...
def temp-table tt-bairro
    field clicod like clilig.clicod
    field dtven  like clilig.dtven
    field numdias as int
    field divida as dec
    field funcod like func.funcod
    field funnom like func.funnom
    field datahora as char
    field dataprom as date
...
then  do.
    if vdatopc8ini <> ?
    then
        for each clilig where clilig.dtven >= vdatopc8ini and
...
find clien where clien.clicod = clilig.clicod no-lock.
        display clien.clicod 
            ...
            status default "H-Hist.Lig/C-AnaliseCred/T-Telefones/S-SimulaNovacao/B-RelBairros".

            choose field clilig.clicod help ""
                go-on(cursor-down cursor-up
                      cursor-left cursor-right
                      page-down   page-up  h H c C t T s S b B
        ...
        if keyfunction(lastkey) = "B" or
           keyfunction(lastkey) = "b"
        then do with frame fimprimeb.
            hide frame f-com1 no-pause.
    ...
    for each tt-bairro.
        delete tt-bairro.
            ...
            varquivo = "./bstelclib." + string(time).
              {mdad_l.i
               ...
               &Page-Line = "66"
               &Nom-Rel   = ""GERAL""
               &Nom-Sis   = """Clientes para Cobrar Por Bairro"""
            ...
            for each tt-bairro.
        find clien where clien.clicod = tt-bairro.clicod no-lock.            
        
                disp tt-bairro.bairro format "x(30)"
                     tt-bairro.clicod format ">>>>>>>>>>>9"
                     clien.clinom
                     tt-bairro.dtven
                     tt-bairro.numdias
                     tt-bairro.divida
                     tt-bairro.dataprom
                     tt-bairro.endereco
              with frame fimprimeb 7 down centered
            row 7 title vtitle.
            down with frame fimprimeb.  
...
/*                             cpfis.estado-fone label "Tipo"*/
                        ...
                        then do:
                            ...
                            recatu1 = ?.
                            if clitel.codcont = 2 or clitel.codcont = 3
                            then
                            clilig.dtultlig = /*today Bernardo*/ 01/01/0001.
                            else clilig.dtultlig = today.
                            /*
...
then  
    ...
    then  
        if vdatopc8ini <> ?
        then
        find first clilig where clilig.dtven    >= vdatopc8ini and
                                clilig.dtven    <= vdatopc8fin and
                                clilig.dtacor    = ?      and
                                clilig.DtUltLig  < vtoday  and
                                clilig.titvlcob >= vvalini     and
...
and (if vnovos = yes then clilig.tipo = "N" else true)                         
      ...
      if vdatopc7 <> ?
      then
            find last clilig  where /*clilig.dtacor    >= vdatopc7*/
                                 clilig.dtacor <> ? and
                                (if clilig.modcod = "CRE"
                                 then clilig.dtacor = vdatopc7
                                 else clilig.dtacor >= vdatopc7) /*Luand
                                clilig.dtacor    <= vtoday - 1Lu*/     and
                                clilig.DtUltLig  < vtoday  and
                                clilig.titvlcob >= vvalini     and
...
then  
    ...
    then  
        if vdatopc8ini <> ?
        then 
        find next clilig  where  clilig.dtven    >= vdatopc8ini and
                                 clilig.dtven <= vdatopc8fin and
                                clilig.dtacor    = ?      and
                                clilig.DtUltLig  < vtoday  and
                                clilig.titvlcob >= vvalini     and
...
and (if vnovos = yes then clilig.tipo = "N" else true)                                                              ***/
      ...
      if vdatopc7 <> ?
      then 
        find prev clilig  where /*clilig.dtacor    >= vdatopc7*/
                                 clilig.dtacor <> ? and
                                (if clilig.modcod = "CRE"
                                 then clilig.dtacor = vdatopc7
                                 else clilig.dtacor >= vdatopc7) /*Luand
...
then                  
    ...
    if vdatopc8ini <> ?
    then 
                   find prev clilig where   clilig.dtven  >= vdatopc8ini
...
and (if vnovos = yes then clilig.tipo = "N" else true)                                                               ***/
      ...
      if vdatopc7 <> ?
      then
        find next clilig where  /*clilig.dtacor    >= vdatopc7*/
                                 clilig.dtacor <> ? and
                                (if clilig.modcod = "CRE"
                                 then clilig.dtacor = vdatopc7
                                 else clilig.dtacor >= vdatopc7) /*Luand
...
and (if vnovos = yes then clilig.tipo = "N" else true)                         
        ...
        for each clilig   where  clilig.dtacor    <= vtoday - 1     and
                                clilig.DtUltLig  < vtoday  and
                                clilig.titvlcob >= vvalini     and
                                (if vetbcod = 0
                                 then true
...
procedure p-arquivosms.
    ...
    then do: 
        if vdatopc8ini <> ?
        then do:
        for each celclilig  where  celclilig.dtven    >= vdatopc8ini and
                                ...
                                (if vetbcod = 0
                                 then true
                                 else celclilig.etbcod = vetbcod) and
                                (if vrgccod = 0
                                 then true
                                 else celclilig.rgccod = vrgccod) and
                                (if vmodcod = "GERAL"
                                 then true
                                 else celclilig.modcod = vmodcod) 
                                 /***and
(if ((vnovacao = yes and clilig.modcod = "NOV") or
                           (vcrediario = yes and clilig.modcod <> "NOV"))
                                then true
...
***/
        ...
        for each  celclilig  where  celclilig.dtven    = vdtini and 
                                celclilig.dtacor    = ?      and
                                celclilig.DtUltLig  < vtoday  and
                                celclilig.titvlcob >= vvalini     and
                                (if vetbcod = 0
...
and (if vnovos = yes then clilig.tipo = "N" else true)                                                          ***/
      ...
      if vdatopc7 <> ?
      then do:
        for each celclilig  where /*clilig.dtacor    >= vdatopc7*/
                                 celclilig.dtacor <> ? and
...
and (if vnovos = yes then clilig.tipo = "N" else true)                                                          ***/
        ...
        for each celclilig   where  celclilig.dtacor    <= vtoday - 1     and
                                celclilig.DtUltLig  < vtoday  and
                                celclilig.titvlcob >= vvalini     and
                                (if vetbcod = 0
                                 then true
...
procedure p-imprimeb.
    ...
    find first tt-bairro where tt-bairro.clicod = clilig.clicod no-error.
    if not avail tt-bairro
    then do:
        create tt-bairro.
        tt-bairro.clicod = clilig.clicod.
    end.
    tt-bairro.dtven = clilig.dtven.
    tt-bairro.numdias = vnumdias.
    tt-bairro.divida = vdivida.
    tt-bairro.funcod = clitel.funcod.
    tt-bairro.funnom = func.funnom.
    tt-bairro.datahora = vdatahora.
    tt-bairro.dataprom = vdataprom.
    tt-bairro.bairro = clien.bairro[1].
    tt-bairro.endereco = clien.endereco[1] + " " + string(clien.numero[1]) + " " + clien.compl[1] + " " + 
    cpclien.var-char9 + " " + clien.cidade[1].
