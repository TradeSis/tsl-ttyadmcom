...
do:
    ...
    find tipmov where tipmov.movtdc = 28 no-lock.
    ...
    repeat with 1 down:
        ...
        do on error undo, retry:
            ...
            if v-kit
            then do:       
                find autctb where autctb.movtdc = tipmov.movtdc and
                                  autctb.emite  = vforcod and
                                  autctb.datemi = vpladat and
                                  autctb.serie  = vserie  and
                                  autctb.numero = vnumero no-lock no-error.
    ...
    do transaction:
        find first plani where plani.etbcod = estab.etbcod
                           ...
                           and plani.emit = vforcod
                           and plani.numero = vnumero
                           and plani.serie = vserie
                           no-error.
    ...
    for each w-movim:
        ...
        do transaction:
            ...
            find movim where movim.etbcod = plani.placod
                         and movim.placod = plani.placod
                         and movim.procod = produ.procod no-error.
            if not avail movim
            then create movim.
    ...
    find fiscal where fiscal.emite  = plani.emit and
    ...
    then  do transaction:
            create fiscal.
            assign fiscal.emite  = plani.emit
                   fiscal.desti  = plani.desti
                   fiscal.movtdc = tipmov.movtdc
    ...
    message "Nota Fiscal Incluida: Numero" vnumero " Serie:"  vserie. pause.
