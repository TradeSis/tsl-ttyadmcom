...
def var vlei            as char format "x(13)".
def var vetb            as i    format ">>9".
def var vcod            as i    format "999999".
...
def var vac like titulo.titvlcob format "->,>>>,>>9.99".
    ...
    update vpath label "Arquivo Origem" with frame f1.
    ...
    def var vc as char format "x(20)".
       ...
       input from value(vpath).
        repeat:
            vqtd = 0.
            import vlei.
            assign vcod = int(substring(string(vlei),1,6))
                   vqtd = int(substring(string(vlei),8,6)).
            disp vcod vqtd.
