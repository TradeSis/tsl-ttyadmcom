...
/*
message tt-plani.emite nfe-emite  serie-nfe versao-nfe.
pause.
*/
find first tt-plani.
...
do:
    DO:
        ...
        **/
        ...
        if v-ok = no
        then do:
            message "Erro na Geração da NFE." view-as alert-box.
            return.
        end.
        ...
        for each tt-movim where tt-movim.etbcod = tt-plani.etbcod and
            ...
            then do:
                ...
                then do:
                    find first bi01_prod where
                           ...
                           no-lock no-error.
                    if not avail bi01_prod
                    then find first bi01_prod where
                                 bi01_prod.chave = vchave-nfref and
                                 bi01_prod.procod = tt-movim.procod
                                 no-lock no-error.
                    if avail bi01_prod
                    then do:
                        ...
                        then assign
                                ...
                                N01_icms.vicmsop = 
                                    (bn01_icms.vicmsop / bi01_prod.qcom)
                                    * i01_prod.qcom
                                N01_icms.pdif = bn01_icms.pdif
                                N01_icms.vicmsdif = 
                                    (bn01_icms.vicmsdif / bi01_prod.qcom)
                                    * i01_prod.qcom
                                N01_icms.vicms = n01_icms.vicmsop -
                                n01_icms.vicmsdif
                                .
                    end. 
                    else do:
                ...
                then do:
                    ...
                    if avail clafis and
                       clafis.perred > 0
                                and (tt-plani.opccod = 1102
                                      or tt-plani.opccod = 5202)
                    then
            ...
            then do:
                if tt-movim.movbpiscof > 0 and
                   tt-movim.movalpis > 0 and
                   tt-movim.movpis = 0
                then tt-movim.movpis = 
                     tt-movim.movbpiscof * (tt-movim.movalpis / 100).
            ...
            then do:
                if tt-movim.movbpiscof > 0 and
                   tt-movim.movalcofins > 0 and
                   tt-movim.movcofins = 0
                then tt-movim.movcofins = 
                     tt-movim.movbpiscof * (tt-movim.movalcofins / 100).
