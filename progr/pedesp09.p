...
for each estab where ( if vetbcod > 0
    ...
    for each pedid where pedid.etbcod = estab.etbcod and
        ...
        for each liped where liped.pedtdc = pedid.pedtdc and
                             ...
                             liped.lipsit = "A"
                             no-lock.
            find produ where produ.procod = liped.procod 
                and produ.proipival = 1 no-lock no-error.
            if not avail produ
            then next.
...
repeat:
    ...
    if recatu1 = ?
    then find last tt-pedid no-error.
    else find tt-pedid where recid(tt-pedid) = recatu1.
    ...
    repeat with frame frame-a:
        ...
        hide frame  frame-a no-pause.
          ...
          then do:
            ...
            then do with frame f-inclui overlay row 9 1 column centered
                ...
                then do:
                    message color red/with "Compra ja emitida para pedido."
                    view-as alert-box.
                end.
                else do:
                find first tt-marca where 
            ...
            then do with frame f-Lista.
                ...
                find func where func.etbcod = tt-pedid.etbcod and
                                func.funcod = tt-pedid.vencod no-lock no-error.
                put skip(7)
                "PEDIDO: " at 10 tt-pedid.pednum format ">>>>>>>>9"
                "    ORDEM DE COMPRA: " tt-pedid.comcod format ">>>>>>>>9"
                SKIP(1)
            ...
            then do:
                ...
                find first tt-marca where tt-marca.pedtdc = par-pedtdc
                            no-error.
                if not avail tt-marca
                then do:
                    message "Marcar pedido antes de gerar compra."
                    view-as alert-box.      
                    leave.          
                end.
                
                /*Nede 18-01-2011*/
                find first pedid  where tt-marca.pednum = pedid.pednum
                no-lock no-error.
                if avail pedid
                then do:
        ...
        for each nliped where 
               ...
               nliped.pedtdc = tt-pedid.pedtdc
               no-lock,
        first nprodu of nliped where
                nprodu.proipival = 1 no-lock.
        find first nforne where nforne.forcod = nprodu.fabcod no-lock.
        vfornom = string(nforne.forcod,">>>>>>9") + " " +  nforne.fornom. 
        end.            
        display vmarca
        ...
        for each tt-marca where tt-marca.pednum > 0 no-lock: 
            ...
            then do:
                ...
                put fill("_",80) format "x(80)" skip
                    "PEDIDO: " tt-numped.pedido format ">>>>>>>>9"  "" skip
