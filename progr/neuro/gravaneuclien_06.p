...
/* #1 22.05.18 - teste se cpfcnpj eh nulo, nao permite a criacao de neuclien */    
...
def shared temp-table PreAutorizacao
    ...
    vdtnasc = date(int(substring(preAutorizacao.data_nascimento,6,2)),
     ...
     vcpfcnpj = dec(p-cpf) no-error. /* #1 */
     if vcpfcnpj = 0 or vcpfcnpj = ?
     then do:
        p-recid-neuclien = ?.
        return.
     end.
     ...
     do on error undo.
        find neuclien where neuclien.cpfcnpj = vcpfcnpj
            exclusive
            no-wait 
            no-error.
