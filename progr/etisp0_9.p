...
then do:
    ...
    if opsys = "UNIX"
    then do:
        varquivo = "/admcom/zebra/sp/eti-sp.bat".
        if search(varquivo) <> ?
        then do:
            unix silent value(" rm " + varquivo + " -f").
            unix silent value(" rm /admcom/zebra/sp/c*.* -f"). 
        end.
    end.
    else do:
        varquivo = "l:~\zebra~\sp~\eti-sp.bat".
    
        if search(varquivo) <> ?
        then do:
            dos silent del l:~\zebra~\sp~\eti-sp.bat.
            dos silent del l:~\zebra~\sp~\c*.* .
        end.        
    end.

    
    /*
    if search("l:\zebra\sp\eti-sp.bat") <> ?
    then do:
    ...
    */
    ...
    for each wetique:
        ...
        run /*etisp_9.p*/ etiqbz02.p (input wetique.wrec,
                     input wetique.wqtd,
