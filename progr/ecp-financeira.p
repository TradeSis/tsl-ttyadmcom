...
disp "IMPORTANDO DADOS DO ARQUIVO... " at 1  VV
     WITH FRAME F2 down no-label no-box row 7 overlay.
pause 0.
...
disp "VALIDANDO DADOS DO ARQUIVO.... "
    AT 1 WITH FRAME F3 down no-label no-box row 8 overlay.
pause 0.
IF VV = ""
    THEN VV = "AGUARDE!".
    ELSE VV = "" .
...
for each tt-cont where tt-cont.contnum > 0 :
    find contrato where contrato.contnum = tt-cont.contnum NO-LOCK.
...
disp stream st "GERANDO ARQUIVO DE RETORNO.... "
    AT 1 WITH FRAME F4 down no-label no-box row 9 overlay.
pause 0.
...
disp "ARQUIVOS DE RETORNO GERADOS.... "
    AT 1 WITH FRAME F5 down no-label no-box row 10 overlay.
pause 0.
disp "FIM DO PROCESSO.... "
    AT 1 WITH FRAME F6 down no-label no-box row 11 overlay.
pause .
...
procedure p-registro-10.
  ...
  /*if avail btitulo then 
    assign*/
  v-data-vecto = titulo.titdtpag. /*btitulo.titdtven*/ 
