...
def var vtitvlcob like titulo.titvlcob label "TOTAL". 
...
update vsit at 8 label "Situacao" format "x(3)"
        validate(vsit = "LIB" or
                 ...
                 Vsit = "PEN" or
                 Vsit = "   ", "")
     help "LIB=Liberado PAG=Pago BLO=Bloqueado EXC=Excluido PEN=Pend.P.Bis"
    with frame f1.
...
if opsys = "UNIX"
then varquivo = "/admcom/relat/rpgprem1u" + string(time) + ".txt".
else varquivo = "l:~\relat~\rpgprem1u" + string(time) + ".txt".
...
for each tt-titluc use-index i1
    ...
    then  disp tt-titluc.etbcod column-label "Filial"
    ...
    if first-of(tt-titluc.vencod)
    then vtitvlcob = 0.           
               
    vtitvlcob = vtitvlcob + tt-titluc.titvlcob.           
        
    disp tt-titluc.vencod    
    ...
    if last-of(tt-titluc.vencod)
    then disp vtitvlcob colon 31 with frame f-disp1 no-labels.
