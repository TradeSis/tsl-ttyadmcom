...
def var esqcom1         as char format "x(15)" extent 5
    initial ["Consulta","Marca","Marca Tudo","Extorna","Exclui da base"].
...
repeat:
    ...
    display
        ...
        mapctb.t02       column-label "Val.12%" format ">,>>9.99"
        mapctb.t03       column-label "Val.07%" format ">,>>9.99"
    ...
    repeat with frame frame-a:
        ...
        hide frame frame-a no-pause.
          ...
          then do:
            ...
            if esqcom1[esqpos1] = "Extorna"
            then do:
                find first tt-mapctb where 
                            tt-mapctb.marca = "*" no-error.
                if not avail tt-mapctb
                then do:
                    bell.
                    message "Nenhum registro selecionado para EXTORNO."
                    view-as alert-box.
                    next bl-princ.
                end.            
                sresp = no.
                message "Confirma EXTORNO das Red. Z selecionadas ?" 
                update sresp.
                if sresp 
                then do:
                for each tt-mapctb where
                         tt-mapctb.marca = "*":
                    find mapctb where recid(mapctb) = tt-mapctb.rec.
            ...
            if esqcom1[esqpos1] = "Exclui da base"
            then do:
                find first tt-mapctb where 
                            tt-mapctb.marca = "*" no-error.
                if not avail tt-mapctb
                then do:
                    bell.
                    message "Nenhum registro selecionado para EXCLUIR DA BASE."
                    view-as alert-box.
                    next bl-princ.
                end. 
                sresp = no.
                message 
                "Confirma EXCLUIR Red. Z selecionadas da base de dados?" 
                update sresp.
                if sresp 
                then do:
                for each tt-mapctb where
            ...
            then do:
                if tt-mapctb.marca = ""
                then tt-mapctb.marca = "*".
                else tt-mapctb.marca = "".   
            end.
