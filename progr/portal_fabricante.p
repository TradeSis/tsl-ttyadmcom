...
define temp-table tt-valores no-undo serialize-name 'fabricante'
    field codigo    as integer   serialize-name 'codigo'
    field descricao as character serialize-name 'nomeFantasia'
...
for each fabri no-lock:
    ...
    create tt-valores.
    assign tt-valores.codigo    = fabri.fabcod
           tt-valores.descricao = fnremoveacento(fabri.fabfant)
           tt-valores.cod-pai   = 1.

    if i-cont = 4000 then do:
        ...
        RUN PostRequest (INPUT 'produtos/integracaoFabricante', 
                         INPUT lo-arquivo).
