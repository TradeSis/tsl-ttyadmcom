...
put "TIPO DE OPERACAO" skip
    "(  ) CDC"  " (  ) CREDITO LOJISTA" skip
    "(XX) RENEGOCIACAO CDC" skip
    " N: "  vcontrato  skip.
...
put "DT Financ: " string(v-pladat,"99/99/99") skip
    ...
    "         Valor IOF: " + string(dec(vret-ValorIOF),">>>>>>>9.99") + " " +  string((dec(vret-ValorIOF) / dec(vret-ValorFinanciado)) * 100,">>9.99%")
    
                                format "x(40)" skip
    "    Valor da Venda: " + string(vvenda,">>>>>>>9.99") /*+ " " + string((vvenda / dec(vret-ValorFinanciado)) * 100,">>9.99%")*/
                                    format "x(40)"  skip
    "  Valor Financiado: " + string(dec(vret-ValorFinanciado),">>>>>>>9.99")
             /*+ " " + string((dec(vret-ValorFinanciado) / 
                            (vprest * int(vparc)) * 100),">>9.99%")*/   
                                    format "x(40)" skip
    "Valor Total Devido: " + string(vtotaldevido,">>>>>>>9.99")
                                    format "x(40)"
                            /*contrato.vltotal format ">>>>>9.99"*/ skip(1)
    "Contrato(s) renegociado(s): "  skip.
    
    for each tc-contrato :
        put unformatted tc-contrato.contnum skip.
    end.
...
put unformatted "Eldorado do Sul, "  trim(string(day(v-pladat)))
 " de " vmescomp[month(v-pladat)] 
    " de " string(year(v-pladat)) "."
    skip(2).
