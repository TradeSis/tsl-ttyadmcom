...
def var dtfim as date format "99/99/9999" initial today.
 ...
 update dtini colon 15 label "EP de" 
               dtfim label "ate".
       

varquivo2 = "/admcom/import/cdlpoa/EMP-behaviour-"
+ string(month(dtini))
+ string(year(dtini))
+ string(month(dtfim))
+ string(year(dtfim))
...
pause 0.
 ...
 find first titulo use-index iclicod where
    ...
    for each contrato use-index iconcli where contrato.clicod = clien.clicod and
                        (contrato.modcod begins "CP0"  or contrato.modcod begins "CP1"  or contrato.modcod begins "CP2" )
                                                and 
