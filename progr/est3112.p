...
repeat:
    ...
    do on error undo:
        ...
        update skip 
               vcatcod label "Departamento" at 12 
    ...
    do on error undo:
        ...
        update skip
               vmes   label "Mes"
               vano   label "Ano"
               with frame f-dados2 centered side-labels title " Estoque ".
    ...
    update skip
           ...
           with frame f-dados3 centered side-labels title " Vendas ".
    ...
    do vdata = vdtini to vdtfin:
        for each tt-est:
            ...
            for each movim use-index icurva
                           where movim.etbcod = tt-est.etbcod
                             and movim.movtdc = 5
                             and movim.procod = tt-est.procod
                             and movim.movdat = vdata no-lock:
                
                tt-est.venda = tt-est.venda + movim.movqtm.
    ...
    if opsys = "UNIX"
    then assign varquivo = "/admcom/relat/est3112." + string(time).
    else assign varquivo = "l:\relat\est3112." + string(time).
    ...
    {mdadmcab.i 
        ...
        &Tit-Rel   = """PRODUTOS VENDIDOS ACIMA DO ESTOQUE """
        &Width     = "120"  
        &Form      = "frame f-cabcab"}
    

        for each tt-est where tt-est.venda > tt-est.qtd
                        break by tt-est.etbcod
                              by tt-est.pronom:
            ...
            disp space(3)
                 ...
                 tt-est.qtd    column-label "Qtd Est" format ">>>>>9"
                 tt-est.venda  column-label "Qtd Ven" format ">>>>>9"
                 (total by tt-est.etbcod) with width 100.
