...
def new shared temp-table tt-movim-aux like movim
...
def var vprotot   as dec format ">>,>>>,>>9.9999".
...
def temp-table w-movim
               ...
               field subtotal  as decimal format ">>,>>>,>>9.9999"
               field movpc     as decimal format ">>,>>>,>>9.9999".
form titulo.titpar
     titulo.titnum
     prazo
     titulo.titdtven
...
form produ.procod
     ...
     w-movim.movpc       column-label "V.Unit."
...
find tipmov where tipmov.movtdc = 70 no-lock.
...
REPEAT:
   ...
   find tipmov where tipmov.movtdc = 70 no-lock.
    ...
    do on error undo:
        ...
        if vforcod > 0 or vmovtdc = 70 /* Saída para montagem */
        then do:
            ...
            /*
            vforcod = vetbcod.
            find bestab where bestab.etbcod = vetbcod
                    no-lock .
            if bestab.ufecod = "RS"
            then find first opcom where opcom.movtdc = tipmov.movtdc no-lock.
            else find last  opcom where opcom.movtdc = tipmov.movtdc no-lock.
        ...
        then do with frame f-produ2:
            ...
            for each w-movim:
                ...
                disp produ.procod
                     produ.pronom
                     w-movim.movqtm format ">>,>>9.99" column-label "Qtd"
                     w-movim.movpc                     column-label "Custo"
    ...
    */
    ...
    for each w-movim:
        ...
        create tt-movim-aux.
        buffer-copy tt-movim to tt-movim-aux.   
         
        /* Esse campo mantém as 4 casas decimais */ 
        assign tt-movim-aux.movpc-aux = w-movim.movpc.
    ...
    then 
    */
    run manager_nfe.p (input "5555", input ?, output v-ok). 
    pause 5 no-message.
    for each w-movim:
        delete w-movim.
    end.
