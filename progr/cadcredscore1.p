...
assign
    tt-grupo.codigo = 0
    tt-grupo.descri = "GERAL"
    .

for each agfilcre no-lock:
...
def var p-grupo as int.
if sretorno = ""
then run sel-grupo(output p-grupo).
else p-grupo = int(sretorno).
...
for each credscore where credscore.desc-campo = "PC PAGAS".
credscore.int2 = integer(credscore.campo).
end.
...
repeat:
    ...
    repeat with frame frame-a:
        ...
        then do:
            form credscore.campo credscore.valor credscore.vl-ini  credscore.vl-fin
            ...
            then do:
                ...
                disp with frame frame-a overlay.
                pause 0.
                if esqcom1[esqpos1] = " Inclusao " or esqvazio
                then do with frame f-credscore overlay on error undo.
                    run p-guardaval (input "Inclusao").                
                    create credscore.
                    credscore.grupo = p-grupo.
                    credscore.desc-campo = "PC PAGAS".
                    update
                         credscore.campo  label "PARCELAS PAGAS......"
                         format "x(5)"
                         skip
                         credscore.valor  label "DIAS DE ATRASO......"
                         format ">>9"
                         skip
                         credscore.vl-ini label "% LIMITE (<= ATRASO)" 
                         skip
                         credscore.vl-fin label "% LIMITE (> ATRASO)."
                         with frame f-mostra centered side-labels
                             color white/red row 10 overlay.
                     recatu1 = recid(credscore).
                ...
                then do with frame f-credscore.
                    pause 0.                
                     disp credscore.campo  label "PARCELAS PAGAS......"
                         format "x(5)"
                         skip
                         credscore.valor  label "DIAS DE ATRASO......"
                         format ">>9"
                         ...
                         credscore.vl-fin label "% LIMITE (> ATRASO)."
                         skip(1)
                         credscore.int1   label "DIAS EM ATRASO TOLERAVEL"
                                          format ">>9"
                         with frame f-mostra centered side-labels
                             color white/red row 10 overlay.
                ...
                then do with frame f-exclui  row 5 1 column centered
                    ...
                    then undo, leave.
                    find next credscore 
                        use-index grupo where  credscore.grupo = p-grupo and
                        credscore.desc-campo = "PC PAGAS"
                        no-error.
                    if not available credscore
                    then do:
                        ...
                        find prev credscore
                          use-index grupo where  credscore.grupo = p-grupo and                            credscore.desc-campo = "PC PAGAS"
                            no-error.
                    end.
...
procedure frame-a.
find first tt-grupo where
           tt-grupo.codigo = p-grupo no-error.
display credscore.campo  column-label "PC PAGAS" format "x(5)"
        credscore.valor  column-label "DIAS DE ATRASO" format ">>9"
        credscore.vl-ini column-label "% LIMITE <=ATRASO" 
        credscore.vl-fin column-label "% LIMITE >ATRASO "
        credscore.int1   column-label "DIAS EM ATRASO!TOLERAVEL"
        with frame frame-a 11 down centered color white/red row 5
        title " parametros do grupo " + tt-grupo.descri + " "
        .
end procedure.
...
credscorelog.campo          = ant-campo + " PC PAGAS".
...
procedure sel-grupo:
    def output parameter p-grupo as int.
    assign
        a-seeid = -1
        a-recid = -1
        a-seerec = ?
    ...
    {sklclstb.i  
        ...
        &aftfnd1 = " "
        &where  = " true "
        &aftselect1 = " p-grupo = tt-grupo.codigo.
                        leave keys-loop.
                      "
        &go-on = TAB 
        &naoexiste1 = " p-grupo = 0.
                        leave keys-loop.
                      "  
        &otherkeys1 = " "
        &locktype = " "
        &form   = " frame f-linha down centered 
                    row 6 title "" grupos "" "
    }  
end procedure.
