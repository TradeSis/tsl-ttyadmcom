...
repeat:
    ...
    dos silent dir value("i:\spc\DRIN" + string(day(vdata),"99") +
    ...
    for each tt-arq where tt-arq.arq <> "":
        ...
        repeat:
            ...
            then do:
                ...
                tt-spc.reg   = vreg.
                tt-spc.cpf   = substring(vtexto,22,11).
                
                find first clien where 
                           clien.clinom = substring(vtexto,43,50) and
                           clien.ciccgc = tt-spc.cpf and
                           clien.dtnasc = date(tt-spc.dtnas)
                                        no-lock no-error.
                if avail clien
    ...
    if opsys = "UNIX"
    then assign varquivo = "/admcom/relat/spc" + string(day(today)).  
    else assign varquivo = "..\relat\spc" + string(day(today)).
    ...
    {mdad.i
            ...
            &Tit-Rel   = """RETORNO DE SPC - INCLUSAO - DO DIA "" +
