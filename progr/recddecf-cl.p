...
/*
sresp = no.
run ecfstatus.p (output sresp).
if not sresp 
then do:
    message color red/with
    "ECF não esta respondendo."
    view-as alert-box.
    return.
end.
*/
...
do on error undo:
    ...
    update vdti AT 1 label "DATA" format "99/99/9999"
           WITH FRAME F1.
    VDTF = VDTI.
           /*
...
if vindex = 1
then varq = "bemamfd" + string(vcxacod,"99") +
            string(day(vdti),"99") +
            string(month(vdti),"99") +
            substr(string(year(vdti),"9999"),3,2) + ".txt"
            .
else varq = "bemamfd" + "_" + string(vcxacod,"99") + 
            string(month(vdti),"99") +
            substr(string(year(vdti),"9999"),3,2) + ".txt"
            .
     
            
update varq label "Arquivo" with side-label
width 80.
...
then do:
    ...
    unix  value("scp root@filial" + string(vetbcod,"999") +
        ":/usr/admcom/ecfinfo/" + varq + " /admcom/work/").
        
    /*
    if search(varqmfd) <> ?
    then do:
     unix silent value("/usr/admcom/progr/bemamfd2 " + varqret + " " + varqmfd +
                " 1 " + vini + " " + vfin + " >> bemamfd.log"). 
    end.
    hide message no-pause.
    */
end.

if search("/admcom/work/" + varqret) <> ?
then do:
    input from value("/admcom/work/" + varqret).
    repeat:
        create tt-linha.
    ...
    then do:
            ...
            then do:
                ...
                then do:
                    ...
                    assign
                        ...
                        vcharmovpc = replace(vcharmovpc,":",".")
                        vcharmovpc = replace(vcharmovpc,"G","")
                        vmovpc = dec(substr(string(vcharmovpc,"x(14)"),2,12))
                            ad-linha = qt-linha + 1
                        . 
                end.
                run cria-movim.
        ...
        then do:
            ...
            assign    
                ...
                assign
                    ...
                    vmovqtm = dec(substr(string(vcharicms),1,3))
                    vcharicms = substr(string(entry(1,linha,"F1")
                        ),length(string(entry(1,linha,"%"))) - 2,2)
                        /*vcharicms = replace(vcharicms,",",".")*/
                    vmovalicms = 0 /*dec(entry(2,vcharicms,"T"))*/    
                    vcharmovpc = entry(2,linha,"F1")
                    vcharmovpc = replace(vcharmovpc,".",":")
        ...
        then do:
            assign
                ...
                vvencod = if substr(string(entry(5,linha,":")),1,4) = " ???"
                          then 0 
        ...
        else if substr(string(linha),1,5) = "TOTAL" 
        then do:
...
/**
disp "GRAVANDO DADOS NO ADMCOM... AGUARDE!"  at 1
    with frame f-log 1 down no-box color message.

def var vqtdimp as int.
def var vqtdrec as int.
for each wf-plani where wf-plani.etbcod = vetbcod and
                        wf-plani.numero > 0 and
                        wf-plani.numero <> ?
                        :
    vqtdimp = vqtdimp + 1.
    find first  plani where plani.etbcod = wf-plani.etbcod and
                     plani.placod = wf-plani.placod and
...
for each wf-movim where wf-movim.etbcod = vetbcod and
                        wf-movim.placod <> ? and
                        wf-movim.placod > 0
                        :
        find first movim where movim.etbcod = wf-movim.etbcod and
                         movim.placod = wf-movim.placod and
                         movim.procod = wf-movim.procod
                         no-lock no-error.
        if not avail movim
         then do:
            create movim.
            ASSIGN movim.movtdc    = wf-movim.movtdc 
               movim.PlaCod    = wf-movim.placod  
               movim.etbcod    = wf-movim.etbcod  
               movim.emite     = wf-movim.emite  
               movim.desti     = wf-movim.desti  
               movim.movseq    = wf-movim.movseq  
               movim.movdes    = wf-movim.movdes  
               movim.procod    = wf-movim.procod  
               movim.movqtm    = wf-movim.movqtm  
               movim.movpc     = wf-movim.movpc  
               movim.movalicms = wf-movim.movalicms  
               movim.movdat    = wf-movim.movdat  
...
for each wf-plani where wf-plani.etbcod = vetbcod and
    ...
    then.
        ...
        for each movim where 
             ...
             movim.etbcod = plani.placod and
             movim.movtdc = plani.movtdc
             no-lock:
            plani.protot = plani.protot + (movim.movqtm * movim.movpc).
            plani.platot = plani.protot.
        end.
    end.
end.
         
disp "FIM DA IMPORTACAO... AGUARDE!"     at 1
    with frame f-log 1 down no-box color message.
    
disp vqtdimp label "Quntidade de cupons  "
     vqtdrec label "Qunatidade recuperada"
     with frame frec 1 down centered width 40
     side-label.
***/
