...
def input parameter p-toper as char.
def input parameter p-modelo as char.
def input parameter p-numero as dec.
def input parameter p-serie as char.
def input parameter p-emite as char.
def input parameter p-emis as date.
    
                                                            
find first  NF_Serv_Capa where 
            NF_Serv_Capa.Cod_Estab = p-estab and
            NF_Serv_Capa.Tipo_Operacao = p-toper and
            NF_Serv_Capa.Cod_Modelo = p-modelo and
            NF_Serv_Capa.Num_Docto  = p-numero and
            NF_Serv_Capa.Num_Serie  = p-serie and
            NF_Serv_Capa.Cod_Parceiro = p-emite and
            NF_Serv_Capa.Data_Emissao = p-emis
            no-lock no-error.
if not avail NF_Serv_Capa then return.            

def temp-table tt-nfser like fatudesp.
/*
    ...
    .
*/
find fatudesp where fatudesp.etbcod = int(p-estab) and
                    fatudesp.clicod = int(p-emite) and
                    fatudesp.fatnum = int(p-numero)
                     no-error.
if not avail fatudesp 
then do:
    create tt-nfser.
    assign
        tt-nfser.etbcod     = int(p-estab)
        tt-nfser.clicod     = int(p-emite) 
        tt-nfser.fatnum     = int(p-numero)
        tt-nfser.emissao    = p-emis
        tt-nfser.inclusao   = NF_Serv_Capa.Data_ES
        tt-nfser.val-ir     = 0
        tt-nfser.val-iss    = 0
        tt-nfser.val-inss   = 0
        tt-nfser.val-pis    = 0
        tt-nfser.val-cofins = 0
        tt-nfser.val-csll   = 0
        tt-nfser.val-total  = NF_Serv_Capa.Val_Tot_Docto
        tt-nfser.val-liq    = NF_Serv_Capa.Val_Tot_Docto
        tt-nfser.qtd-par    = 0
        tt-nfser.val-icms   = 0
        tt-nfser.val-ipi    = 0
        tt-nfser.val-des    = NF_Serv_Capa.Val_Desconto
        tt-nfser.val-acr    = 0
        tt-nfser.situacao   = ""
         .
end.
else do:
    /*disp fatudesp.modcod fatudesp.situacao. pause.
    */
    create tt-nfser.
    buffer-copy fatudesp to tt-nfser.

end.                

def var vsetcod like setaut.setcod.
def var vprocimp as char format "x(20)".
def var vmodcod as char.
def var vmod-ctb as char.
...
*/
def var v-existe as log.

assign vtitnat = yes
       vmodcod = tt-nfser.modcod
       vetbcod = tt-nfser.etbcod
       vclifor = tt-nfser.clicod
       vsetcod = tt-nfser.setcod
       .
...
procedure rateio-cria-titulo:
    ...
    assign
        ...
        vtitnum = string(tt-nfser.fatnum)
...
procedure inclusao.
    ...
    v-existe = no.
    do on error undo:
        run dd-nfser(input par-rec).
        if keyfunction(lastkey) = "END-ERROR"
        then return.
    end.
    if v-existe or keyfunction(lastkey) = "END-ERROR"
    then return.
    sresp = no.
...
procedure relatorio:
    ...
    for each fatudesp where fatudesp.etbcod = int(p-estab) and
                        fatudesp.clicod = int(p-emite) and
                        fatudesp.fatnum = int(p-numero)
                        no-lock:
...
procedure dd-nfser:
    ...
    */
    ...
    then do: 
        ...
        find first  fatudesp where 
                ...
                no-error.
        vtt-titulo = 0.
        for each ftitulo where
                 ftitulo.empcod = titulo.empcod and
                 ftitulo.titnat = titulo.titnat and
                 ftitulo.modcod = titulo.modcod and
                 ftitulo.etbcod = titulo.etbcod and
                 ftitulo.clifor = titulo.clifor and
                 ftitulo.titnum = titulo.titnum
                 no-lock .
            vtt-titulo = vtt-titulo + titulo.titvlcob.
        end.         
        if vtt-titulo = fatudesp.val-liq
        then do:
            v-existe = yes.
            return.
        end.
    ...
    find first  fatudesp where 
                fatudesp.fatnum = int(tt-nfser.fatnum) and
    ...
    */
    ...
    vsetcod = tt-nfser.setcod.
    vmodcod = tt-nfser.modcod.
    vmod-ctb = tt-nfser.modctb.
    vprocimp = tt-nfser.numerodi.
    run set-valores.
    if keyfunction(lastkey) = "END-ERROR"
    then return.
    disp tt-nfser.fatnum    format ">>>>>>>>9"
         tt-nfser.emissao
    ...
    repeat on endkey undo, return:
        ...
        then do:
            ...
            find first titulo where
                       titulo.clifor = vclifor and
                       titulo.titnum = string(int(tt-nfser.fatnum)) and
            ...
            then do:
                bell.
                message color red/with
                "Documento ja existe com Numero " tt-nfser.fatnum
                view-as alert-box.
                v-existe = yes.
                return.
            end. 
    ...
    for each tt-modctb: delete tt-modctb. end.
    
    message "Gravando dados AGUARDE...".
    PAUSE 1 no-message.
    find first tt-nfser where
               tt-nfser.fatnum <> 0 no-error.
    if avail tt-nfser
    then do on error undo:
        find first fatudesp where
                   /*fatudesp.etbcod = vetbcod and*/
                   fatudesp.clicod = vclifor and
                   fatudesp.fatnum = int(tt-nfser.fatnum)
                   no-error.
        if not avail fatudesp
        then do:
            create fatudesp.
        ...
        for each tt-titudesp where
            ...
            then do:
                ...
                create tt-modctb.
                assign
                    tt-modctb.modcod = vmod-ctb
                    tt-modctb.etbcod = tituctb.etbcod
                    tt-modctb.titnum = tituctb.titnum
                    .
            end.
        ...
        then do:
            ...
            find first  blancactb where
                                blancactb.id = 0  and
                                blancactb.etbcod = 0 and
                                blancactb.forcod = 0 and
                                blancactb.modcod = fatudesp.modctb
                                no-lock no-error.
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA",blancactb.contadeb,"93",
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA",blancactb.contadeb,"40",
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA",blancactb.contadeb,"102",
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA",blancactb.contadeb,"101",
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA",blancactb.contadeb,"235",
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA","228",blancactb.contadeb,
            ...
            /***
            for each tt-modctb where tt-modctb.modcod <> "" no-lock:
                find first tituctb where 
                       tituctb.empcod = wempre.empcod and
                       tituctb.titnat = vtitnat         and
                       tituctb.modcod = tt-modctb.modcod and
                       tituctb.etbcod = tt-modctb.etbcod and
                       tituctb.clifor = fatudesp.clicod       and
                       tituctb.titnum = tt-modctb.titnum /*and
                       tituctb.titpar = titudesp.titpar */
                       no-lock no-error.
                if avail tituctb
                then do:
                find first  lancactb where
                                ...
                                no-lock no-error.
                run  lan-contabil ("EXTRA-CAIXA",lancactb.contadeb,
                    lancactb.contacre,tituctb.modcod,today,tituctb.titvlcob,
                  tituctb.clifor,int(tituctb.titnum),tituctb.etbcod,
                              lancactb.int1,vcompl).
                end.
            ...
            ***/ 
            ...
            for each tituctb use-index clifor where 
                       tituctb.clifor = fatudesp.clicod  and
                       tituctb.titnum = string(fatudesp.fatnum) and
                       tituctb.titdtemi = fatudesp.inclusao
                       no-lock.
            ...
            if fatudesp.modctb = "DIP" or
               fatudesp.modctb = "IIA"
            then
            run despatucusto.p(input fatudesp.etbcod, 
                               input fatudesp.clicod, 
                               input fatudesp.fatnum).
        end.
    end.
end.  
...
procedure gera-titulo-parcial:
    ...
    disp par-setcod
         ...
         .
    pause 0.
    update val-parcial column-label "Valor Parcelar" with frame f-par0.
...
procedure set-valores:

form with frame ff2 side-labels row 4 width 80 no-box .
        ...
        do on error undo, retry:
            update vmod-ctb label "Mod.CTB"
                    validate(can-find(modal where modal.modcod = vmod-ctb),
                                "Modalidade Invalida") with frame ff2.
            find bmodal where bmodal.modcod = vmod-ctb NO-LOCK.
            display bmodal.modnom no-label with frame ff2.
