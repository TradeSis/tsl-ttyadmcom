...
def temp-table tt-tipmov 
    ...
    field custo   like plani.platot extent 100.

def var totcus like plani.platot extent 100.
...
repeat:
    ...
    for each tipmov where tipmov.movtdc = 03 or
        ...
        */
        ...
        find first tt-tipmov where tt-tipmov.data  = plani.datexp no-error.
    ...
    for each tipmov no-lock:
        if /*tipmov.movtdc = 9 or */
           ...
           tipmov.movtdc = 31
        then next.   
        for each plani where plani.datexp >= (vdti /* - 15*/)    and
                             plani.datexp <= (vdtf /*+ 15*/)   and
                             plani.movtdc = tipmov.movtdc  and
    ...
    if opsys = "UNIX"
    then varquivo = "/admcom/relat/pla" + STRING(day(today)) +
                            string(estab.etbcod,">>9").
    else varquivo = "..\relat\pla" + STRING(day(today)) + 
                            string(estab.etbcod,">>9").
    ...
    put "Vl.Custo (ENTRADAS):   "  at 20 totent 
        
        "Vl.Custo (SAIDAS):     "  at 20 totsai .
