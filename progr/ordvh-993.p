...
for each tdocbpro of tdocbase :
    ...
    v-lote = "".
    if tip-ped = "PESP"
    then v-lote = string(tdocbpro.campo_int1).
    else v-lote = "".
    
    v-serie = "".

    if length(tdocbpro.campo_char1) = 1
    then v-serie = "00" + string(tdocbpro.campo_char1).
    else if length(tdocbpro.campo_char1) = 2
    then v-serie = "0" + string(tdocbpro.campo_char1).
    else if length(tdocbpro.campo_char1) = 3
    then v-serie = string(tdocbpro.campo_char1).
...
unix silent value("mv -f " + par-arq +  " " + vdiretorio-apos).
