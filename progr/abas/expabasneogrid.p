...
/*#33 - helio.neto - 20.08.2019 - Elimina "AC" em geral */
...
for each abaswms no-lock.
    for each abastransf where            
            ...
            abastransf.dttransf <= today /*#33and 
            abastransf.qtdemwms = 0 and    #33
            abastransf.qtdatend = 0        #33**/ .
                                                .
    ...
    for each abastwms of abaswms no-lock.
        ...
        then next.
            ...
            for each abastransf where            
                ...
                abastransf.abtsit = "AC" and
                abastransf.dttransf < today - 45 /*#33and
                abastransf.qtdemwms = 0 and        #33
                abastransf.qtdatend = 0            #33*/ .
                abastransf.abtsit = "EL".        
                vconta = vconta + 1.
...
THEN DO:
    def stream ORIGENS.   
    output stream ORIGENS   to value(tmp-arquivo[2]).
END.
IF NOME-ARQUIVO[8] <> ""
THEN DO:    
    def stream SKU_ESTOQUE. 
    output stream SKU_ESTOQUE to value(tmp-arquivo[8]).
END.    
...
def temp-table temp-sku NO-UNDO
    ...
    field leadTIMEDP            as char
    field LOTE                  as char
    field unitizacao            as char
    
    index temp-sku   is primary unique item asc local asc local-origem asc.
...
def temp-table temp-mix no-undo
    field etbcod    like estoq.etbcod
    field procod    like produ.procod
    field grade     as int
    index mix is unique primary procod asc etbcod asc.
...
for each produ no-lock.
    ...
    THEN DO:
        create temp-item.
        assign 
           temp-item.item    = string(produ.procod)
           temp-item.ean     = if vproindice <> ?
                               then if trim(vproindice) = "SEM GTIN"
                                    then ""
                                    else vproindice                               
                               else "".
           temp-item.descr   = retira-acento(produ.pronom).
           temp-item.fornecedor = (if avail forne then string(forne.forcod) else "N/A").
           temp-item.unidade = "UN" /*if avail produ
                                  then produ.prouncom
                                  else "" */ .
           temp-item.emb     = 1                .
           ...
           temp-item.dgrupo1 = (if avail depto then string(depto.clacod) else "N/A" ) + 
           ...
           if temp-item.categoria = "" or temp-item.categoria = "-"
           then temp-item.categoria  = "N/A".
           if temp-item.dgrupo1 = "" or temp-item.dgrupo1 = "-"
           then temp-item.dgrupo1  = "N/A".
           if temp-item.dgrupo2 = "" or temp-item.dgrupo2 = "-"
           then temp-item.dgrupo2  = "N/A".
           if temp-item.dgrupo3 = "" or temp-item.dgrupo3 = "-"
           then temp-item.dgrupo3  = "N/A".
           if temp-item.dgrupo4 = "" or temp-item.dgrupo4 = "-"
           then temp-item.dgrupo4  = "N/A".
           if temp-item.dgrupo5 = "" or temp-item.dgrupo5 = "-"
           then temp-item.dgrupo5  = "N/A".
            

           temp-item.estacao = string(produ.etccod) + "-" + (if avail estac then  retira-acento(estac.etcnom) else "N/A") .
           temp-item.temporada = string(produ.temp-cod)  + "-" + (if avail temporada then retira-acento(temporada.tempnom) else "N/A") .
        ...
        */
           ...
           temp-item.caract2   = /* 10.05.19 - sempre vai  descontinuado
                                 if produ.catcod = 41
                                 then*/ "Descontinuado"
                                 /* 10.05.19
                                 else if vcar2 <> ""
           ...
           temp-item.subcaract2   = /* 10.05.19 - sempre vai descontinuado
                                 if produ.catcod = 41
    ...
    for each estab no-lock.
        ...
        THEN DO:
            ...
            for each mixmprod where mixmprod.procod = produ.procod and
                ...
                find first temp-mix where temp-mix.procod = produ.procod and
                                    temp-mix.etbcod = estab.etbcod
                                    no-error.
                if not avail temp-mix                    
                then do:
                    create temp-mix.
                    temp-mix.etbcod = estab.etbcod.
                    temp-mix.procod = produ.procod.
                end.
                temp-mix.grade = vgrade.
            end.      
        END.    
        /* mix Grade */
        ...
        THEN DO:
            ...
            create temp-sku.
                assign 
                    ...
                    temp-sku.item-origem = if estab.etbcod >= 500
                                           then ""
                                           else string(produ.procod).
                    temp-sku.local-origem = if estab.etbcod >= 500
                                            then ""
                                            else string(vemite).
                    temp-sku.empresa-origem = if estab.etbcod >= 500
                                              then ""
                                              else string(1).
                    temp-sku.forne-origem = if estab.etbcod >= 500
                                            then string(vemite)
                                            else "".
                    temp-sku.LEADTIME       = replace(string(vleadtime,">>>>>9.999"),".",",").
                    temp-sku.LEADTIMEDP     = replace(string(if (vleadtime * 0.10) < 1 then 1 
                                                           else (vleadtime * 0.10),">>>>>9.999"),".",",").

                    temp-sku.lote           = replace(string(
                                                    (if estab.etbcod >= 500
                    ...
                    temp-sku.unitizacao     =              replace(string(vpack,">>>>>>>>9.999"),".",",").
        ...
        THEN DO:
            ...
            assign 
                ...
                temp-stock.EstDisp        =  trim(replace(string(if vestatual > 0 then vestatual else 0,">>>>>>>9.999"),".",",")).
                temp-stock.EstCusto       =  trim(replace(string(vestcusto,">>>>>9.99"),".",",")).
                temp-stock.Estmargem       =  trim(replace(string(if vestvenda - vestcusto > 0
                                                              then vestvenda - vestcusto
                                                              else 0 , ">>>>>9.99"),".",",")).
            ...
            put stream SKU_ESTOQUE unformatted 
                 temp-stock.Item           vcp     
...
IF NOME-ARQUIVO[8] <> ""
THEN DO:    
    output stream SKU_ESTOQUE close.
END.    
...
for each temp-mix.
      ...
      put unformatted
                    string(temp-mix.procod) vcp
                    string(temp-mix.etbcod,"999") vcp
                    trim(replace(string(temp-mix.grade,">>>>>9"),".",","))
                    skip.
