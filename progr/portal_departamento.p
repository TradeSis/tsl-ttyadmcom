...
define temp-table tt-valores no-undo serialize-name 'departamento'
...
for each categoria no-lock:
    create tt-valores.
    assign tt-valores.codigo    = categoria.catcod
           tt-valores.descricao = fnremoveacento(categoria.catnom)
...
RUN PostRequest (INPUT 'produtos/integracaoDepartamento', 
                 INPUT lo-arquivo).
