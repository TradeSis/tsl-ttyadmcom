...
repeat:
    ...
    output to l:\relat\livger2.txt.
    /*output to /admcom/relat/livger-new.txt.*/

    
    for each produ where produ.catcod = categoria.catcod
                     and produ.procar <> "INATIVO"
                                 no-lock break by pronom.
             /*
        vx = vx + 1.
        if vx > 100 then leave.
               */
    ...
    varquivo = "l:\relat\livro2" + string(time).
    
    /*varquivo = "/admcom/relat/livro-new" + string(time).*/
   ...
   input from l:\relat\livger2.txt.
   /*input from /admcom/relat/livger-new.txt.*/
   ...
   repeat:
        ...
        then do:
            ...
            put produ.procod at 70 space(1)
                produ.pronom format "x(36)" space(1)
