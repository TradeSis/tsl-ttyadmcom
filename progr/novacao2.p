...
repeat:    
    ...
    */
     ...
     do vv = 1 to 5:
        ...
        for each tp-titulo use-index dt-ven:     
            ...
            display tp-titulo.etbcod
                    ...
                    tp-titulo.titsit
                    tp-titulo.titdtven
                    tp-titulo.titvlcob(total) 
                    vtitvlp 
                    vtot
                    with frame f2 centered down.*/
        ...
        ******/
        ...
        then do:
            if today <= 12/31/2008
            then vtitj = vtitj + (vtot + vtitj).
            else vtitj = vtitj + ((vtot + vtitj) * 0.10).
        end.
        if vplano = 3
    ...
    /********************
    message vjuro-ori valor_juro[1]
                    valor_juro[2] valor_juro[3] valor_juro[4] valor_juro[5]. 
        ****************/
...
procedure cal-juro:
    ...
    def var mjuro     as dec extent 5 init [0.05, 0.1, 0.15, 0.35, 0.4]. 
    def var vv    as int.
    def var vvalor_novacao as dec.

    vvalor_novacao = valor_novacao[1].

    do vv = 1 to 5:
        if vdia > 600
        then valor_juro[vv] = valor_juro[vv] / 15 .

        valor_novacao[vv] = valor_cobrado[vv] + valor_juro[vv].

        valor_juro[vv]    = valor_juro[vv] + (vvalor_novacao * mjuro[vv]).
...
/***
        ...
        then do:
            ...
            then valor_juro[vv] = valor_juro[vv].
            else.
                    ...
                    then do:
                        ...
                        then do:
                            find first tp-titulo where
                                tp-titulo.titnum = string(tp-contrato.contnum)
                                no-error.
                            if avail tp-titulo and tp-titulo.titpar > 30
                            then do:
                                /*** pre-corretiva 241271
                                message color red/wit
                             ""Contrato de NOVAÇÃO, operação não permitida.""
                                view-as alert-box.
                                next keys-loop.
                                ***/
                            end.    
