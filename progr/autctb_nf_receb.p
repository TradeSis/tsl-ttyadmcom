...
def var esqcom1         as char format "x(12)" extent 4
            initial ["Libera","Consulta","Alteracao","Inclusao"].
...
def buffer bautctb       for autctb.
    ...
    ***/
def var vsituacao as char format "x(15)".
...
repeat:
    ...
    if recatu1 = ?
    then find first autctb  where 
                    autctb.movtdc = ? no-error.
    else find autctb where recid(autctb) = recatu1.
    vinicio = yes.
    ...
    then do:
        ...
        do with frame f-inclui1 overlay row 6 centered side-label.
            ...
            find first forne where forne.forcod = vemite no-lock no-error.
            if not avail forne
            then do:
    ...
    find first forne where forne.forcod = autctb.emite no-lock. 
    if autctb.hora = 111
    then vsituacao = "Bloqueada".
    if autctb.hora = 222
    ...
    disp autctb.motivo format "x(80)"with frame f-mot 1 down
    no-label no-box color message row 20. 
    ...
    then vsituacao = "Liberada".
        display autctb.emite
                ...
                vsituacao
                    with frame frame-a.
        disp autctb.motivo with frame f-mot.
    ...
    repeat with frame frame-a:
        ...
        find autctb where recid(autctb) = recatu1.
        ...
        hide frame frame-a no-pause.
          ...
          then do:
            ...
            then do on error undo:
                if autctb.hora = 111
                then autctb.hora = 222.
                else if autctb.hora = 222
                then autctb.hora = 111.
            end.
