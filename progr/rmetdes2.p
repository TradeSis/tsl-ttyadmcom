...
for each wpag where wpag.wmod <> "" and
    ...
    find first modgru where modgru.modcod = wpag.wmod and 
                            modgru.mogsup <> 0 no-lock no-error.
    find first bmodgru where bmodgru.mogcod = modgru.mogsup no-lock no-error.
    if avail bmodgru then do:
    assign wpag.wgru = bmodgru.modcod
        ...
        then do:
            ...
            for each bwgru where bwgru.wset = wgru.wset and
                ...
                disp 
                    ...
                    val-sal   format "->>,>>>,>>9.99" column-label "Saldo" 
                    with frame f-disp3 down width 124.
        ...
        ***/
        ...
        disp setaut.setcod label "Setor"
         ...
         for each swgru where swgru.wset = wsetor.wsetcod and
            ...
            for each swpag where swpag.wgru = swgru.wmod and
                            swpag.wset = swgru.wset and
                            swpag.wano = swgru.wano and
                            swpag.wmes = swgru.wmes
                            no-lock:
                            
              assign val-salac = val-salac + swpag.wpag.
         ...
         for each wacu where wacu.wset = wsetor.wsetcod:
            ...
            if avail tt-totgru then do:
                assign val-salac = /*val-sal -*/ tt-totgru.tot 
                       vtotgeral = vtotgeral + val-salac.
            end.   
            ...
            disp 
                ...
                val-sal    format "->>,>>>,>>9.99" column-label "Saldo"
                val-salac  format "->>,>>>,>>9.99" column-label "A pagar"
            ...
            vtot-val-sal = vtot-val-sal + val-sal - val-deb.
        end.
        val-sal = 0.
        val-rec = 0.
        val-pag = 0.
        ...
        val-sal = v-wacumeta - val-pag + val-rec - val-deb.
        val-salac = vtotgeral.
        disp    "Totais" @ modgru.mognom
                val-pag @ wacu.wpag
                /*val-met @ wacu.wmet */
...
/*do vdta = vdtini to vdtfim:
    ...
    mes-ano = vmes + vano.*/*/
     ...
     for each bfmetadesp where  bfmetadesp.etbcod = setbcod and                     (if vsetcod > 0 then bfmetadesp.setcod = vsetcod else true) and
   ...
   /* disp "Processando... Aguarde! " vdt
    ...
    for each wmodal:
        ...
        for each banfin.titulo where 
                             ...
                             banfin.titulo.titsit =   "LIB" no-lock:
            ...
            find first swpag where 
                       ...
                       swpag.wano = year(banfin.titulo.titdtven) and
                       swpag.wmes = month(banfin.titulo.titdtven) no-error.
            if not avail swpag
            then do:
                /*next.*/
                create swpag.
                assign
                    ...
                    swpag.wano = year(banfin.titulo.titdtven) 
                    swpag.wmes = month(banfin.titulo.titdtven)  .
            end.
        ...
        for each fin.titluc where fin.titluc.empcod = wempre.empcod and
            ...
            find first swpag where 
                       ...
                       swpag.wano = year(fin.titluc.titdtven) and
                       swpag.wmes = month(fin.titluc.titdtven) no-error.
            if not avail swpag
            then do:
                /*next.*/
                create swpag.
                assign
                    ...
                    swpag.wano = year(fin.titluc.titdtven) 
                    swpag.wmes = month(fin.titluc.titdtven)  .
            end.
       ...
       for each banfin.titluc where banfin.titluc.empcod = wempre.empcod and
            ...
            find first swpag where 
                       ...
                       swpag.wano = year(banfin.titluc.titdtven) and
                       swpag.wmes = month(banfin.titluc.titdtven) no-error.
            if not avail swpag
            then do:
                /*next.*/
                create swpag.
                assign
                    ...
                    swpag.wano = year(banfin.titluc.titdtven) 
                    swpag.wmes = month(banfin.titluc.titdtven)  .
            end.
