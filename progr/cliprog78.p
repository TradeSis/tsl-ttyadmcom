...
def var vesc as char format "x(16)" extent 3
        init["",
             "Venda Prazo",
             ""].
...
output stream s-mostra to terminal.

def var rec-plani as recid.
def var varquivo as char.
if vindex = 2
then do:
    vnomarq = "venda-prazo-" + string(vmes,"99") + "-" 
                + string(vano,"9999") + "-" + string(time)
                + ".csv".
    varquivo = "/admcom/relat/" + vnomarq.
    output to value(varquivo).
    put "Filial;Data;Valor;Nome;CPF" skip.
    
    for each tabdac17 use-index indx3 where tabdac17.meslan = vmes and
                            tabdac17.anolan = vano and
                            tabdac17.tiplan = "EMISSAO" and 
                            tabdac17.sitlan = "LEBES"
                            no-lock.
        if tabdac17.vallan = 0 then next.
        disp stream s-mostra tabdac17.numlan with frame f-processa.
        pause 0.
        find fin.contrato where 
                fin.contrato.contnum = int(tabdac17.numlan) 
                no-lock no-error.
        rec-plani = ?.
    ...
    output close.
    varquivo = "l:~\relat~\" + vnomarq .
    message color red/with
    "Arquivo gerado:" skip
    varquivo
    view-as alert-box.
end.
output stream s-mostra close.
...
procedure ctb-novacoes:
    ...
    for each bestab no-lock:
        ...
        for each ninja.tabdicre where
                 ...
                 no-lock:
            assign
                rec-novacao = rec-novacao + tabdicre.recebimento_novacao
                . 
        end.
        for each ninja.tabdicem where
                 ninja.tabdicem.etbcod = bestab.etbcod and
                 ninja.tabdicem.datref = vdata 
                 no-lock:
            assign
                new-novacao = new-novacao + 
                        (tabdicem.novacao_lebes + tabdicem.novacao_fdrebes)
                acr-novacao = acr-novacao +
                        (tabdicem.acrescimo_novacao_lebes + 
                         tabdicem.acrescimo_novacao_fdrebes)
                new-lebes    = new-lebes + tabdicem.novacao_lebes
                acr-lebes    = acr-lebes + tabdicem.acrescimo_novacao_lebes
                new-nfdrebes = new-nfdrebes + tabdicem.novacao_fdrebes
