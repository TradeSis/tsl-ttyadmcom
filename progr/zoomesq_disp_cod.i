...
repeat with frame f2:
    for each {1} where {6} no-lock by {3} with frame f2 i = 1 to 13:
        array[i] = string({3},"x({7})") + string({2}).
        array-cod[i] = string({2}).
    end.
    display array.
    campo = array[1].
    display campo with frame f1.
    color display message /*white/black*/ array[1].
    i = 1.
    prompt-for campo with frame f1 editing:
        color display message /*white/black*/ array[i].
        ...
        color display normal /*black/cyan*/ array[i].
        hide message.
        ...
        if lastkey = keycode("CURSOR-UP")
            ...
            if i > 1
                ...
                for each {1} where ({6})
                               and string({3},"x({7})") + string({2}) < array[1]                             no-lock by string({3},"x({7})") + string({2})
                             descending j = 1 to 1:
                    aux = string({3},"x({7})") + string({2}).
        ...
        if lastkey = keycode("CURSOR-DOWN")
            ...
            if i < 13
                ...
                for each {1} where ({6})
                             and string({3},"x({7})") + string({2}) > array[13]                             no-lock by {3} j = 1 to 1:
                     aux = string({3},"x({7})") + string({2}).
                     aux-cod = string({2}).
                end.
                if j > 0
