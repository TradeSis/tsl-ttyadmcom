                ...
                then do with frame frame-a on error undo.
                    ...
                    then do:
                        ...
                        then do:
                            ...
                            update v-val-min-compra label "Valor de "  "até"                                      v-val-max-compra no-label
                               with frame fval row 12 overlay
                                     side-labels centered title "Insira uma faixa de valores de compra.".
                            hide frame fval no-pause.

                            update v-qtd-min-compra label "Valor de "  "até"
                                   v-qtd-max-compra no-label
                                        with frame fqtd row 12 overlay
                                     side-labels centered title "Insira a quantidade comprada mínima e máxima para o filtro.".
...
procedure p-filtro-considerar.
    ...
    if tt-filtro.descricao = "ELEGIVEIS CARTAO"
    then do:
        DISP "Clientes com prestações em aberto e ou quitadas"
                with frame f-car.
        repeat on error undo:
            ...
            update tt-filtro.log at 1 format "Sim/Nao" 
                        label "Considerar Novação?"  with frame f-car.
    ...
    then do:
        ...
        then do:
          ...
          update vval-aumento
                  label "Valor mínimo de Aumento" format ">>>>9.99"
                  with frame f-aument2 row 12 overlay
                      side-labels                                                                 centered title "Digite o Valor mínimo de aumento do Limite.".

            
        end.
        else do:     /* Percentual de Aumento */
          update vperc-aumento
                  label "Perc. mínimo de Aumento" format ">>>>9.99 %"
                  with frame f-aument row 12 overlay
                        side-labels
            centered title "Digite o Percentual mínimo de aumento do Limite.".
        end.
