...
def new shared temp-table tt-principal no-undo
    ...
    field titdes like fin.titulo.titdes
...
def buffer titulo for SC2015.
def buffer btitulo for SC2015.

for each titulo use-index INDX2  where
    ...
    if titulo.titdtmovref > vdatref or
       titulo.titdtmovref = ?
    then next.
    ...
    then do:
        p-catcod = 0.
        find first fin.contnf where contnf.etbcod = titulo.etbcod and
        ...
        for each btitulo  where
             ...
             btitulo.titdtemi <= vdatref and
             btitulo.clifor = titulo.clifor and
             btitulo.titnum = titulo.titnum and
             ((btitulo.titdtpag > vdatref and
             btitulo.titsit = "PAG") or
             btitulo.titsit = "LIB")
             no-lock.
                        
            if btitulo.titdtmovref > vdatref or
               btitulo.titdtmovref = ?
            then next.
            ...
            then*/ do:
                assign
                    p-principal = p-principal + 
                                  (if v-principal > btitulo.titvlcob
                                   then btitulo.titvlcob else v-principal)
...
procedure principal-renda:
    ...
    then do:
        ...
        /*find first btitulo where btitulo.empcod = titulo.empcod and
                                  btitulo.titnat = titulo.titnat and
                                  btitulo.modcod = titulo.modcod and
                                  btitulo.etbcod = titulo.etbcod and
                                  btitulo.clifor = titulo.clifor and
                                  btitulo.titnum = titulo.titnum and
                                  btitulo.titpar = titulo.titpar                                                 no-lock no-error.
        if avail btitulo
        then*/ do:
            
            run retorna-principal-acrescimo-sc2015.p
                                        (input recid(titulo),
                                         output p-principal,
                                         output p-acrescimo,
                                         output p-seguro,
