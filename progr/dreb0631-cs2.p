...
def var valortotal like plani.platot init 0.
def var segurototal as int init 0.
...
def temp-table wfcomis field etbcod       like estab.etbcod
                       ...
                       field acre         as dec
                       field qtdseguro as int.  

def var totseguro as int.
...
repeat:
    ...
    tipo = 0.
    update vtprcod  colon 20 with title " Relatorio vendas de seguro ".
    tipo = vtprcod.
    ...
    */
    ...
    do vdt = vdtini to vdtfin:
        ...
        for each estab where if vetbcod = 0
            ...
            find first movim where movim.etbcod = plani.etbcod and
                                   ...
                                   (movim.procod = 559910 or
                                    movim.procod = 559911 or
                                    movim.procod = 578790 or
                                    movim.procod = 579359 )
                                    no-lock no-error.
            ...
            ***/
            ...
            if vcatcod = 2 and val_acr > 0
            then val_acr = val_acr * 
                        (plani.seguro / (plani.platot - plani.vlserv)).
            else val_acr = 0.
            
            valtotal = valtotal + plani.seguro + val_acr.
            ...
            */
            ...
            then do:
                ...
                assign wfcomis.etbcod = wf-com.etbcod
                       ...
                       wfcomis.qtdseguro = wfcomis.qtdseguro + 1 
    ...
    find tippro where tippro.tprcod = vtprcod no-lock no-error.
    for each wfcomis:
    valortotal = valortotal + wfcomis.vtot.
    segurototal = segurototal + wfcomis.qtdseguro.
    ...
    then do:
        if opsys = "UNIX"
        then varquivo = "/admcom/relat/drebseguro" + string(time).
        else varquivo = "l:~\relat~\drebseguro" + string(time).
        {mdadmcab.i &Saida     = "value(varquivo)"
                ...
                &Tit-Rel   = """COMISSAO VENDA SEGURO PERIODO DE "" +
        ...
        for each wfcomis break by wfcomis.etbcod 
            ...
            if first-of(wfcomis.etbcod)
            then do:
                valormeta = 0.
            find first tvendfil where
                   tvendfil.anoref = year(vdtfin) and
                   tvendfil.mesref = month(vdtfin) and
                   tvendfil.etbcod = wfcomis.etbcod
                    no-lock no-error.
            ...
            then do:
                find first tabaux where
                     ...
                     tabaux.nome_campo = string(wfcomis.etbcod,"999") +
                            ";" + string(month(vdtfin),"99")
                    no-lock no-error.
                if avail tabaux and
                   num-entries(tabaux.valor_campo,";") > 1
                then do:
                    valormeta = int(entry(2,tabaux.valor_campo,";")).
                    if not avail tvendfil or
                          tvendfil.moda >                                        int(entry(1,tabaux.valor_campo,";"))
                    then valormeta = valormeta / 
            ...
            disp wfcomis.etbcod column-label "Fil"
                 tippro.tprnom  column-label "Tipo produto" format "x(12)"
                 wfcomis.vencod column-label "Cod"
                 func.usercod   column-label "Folha" when avail func
                 func.funnom    format "x(15)" when avail func
                 wfcomis.vtot   column-label "Valor" 
                 wfcomis.qtdseguro column-label "Qtd!Seguro"
                 valormeta      column-label "Meta"
                 with frame f-disp1 down width 110.
        ...
        message "Valor total: "valortotal.
        message "Seguro total: "segurototal.
        output close.
        if opsys = "UNIX"
        then do:
    ...
    then next.
    if opsys = "UNIX"
    then varquivo = "/admcom/import/folha/folha-seguro" + string(vetbcod,"99") 
                        + ".txt".
    else varquivo = "m:~\folha-seguro" + string(vetbcod,"99") + ".txt".            
    ...
    for each wfcomis break by wfcomis.etbcod
        ...
        totseguro = totseguro + wfcomis.qtdseguro.
        ...
        then do:
            ...
            then do:
                ...
                if avail tabaux and
                   num-entries(tabaux.valor_campo,";") > 1
                then valormeta = int(entry(2,tabaux.valor_campo,";")).
            end.
            if wfcomis.tprcod  = 2
            then do:
                find first tabaux where
             ...
             */
             put substring(func.usercod,1,5) format "99999"
                        ...
                        string(year(vdtfin),"9999") format "9999"
                        "        " format "x(09)" 
                        totven format "999999999.99" 
                        totseguro format "99999"
                        valormeta format "99999"
                        skip.
