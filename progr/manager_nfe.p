...
def new shared var sfusohr as char.
sfusohr = fusohr().
...
if avail tt-plani and tt-plani.etbcod = 700
then do:
    message "A Filial 200 somente pode emitir NFE pelo Sistema Ábacos!"
            "Impossível Continuar." view-as alert-box.
    undo, return.
end.

if (par-operacao <> "envio" and
    par-operacao <> "estorno" and
    par-operacao <> "3102"  and
    par-operacao <> "5927") or
   par-operacao = ?  
then do.
    run piscofins.p.
...
case par-operacao:    
    ...
    then do:
/***
        if par-operacao = "5102_j"
        then vmovimimp = yes.
***/
        run nfe_5102_j.p(output sresp, output vrec-nota) no-error.
    end.

    when "1202"   /*** DEVOLUCAO DE VENDA  ***/  /*[[]]*/
    then do:
        run nfe_1202.p(output sresp, output vrec-nota) no-error.    
    end.

    when "1202_310"   /*** DEVOLUCAO DE VENDA  ***/
    then do.
        run nfe_1202_310.p(input p-recid, output sresp,
                           output vrec-nota) no-error.
    end.

    when "5949_e"   /*** ESTORNO DE NF OUTRAS ENTRADAS  ***/  /*[[]]*/
    then do:
        run nfe_5949_e.p(output sresp, output vrec-nota) no-error.
    end.

    when "5949_c"   /*** ESTORNO DE NF OUTRAS ENTRADAS  ***/
    then do:
        run loj/nfe_5949_c.p(output sresp, output vrec-nota) no-error.
    end.

    when "1949_e"   /*** ESTORNO DE NF OUTRAS SAIDAS  ***/      /*[[]]*/
    then do:
        run nfe_1949_e.p(output sresp, output vrec-nota) no-error.
    end.

/***
    when "5152"   /*** TRANSFERENCIAS  ***/
    then do:
        run nfe_5152.p(output sresp, output vrec-nota) no-error.
    end.
***/
    when "1152_e"   /*** ESTORNO TRANSFERENCIAS  ***/
    then do:
        run nfe_1152_e.p(output sresp, output vrec-nota) no-error.
    end.

    when "5949"   /*** OUTRAS SAÍDAS  ***/
    then do:
        run nfe_5949.p(output sresp, output vrec-nota) no-error.
    end.
    
    when "1949_l"   /*** OUTRAS ENTRADAS  ***/
    then do:
        run nfe_1949_l.p(output sresp, output vrec-nota) no-error.
    end.

    when "1949_c"   /*** OUTRAS ENTRADAS  ***/
    then do:
        run nfe_1949_c.p(output sresp, output vrec-nota) no-error.
    end.

    when "1915"   /*** RETORNO DE CONSERTO   ***/
    then do:
        run nfe_1915.p(output sresp, output vrec-nota) no-error.
    end.
    
    when "1949"   /*** OUTRAS ENTRADAS   ***/
    then do:
        run /*** loj/ ***/ nfe_1949.p(output sresp,
                       output vrec-nota) no-error.
    end.
...
/***
        vmovimimp = yes.
***/
        run nfe_5102.p(output sresp, output vrec-nota) no-error.
    end.

    when "5202"   /*** DEVOLUCAO DE COMPRAS  ***/
    then do:
        run nfe_5202.p(output sresp, output vrec-nota) no-error.
    end.

    when "5152" or /*** TRANSFERENCIAS  ***/
    when "5552"    /*** TRANSFERENCIA DE ATIVO IMOBILIZADO  ***/
    then do:
        run nfe/monta_nfe.p (output sresp, output vrec-nota) no-error.
        /*** run nfe_5552.p(output sresp, output vrec-nota) no-error. ***/
    end.

    when "5555"
    then do:
        run nfe_5555.p(output sresp, output vrec-nota) no-error.
    end.
    
    when "5557"   /*** TRANSFERENCIA DE USO E CONSUMO  ***/
    then do:
        run nfe_5557.p(output sresp, output vrec-nota) no-error.
    end.
    
    when "5602"   /*** TRANSFERENCIA DE SALDO CREDOR DE ICMS  ***/
    then do:
        run nfe_5602.p(output sresp, output vrec-nota) no-error.
    end.
    
    when "5901"   /*** REMESSA PARA INDUSTRIALIZAÇÃO ***/
    then do:
        run nfe_5901.p(output sresp, output vrec-nota) no-error.
    end.

    when "5915"   /*** REMESSA PARA CONSERTO ***/
    then do:
        run nfe_5915.p(output sresp, output vrec-nota) no-error.
    end.

    when "os_1949_l"   /*** OUTRAS ENTRADAS  ***/
    then do:
        run loj/nfeos_1949_l.p(output sresp, output vrec-nota) no-error.
    end.
    when "os_5915"   /*** REMESSA PARA CONSERTO ***/
    then do:
        run nfe_os_5915.p(output sresp, output vrec-nota) no-error.
    end.
    when "os_5152"   /*** TRANSFERENCIA - OS ***/
    then do:
         run nfe_os_5152.p(output sresp, output vrec-nota) no-error.
    end.

    when "5411"   /*** ??? ***/
    then do:
         run nfe_5411.p(output sresp, output vrec-nota) no-error.
    end.
    when "1603"   /*** ??? ***/
    then do:         
         run nfe_1603.p(output sresp, output vrec-nota) no-error.
    end.

    when "5603"   /*** ??? ***/
    then do:
         run nfe_5603.p(output sresp, output vrec-nota) no-error.
    end.

    when "devipi_5202" /*** DEVOLUÇÃO IPI ***/
    then do:
         run nfe_devipi_5202.p(output sresp, output vrec-nota) no-error.
    end.
    when "devipi_5411" /*** DEVOLUÇÃO IPI ***/
    then do:
         run nfe_devipi_5411.p(output sresp, output vrec-nota) no-error.
    end.
    when "devipi_1603" /*** DEVOLUÇÃO IPI ***/
    then do:    
         run nfe_devipi_1603.p(output sresp, output vrec-nota) no-error.
    end.

    when "wms_5152" /*** ???? ***/
    then do:
         run nfe_wms_5152.p(output sresp, output vrec-nota) no-error.
    end.
    when "wms_alcis_5152"
    then do:
        run nfe_wms_alcis_5152.p(output sresp, output vrec-nota) no-error. 
    end.
    when "wms_ebljh_5152"
    then do:
        run nfe_wms_ebljh_5152.p(output sresp, output vrec-nota) no-error. 
    end.
    when "5905" /*** ???? ***/
    then do:
         run nfe_5905.p(output sresp, output vrec-nota) no-error.
    end.

    when "devipi_5603" /*** DEVOLUÇÃO IPI ***/
    then do:
         run nfe_devipi_5603.p(output sresp, output vrec-nota) no-error.
    end.

    when "5551_j" /*** VENDA ATIVO IMOBILIZADO PESSOAO JURIDICA ***/
    then do:
         run nfe_5551_j.p(output sresp, output vrec-nota) no-error.
    end.

    when "995_5152" /***  ***/
    then do:
         run nfe_995_5152.p(output sresp, output vrec-nota) no-error.
    end.

    when "1949_f"   /*** OUTRAS ENTRADAS MUDANÇA EMDEREÇO FILIAL ****/
    then do:
        run nfe_1949_f.p(output sresp, output vrec-nota) no-error.
    end.

    when "5923"   /*** REMESSA A ORDEM DE TERECIROS ****/
    then do:
        run nfe_5923.p(output sresp, output vrec-nota) no-error.
    end.
    ...
    when "3102"  /**** IMPORTAÇÃO *******/
    then do:
        run nfe_3102.p(output sresp, output vrec-nota).
    end.
    when "5502_j"
    then do:
        run nfe_5502_j.p(output sresp, output vrec-nota).
    end.
    when "5502_je"
    then do:
        run nfe_5502_je.p(output sresp, output vrec-nota).
    end.     
    when "5927"     /*** Ajuste baixa de estoque ***/
    then do:
        run nfe_5927.p(output sresp, output vrec-nota).
    end.
    when "ESTORNO" /*** ESTORNO NOTA FISCAL ***/
    then do:
        run nfe_estorno.p(output sresp, output vrec-nota).
        vmovimimp = yes.
    end.                 
end case.    
...
**/
if vmovimimp
then run nfe-icms-efetivo.p (vrec-nota). /* #1 */
...
p-valor = "".
run le_tabini.p (A01_infnfe.emite, 0,
            "NFE - VERSAO", OUTPUT p-valor) .

/*message "1111 " p-valor. pause.
  */                     
if p-valor = "4.00"
then run arq_xml_nfe_400.p (input recid(A01_infnfe),
                   input v-idamb, 
                   output varquivo).    
else run arq_xml_nfe_new.p (input recid(A01_infnfe),
                   input v-idamb, 
                   output varquivo).                       

disp skip(1)
    "NFE " A01_infnfe.serie no-label a01_infnfe.numero no-label
    " GERADA NO SISTEMA!!"
    skip(1)
...
then do:
    ...
    v-ok = no.                         
                                /* Manda 9999 para não conflitar com
                                   retorno "1 - Aguardando Geracao da NFE" */  end.
    ...
    repeat on endkey undo:
        ...
        case vnum-erro:
            ...
            otherwise do:
                ...
                then do:
                    if program-name(2) begins "emite-NFe-ajuste-estoque"
                    then.
                    else do:
                    if sremoto
                    then run danfe_nfe_remoto.p (input recid(A01_InfNFe)).
                    else run p-imprime-danfe (input recid(A01_InfNFe)).
                    end.
                    assign v-ok = yes.
                end.    
                else assign v-ok = no.
...
procedure p-imprime-danfe:
    ...
    if avail a01_infnfe
    then find first bf-estab where bf-estab.etbcod = a01_infnfe.etbcod
                                    no-lock no-error.
    ...
    if avail bf-estab
        and bf-estab.tipoloja = "Normal"
        and bf-estab.usap2k = true
    then do:
        put unformatted
            "|"
            string(A01_infnfe.etbcod) skip.
    end.
