...
def temp-table tt-pdvd2016 like pdvd2016.
...
form pdvd2016.Faixa_risco column-label "Faixa" format "x(5)"
     pdvd2016.descricao_faixa no-label format "x(12)"  
     pdvd2016.saldo_curva column-label "Curva"      format ">>>>>>>>9.99"
     pdvd2016.pctprovisao column-label "%"          format ">>>9.99"
     pdvd2016.provisao    column-label "Provisao"   format ">>>>>>>>9.99"
     pdvd2016.principal   column-label "Principal"  format ">>>>>>>>9.99"
     pdvd2016.renda       column-label "Renda"      format ">>>>>>>>9.99"
...
for each pdvd2016 where pdvd2016.data_visao = vdatref     and
                        pdvd2016.tipo_visao = vtpfaixa /*vtipo_visao*/ and
                        pdvd2016.modo_visao = vmodo_visao and
                        pdvd2016.modalidade = "CRE" and
                        pdvd2016.cobcod     = ? and
                        pdvd2016.categoria  = ? and
                        pdvd2016.etbcod = 0
                        no-lock:
    assign
        tsaldo_curva = tsaldo_curva + pdvd2016.saldo_curva
        tprovisao    = tprovisao + pdvd2016.provisao
        tprincipal   = tprincipal + pdvd2016.principal
        trenda       = trenda + pdvd2016.renda
        .                
end.                    
...
l1: repeat:
    ...
    {sklclstb.i  
        ...
        &file  = pdvd2016  
        &help  = " "
        &cfield = pdvd2016.faixa_risco
        &noncharacter = /* */  
        &ofield = " pdvd2016.descricao_faixa
                    pdvd2016.saldo_curva
                    pdvd2016.pctprovisao
                    pdvd2016.provisao
                    pdvd2016.principal
                    pdvd2016.renda
                    "  
        &aftfnd1 = " "
        &where  = " pdvd2016.data_visao = vdatref and
                    pdvd2016.tipo_visao = vtpfaixa  and
                    pdvd2016.modo_visao = vmodo_visao and 
                    pdvd2016.modalidade = ""CRE"" and
                    pdvd2016.cobcod = vcobcod and
                    pdvd2016.categoria = vcatcod and
                    pdvd2016.etbcod = 0 "
        &aftselect1 = " 
                        run aftselect.
...
procedure aftselect:
    ...
    THEN DO:
        sresp = no.
        message 
        "Confirma gerar relatorio analitico faixa" pdvd2016.faixa_risco 
        update sresp.
        if sresp
        then do: 
            /*if vtipo_visao = "CONTABIL"
            then run relatorio-faixa-analitico-outros(pdvd2016.faixa_risco).
            else*/ run relatorio-faixa-analitico(pdvd2016.faixa_risco).
        end.
    END.
...
procedure relatorio:
    ...
    def var vencer90 like pdvd2016.vencer_90 .
    def var vencer360 like pdvd2016.vencer_360.
    def var vencer1080 like pdvd2016.vencer_1080.
    def var vencer1800 like pdvd2016.vencer_1800 .
    def var vencer5400 like pdvd2016.vencer_5400  .
    def var vencer9999 like pdvd2016.vencer_9999   .
    ...
    for each pdvd2016 where 
             pdvd2016.data_visao = vdatref and
             pdvd2016.tipo_visao = vtipo_visao and
             pdvd2016.modo_visao = vmodo_visao and
             pdvd2016.modalidade = "CRE" and
             pdvd2016.cobcod     = vcobcod and
             pdvd2016.categoria  = vcatcod and
             pdvd2016.faixa_risco <> "" no-lock:
        
        /*
        find first tbcntgen where
                   tbcntgen.tipcon = 13 and
                   tbcntgen.campo1[1] = pdvd2016.faixa_risco 
                   no-lock no-error.
        if avail tbcntgen
        then pdvd2016.descricao_faixa = tbcntgen.numini + "-" +
                            tbcntgen.numfim.
        */

        if pdvd2016.faixa_risco < "H"
        then assign
            vencidos = pdvd2016.vencido
            vencer90 = pdvd2016.vencer_90
            vencer360 = pdvd2016.vencer_360
            vencer1080 = pdvd2016.vencer_1080
            vencer1800 = pdvd2016.vencer_1800
            vencer5400 = pdvd2016.vencer_5400 
            vencer9999 = pdvd2016.vencer_9999
            .
        else assign
            vencidos = pdvd2016.vencido +
                       pdvd2016.vencer_90 + pdvd2016.vencer_360 +
                       pdvd2016.vencer_1080 + pdvd2016.vencer_1800 +
                       pdvd2016.vencer_5400 + pdvd2016.vencer_9999
            vencer90 = 0
            vencer360 = 0
            vencer1080 = 0
            vencer1800 = 0
        ...
        disp pdvd2016.faixa_risco column-label "Faixa"
             pdvd2016.descricao_faixa no-label   format "x(12)"
             ...
             format ">>>,>>>,>>9.99"  (total)
             vencer5400 column-label "Vencer!5 a 16 anos"
             format ">>>,>>>,>>9.99"  (total)
             vencer9999 column-label "Vencer!+ 16 anos"
             format ">>>,>>>,>>9.99"  (total)
             pdvd2016.saldo_curva  column-label "Saldo Curva"
             format ">>,>>>,>>>,>>9.99"  (total)
             pdvd2016.provisao  column-label "Provisao"
             format ">>>,>>>,>>9.99"  (total)
             pdvd2016.pctprovisao column-label "%"
             pdvd2016.principal(total) column-label "Principal"
             format ">>>,>>>,>>9.99"
             pdvd2016.renda(total) column-label "Renda"
             format ">>>,>>>,>>9.99"
    ...
    for each fn-risco NO-LOCK:
        ...
        ****/
        ...
        disp fn-risco.risco column-label "Faixa"
             ...
             format ">>>,>>>,>>9.99"  (total)
             fn-risco.ven-5400 column-label "Vencer!5 a 16 anos"
             format ">>>,>>>,>>9.99"  (total)
             fn-risco.ven-9999 column-label "Vencer!+ 16 anos"
             format ">>>,>>>,>>9.99"  (total)
...
procedure relatorio-faixa-analitico:
    ...
    for each GClasOPC where
                   ...
                   GClasOPC.riscon  = pdvd2016.faixa_risco and
                   GClasOPC.situacao = ""
                   no-lock:
        ...
        then vdtven = titulo.titdtven.
            ...
            find first sc2016 where
                   sc2016.clifor = GClasOPC.clifor and
                   sc2016.titnum = GClasOPC.titnum and
                   (sc2016.titsit = "LIB" or
                   (sc2016.titsit = "PAG" and
                    sc2016.titdtpag > vdatref))
                   no-lock no-error.
            if avail sc2016
            then vdtven = sc2016.titdtven.
        end.
        vprovisao = (GClasOPC.vencido + GClasOPC.vencer) *                             (pdvd2016.pctprovisao / 100).
    ...
    for each GClasOPC where
                   ...
                   /*GClasOPC.catcod  => 0 and*/
                   GClasOPC.modcod  = "CRE" and
                   GClasOPC.riscli  = pdvd2016.faixa_risco and
                   GClasOPC.situacao = ""
                   no-lock:
        find clien where clien.clicod = GClasOPC.clifor no-lock no-error.
        if avail clien
    ...
    **************/
    ...
    for each visdev16 where visdev16.data_visao = vdatref  and
                            visdev16.tipo_visao = vtpvisao and
                            visdev16.risco_cliente = p-risco and
                            visdev16.situacao = ""
                            no-lock:

        find clien where clien.clicod = visdev16.clifor no-lock no-error.
        if avail clien
        then assign
                vclinom = clien.clinom
                vciccgc = clien.ciccgc.
        ...
        for each titulo where
                 titulo.empcod = visdev16.empcod and
                 titulo.titnat = visdev16.titnat and
                 titulo.modcod = visdev16.modcod and
                 titulo.titdtemi = visdev16.titdtemi and
                 titulo.etbcod = visdev16.etbcod and
                 titulo.clifor = visdev16.clifor and
                 titulo.titnum = visdev16.titnum and
                 (titulo.titsit = "LIB" or
                 (titulo.titsit = "PAG" and
                  titulo.titdtpag > vdatref)) 
                 no-lock: 
            if visdev16.qtdatr > 0 /*220*/
            then vdtven = titulo.titdtven - visdev16.qtdat.
        ...
        /****
        for each titadd16 where
                 titadd16.empcod = visdev16.empcod and
                 titadd16.titnat = visdev16.titnat and
                 titadd16.modcod = visdev16.modcod and
                 titadd16.titdtemi = visdev16.titdtemi and
                 titadd16.etbcod = visdev16.etbcod and
                 titadd16.clifor = visdev16.clifor and
                 titadd16.titnum = visdev16.titnum and
                 (titadd16.titsit = "LIB" or
                 (titadd16.titsit = "PAG" and
                  titadd16.titdtpag > vdatref)) 
                 no-lock: 
            if visdev16.qtdatr > 0 /*220*/
            then vdtven = titadd16.titdtven - visdev16.qtdat.
            else vdtven = titadd16.titdtven.
            vtitnum = titadd16.titnum.
            vtitvlcob = titadd16.titvlcob.
            vdatraso = vdatref - vdtven.
            disp vclinom 
                 vciccgc 
                 vtitnum 
            ...
            down with frame fd2.
            vtotal = vtotal + titadd16.titvlcob.
        end.
        ***/
    end.    
    ...
    *********************/
    ...
    /*
    varq = "/admcom/Contabilidade/clientes2016/saldo-clientes-"
                    + string(vdatref,"99999999") + "-risco-"
                                    + p-risco + ".csv".
    */                                
    ...
    for each visdev16 where visdev16.data_visao = vdatref  and
                            visdev16.tipo_visao = vtpvisao and
                            visdev16.risco_cliente = p-risco and
                            visdev16.situacao <> "EXCECD"
                            no-lock:

        if visdev16.total_vencido = 0 and
           visdev16.total_vencer = 0
        then next.    
        /*find clien where clien.clicod = visdev16.clifor no-lock no-error.
        if avail clien
        then*/ assign
                vclinom = visdev16.nome
                vciccgc = visdev16.cpf.
        /*else assign
                vclinom = "NOME"
                vciccgc = "CPF".
        */
        vok = no.
        if visdev16.vencimento_arquivo = ?
        then do:
        for each titulo where
                 titulo.empcod = visdev16.empcod and
                 titulo.titnat = visdev16.titnat and
        ...
        then for each titadd16 where
                 ...
                 (titadd16.titsit = "PAG" and
                  titadd16.titdtpag > vdatref)) 
                 no-lock:
            if titadd16.titvlcob > 0  and
               titadd16.titdtven <> ?  and
               titadd16.titdtven < 01/01/2030 and
               titadd16.titdtven > 01/01/1990
            then do:       
                if visdev16.qtdatr > 0 /*220*/
                then vdtven = titadd16.titdtven - visdev16.qtdat.
                else vdtven = titadd16.titdtven.
                valchar = string(titadd16.titvlcob).
                valchar = replace(valchar,",","").
                valchar = replace(valchar,".",",").
                put unformatted 
                vclinom 
        ...
        ***/
            ...
            vdtven = visdev16.vencimento_arquivo.
            valchar = string(visdev16.total_vencido + 
                             visdev16.total_vencer).
                valchar = replace(valchar,",","").
                valchar = replace(valchar,".",",").
                put unformatted 
                vclinom 
                ...
                vtotal = vtotal + 
                            visdev16.total_vencido + visdev16.total_vencer.
