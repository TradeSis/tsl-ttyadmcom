...
def var t5 as dec decimals 4 format ">>,>>>,>>9.9999".
...
def var d5 as dec decimals 4 format ">>,>>>,>>9.9999".
def var dcan as dec.
...
for each estab where estab.etbcod < 200 /*and
    ...
    for each tt-tabecf where tt-tabecf.equipa > 0:
        ...
        do vdata = vdti to vdtf:
            ...
            assign
                    ...
                    tt-caixa.icms  = bmapctb.t05 * .18
                    tt-caixa.idesc = bmapctb.vldes
                    .
        end.
    end.
    ...
    do vdata = vdti to vdtf:
        ...
        for each plani where plani.movtdc = 5 and
            ...
            if (substr(plani.ufemi,1,1) <> "B" and
               substr(plani.ufemi,1,1) <> "E" 
               and length(plani.ufemi) > 30)
               or plani.modcod = "CAN" 
            then next.
            ...
            for each movim where movim.etbcod = plani.etbcod and
                ...
                */
                ...
                then  t4 = t4 + round(vmovpc * movim.movqtm,2)
                                - movim.movdes.
                if movalicms = 18
                then t5 = t5 + round(vmovpc * movim.movqtm,2)
                            - movim.movdes.
                ...
                for each movim where movim.etbcod = plani.etbcod and
                    ...
                    */
                    ...
                    then  d4 = d4 + round(vmovpc * movim.movqtm,2)
                                        - movim.movdes.
                    if movalicms = 18
                    then  d5 = d5 + round(vmovpc * movim.movqtm,2)
                                        - movim.movdes.
                    ...
                    tt-plani.icms = tt-plani.icms +
                            (round(vmovpc * movim.movqtm,2) * .18).
...
for each tt-caixa:
    /*message tt-caixa.icms  tt-caixa.cicms.
            pause.*/
    if tt-caixa.gti <= 0 or 
       ...
       tt-caixa.icms <> tt-caixa.cicms
       /*or tt-caixa.tcan <> tt-caixa.ccan*/ 
    then do:
        /*
        message tt-caixa.t03 tt-caixa.c03 . pause.
        */
        /*message tt-caixa.icms  tt-caixa.cicms.
        pause.*/
        tt-caixa.difer = yes.
        ...
        for each tt-plani where
            ...
            if (tt-plani.platot - tt-plani.seguro) = 
                    (tt-caixa.t01 - tt-caixa.c01) or
               (tt-plani.platot - tt-plani.seguro) =     
                     (tt-caixa.t05 - tt-caixa.c05) or
               (tt-plani.platot - tt-plani.seguro) = 
...
for each tt-caixa where difer = yes no-lock.
    ...
    disp tt-caixa.etbcod column-label "Fil"
         ...
         */
         tt-caixa.t05       column-label "17%R" format ">>,>>9.99"
         tt-caixa.c05       column-label "17%C" format ">>,>>9.99"
         tt-caixa.c05 - tt-caixa.t05  column-label "Dif17%"
                  format "->>,>>9.99"
...
def var esqcom1         as char format "x(15)" extent 5
    initial ["","  CONSULTAR","  ALTERAR","  BUSCAR","  RECUPERAR"].
def var esqcom2         as char format "x(15)" extent 5
            initial ["","RELATORIO","VENDAS","",""].
...
form tt-caixa.etbcod label "Fil"
     ...
     tt-caixa.serie  format "x(20)"
     tt-caixa.t01    label "17%"  format ">>>>9.99"
     tt-caixa.t05    label "18%"  format ">>>>9.99"
     tt-caixa.tsub   label "ST"   format ">>>>9.99"
     tt-caixa.tcan   label "CAN"  format ">>>>9.99"
...
procedure aftselect:
    ...
    THEN DO on error undo:
        ...
        disp tt-caixa.serie skip(1)
             ...
             skip
             tt-caixa.t05  - tt-caixa.c05   format "->>>,>>9.99"
                          label "                18%"
             tt-caixa.d05 label "Rec 18%"
             skip
             tt-caixa.tsub - tt-caixa.csub  format "->>>,>>9.99"
             label "    Sub. Tributaria"
             tt-caixa.dsub label "Rec  ST"
             with frame f-con1
             side-label overlay row 4 centered
    ...
    THEN DO:
        sresp = yes.
        message "Cancelados?" update sresp.
        if sresp
        then RUN valZcup45.p (tt-caixa.etbcod, tt-caixa.datmov, tt-caixa.equip).
        else RUN valZcup4.p (tt-caixa.etbcod, tt-caixa.datmov, tt-caixa.equip).
    END.
    if esqcom2[esqpos2] = "RELATORIO"
    THEN DO on error undo:
        RUN relatorio.
    END.
    if esqcom2[esqpos2] = "VENDAS"
    THEN DO on error undo:
        run recplani.p( tt-caixa.etbcod, tt-caixa.datmov, tt-caixa.equip,
                        tt-caixa.serie).
                        pause.
    END.
...
for each tt-caixa.
    ...
    */
    ...
    then do:
        disp     
         "18%" @ valiq at 20 
         tt-caixa.t05  @  tt-caixa.t01
         tt-caixa.ti05 @  tt-caixa.ti01
         tt-caixa.di05 @  tt-caixa.di01
         skip.
    end.
    if di04 <> 0
    then do:
...
procedure grava:
    find first tt-caixa where
        ...
        assign
            ...
            tt-caixa.c05 = tt-caixa.c05 + t5
            tt-caixa.csub = tt-caixa.csub + t4
            tt-caixa.ccan = tt-caixa.ccan + vl-can
            tt-caixa.cicms = tt-caixa.cicms + (t5 * .18)
            tt-caixa.cdesc = tt-caixa.cdesc + plani.descprod
            tt-caixa.d01 = tt-caixa.d01 + d1
            tt-caixa.d02 = tt-caixa.d02 + d2
            tt-caixa.d03 = tt-caixa.d03 + d3
            tt-caixa.d05 = tt-caixa.d05 + d5
            tt-caixa.dsub = tt-caixa.dsub + d4
            tt-caixa.dcan = tt-caixa.dcan + dcan
            tt-caixa.dicms = tt-caixa.dicms + (d5 * .18)
            tt-caixa.ddesc = tt-caixa.ddesc + plani.descprod
            .
end procedure.
