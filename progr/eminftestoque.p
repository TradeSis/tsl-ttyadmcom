...
def temp-table tt-arq
    ...
    update vetb-emite label "Filial emitente" .
    find first estab where estab.etbcod = vetb-emite no-lock.
    disp estab.etbnom no-label.         
    update vetb-desti at 2 label "Filial destino" .
    ...
    then do:
        sresp = no.
        message "Confirma ultima NFe emitida?" update sresp.
    ...
    varq-est = "/admcom/work/estoque_" + string(vetb-emite,"999") + ".d".
    ...
    vnfs = int(substr(string(vmovseq / 300,">>9.999"),1,3)).
    if int(substr(string(vmovseq / 300,">>9.999"),5,3)) > 0
    then vnfs = vnfs + 1.

    bell.
    message color red/with
                    "Numero de itens no pedido: " vmovseq skip
                    "Numero maxtimo de itens NF: 300" skip
    ...
    varq-n = "/admcom/work/est_" + string(vetb-emite,"999")
                        + "_" + string(vetb-desti,"999") + "_" +
    ...
    for each tt-estoq where tt-estoq.procod > 0 and
        ...
        if vqtd-item = 300 then vqtd-item = 0.
