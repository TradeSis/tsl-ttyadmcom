...
def var cgc-origem like forne.forcgc.
...
def var vplaori like plani.placod.
form titulo.titpar
     titulo.titnum
     prazo
     titulo.titdtven
...
form produ.procod column-label "Codigo" format ">>>>>9"
     ...
     with frame f-produ1 row 12 down overlay
                centered color white/black width 80.

form /* produ.procod column-label "Codigo" format ">>>>>9" */
     w-movim.movqtm format ">>>>>9" label "Qtd"
     w-movim.movpc  format ">>,>>9.99" label "Val.Unit."
     w-movim.subtotal  format ">>>,>>9.99" label "Subtot"
     w-movim.valdes format ">>>,>>9.999" label "Val.Desc"
     w-movim.movdes format ">9.9999" label "%Desc"
     w-movim.movalicms label "ICMS" format ">9.99"
     w-movim.movalipi  label "IPI"  format ">9.99"
     w-movim.movfre    label "Frete" format ">>>,>>9.99"
     with frame f-produ9 row 12 down overlay
                centered color white/black width 80 1 column
                side-label.
...
form estab.etbcod label "Filial" colon 15
    ...
    title "   Nota Fiscal Complementar   " overlay.
...
def var vforori like forne.forcod.
form cgc-origem label "Emitente "
     cforne.fornom no-label
     vserori like vserie  at 4 label "Serie "
     vnumori like vnumero label "Numero "
     with frame f-complementar 1 down width 80
     title "    Nota Fiscal Origem   " side-label
     .
...
do:
    ...
    **/
    ...
    repeat with 1 down:
        ...
        disp w-movim.movqtm  w-movim.movpc   w-movim.subtotal  
             w-movim.valdes  w-movim.movdes 
             w-movim.movalicms  w-movim.movalipi   w-movim.movfre    
             with frame f-produ9 .
             
        update w-movim.movqtm with frame f-produ9.
                       
        w-movim.movpc = 0 /*estoq.estcusto*/. 
        w-movim.movqtm = vmovqtm + w-movim.movqtm. 
        display w-movim.movqtm with frame f-produ9. 
        update w-movim.subtotal with frame f-produ9.  
        
        w-movim.movpc = w-movim.subtotal / w-movim.movqtm.
        display w-movim.movpc with frame f-produ9. 
        ...
        display w-movim.valdes
                w-movim.movdes with frame f-produ9.
                    
        if tipo_desconto = 3 
        then do: 
            update w-movim.movdes with frame f-produ9. 
        ...
        if tipo_desconto = 4 
        then do on error undo, retry: 
            update w-movim.valdes with frame f-produ9.
        ...
        update w-movim.movalicms with frame f-produ9.
        ...
        then.
            ...
            update w-movim.movalipi with frame f-produ9.
        end.
        
        update w-movim.movfre with frame f-produ9.
    ...
    for each w-movim:
        ...
        for each estoq where estoq.procod = produ.procod.
            do transaction:
                estoq.estcusto = estoq.estcusto + vcusto.
            end.    
        end.
...
procedure complementar:
    cgc-origem = cgc-admcom.
    disp cgc-origem with frame f-complementar.
    find first cforne where cforne.forcgc = cgc-origem no-lock.
    disp cforne.fornom with frame f-complementar.
    vforori = vforcod.
    vserori = "1".
    update vserori vnumori with frame f-complementar.
    
    find cplani where cplani.etbcod = vetbcod and
                      cplani.emite  = vforori and
                      cplani.serie  = vserori and
                      cplani.numero = vnumori
                      no-lock no-error.
    if not avail cplani
    then do:
        bell.
    ...
    for each cmovim where cmovim.etbcod = cplani.etbcod and
                             ...
                             cmovim.movdat = cplani.pladat no-lock:
                             
            find produ where produ.procod = cmovim.procod no-lock.
                create w-movim. 
                assign w-movim.wrec      = recid(produ)
                       w-movim.movqtm    = cmovim.movqtm  
                       w-movim.subtotal  = (cmovim.movpc * cmovim.movqtm) 
                       w-movim.movpc     = cmovim.movpc  
                       w-movim.movalicms = cmovim.movalicms
                       w-movim.valicms   = cmovim.movicms 
                       /*w-movim.movalipi  = cmovim.movalipi   
                       w-movim.movipi    = cmovim.movipi*/ 
                       w-movim.movdes    = cmovim.movpdes  
                       w-movim.valdes    = cmovim.movdes.

        end. 
     ****/
     find produ where produ.procod = 0
                no-lock no-error.
     if avail produ
     then do:           
        create w-movim.
