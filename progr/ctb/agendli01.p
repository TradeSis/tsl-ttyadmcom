        ...
        */
        ...
        then do:
            for each wmodal where wmodal.modcod <> "CHP"
                ...
                find first titudesp where
                               titudesp.empcod = titulo.empcod and
                ...
                */
                ...
                find first  hiscampotb where
                        ...
                        no-lock no-error.
                if avail hiscampotb
                then tt-titlb.dtvenori = date(hiscampotb.valor_campo). 
        ...
        for each tt-titlb no-lock 
            ...
            find first setaut where
                       setaut.setcod = tt-titlb.vimp-setor
                       no-lock no-error.
            find first modal where 
                       modal.modcod = tt-titlb.modcod     
                       no-lock no-error.
                              
            disp tt-titlb.titdtven format "99/99/99"
                                column-label "Dt.Venc."
                 tt-titlb.dtvenori format "99/99/99"
                                column-label "Ven.Antes"
                 tt-titlb.vimp-setor when vimp-setor <> 0 column-label "Set"
                                format ">>>>>9"
                 setaut.setnom when avail setaut column-label "Setor"
                            format "x(20)"
                 tt-titlb.clifor 
                 ...
                 modal.modnom when avail modal column-label "Modalidade"
                                      format "x(20)"  
                 tt-titlb.titnum 
                 tt-titlb.titpar
...
procedure fin-titudesp:
    for each titudesp where titudesp.empcod = wempre.empcod and
                ...
                if wetbcod > 0 and titudesp.etbcod <> wetbcod
                then next.
