...
def temp-table tt-doc
    ...
    field vencimento_inicial as date
...
for each tt-doc: delete tt-doc. end.
...
procedure temp-documento-contrato:

    find first tt-doc where
               tt-doc.clifor = titulo.clifor and
               tt-doc.titnum = titulo.titnum
               no-error.
    if not avail tt-doc
    then do:
        create tt-doc.
        assign
            tt-doc.clifor = titulo.clifor
            tt-doc.titnum = titulo.titnum
            tt-doc.titdtemi = titulo.titdtemi
                .
    end.
    tt-doc.titvlcob = tt-doc.titvlcob + titulo.titvlcob.        
    if tt-doc.vencimento_inicial = ? or
       tt-doc.vencimento_inicial > titulo.titdtven
    then tt-doc.vencimento_inicial = titulo.titdtven.   
    if tt-doc.vencimento_final = ? or
       tt-doc.vencimento_final < titulo.titdtven
    then tt-doc.vencimento_final = titulo.titdtven.    
...
procedure relatorio-analitico-GERAL:
    ...
    disp "Gerando relatorio saldo gerencial por documento... Aguarde!" skip
    ...
    vqa = 1.
    if not v-consulta-parcelas-LP and
       not v-feirao-nome-limpo 
    then varq = "/admcom/relat/saldo-gerencial-" 
                + string(vdtref,"99999999") 
                + "-arquivo-" + string(vqa,"9") 
                + ".csv". 
    if v-consulta-parcelas-LP
    then varq = "/admcom/relat/saldo-LP-"
                 + string(vdtref,"99999999")
                 + "-arquivo-" + string(vqa,"9")
                 + ".csv".
    if v-feirao-nome-limpo
    then varq = "/admcom/relat/saldo-feirao-"
                 + string(vdtref,"99999999")
                 + "-arquivo-" + string(vqa,"9")
                 + ".csv".
    ...
    put "Codigo;Nome;CPF;Contrato;Valor;Emissao;Vencimento_inicial;Vencimento_f~inal"
                    skip.
    
    vtotal = 0.                  

    for each tt-doc no-lock use-index i1:
        /**********
        ...
        then next. 
        ********/
        find clien where clien.clicod = tt-doc.clifor no-lock no-error.
        if avail clien
        then    assign
                ...
                valchar = string(tt-doc.titvlcob). 
                valchar = replace(valchar,",","").
                valchar = replace(valchar,".",",").
                put unformatted 
                tt-doc.clifor format ">>>>>>>>>9"
                ";"
                vclinom 
                ";"
                vciccgc 
                ...
                valchar
                ";"
                tt-doc.titdtemi format "99/99/9999"
                ";"
                tt-doc.Vencimento_inicial format "99/99/9999"
                ";"
                tt-doc.Vencimento_final format "99/99/9999"
                skip
                .
        ...
        then do:
            ...
            output to value(varq).
            put "Codigo;Nome;CPF;Contrato;Valor;Emissao;Vencimento_inicial;Venc~imento_final"
            skip.
         
        end.
                
        vtotal = vtotal + tt-doc.titvlcob.
