...
do on error undo with frame f-filtro side-label centered
    ...
    disp vpasta       colon 15 label "Diretorio" format "x(40)".
    update varquivo colon 15 label "Arquivo"   format "x(40)"
        help "Informar somente o nome do arquivo".
...
vdtaux = date(int(substr(vnomarq,5,2)),int(substr(vnomarq,7,2)),
                int(substr(vnomarq,1,4))).
def var vtime as char.
vtime = string(time).
vtime = entry(2,varquivo,"_").
...
DO ON ERROR UNDO.
    ...
    assign lotefidc.lotnum  = (if avail blotefidc 
          ...
          lotefidc.data    = vdtaux
          lotefidc.hora    = int(vtime)
...
*/
        ...
        assign
            ...
            titulolog.data   = vdtaux
            titulolog.hora   = int(vtime)
