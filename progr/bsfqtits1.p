...
def var vclfcod like clien.clicod init 1101103925.
...
run bsfqtitlp.p (vclfcod).

disconnect d.
hide message no-pause.

find first tt-titulo no-error.
message avail tt-titulo view-as alert-box.
...
form
            ...
            tt-titulo.modcod column-label "Modal"
            tt-titulo.titnum   format "x(13)"
            tt-titulo.titdtemi format "99/99/99"
            tt-titulo.titdtven format "99/99/99" column-label "Vecto"
            vdias           column-label "Dias"
            vtotal         format ">>>>,>>9.99" column-label "Saldo"
            vsaldo  column-label "C/Jur+Multa"
                with frame frame-a .

find first tt-titulo where tt-titulo.titnat = vtitnat and
                        tt-titulo.clifor  = clien.clicod and
                        tt-titulo.titsit = "LIB" no-lock no-error.
if avail tt-titulo
then
    ...
    then
        ...
        find first tt-titulo where tt-titulo.titnat = vtitnat and
                                tt-titulo.clifor  = clien.clicod and
                                tt-titulo.titsit = "LIB" and
                                tt-titulo.titdtven >= today no-lock no-error.
        if avail tt-titulo
        then
            ...
            then
                ...
                then
                    ...
                    run message.p (input-output sresp,
                           input "Nenhum ttitulo para este Cliente " ,
                                   input " !! ATENCAO !! ",
                                   input "    OK").
                    */                                   
                    return.
...
find first tt-titulo where tt-titulo.titnat = vtitnat and
                        tt-titulo.clifor  = clien.clicod and
                        tt-titulo.titdtpag <> ? and
                        tt-titulo.titdtpag <> 01/01/1111
...
find first tt-titulo where tt-titulo.titnat = vtitnat and
                        tt-titulo.clifor  = clien.clicod no-lock no-error.
if avail tt-titulo
then.
else vposicao[5] = "            ".
if rectit <> ?
...
/*    else color display message esqcom2[esqpos2] with frame f-com2. */
    ...
    color disp messages
            ...
            find ttmarc where ttmarc.rec = recid(tt-titulo) no-error.
    ...
    color disp normal
            ...
            then
                find first tt-titulo where
                          ...
                          tt-titulo.clifor  = clien.clicod and
                          tt-titulo.titsit = "LIB"
                                            and tt-titulo.titnum = vbusca
                                            and (if vtitpar <> ""
                                             then tt-titulo.titpar = int(vtitpar)
                                                 else true)
                                        no-lock no-error.
            else
                if vchoose = "venci"
...
/*                  color display normal esqcom2[esqpos2] with frame f-com2. */
            ...
            then do:
                ...
                if not avail tt-titulo
                then next.
/*                color display white/red tt-titulo.titnum.*/
                if frame-line(frame-a) = frame-down(frame-a)
...
/*                    view frame ftot2. */
                ...
                then do.
                    find tt-titulo where recid(tt-titulo) = recatu1 no-lock.
                    find ttmarc where ttmarc.rec =  recid(tt-titulo)
                                            no-error.
                    if avail ttmarc
                    then delete ttmarc.
                    else do.
                        create ttmarc.
                        ttmarc.rec =  recid(tt-titulo).
                    end.
                    run marcados.
                    leave.
                end.
                ...
                then do.
                    hide frame frame-a no-pause.
                    hide frame f-com1 no-pause.
                    run fqnottit.p (input recid(tt-titulo)).
        pause 0.
                    view frame fhelp.
/*                    view frame f-com2. */
pause 0.
...
/*                    view frame f-com2. */
                ...
                then do.
                    hide frame frame-a no-pause.
                    hide frame f-com1 no-pause.
                    run fqcontrato.p (input tt-titulo.clifor).
...
/*                    hide frame f-com2   no-pause.*/
                    ...
                    hide frame fhelp    no-pause.
                    hide frame frame-a  no-pause.
                    run bsfqtitulo.p (input recid(tt-titulo)).
...
procedure frame-a.
    /*def var vsaldo           like tt-titulo.titvlcob.*/
    def var vsaldoatualizado like tt-titulo.titvlcob.
    def var vperc            as   dec.
    ...
    def var vvalormulta as dec format ">>>>9.99".
            ...
            assign
                vtotal = if tt-titulo.titdtpag = ?
                         then tt-titulo.titvlcob - tt-titulo.titvlpag
                         else 0.
        find cobra    of tt-titulo no-lock.
        vcartcobra = trim(substr(cobra.cobnom,1,5)).
        find banco of tt-titulo no-lock no-error.
        if avail banco
        then
            substr(vcartcobra,1,5) = banco.banfan.
        if tt-titulo.titdtpag = ?
        then vdias = vtoday - tt-titulo.titdtven.
        else vdias = tt-titulo.titdtpag - tt-titulo.titdtven.
        find estab where
                        estab.etbcod = tt-titulo.etbcod no-lock.
        if tt-titulo.titdtpag <> ?
        then
        ...
        vsaldoatualizado    = tt-titulo.titvlcob - tt-titulo.titvlpag.
        if vtoday > tt-titulo.titdtven
        then do:
            if tt-titulo.modcod = "GE"
            then do.
            end.
            else do.
                vsaldobase = vsaldo.
                /*
                run fbjuro.p (input tt-titulo.cobcod, 
                          input tt-titulo.carcod, 
                          input tt-titulo.titnat, 
                          input vsaldo,
                          input tt-titulo.titdtven, 
                          input vtoday, 
                          output vsaldoatualizado, 
                          output vperc) .
                */
            end.  /*
            run fbmulta.p (input tt-titulo.cobcod,  
                       input tt-titulo.carcod,  
                       input tt-titulo.titnat,  
                       input vsaldobase,
                       input tt-titulo.titdtven,  
        ...
        find first titulo where titulo.empcod = tt-titulo.empcod
        ...
        vtitulo = trim(tt-titulo.titnum + "/" + string(tt-titulo.titpar)).
                
        display
            vaster
            estab.etbcod  column-label "Fil" format ">>9"
            tt-titulo.modcod column-label "Modal"
            vtitulo @ tt-titulo.titnum
            tt-titulo.titdtemi format "99/99/99"
...
procedure marcados.
def var vmarcsaldo   like tt-titulo.titvlcob.
def var vmarctotal   like tt-titulo.titvlcob.
    def var vsaldoatualizado like tt-titulo.titvlcob.
    def var vperc            as   dec.
    def var vvalordesc       as   dec.
    def var vsaldobase    as dec.
...
for each ttmarc.
    find tt-titulo where recid(tt-titulo) = ttmarc.rec no-lock.
    vtotal = if tt-titulo.titdtpag = ? 
             then tt-titulo.titvlcob - tt-titulo.titvlpag 
             else 0.
    do.
    ...
    def var vjuros as dec.
    vvalormulta = 0.
    run bstitjuro.p (input  recid(tt-titulo),  
                     input  vtoday,          
                     output vjuros,         
                     output vsaldoatualizado).   
...
procedure nota-fiscal.
        ...
        pause 0.
            do:
                ...
                for each contnf where contnf.etbcod  = tt-titulo.etbcod and
                                      contnf.contnum = int(tt-titulo.titnum) 
                                                    no-lock:
                    for each plani where plani.etbcod = tt-titulo.etbcod and
