...
varqexp = "op" + string(today,"999999") + "cp.rem".
    ...
    */
    ...
    for each tt-estab no-lock:
        ...
        for each contrato where contrato.etbcod = tt-estab.etbcod 
          ...
          /************************************************
          **** Se não achar plani usando o pladat      ****
          **** tenta novamente sem esse campo          ****
          **** Crédito pessoal pode ter                ****
          ...
          ************************************************/
            ...
            valor-av = valor-av - valor-entrada.
            if plani.biss <= valor-av /*(plani.platot - plani.vlserv)*/ 
            then next.
            
            if plani.seguro <> ?
            then valor-av = valor-av - plani.seguro.
...
procedure p-registro-10.
  ...
  def var vcod-plano    as integer.  
  def var vcod-produto  as integer.
  def var vvalor-tfc    as dec.
  def var vval-seguro   as dec.
  def var vval-operacao as dec.
  ...
  else assign vcod-produto = 0.
    
  if avail plani
  then assign vcod-plano = plani.pedcod.
  else assign vcod-plano = 0.

  assign vval-operacao = 0.  
  ...
  for each bmovim where bmovim.etbcod = plani.etbcod
      ...
      /*******************************************
      Verifica se o item da nota é Crédito Pessoal
      *******************************************/
      
      find first bprofin where bprofin.procod = bmovim.procod
                                   no-lock no-error.
        
      if avail bprofin
      then do:
          assign
              vvalor-tfc = (bmovim.movpc * bmovim.movqtm) - bmovim.movctm.
      
          if vvalor-tfc = ?
          then vvalor-tfc = 0.
          
          assign vval-operacao = bmovim.movctm.
          
          if vval-operacao = ?
          then vval-operacao = 0.
      ...
      /**********************************************
      Verifica se o item da nota é Seguro Prestamista
      **********************************************/
      
      find first bprofin where bprofin.procod_seguro = bmovim.procod
                                         no-lock no-error.

      if avail bprofin
      then do:
          assign
               vval-seguro = (bmovim.movpc * bmovim.movqtm).
      end.
  ...
  put unformat skip 
      ...
      vcod-produto format "999999"                  /* 29-34 PRODUTO */
      vcod-plano          format "9999"      /* 35-38 PLANO    */
      tt-contrato.numparc format ">>9"
      /*finan.finnpc    format ">>9"       /* 39-41 PRAZO ???   */
      */
      contrato.DtEfetiva format "99999999"  /* 42-49 DATA EMISSÃO */
      /* titulo.titdtven format "99999999"  /* 50-57 DATA 1° VENCIMENTO */ */
      v-data-vecto    format "99999999"  /* 50-57 DATA 1° VENCIMENTO */
      (vval-operacao * 100) format "99999999999999999"
    ...
    (contrato.TxJuros * 1000000) format "999999999"  /* 132-140 TAXA NOMINAL */
      ...
      ******************************/
      ...
      vvalor-tfc * 100    format "99999999999999999" /* 198 - 214 VALOR TFC */
      vval-seguro * 100   format "99999999999999999" /* 215 - 231 VAL SEGURO*/
...
PROCEDURE p-registro-12.
    ...
    def var vval_libera     as character.
    def var vcpf_cgc_desti  as character.
    def var vnome_desti     as character. 
    def var vtipo_liberacao as character.
    def var vnotobs         as character.
    def var vnotobs2        as character.

    assign vnotobs = acha("Deposito",plani.notobs[1]).
    assign vnotobs2 = acha("Favorecido",plani.notobs[2]).

    assign vseq = vseq + 1.

    /* banco|agencia|tipo|conta|cpf */
    
    if vnotobs <> "" and vnotobs <> "?" and num-entries(vnotobs,",") = 5
    then do:
        ...
        for each bmovim where bmovim.etbcod = plani.etbcod
            ...
            first bprofin where bprofin.procod = bmovim.procod
                                       no-lock.
        
            assign
          vval_libera = trim(replace(string(bmovim.movctm,">>>>>9.99"),".","")).

        end. 
        

        if length(entry(1,vnotobs,",")) >= 4
        then
        assign vbanco = integer(substr(entry(1,vnotobs,","),
                                length(entry(1,vnotobs,",")) - 2,3)).
        
        assign
        vagencia        = integer(substr(entry(2,vnotobs,","),2,4))
        vconta          = entry(4,vnotobs,",")
        vtipo_conta     = entry(3,vnotobs,",")
        vcpf_cgc_desti  = entry(5,vnotobs,",")
        vtipo_liberacao = "5".

        assign
        vval_libera = fill(" ",17 - length(vval_libera)) + trim(vval_libera).

    end.
    
    assign vnome_desti = vnotobs2.
...
procedure criaenvfinan:
    ...
    for each tt-contrato where
        ...
        ***/
        ...
        if num-entries(acha("Deposito",plani.notobs[1]),",") > 2
        then    
        run p-registro-12.
