...
define temp-table tt-valores no-undo serialize-name 'precoCustoProduto'
    field codigo    as integer serialize-name 'codigoProduto'
    field descricao as decimal serialize-name 'precoCustoProduto'
...
for each estoq fields(etbcod procod estcusto)
   where etbcod = 1 no-lock:
    ...
    assign tt-valores.codigo    = estoq.procod
           tt-valores.descricao = estoq.estcusto
    ...
    if i-cont = 3000 then do:
        ...
        run /home/kbase/bkps/produtos/exporta-prod.p(input-output lo-arquivo).             
        
        RUN PostRequest (INPUT 'produtos/integracaoPrecoCustoProduto', 
                         INPUT lo-arquivo).
