...
unix silent value("tty > lognf.tty" ).
input from ./lognf.tty.
repeat.
      import vtty.
end.
input close.
        ...
        then do:
            ...
            then do:
                ...
                then do with frame f-plani on error undo.
                    ...
                    /***
                    find divplani where 
                         divplani.plani-PlaCod     = plani.PlaCod  and
                         divplani.plani-EtbCod     = plani.EtbCod  
                          no-lock no-error.
                    if avail divplani 
                    then do.
                        find wmsplani where  
                             wmsplani.EtbCod = divplani.wmsplani-EtbCod  and
                             wmsplani.PlaCod = divplani.wmsplani-PlaCod
                             no-lock no-error.   
                        if avail wmsplani and
                           wmsplani.plasit = "X"
                        then do on error undo. 
                            run divplani.p (recid(divplaniwms)).
                        /*  leave.*/
                        end.
                    end.
                    ***/
                    ...
                    if ped-especial /*Cross Docking*/
                    then do:
                        run orech-993.p(recid(plani), "TR"). 
                    end.
                    else do:
                        run orech-993.p(input recid(plani), input "LI"). 
                    end.
                    if ped-especial
                    then do:
                        run pedido-especial.
...
procedure atu-fat-finan:
    ...
    update vezes label "Parcelas"
            validate(vezes > 0,"Favir informar a quantidade de parcelas.")
