...
for each tt-docbase :
    ...
    if avail tdocbase
    then run ordvh-moveis.p(recid(tdocbase), "NORM").
    delete tt-docbase.
end. 
...
procedure frame-a.
    ...
    find tbgenerica where tbgenerica.TGTabela = "TP_PEDID" and
                          ...
                          no-lock no-error.
    if avail tbgenerica
    then ped-tipo = tbgenerica.tgdescricao.
    ...
    for each liped of pedid no-lock.
        ...
        for each tt-reservas.
            delete tt-reservas. 
             ...
             /****                             
             find wbsprodu where wbsprodu.procod = com.produ.procod no-lock
                                    no-error.
             if not avail wbsprodu
             then next.  
             *****/
                          
             {tbcntgen6.i pedid.peddat}
             if avail tbcntgen
             then next.                 
    ...
    display tab_box.box when avail tab_box
            ...
            tbgenerica.tgint / 10 format ">>>" label "Pri"
                              when tgint  <> 9999
    ...
    then  do:
        ...
        then
            find first com.pedid where com.pedid.pedtdc = vpedtdc 
                               ...
                               and com.pedid.sitped <> "C" no-lock no-error.
        else 
            find first com.pedid where com.pedid.pedtdc = vpedtdc
                        and (if vsitped <> ""
                             then com.pedid.sitped = vsitped else true)
                        and (if vetbped > 0
                             then com.pedid.etbcod = vetbped else true)
                        and (if vproped > 0
                             then can-find(first liped of pedid where
                                                 liped.procod = vproped)
                             else true)
                             no-lock no-error.
        ...
        then
            ...
            find last com.pedid where com.pedid.pedtdc = vpedtdc
             ...
             /*
             find last com.pedid where com.pedid.pedtdc = vpedtdc
                              and com.pedid.sitped = vsitped no-lock no-error.
                */
...
procedure p-consulta:  
    ...
    for each com.liped of com.pedid where com.liped.predt = com.pedid.peddat no-lock:
...
then do:     
    ...
    if vbox-num = 0
    then return.
    for each tab_box where tab_box.box = vbox-num no-lock: 
      create tt-estab. 
      assign tt-estab.etbcod = tab_box.etbcod 
             tt-estab.ordem  = tab_box.ordem. 
    end.  
...
"------------------------------------------------------------------------------"     skip(1).
  ...
  for each tt-estab no-lock break by tt-estab.ordem:
     ...
     disp tt-estab.etbcod ger.estab.etbnom
            with  frame fcorte .
     ...
     for each xpedid where xpedid.etbcod = ger.estab.etbcod 
                       ...
                       and xpedid.peddat <= today no-lock:
        ...
        then if xpedid.pednum <> par-pednum  
         ...
         for each com.liped where com.liped.pedtdc = xpedid.pedtdc 
             ...
             west = 0. 
             /***
             if wbsprodu.endpav = 981
             then find com.estoq where com.estoq.etbcod = 981 
             ...
             ****/
             ...
             then do on error undo. 
                ...
                next.
                 ...
                 find tt-ped where tt-ped.etbcod = com.liped.etbcod
                 ...
                 find com.produ where com.produ.procod = com.liped.procod no-lock no-error.
                 ...
                 then do:  
                     ...
                     assign wroma.wped = wroma.wped + (if wdisp >= com.liped.lipqtd
                                                       then com.liped.lipqtd 
                                                       else wdisp)
             ...
             find bbliped where bbliped.etbcod = com.liped.etbcod
     ...
     for each wroma where wroma.wped > 0 no-lock by wroma.etbcod
        ...
        find tt-pav where tt-pav.etbcod = wroma.etbcod
     ...
     for each wroma where wroma.wped > 0
        ...
        /***** CONJUNTO
        find wbsprodu where wbsprodu.procod = wroma.procod no-lock.
        if wbsprodu.endpav = 981
        then
        find tdocbase where recid(tdocbase) = rec_981 no-lock.
        if wbsprodu.conj-qtd > 1 and tdocbase.ecommerce = no
        *****/
        
        find conjunto where conjunto.procod = wroma.procod no-lock no-error.
        ...
        ASSIGN  
                ...
                tdocbpro.pednum  = if avail conjunto and conjunto.qtd > 1
                                     then ? 
                                     else wroma.pednum.
        ...
        find peddocbpro where peddocbpro.dcbcod  = tdocbpro.dcbcod and
        ...
        find nffped where nffped.forcod = setbcod               and 
