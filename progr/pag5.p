...
def temp-table wpag
    ...
    field clinom like forne.fornom
    ...
    field emis   like titulo.titdtemi
    ...
    repeat:
        ...
        then do:
            ...
            vmodnom = if avail modal
                      then modal.modnom
                      else "TODAS MODALIDADES".
        end.

        if opsys = "UNIX"
        then varquivo = "../relat/pag5." + string(time).
        else varquivo = "..\relat\pag5." + string(time).              
        {mdadmcab.i
        ...
        for each wmodal:
            ...
            for each titulo where titulo.empcod = wempre.empcod  and
                              ...
                              titulo.titdtemi >= wdti        and
                              titulo.titdtemi <= wdtf        and 
                            (if wetbcod = 0
                                 then true
            ...
            find first wpag where wpag.clicod = titulo.clifor and
                                  wpag.titnum = titulo.titnum no-error.
            if not avail wpag
            then do:
                create wpag.
                assign wpag.clicod = titulo.clifor
                       wpag.clinom = vnome
                       wpag.titnum = titulo.titnum.
            end.
            ...
            if avail plani
            then assign
                    wpag.emis = plani.pladat
                    wpag.chave = plani.ufdes.
            else do:
                find first fatudesp where
                           ...
                           no-lock no-error.
                if avail fatudesp
                then wpag.emis = fatudesp.emissao.
            end.
        end.
        end.
        for each wpag break by wpag.clinom
                            by wpag.titnum:
            ...
            display wpag.clicod
                    ...
                    wpag.emis column-label "Emissao"
                    wpag.chave column-label "Chave de acesso"
                            with frame f-pag width 200 down.
            if last-of(wpag.titnum)
            then do:
                vacum = 0.
                /* put skip(1). */
            end.
