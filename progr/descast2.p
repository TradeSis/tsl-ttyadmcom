...
def input parameter vpercdiver as dec format ">>9.99 %".
def input parameter vcatcod like categoria.catcod.
...
def temp-table wpreco
    ...
    field  prejus  like divpre.divjus
    ...
    find first bmovim where bmovim.movdat >= vdti and
                            bmovim.movdat <= vdtf no-lock no-error.
    if not avail bmovim
    then leave.

    if opsys = "UNIX"
    then varquivo = "../relat/confp1" + string(time).
    else varquivo = "..\relat\confp1" + string(time).
    ...
    {mdad.i &Saida     = "value(varquivo)"
                ...
                &Tit-Rel   = """DIV. DE PRECOS DIA FIL"" +
                             string(vdti,""99/99/9999"") + "" A "" +
                             string(vdtf,""99/99/9999"") + "" - Fil."" + 
                             string(estab.etbcod,"">>9"") + "" Fone: "" +  
    ...
    for each movim where movim.etbcod = plani.etbcod and
        ...
        find divpre where divpre.etbcod = plani.etbcod and
                          ...
                          divpre.procod = movim.procod no-lock no-error.
        if avail divpre
        then wpreco.prejus  = divpre.divjus.
    end.
    end.
