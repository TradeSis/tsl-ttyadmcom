...
update vdt
       ...
       vcxa with frame f1 side-label centered 1 column width 55.
...
for each titulo where titulo.empcod = wempre.empcod and
                      ...
                      titulo.titpar = 0 no-lock.
    if titulo.cxacod = vcxa then do:
        ct-entr = ct-entr + 1.
...
for each titulo where titulo.empcod = wempre.empcod and
                      ...
                      titulo.titpar > 0     and
                      titulo.cxacod = vcxa no-lock
                      use-index titdtpag:

    if titulo.clifor = 1 then undo,next.
     /*
    if titulo.moecod = "PRE"
    then
        assign
            vlpred = vlpred + titulo.titvlcob
