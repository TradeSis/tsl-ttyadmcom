...
output to /tmp/etique.bat append.
        
     put  /*"mode com2:9600,e,7,2,r" skip  */
         "cp /admcom/zebra/LEBES.GRF /dev/ttyS1" skip. 
output close.

input from /admcom/gener/admcom.ale no-echo.
vlinha = 0.
repeat:
    create wfetq.
    import
...
input close.
    ...
    then do:
      for each wfetq break by linha:
        ...
        output to /tmp/etique.bat append.
            put trim(" cat /tmp/cris" + 
                 string(produ.procod) + " > /dev/ttyS1") format "x(40)" skip.
        output close.
