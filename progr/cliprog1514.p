...
def buffer tt-venda for tabcem17.
def buffer tt-receb for tabcre17.
...
DEFINE temp-table tt-total  no-undo
  ...
  FIELD acrescimo_novacao_lebes_ol       AS DECIMAL     DECIMALS 2
  FIELD acrescimo_novacao_lebes_of       AS DECIMAL     DECIMALS 2
  ...
  FIELD define_venda_cartaopresente      AS CHARACTER   EXTENT 15 FORMAT "x(20)"
  ...
  FIELD novacao_lebes_ol                 AS DECIMAL     DECIMALS 2
  FIELD novacao_lebes_of                 AS DECIMAL     DECIMALS 2
  ...
  FIELD receber_principal                AS DECIMAL     DECIMALS 2
  FIELD receber_principal_lebes          AS DECIMAL     DECIMALS 2
  FIELD receber_principal_fdrebes        AS DECIMAL     DECIMALS 2
  FIELD receber_acrescimo                AS DECIMAL     DECIMALS 2
  FIELD receber_acrescimo_lebes          AS DECIMAL     DECIMALS 2
  FIELD receber_acrescimo_Fdrebes        AS DECIMAL     DECIMALS 2
  ...
  FIELD venda_cartaopresente             AS DECIMAL     EXTENT 15 DECIMALS 2
  ...
  FIELD venda_cartao_cd                  AS DECIMAL     DECIMALS 2
...
def var ok-cartaopresente as log.
...
for each estab no-lock:
    if vetblan > 0 and vetblan <> estab.etbcod
    then next.   
for each tt-venda where 
    ...
    assign
        ...
        tt-total.acrescimo_venda_outras = tt-total.acrescimo_venda_outras +
                        ...
                        .
    if tt-venda.novacao_lebes_ol <> ?
    then tt-total.novacao_lebes_ol = tt-total.novacao_lebes_ol +
                                         tt-venda.novacao_lebes_ol.
    if tt-venda.novacao_lebes_of <> ?
    then tt-total.novacao_lebes_of = tt-total.novacao_lebes_of +
                                         tt-venda.novacao_lebes_of.
    if tt-venda.acrescimo_novacao_lebes_ol <> ?
    then tt-total.acrescimo_novacao_lebes_ol = 
                            tt-total.acrescimo_novacao_lebes_ol +
                            tt-venda.acrescimo_novacao_lebes_ol.
    if tt-venda.acrescimo_novacao_lebes_of <> ?
    then tt-total.acrescimo_novacao_lebes_of = 
                            tt-total.acrescimo_novacao_lebes_of +
                            tt-venda.acrescimo_novacao_lebes_of
                            .         
    ...
    do vi = 1 to 15:
        ...
        do vb = 1 to 15:
            ...
            if tt-total.define_venda_moeda[vb] =
                    tt-venda.define_venda_moeda[vi]
            then do:
                if substr(tt-venda.define_venda_moeda[vi],1,3) = "CAR" or
                   substr(tt-venda.define_venda_moeda[vi],1,1) = "T"
                then tt-total.venda_cartao_cd = 
                     tt-total.venda_cartao_cd + tt-venda.venda_moeda[vi]. 
                ok-venda = yes.
                leave.
            end.
        end.          
        ...
        ok-cartaopresente = no.
        if tt-venda.define_venda_cartaopresente[vi] <> ""
        then 
        do vb = 1 to 15:
            if tt-total.define_venda_cartaopresente[vb] = ""
            then tt-total.define_venda_cartaopresente[vb] =
                tt-venda.define_venda_cartaopresente[vi].
            if tt-total.define_venda_cartaopresente[vb] =
                    tt-venda.define_venda_cartaopresente[vi]
            then do:
                ok-cartaopresente = yes.
                leave.
            end.
        end.    
        if ok-cartaopresente
        then tt-total.venda_cartaopresente[vb] = 
                tt-total.venda_cartaopresente[vb] + 
                    tt-venda.venda_cartaopresente[vi].
...
for each tt-receb where 
         tt-receb.etbcod = estab.etbcod and
        tt-receb.datref >= vdti and
    ...
    assign
        ...
        tt-total.receber_principal =
        tt-total.receber_principal + tt-receb.receber_principal
        tt-total.receber_principal_lebes =
        tt-total.receber_principal_lebes + tt-receb.receber_principal_lebes
        tt-total.receber_principal_fdrebes =
        tt-total.receber_principal_fdrebes + tt-receb.receber_principal_fdrebes
        tt-total.receber_acrescimo =
        tt-total.receber_acrescimo + tt-receb.receber_acrescimo
        tt-total.receber_acrescimo_lebes =
        tt-total.receber_acrescimo_lebes + tt-receb.receber_acrescimo_lebes
        tt-total.receber_acrescimo_fdrebes =
        tt-total.receber_acrescimo_fdrebes + tt-receb.receber_acrescimo_fdrebes
                         .
...
do vi = 1 to 15:
    ...
    then  do:
        if substr(tt-total.define_venda_moeda[vi],1,3) <> "CAR" and
           substr(tt-total.define_venda_moeda[vi],1,1) <> "T"
        then   
        put "         " 
...
put  "Venda CARTAO D/C        ;"
    replace(string(tt-total.venda_cartao_cd),".",",")
    format "x(15)"
    skip.
vi = 0.
do vi = 1 to 15:
    if tt-total.define_venda_moeda[vi] <> ""
    then  do:
        if substr(tt-total.define_venda_moeda[vi],1,3) = "CAR" or
           substr(tt-total.define_venda_moeda[vi],1,1) = "T"
        then   
        put "         " 
            tt-total.define_venda_moeda[vi] " ; "
...
*/  
put  "Venda a RECARGA           ;" format "x(40)"
        replace(string(tt-total.venda_recarga[1] + tt-total.venda_recarga[2])
                    ,".",",")
        format "x(15)"            
        skip.
    vi = 0.
    do vi = 1 to 15:
        if tt-total.define_venda_recarga[vi] <> ""
        then put "         " tt-total.define_venda_recarga[vi]  ";" 
                 replace(string(tt-total.venda_recarga[vi]),".",",")
                 format "x(15)"
                 skip.
    end.
put "Venda CARTAO PRESENTE     ;" format "x(40)"
        replace(string(tt-total.venda_cartaopresente[1] +
         tt-total.venda_cartaopresente[2]),".",",")
         format "x(15)"
        skip.
     vi = 0.
    do vi = 1 to 15:
        if tt-total.define_venda_cartaopresente[vi] <> ""
        then put "         "  
            tt-total.define_venda_cartaopresente[vi] ";"
            replace(string(tt-total.venda_cartaopresente[vi]),".",",")
             format "x(15)"
                 skip.
    end. 
...
put  skip "JUROS recebimento   ;"
     ...
     skip "            Origem LEBES           ;"
     replace(string(tt-total.novacao_lebes_ol),".",",") format "x(15)"
     skip "            Origem FINANCEIRA      ;"
     replace(string(tt-total.novacao_lebes_of),".",",") format "x(15)"
     ...
     skip "            Origem LEBES        ;"
     replace(string(tt-total.acrescimo_novacao_lebes_ol),".",",") 
     format "x(15)"
     skip "            Origem FINANCEIRA   ;"
     replace(string(tt-total.acrescimo_novacao_lebes_of),".",",")
     format "x(15)"
    ...
    if vdti > 08/31/17
    then do:
    put   "Receber ACRESCIMO         ;". 
    put replace(string(tt-total.receber_acrescimo),".",",") format "x(15)". 
    put skip.
    put   "   Receber ACRESCIMO LEBES;".
    put replace(string(tt-total.receber_acrescimo_lebes),".",",") 
                format "x(15)". 
    put skip.            
    put   "   Receber ACRESCIMO FINANCEIRA;". 
    put replace(string(tt-total.receber_acrescimo_fdrebes),".",",")
                format "x(15)".
    put skip.            
    end.
output close.
...
procedure relatorio:
    ...
    do vi = 1 to 15:
        if tt-total.define_venda_moeda[vi] <> ""
        then if substr(tt-total.define_venda_moeda[vi],1,3) <> "CAR" and
                substr(tt-total.define_venda_moeda[vi],1,1) <> "T"
            then    
        put "     " + tt-total.define_venda_moeda[vi] format "x(40)"
                  tt-total.venda_moeda[vi] format ">>>,>>>,>>9.99"
                  skip.
    end. 
    put "Venda CARTAO C/D "  format "x(40)" 
            tt-total.venda_cartao_cd  format ">>>,>>>,>>9.99"
            skip.
    vi = 0.
    do vi = 1 to 15:
        if tt-total.define_venda_moeda[vi] <> ""
        then if substr(tt-total.define_venda_moeda[vi],1,3) = "CAR" or
                substr(tt-total.define_venda_moeda[vi],1,1) = "T"
            then    
        put "     " + tt-total.define_venda_moeda[vi] format "x(40)"
                  tt-total.venda_moeda[vi] format ">>>,>>>,>>9.99"
    ...
    put "Venda RECARGA " format "x(40)"
        tt-total.venda_recarga[1] + tt-total.venda_recarga[2]
        format ">>>,>>>,>>9.99"
        skip.
    vi = 0.
    do vi = 1 to 15:
    ...
    put "Venda CARTAO PRESENTE" format "x(40)"
        tt-total.venda_cartaopresente[1] + tt-total.venda_cartaopresente[2]
        format ">>>,>>>,>>9.99"
        skip.
     vi = 0.
    do vi = 1 to 15:
        if tt-total.define_venda_cartaopresente[vi] <> ""
        then put "     " + 
            tt-total.define_venda_cartaopresente[vi] format "x(40)"
                 tt-total.venda_cartaopresente[vi] format ">>>,>>>,>>9.99"
                 skip.
    end.   
    vi = 0.
    /***
    ...
    *************/
    ...
    put  "Venda a PRAZO "   format "x(40)"
         ...
         skip "     Venda prazo SEM DOCFIS "   format "x(40)"
    ...
    */
    ...
    put  "JUROS recebimento "   format "x(40)"
         ...
         skip "        Origem LEBES "   format "x(40)"
         tt-total.novacao_lebes_ol format ">>>,>>>,>>9.99"
         skip "        Origem FINANCEIRA "   format "x(40)"
         tt-total.novacao_lebes_of format ">>>,>>>,>>9.99"
         ...
         skip "        Origem LEBES " format "x(40)"
         tt-total.acrescimo_novacao_lebes_ol format ">>>,>>>,>>9.99"
         skip "        Origem FINANCEIRA " format "x(40)"
         tt-total.acrescimo_novacao_lebes_of format ">>>,>>>,>>9.99"
    ...
    if vdti >= 08/31/17
    then do:
    put       "Receber ACRESCIMO        "  format "x(40)"
         tt-total.receber_acrescimo 
         format ">>>,>>>,>>9.99" skip.
    put       "   Receber ACRESCIMO LEBES     "  format "x(40)"
         tt-total.receber_acrescimo_lebes 
         format ">>>,>>>,>>9.99" skip.
    put       "   Receber ACRESCIMO FINANCEIRA"  format "x(40)"
         tt-total.receber_acrescimo_fdrebes 
         format ">>>,>>>,>>9.99" skip.
    end.
end.