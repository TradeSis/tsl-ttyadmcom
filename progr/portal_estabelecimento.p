...
define temp-table tt-valores no-undo serialize-name 'estabelecimento'
    ...
    field grupo	    as integer   serialize-name 'grupo'.
...
def var aa as int initial 0 no-undo.

for each estab no-lock:
    
    for each mixmgruetb no-lock
         where mixmgruetb.etbcod = estab.etbcod 
   	   and mixmgruetb.situacao = true:

        create tt-valores.
        assign tt-valores.codigo    = estab.etbcod
               tt-valores.descricao = fnremoveacento(estab.etbnom)
               tt-valores.cod-pai   = 1
	       tt-valores.grupo     = mixmgruetb.codgrupo.
	aa = aa + 1.
...
output to value("/home/kbase/teste.log").
put aa.
output close.
...
RUN PostRequest (INPUT 'produtos/integracaoEstabelecimento', 
                 INPUT lo-arquivo).
