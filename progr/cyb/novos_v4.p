...
for each contrato use-index datexp
    ...
    xtime = time - vtime.    
    if vi mod 10000 = 0 or vi < 2 or (xtime mod 60 = 0 and vi > 10000) 
    then do:
    ...
    if can-find(cyber_controle where cyber_controle.contnum = contrato.contnum
        no-lock)
    then do:
        next.
    end.    
    ...
    then do:
        ...
        **/         
        ...
        find cyber_controle where cyber_controle.contnum = contrato.contnum
          exclusive no-error.
