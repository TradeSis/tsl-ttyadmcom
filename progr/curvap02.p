...
repeat:
    ...
    update vcla-cod label "Clase Superior" colon 16 with frame f-dep.
    vclacod = vcla-cod.
/******/
...
/******/    
    ...
    update vcomcod label "Comprador" format ">>>9" 
            with frame f-dep centered color blue/cyan row 15.
    ...
    if opsys = "UNIX"
    then
        varquivo = "/admcom/relat/p02" + string(time) + STRING(day(today)) +
                                  STRING(month(today)) +
                                  string(categoria.catcod,"99").
    else        
        varquivo = "l:\relat\p02" + string(time) + STRING(day(today)) +
                                  STRING(month(today)) +
                                  string(categoria.catcod,"99").
