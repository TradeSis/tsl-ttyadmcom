...
/* ABAS */
def var vabas_estoq_depos as dec.
def var vabas_reserv_ecom as dec.
def var vabas_reservas as dec.
def var vabas_atende as dec.
def var vabas_disponivel as dec.
...
vestoq_depos = 0.  
/*find estoq where estoq.etbcod = 981  
                 and estoq.procod = par-procod  
                     no-lock no-error. 
vestoq_depos = if avail estoq
               then estoq.estatual
               else 0.
*/
find estoq where estoq.etbcod = 900
                 and estoq.procod = par-procod  
                     no-lock no-error.
vestoq_depos = vestoq_depos + (if avail estoq 
                               then estoq.estatual 
                               else 0).
...
run /admcom/progr/reserv_ecom.p (input  par-procod, 
    ...
    /*neo*/
    run abas/atendcalc.p    (input par-procod,
                             input ?,
                             output vabas_estoq_depos, 
                             output vabas_reserv_ecom,
                             output vabas_reservas,
                             output vabas_atende,
                             output vabas_disponivel).
    /*neo*/
    ...
    delete tt-reservas.
        ...
        then do.
            ...
            do vdata = today - 45 to today.
                for each liped use-index liped2 where liped.pedtdc = 95
...
vreservas = reserva + vespecial + vabas_reservas .
...
recalcula = no.
def var vatendeu-ecom as dec.
vatendeu-ecom = vreserv_ecom.
            ...
            then do.
                ...
                do vdata = today - 45 to today.
                    for each liped where liped.pedtdc = 95
                                     and liped.predt  = vdata
    ...
    vatendeu-ecom = vatendeu-ecom - liped.lipqtd.
    find last btt-reservas where 
              btt-reservas.prioridade < tt-reservas.prioridade
              use-index sequencia no-error.
...
if vdisponivel > 0 and vatendeu-ecom < 0 and vreserv_ecom > 0
then do:
    for each tt-reservas  use-index sequencia where tt-reservas.sequencia <> 50
                by tt-reservas.prioridade.
        find liped where recid(liped) = tt-reservas.rec_liped no-lock.
        if liped.etbcod <> 200
        then next.
        if vdisponivel >= ((-1) * vatendeu-ecom)
        then do:
            tt-reservas.atende = tt-reservas.atende +
                            ((-1) * vatendeu-ecom).
            vdisponivel = vdisponivel - ((-1) * vatendeu-ecom).
        end.
        else do:
            tt-reservas.atende = tt-reservas.atende + vdisponivel.
            vdisponivel = vdisponivel - ((-1) * vatendeu-ecom).
        end.                    
    end.
 end.
