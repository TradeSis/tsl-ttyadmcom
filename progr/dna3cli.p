...
def var graucompleto as char.
 def var valorlimite as dec.
  ...
  def var dtfuturo as date format "99/99/9999" initial today.
...
def var varquivo2 as character.
  ...
  update dtini colon 15 label "Contratos emitidos de"
                 dtfim label "ate".
				 
				 
				  dtfuturo = dtfim + 180.
				 
  update dtfuturo colon 15 label "Comportamento ate".
              
				 
				 
                 
 
varquivo2 = "/admcom/relat/dataset_cli-"
 + string(month(dtini))
 + string(year(dtini))
 +  "-" + string(time) + ".csv".
