...
for each tt-sel:  
    find wbsdocbase where recid(wbsdocbase) = tt-sel.rec no-lock.
    for each nffped where nffped.forcod = A01_infnfe.etbcod
                          and nffped.movndc = wbsdocbase.dcbcod no-lock:
    ...
    for each wbspeddocbpro of wbsdocbase no-lock. 
            find com.pedid where pedid.etbcod = wbspeddocbpro.etbdes
                             and pedid.pedtdc = 3
                             and pedid.pednum = wbspeddocbpro.pednum 
                             no-lock
                             no-error.                    
            if avail pedid
            then do on error undo:  
                ...
                for each liped of pedid. 
                    liped.transf-placod = A01_infnfe.placod.
                end. 
            end.
            find current pedid no-lock.
    end.
...
for each tt-sel.
    for each wbsdocbase where recid(wbsdocbase) = tt-sel.rec :
            wbsdocbase.situacao = "E".
            wbsdocbase.dtrettar = today.
            wbsdocbase.hrrettar = time.
            wbsdocbase.plani-etbcod = A01_infnfe.etbcod.
            wbsdocbase.plani-placod = A01_infnfe.placod.
            for each wbsdocbpro of wbsdocbase:
                wbsdocbpro.situacao = "E".
            end.
    end.
    delete tt-sel.
end.
