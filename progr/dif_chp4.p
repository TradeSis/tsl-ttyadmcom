...
repeat:
    ...
    if recatu1 = ?
    then
        find first tt-dif where tt-dif.vstatus = "NAO LIDO"
             no-error.
    else
    ...
    find chq where chq.banco   = tt-dif.banco   and
                   ...
                   chq.numero  = tt-dif.numero  no-lock no-error.
    ...
    */
    ...
    color display message
        esqcom1[esqpos1]
            with frame f-com1.
    repeat:
        find next tt-dif where tt-dif.vstatus = "NAO LIDO" no-error.
        if not available tt-dif
        then leave.
    ...
    repeat with frame frame-a:
        ...
        choose field tt-dif.valor
            go-on(cursor-down cursor-up
                  ...
                  page-down page-up
                  tab PF4 F4 ESC return 1 2 3 4 5 6 7 8 9 0).
        ...
        if  keyfunction(lastkey) = "1" or
            ...
            keyfunction(lastkey) = "0"
        then do with frame f-proc 1 down centered row 10 side-label
                    overlay:
            vprocura = keyfunction(lastkey).
            pause 0.
            sresp = yes.
            update vprocura
                editing:
            ...
            find first tt-dif where
                       tt-dif.valor >= dec(vprocura) no-error.
            if avail tt-dif
            then recatu1 = recid(tt-dif).
            next bl-princ.           
        end.   
        ...
        then do:
            do reccont = 1 to frame-down(frame-a):
                find prev tt-dif where tt-dif.vstatus = "NAO LIDO" no-error.
                if not avail tt-dif
                then leave.
                recatu1 = recid(tt-dif).
            end.
        ...
        then do:
            find next tt-dif where tt-dif.vstatus = "NAO LIDO"
                 no-error.
            if not avail tt-dif
            then next.
            color display normal
        ...
        then do:
            find prev tt-dif where tt-dif.vstatus = "NAO LIDO"
                 no-error.
            if not avail tt-dif
            then next.
            color display normal
