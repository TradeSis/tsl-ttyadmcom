...
if sresp then do:
    if opsys = "UNIX" then do:
        vdir = "/admcom/zebra/03/".
        varquivo = vdir + "eti-03.bat".

        unix silent value("rm -f " + varquivo).
        unix silent value("rm -f " + vdir + "c*.*").

        unix silent value("rm -f " + vdir + "03.zip").
        unix silent value("rm -f " + vdir + "03.ZIP").
        ...
        put unformatted "zip -q " vdir 
                "03.zip c*.* eti-03.bat *.GRF PKUNZIP.EXE UZIPA-01.BAT".
        ...
        varquivo = "l:~\zebra~\03~\eti-03.bat".
    
        if search(varquivo) <> ?
        then do:
            dos silent del l:~\zebra~\03~\eti-03.bat.
            dos silent del l:~\zebra~\03~\c*.* .
        end.        
    end.
    ...
    for each tt-etiq:
        run leb-etiq-imprime03.p (input tt-etiq.rec,
                                input tt-etiq.qtd,
    ...
    if opsys = "UNIX" then unix silent value("/admcom/zebra/03/zipa.sh").
end.
