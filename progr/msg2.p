...
FORM
  SPACE(2) pergunta FORMAT "x(50)" SPACE(2) SKIP
...
FORM
  SPACE(10) msg[1] FORMAT "x(10)" SPACE(6) SKIP
...
ASSIGN
  d      = 1
  msg[d] = pergunta.
DO WHILE (LENGTH(msg[d]) > 50 OR INDEX(msg[d],"!") > 0) AND d < 20:

  ASSIGN
    i          = MAXIMUM(
                 R-INDEX(SUBSTRING(msg[d],1,50)," "),
                 R-INDEX(SUBSTRING(msg[d],1,50),","))
    i          = (IF i = 0 THEN 50 ELSE i)
    j          = (IF INDEX(msg[d],"!") = 0 THEN 50 ELSE INDEX(msg[d],"!"))
    i          = MINIMUM(i,j)
...
PAUSE 0.
VIEW FRAME dbox-top.
DISPLAY new_lang[1] @ msg[1] WITH FRAME dbox-top.
IF NOT resposta THEN NEXT-PROMPT msg[1] WITH FRAME dbox-top.
DO ON ERROR UNDO,LEAVE ON ENDKEY UNDO, retry:
  CHOOSE FIELD msg[1] COLOR normal  AUTO-return WITH FRAME dbox-top.
END.
