...
repeat:
    ...
    for each tt-estab no-lock by tt-estab.qtdven:
        ...
        tt-estab.proper  = int(((tt-estab.qtdven * 100) / totven)).
    ...
    then display produ.procod at 15
                 ...
                 vqtd   label "Qdt Distribuida" at 15 format ">>>>9"
                 totven label "Total Vendido  " at 15 format ">>>>9"
                        with frame f-tit2 side-label width 120.
     
    
    for each tt-estab no-lock by tt-estab.etbcod:

          disp tt-estab.etbcod
               tt-estab.qtdven     column-label "Qtd Vendida"  format "->>>>9"
               tt-estab.proper(total) column-label "% Venda" 
                  format "->>9.99 %"
               tt-estab.qtddis(total) column-label "Qtd Distr" format "->>>>9"
               tt-estab.estatual(total) format "->>>>9"
                    with frame f3 centered down.
