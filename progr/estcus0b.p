...
def var qtot31 as int.
def var qtot41 as int.
def var qtot35 as int.
def var wnp as i.
...
def temp-table tt-estoq
    ...
    field qest41  like estoq.estatual
    field qest31  like estoq.estatual
    field qest35  like estoq.estatual.
...
repeat:
    ...
    for each produ no-lock:
        ...
        for each estab where estab.etbcod >= vetbi and
            ...
            */
            ...
            if produ.catcod = 31
            then assign vtot31 = (vqtd * estoq.estcusto)
                        qtot31 = qtot31 + vqtd.
            if produ.catcod = 41
            then assign vtot41 = (vqtd * estoq.estcusto)
                        qtot41 = qtot41 + vqtd.

            if produ.catcod <> 31 and
               produ.catcod <> 41
            then assign vtot35 = (vqtd * estoq.estcusto)
                        qtot35 = qtot35 + vqtd. 
            ...
            assign tt-estoq.est41 = tt-estoq.est41 + vtot41
                   ...
                   tt-estoq.qest41 = tt-estoq.qest41 + qtot41
                   tt-estoq.qest31 = tt-estoq.qest31 + qtot31
                   tt-estoq.qest35 = tt-estoq.qest35 + qtot35.
        end.
    end.
    ...
    for each tt-estoq by tt-estoq.etbcod:
        ...
        disp space(10) "Filial - " 
             ...
             tt-estoq.qest31(total)       column-label "Qt. Moveis"
                                format ">>>>>>9"
             tt-estoq.est41(total) column-label "Vl.Custo Confeccoes" 
                                format "->>,>>>,>>9.99" space(10)
             tt-estoq.qest41(total)        column-label "Qt. Confec."
                                 format ">>>>>>9"
             tt-estoq.est35(total) column-label "Vl.Custo Outros"
                                format "->>,>>>,>>9.99"    
             tt-estoq.qest35(total)        column-label "Qt. Outros"
                                format ">>>>>>9"    
