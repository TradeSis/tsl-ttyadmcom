...
* pfoestn.p - PERFORMANCE DE ESTOQUES
*******************************************************************************/
...
def var vfapro-op as char extent 7  format "x(10)"
init["DESCER","PRODUTO","FABRICANTE",
" ESTACAO "," CARACTER. ","PREC.VENDA","PREC.CUSTO"].
...
def temp-table tt-estacao no-undo
    field valorven  like plani.platot 
    field etbcod    like plani.etbcod
    field qtd       like movim.movqtm
    field valorcto  like plani.platot  
    field etccod    like estac.etccod
    field etcnom    like estac.etcnom 
    field clacod    like plani.placod 
    index etccod    etccod   asc
    index valor1    valorcto desc.

def temp-table tt-caract no-undo
    field valorven  like plani.platot label "Vl.Venda"
    field etbcod    like plani.etbcod
    field qtd       like movim.movqtm
    field valorcto  like plani.platot label "Vl.Custo"
    field carcod    like caract.carcod
    field subcod    like subcaract.subcod 
    field estilo    like caract.cardes
    field clacod    like plani.placod 
    index carcod    carcod   
                    subcod  
    index valor1    valorcto desc.
...
def temp-table tt-preco no-undo
    ...
    field qtd       like movim.movqtm
    field valorcto  like plani.platot  label "Vl.Custo"
    field preco     as dec label "Pr.Venda"
    field clacod    like plani.placod 
    index preco     preco   asc
    index valor1    valorcto desc.
...
def temp-table tt-custo no-undo
    ...
    field qtd       like movim.movqtm
    field valorcto  like plani.platot  label "Vl.Custo"
    field custo     as dec  label "Pr.Custo"
    field clacod    like plani.placod 
    index custo     custo asc
    index valor1    valorcto desc.
...
form
    tt-estacao.etccod  column-label "Cod" 
    help "F4=Retorna"
    tt-estacao.etcnom   format "x(35)"      /*9.99*/
    tt-estacao.qtd        format "->>>>>>>9" column-label "Qtd.Est" 
    tt-estacao.valorcto   format "->>>>9.99" column-label "Est.Cus" 
    tt-estacao.valorven   format "->>,>>9.99" column-label "Est.Ven" 
    /*v-perc              column-label "% V/E"  format "->>9.99" */
    v-perdia            format ">>9.99"
    with frame f-estac width 80 down title v-titpro.

form
    tt-caract.carcod  column-label "Cod" 
    help "F4=Retorna"
    tt-caract.subcod column-label "Sub-Caract."
    tt-caract.estilo  no-label format "x(20)"
    tt-caract.qtd        format "->>>>>>>9" column-label "Qtd.Est" 
    tt-caract.valorcto   format "->>>>9.99" column-label "Est.Cus" 
    tt-caract.valorven   format "->>,>>9.99" column-label "Est.Ven" 
    /*v-perc             column-label "% V/E"  format "->>9.99" */
    v-perdia            format ">>9.99"
    with frame f-caract width 80 down title v-titpro.

form
    tt-preco.preco  column-label "Preco" 
    help "F4=Retorna"
    vnomabr             format "x(20)"      /*9.99*/
    tt-preco.qtd        format "->>>>>>>9" column-label "Qtd.Est" 
    tt-preco.valorcto   format "->>>>9.99" column-label "Est.Cus" 
    tt-preco.valorven   format "->>,>>9.99" column-label "Est.Ven" 
    /*v-perc              column-label "% V/E"  format "->>9.99" */
    v-perdia            format ">>9.99"
    with frame f-preco width 80 down title v-titpro.

form
    tt-custo.custo  column-label "Custo" 
    help "F4=Retorna"
    vnomabr             format "x(20)"      /*9.99*/
    tt-custo.qtd        format "->>>>>>>9" column-label "Qtd.Est" 
    tt-custo.valorcto   format "->>>>9.99" column-label "Est.Cus" 
    tt-custo.valorven   format "->>,>>9.99" column-label "Est.Ven" 
    /*v-perc              column-label "% V/E"  format "->>9.99" */
    v-perdia            format ">>9.99"
    with frame f-custo width 80 down title v-titpro.
...
form
    ttgrupo.clacod   column-label "Grupo"
        help "ENTER=Seleciona F4=Retorna F8=Imprime P=Informacoes Filiais"
    clase.clanom   format "x(16)" column-label "Nome"
    ...
    v-perc           column-label "% V/E" format "->>9.99" 
    v-perdia         format "->>9.99"
    with frame f-grupo width 80 down title v-titgru.
...
form
    ...
    v-perc          column-label "% V/E"    format "->>9.99"
    v-perdia        format "->>9.99"
    with frame f-clase width 80 down title v-titcla.
...
help "ENTER=Seleciona F4=Retorna F8=Imprime P=Informacoes Filiais"
    ...
    v-perc            column-label "% V/E"     format "->>9.99"
    v-perdia          format "->>9.99"
    with frame f-sclase width 80 down title v-titscla.
...
repeat:
    ...
    if v-carcod > 0
    then  find caract where caract.carcod = v-carcod no-lock no-error.
    if v-carcod > 0
    then do on error undo: 
        ...
        find first  subcaract where 
                    ...
                    subcaract.subcar = v-subcod no-lock no-error.                      if avail subcaract
        then disp subcaract.subdes with frame f-etb.
        else undo, retry.
    end.
    else do:
         v-subcod = 0.
        ...
        {anbrowse.i
            ...
            &Where = " (if vetbcod = 0
                        then ttloja.etbcod  = ttloja.etbcod
                              ...
                              ttloja.etbcod = 0))"
            &NonCharacter = /*
            &Aftfnd = "assign v-perc = ttloja.valorven / ttloja.valorcto.
        ...
        repeat :
            ...
            {anbrowse.i
                ...
                &OtherKeys = " 
                            ...
                            then do.  run imprime-setor(input p-loja). 
                                an-seeid = -1. 
                                next keys-loop.  
                            end. 
            ...
            repeat :
                ...
                &OtherKeys = "if keyfunction(lastkey) = ""CLEAR""
                    ...
                    repeat:
                        ...
                        run disp-ttsclase-pfoestn.p( input p-setor,
                                                     input p-clase,
                                                     input p-loja,
                                                     input v-titscla,
                                                     output p-sclase
      ...
      assign v-totdia = 0 v-totger = 0.
        ...
        v-perdia = ttproduaux.valorven * 100 / v-totger."
                   ...
                   &Otherkeys = "
                       ...
                       then do:
                           find ttproduaux where 
                       recid(ttproduaux) = an-seerec[frame-line] 
                               no-error.
           ...
           repeat :
               ...
               for each ttproduaux:
                   ...
                   assign
                        ...
                        tt-fabri.valorcto = tt-fabri.valorcto + 
                                            ttproduaux.valorcto.
                    assign 
           ...
           repeat :
               ...
               for each ttproduaux:
                   ...
                   find first tt-estacao where
                              tt-estacao.etccod = produ.etccod 
                              no-error.
                   ...
                   assign
                    tt-estacao.valorven = tt-estacao.valorven + 
                                          ttproduaux.valorven
                    tt-estacao.qtd      = tt-estacao.qtd    + 
                                          ttproduaux.qtd
                    tt-estacao.valorcto     = tt-estacao.valorcto + 
                                              ttproduaux.valorcto.
                    assign 
                        v-totdia = v-totdia + ttproduaux.valorcto
                        v-totger = v-totger + ttproduaux.valorven.
               ...
               {anbrowse.i
                   &File   = tt-estacao
                   &CField = tt-estacao.etccod
                   &color  = write/cyan
                   &Ofield = " 
                   tt-estacao.etcnom 
                   tt-estacao.qtd 
                   tt-estacao.valorcto
                   tt-estacao.valorven  v-perdia "
                   &Where = " true "
                   &NonCharacter = /*
                   &AftFnd = " find first estac where  
                      estac.etccod = tt-estacao.etccod no-lock
                      no-error.
                      if avail estac
                      then tt-estacao.etcnom = estac.etcnom.
                      else tt-estacao.etcnom = ' '.     
                      v-perc = tt-estacao.valorven / tt-estacao.valorcto.
                      v-perdia = tt-estacao.valorven * 100 / v-totger."
                        &AftSelect1 = "next keys-loop."
                        &LockType = " use-index valor "
                        &Form = " frame f-estac down " 
               }.
               if keyfunction(lastkey) = "END-ERROR"
               then leave l3.
           end.
           hide frame f-estac no-pause. 
       end.
       
       /*  Filtro Por Caracterisitica - Sol. 26327 */ 
       if frame-index = 5
       then do:
           l4:
           repeat :
               ...
               assign an-seeid = -1 an-recid = -1 an-seerec = ?.
               v-titpro  = "CACTERISTICAS DA CLASSE ".
               ...
               for each ttproduaux:
                   ...
                   then do:
                        ...
                        then 
                        find first caract 
                        where caract.carcod = subcaract.carcod no-lock no-error.
                        else next.
                   end.
                   else next.
                   
                   find first tt-caract where
                              tt-caract.carcod = (if avail caract 
                                                  then caract.carcod 
                                                  else 0)  and
                              tt-caract.subcod = subcaract.subcar 
                                                             no-error.
                   if not avail tt-caract
                   then do:
                       create tt-caract.
                       assign
                       tt-caract.carcod = (if avail caract then caract.carcod
                                                          else 0) 
                       tt-caract.subcod = subcaract.subcar
                       tt-caract.estilo = vestilo.
                   end.
                   assign
                        tt-caract.valorven = tt-caract.valorven + 
                                    ttproduaux.valorven
                        tt-caract.qtd      = tt-caract.qtd + ttproduaux.qtd
                        tt-caract.valorcto = tt-caract.valorcto + 
                                             ttproduaux.valorcto.
                    assign 
                        v-totdia = v-totdia + ttproduaux.valorcto
                        v-totger = v-totger + ttproduaux.valorven.
               ...
               {anbrowse.i
                   &File   = tt-caract
                   &CField = tt-caract.carcod
                   &color  = write/cyan
                   &Ofield = "tt-caract.subcod 
                       tt-caract.estilo
                       tt-caract.qtd
                       tt-caract.valorcto
                       tt-caract.valorven v-perdia "
                   &Where = " true and tt-caract.carcod <> 0 "
                   &NonCharacter = /*
                   &AftFnd = " v-perc = tt-caract.valorven / tt-caract.valorcto.
                      v-perdia = tt-caract.valorven * 100 / v-totger."
                   &AftSelect1 = "next keys-loop."
                   &LockType = " use-index valor "
                   &Form = " frame f-caract down" 
               }.
               if keyfunction(lastkey) = "END-ERROR"
               then leave l4.
           end.
       ...
       then do:
           ...
           repeat :
               ...
               for each estoq where estoq.etbcod = estab.etbcod
                   ...
                   if estoq.etbcod >= 900 then next.
                   
                   for each ttproduaux where ttproduaux.procod = estoq.procod:
                        ...
                        assign tt-preco.valorven = tt-preco.valorven + 
                               ttproduaux.valorven
                        tt-preco.qtd = tt-preco.qtd + ttproduaux.qtd
                        tt-preco.valorcto = tt-preco.valorcto + 
                                            ttproduaux.valorcto.
                        assign 
                        v-totdia = v-totdia + ttproduaux.valorcto
                        v-totger = v-totger + ttproduaux.valorven.
               ...
               {anbrowse.i
                   ...
                   &color  = write/cyan
                   &Ofield = " tt-preco.valorven column-label ""Vl.Venda""
                   vnomabr format ""x(15)""
                       tt-preco.qtd  v-perdia
                       tt-preco.valorcto "
                   &Where = " true "
                   &NonCharacter = /*
                   &AftFnd = " assign vnomabr = ' '.    
                      v-perc = tt-preco.valorven / tt-preco.valorcto.
                      v-perdia = tt-preco.valorven * 100 / v-totger."
                        &AftSelect1 = "next keys-loop."
                        &LockType = " use-index preco "
                        &Form = " frame f-preco centered down" 
               }.
               if keyfunction(lastkey) = "END-ERROR"
               then leave l5.
           end.
       ...
       then do:
           ...
           repeat :
               ...
               for each estoq where estoq.etbcod = estab.etbcod
                   ...
                   for each ttproduaux where ttproduaux.procod = estoq.procod:
                        ...
                        find first tt-custo where
                                   tt-custo.etbcod = p-loja and
                                   tt-custo.custo  = estoq.estcusto
                                    no-error.
                        ...
                        then do:
                            create tt-custo.
                            assign
                                tt-custo.etbcod = p-loja
                                tt-custo.custo  = estoq.estcusto.
                        end.              
                        
                        assign tt-custo.valorven = tt-custo.valorven + 
                               ttproduaux.valorven
                        tt-custo.qtd = tt-custo.qtd + ttproduaux.qtd
                        tt-custo.valorcto = tt-custo.valorcto + 
                                            ttproduaux.valorcto.
               ...
               {anbrowse.i
                   &File   = tt-custo
                   &CField = tt-custo.custo 
                   &color  = write/cyan
                   &Ofield = " tt-custo.valorcto
                              vnomabr tt-custo.qtd  v-perdia
                       tt-custo.valorcto "
                   &Where = " true "
                   &NonCharacter = /*
                   &AftFnd = " assign vnomabr = ' '.    
                      v-perc = tt-custo.valorven / tt-custo.valorcto.
                      v-perdia = tt-custo.valorven * 100 / v-totger."
                      &AftSelect1 = "next keys-loop."
                      &LockType = " use-index custo "  
                      &Form = " frame f-custo centered down " 
               }.
               if keyfunction(lastkey) = "END-ERROR"
               then leave l6.
           end.
