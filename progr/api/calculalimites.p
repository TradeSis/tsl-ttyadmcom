...
then do:
    ...
    var-propriedades = "" .
      ...
      var-valorParcelasAtraso   = dec(pega_prop("VLRPARCVENC")).
      if var-valorParcelasAtraso = ? then var-valorParcelasAtraso = 0.
    ...
    */
    ...
    ttclien.comprometido   = var-comprometidoPrincipal.
    ttclien.saldoLimite    = vvlrdisponivel.

    ttclien.comprometidoTotal       = var-comprometidoTotal.
    ttclien.comprometidoPrincipal   = var-comprometidoPrincipal.
    ttclien.comprometidoNovacao     = var-comprometidoNovacao.
    ttclien.comprometidoNormal      = var-comprometidoNormal.
    
    /** #092022 */
      vDTULTCPA                 = date(pega_prop("DTULTCPA")).
    ttclien.dataUltimaCompra    =  vDTULTCPA.
    ttclien.quantidadeContratos         = int(pega_prop("QTDECONT")).
    ttclien.quantidadeParcelasPagas     = int(pega_prop("PARCPAG")).
    ttclien.quantidadeParcelasEmAberto  = int(pega_prop("PARCABERT")).
    ...
    then do: 
        ttclien.quantidadeAte15Dias = int(entry(1,r1,"|")). 
        ttclien.quantidadeAte45Dias = int(entry(2,r1,"|")). 
        ttclien.quantidadeAcima45Dias = int(entry(3,r1,"|")). 
    end.    
...
/**
    ...
    vvlrdisponivel = dec(pega_prop("LIMITEDISPEP")).
      ...
      *then vvlrdisponivel = 0.
      */
      ttclien.comprometido   = var-comprometidoEPPrincipal.
      ttclien.saldoLimite    = vvlrdisponivel.

      ttclien.comprometidoTotal       = var-comprometidoEPTotal.
      ttclien.comprometidoPrincipal   = var-comprometidoEPPrincipal.
      ttclien.comprometidoNovacao     = var-comprometidoEPNovacao.
      ttclien.comprometidoNormal      = var-comprometidoEPNormal.
