...
repeat:
    ...
    then do:
        if avail tresu
        then do:
            find first ttloja where ttloja.etbcod = tresu.etbcod no-error.
            if avail ttloja
            then.
             v-part = ( tresu.platot / tresu.geral) * 100.
        end.
        run frame-a.
    end.
...
procedure frame-a.
    display tresu.etbcod column-label "Etb"
            ...
            v-part column-label "% Part" format "->>9.99"
...
procedure p-imprime:
    ...
    if opsys = "UNIX"
    then varquivo = "/admcom/relat-auto/perf-p." + string(time) + ".txt".
    else varquivo = "l:~\relat-auto~\perf-p." + string(time) + ".txt".
