/* ************************************************************************
*  Progrma...: crm20-metr3.p
*  Funcao....: Metrica Detalhada com Filtro
***********************************************************************  */ 
...
def var c-contenz as char form "x(10)".
def var c-contenx as char form "x(10)".
...
assign c-contenx = "NEW"
       c-contenz = "FREE".
       
update c-contenx  label "Filtro" 
       c-contenz  label "ou"
       with frame f-filt side-label 
            2 col title "FILTRO".
...
do w3 = 1 to length(acao.classes[1]). 
    ...
    then do: 
        ...
        message tt-classes.clacod view-as alert-box.
...
/***/
    ...
    then do:
        for each tt-produtos:
            ...
            find first produ where
                     produ.procod = tt-produtos.procod                  and
                     produ.pronom  matches("*" + trim(c-contenz) + "*") or
    ...
    then do:
        for each tt-nprodutos:
            ...
            then do:
            find first produ where
                     produ.procod = tt-nprodutos.procod                  and
                     produ.pronom  matches("*" + trim(c-contenz) + "*") or
                     produ.pronom  matches("*" + trim(c-contenx) + "*")
                     no-lock no-error.
            if avail produ
    ...
    then do:
        for each tt-fabricantes:
            for each produ where
                     produ.fabcod = tt-fabricantes.fabcod              and
                     produ.pronom  matches("*" + trim(c-contenz) + "*") or
                     produ.pronom  matches("*" + trim(c-contenx) + "*")
                     no-lock:
    ...
    then do:
        for each tt-nfabricantes:
            for each produ where
                     produ.fabcod = tt-nfabricantes.fabcod              and
                     produ.pronom  matches("*" + trim(c-contenz) + "*") or
                     produ.pronom  matches("*" + trim(c-contenx) + "*")
                     no-lock:
    ...
    then do:
        for each tt-classes:
            for each produ where
                     produ.fabcod = tt-classes.clacod                   and
                     produ.pronom  matches("*" + trim(c-contenz) + "*") or
                     produ.pronom  matches("*" + trim(c-contenx) + "*")
                     no-lock:
    ...
    then do:
        for each tt-nclasses:
            for each produ where
                     produ.fabcod = tt-nclasses.clacod             and
                     produ.pronom  matches("*" + trim(c-contenz) + "*") or
                     produ.pronom  matches("*" + trim(c-contenx) + "*")
                     no-lock:
...
/*
for each tt-pro exclusive-lock:
...
for each acao-cli where 
    ...
    each plani where plani.movtdc  = 5 
                 ...
                 no-lock break /*by acao-cli.acaocod by acao-cli.clicod*/
                 by plani.desti:
        find first tt-campanha where
               tt-campanha.clicod = acao-cli.clicod no-error.
        if sresp and not avail tt-campanha
        ...
        for each movim where movim.etbcod = plani.etbcod
            ...
            /*
            message movim.movpc skip
                    movim.movqtm skip
                    movim.movdes skip
                    movim.movpdes skip
                    movim.movdev view-as alert-box.
            */
        end.
        ...
        /*
        for each bf-acao-cli where
                 bf-acao-cli.acaocod = acao-cli.acaocod   and
                 bf-acao-cli.clicod  = acao-cli.clicod    no-lock
                 break by acaocod by clicod:
                 
          if first-of(bf-acao-cli.clicod)
          then do:
