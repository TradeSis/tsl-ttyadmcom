...
update vclacod at 9 label "Classe" with frame f-for side-label.
if vclacod = 0
then display "GERAL" @ xclase.clanom with frame f-for.
else do:
    find xclase where xclase.clacod = vclacod no-lock.
    display xclase.clanom no-label with frame f-for.
end.

update vcomcod at 6 label "Comprador" format ">>>9"
         with frame f-for.
...
"MARGEM ---  ULTIMA COMPRA  -  ICMS SUB CUS.TOT. C O M P R A  M E S E S  A N T E R I O R E S"
skip
"                                               VENDA         EST. LUCRO%
 DATA      PRECO QTDE  " space(16)
space(1) vmes2[1] space(3)
vmes2[2] space(3)
vmes2[3] space(3)
vmes2[4] space(3)
vmes2[5] space(3)
...
for each tt-clase no-lock,
    ...
    */
    ...
    for each estab where estab.etbcod >= /*94*/ /*993*/ 900 no-lock:
    ...
    then do:
       do i = 1 to 12: 
            ...
            for each movim where movim.procod = produ.procod
                             and movim.movtdc = 6
                             and movim.emite = 22
                             and (movim.desti = 995 or movim.desti = 900)
                              no-lock:
