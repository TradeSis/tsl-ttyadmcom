...
then do:
    ...
    unix silent value("scp root@filial" + string(vetbcod,"999") +
        ":/usr/admcom/ecfinfo/" + varq + " /admcom/ecfinfo/arqfil" +
        " ; exit").
    
    unix silent value("ls -l /admcom/ecfinfo/arqfil/bemamfd*
                                > /admcom/ecfinfo/arqfil/bmfd.txt ; exit").
    def temp-table tt-arq
        field arquivo as char
        field arquivo1 as char.
    def var vl as char extent 10.    
    input from /admcom/ecfinfo/arqfil/bmfd.txt.
    repeat:
        import vl.
        create tt-arq.
        arquivo = vl[9].
    ...
    input close.    
    for each tt-arq:
        tt-arq.arquivo1 = substr(arquivo,1,16) +
                substr(entry(5,arquivo,"/"),1,7) +
             "_" + string(vetbcod,"999") +
             substr(entry(5,arquivo,"/"),8,12).
        unix silent value("mv " + tt-arq.arquivo + " " + tt-arq.arquivo1).
    end. 
end.
pause 0.
for each tt-arq:
...
varqret = tt-arq.arquivo1.
if search(varqret) <> ?
then do:
    input from value(varqret).
    repeat:
        create tt-linha.
...
for each wf-plani where wf-plani.movtdc <> 45:
    delete wf-plani.
end.
for each wf-plani where wf-plani.movtdc = 45:
