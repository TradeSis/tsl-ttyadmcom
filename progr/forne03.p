...
repeat:
    ...
    then
        find first forne where  
                   forne.fortip = "S" use-index ifornom no-error.
    else
    ...
    then do:
        ...
        do with frame f-inclui1  row 4  centered .
                ...
                do on error undo, retry:          
                   ...
                   if not avail munic 
                   then do:
                        message "Municipio ou UF inexistente." 
    ...
    color display message
        esqcom1[esqpos1]
            with frame f-com1.
    repeat:
        find next forne where forne.fortip = "S" use-index ifornom.
        if not available forne
        then leave.
        ...
        hide frame frame-a no-pause.
          ...
          then do:
            ...
            then do with frame f-inclui
                ...
                */
                ...
                if not avail serv_forne then do:
                    create serv_forne.
                    assign serv_forne.forcod = forne.forcod.
                end.
                
                update serv_forne.ind_org_pub.
                
                if serv_forne.ind_org_pub = yes then do:
                    update serv_forne.tipo_orgao_publico
                 help "Tipo Orgao Publico:AF,EF,SF,OF,AE,EE,SE,OE,AM,EM,SM,OM"
                 validate(lookup(serv_forne.tipo_orgao_publico, 
                        "AF,EF,SF,OF,AE,EE,SE,OE,AM,EM,SM,OM") <> 0,
                            "Opcao invalida").
                end.
                else serv_forne.tipo_orgao_publico = "".
                
                update serv_forne.ind_opt_simples_nacional
                       serv_forne.tipo_nota_servico
                       serv_forne.cod_uf
                       serv_forne.ind_contribui_cprb.
                       
                do on error undo, retry:
                    update serv_forne.ind_nif.
                       
                    if serv_forne.ind_nif < 1 or serv_forne.ind_nif > 3
                    then do:
                        message "Opcao invalida. Verifique as opcoes."
                                view-as alert-box.
                        undo, retry.
                    end.
                end.    
                    
                update serv_forne.ind_proc_admjud
                       serv_forne.ind_recup_falencia
                       serv_forne.ind_retencao_cs
                       serv_forne.ind_retencao_inss
                       serv_forne.ind_retencao_ir
                       serv_forne.inscricao_classe
                       serv_forne.orgao_classe.
            ...
            then do with frame f-altera
                ...
                if forne.fortipo <> 'S' then do:
                     message color red/with
                     "Fornecedor nao e do tipo Servico." view-as alert-box.
                     undo.         
                end.
