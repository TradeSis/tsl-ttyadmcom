...
def input parameter par-qtd as int /*initial 2*/.
...
disp fila with side-label.

if opsys = "UNIX"
then input from /admcom/progr/etifre9.ale.
else input from l:\progr\etifre9.ale no-echo.
/**
if opsys = "UNIX"
then varquivo = "/admcom/progr/etiqueta-d1.d".
else varquivo = "l:~\progr~\etiqueta-d1.d".
...
input close.
    ...
    westac  = trim(
              string(estac.etcnom,"x(8)")   + " " + vdtped +
              " TAM: " + string(par-taman,"x(3)")).
    if wetique > 0
    then do:
    ...
    then do:
      for each wfetq break by linha:
        ...
        then do:
            varquivo-sobra = "".
            varquivo-sobra = "/admcom/zebra-fila/cris" 
                           + string(produ.procod) 
                           + string(vtime).
