...
def new shared var vsel-sit1 as char format "x(15)" extent 1
          init["Nota Fiscal" /*,
               "Folha Pagamento",
               "Drebes Financeira",
               "Drebes Promotora",
...
def var vforcod like forne.forcod format ">>>>>>>>>9".
def var vqtd-lj as int.

def var vprocesso as char format "x(15)".
def var valor-me as dec format ">>,>>>,>>9.99".
def var vmoecod like moeda.moecod.
def var vdocumento as char format "x(9)".
def var vemissao as date format "99/99/9999".
def var val-conv as dec format ">>,>>>,>>9.99".
def var val-cambio as dec decimals 6 format ">>,>>9.999999".
def var vparcelas as int.
def var vdesp-me as dec format ">>,>>>,>>9.99".
def var vdesp-cambio as dec decimals 6 format ">>,>>9.999999".
def var vdesp-conv   as dec format ">>,>>>,>>9.99".
def var vmoe-me like moeda.moecod.
def var vmod-me like moeda.moecod.
...
repeat with frame ff1:
    ...
    ******/
    ...
    update vprocesso label " Numero processo"
           validate(can-find(first tpimport where
                              tpimport.numeropi = input vprocesso),
                              "Cadastrar processo.") 
           .
    find first tpimport where
               tpimport.numeropi = vprocesso no-lock no-error.
    if not avail tpimport
    then return.
    ...
    **/
    ...
    do on error undo.
        ...
        */
        ...
        update vdocumento at 8 label "Documento"
                    validate(vdocumento <> "","Informar o documento.")
                    /*with frame f2*/.
                    
        update vemissao   label "Emissao"
                    validate(vemissao <> ? and vemissao <= today,
                            "Informar a emissao.")
                    /*with frame f2*/.
 
        find first fatudesp where 
            fatudesp.fatnum = int(vdocumento) and
        ...
        update vmod-ctb label " Mod.CTB"
                    validate(can-find(modal where modal.modcod = vmod-ctb),
                                "Modalidade Invalida") with frame f2.
        find bmodal where bmodal.modcod = vmod-ctb NO-LOCK.
        display bmodal.modnom no-label with frame f2.
        if bmodal.modcod = "CRE"
        then do:
            message "modalidade nao cadatrada".
            pause.
        ...
        def var vmodctb-p as char.
        vmodctb-p = vmod-ctb.
        ***************/
                
        update skip vmoecod label "   Moeda" 
        validate(can-find(first moeda where
                    moeda.moecod = vmoecod),"Cadastrar moeda.")
            with frame f2 title " principal ". 
        find moeda where moeda.moecod = vmoecod no-lock.
        disp moeda.moenom format "x(10)" no-label with frame f2.
        vmoe-me = vmoecod.
        update skip valor-me     label "Valor ME"
                with frame f2.
        update skip val-cambio    label "  Cambio" when valor-me > 0
                    validate(val-cambio > 0,"Informat o valor do cambio.")
                    with frame f2.
        val-conv = valor-me * val-cambio.            
        update skip  val-conv     label "Total R$"
                    validate(val-conv > 0,"Informat o valor em R$.")
        ...
        update vmod-ctb label " Mod.CTB"
                    validate(can-find(modal where modal.modcod = vmod-ctb),
                                "Modalidade Invalida") with frame f3.
        find bmodal where bmodal.modcod = vmod-ctb NO-LOCK.
        display bmodal.modnom no-label with frame f3.
        if bmodal.modcod = "CRE"
        then do:
            message "modalidade nao cadatrada".
            pause.
        ...
        **************/
        ...
        update skip vmoecod label "   Moeda" 
            validate(can-find(first moeda where
                    moeda.moecod = vmoecod),"Cadastrar moeda.")
            with frame f3 title " despesas ". 
        find moeda where moeda.moecod = vmoecod no-lock.
        disp moeda.moenom format "x(10)" no-label with frame f3.
        vmod-me = vmoecod.
        update skip vdesp-me label "Valor ME"
                        with frame f3 side-label column 42.
        update skip vdesp-cambio    label "  Cambio" when vdesp-me > 0
                    validate(vdesp-cambio > 0,"Informat o valor do cambio.")
                    with frame f3.
        vdesp-conv = vdesp-me * vdesp-cambio.            
        update   vdesp-conv     label "Total R$"
                    validate(vdesp-conv > 0,"Informat o valor em R$.")
                    with frame f3 side-label .

        def var vtotal-conv as dec format ">>>,>>>,>>9.99".
        vtotal-conv = vdesp-conv + val-conv.
        update vtotal-conv label "Valor Toatal R$"
               validate(vtotal-conv > 0,"Favor informar o VALOR TOTAL em R$")
               with frame f4
               .
        if vdesp-conv > 0 and val-conv > 0
        then vparcelas = 2.
        else vparcelas = 1.
        *****************/
        ...
        update   vparcelas label "Quant. Parcelas"
                    validate(vparcelas > 0,"Informar o numero de parcelas,")
               with frame f4 side-label 1 column .
...
def var q-dias as int.
procedure dias-uteis:
    def input parameter vdata as date.
    def var vdt as date.
    do vdt = today + 1 to vdata:
        if weekday(vdt) = 1 or
           weekday(vdt) = 7
        then.
        else q-dias = q-dias + 1.    
    end. 
end procedure.
...
def temp-table tt-nfser
    field numeroDI  as char   label "Numero DI" format "x(20)"      
    ...
    field vmoe-p    like fatudesp.vmoe-p
    field valor-p   like fatudesp.valor-p
    field vcambio-p like fatudesp.vcambio-p
    field vconv-p   like fatudesp.vconv-p
    field vmoe-d    like fatudesp.vmoe-d
    field valor-d   like fatudesp.valor-d
    field vcambio-d like fatudesp.vcambio-d
    field vconv-d   like fatudesp.vconv-d
          ...
          then do:
            ...
            then do ON ERROR UNDO with frame ftitulo:
                ...
                update titulo.clifor column-label "Fornecedor"
                       ...
                       titulo.titdtemi
                       titulo.titdtven 
                       validate(titulo.titdtven >= data-util
                                    and weekday(titulo.titdtven) <> 7 and
                                        weekday(titulo.titdtven) <> 1,
                                         "Data invalida") 
                       titulo.titvlcob
...
procedure dd-nfser:
    ...
    assign
        tt-nfser.numerodi   = vprocesso
        tt-nfser.documento  = int(vdocumento)
        tt-nfser.emissao    = vemissao
        tt-nfser.val-total  = val-conv
        tt-nfser.val-acr    = vdesp-conv
        tt-nfser.val-liq    = vtotal-conv
        tt-nfser.vmoe-p     = vmoe-me
        tt-nfser.valor-p    = valor-me
        tt-nfser.vcambio-p  = val-cambio
        tt-nfser.vconv-p    = val-conv
        tt-nfser.vmoe-d     = vmod-me
        tt-nfser.valor-d    = vdesp-me
        tt-nfser.vcambio-d  = vdesp-cambio
        tt-nfser.vconv-d    = vdesp-conv
        tt-nfser.qtd-par    = vparcelas
        .
    /****************    
    disp tt-nfser.documento 
         ...
         ***/
         tt-nfser.qtd-par
         with frame f-doc side-label column 45 row 8
         /*
         title " " + vsel-sit1[vtipo-documento] + " "
         */
         .
    **********/
    repeat:
        ...
        */   
        ...
        then do:
            ...
            assign         
                ...
                tt-nfser.vmoe-p     = fatudesp.vmoe-p
                tt-nfser.valor-p    = fatudesp.valor-p
                tt-nfser.vcambio-p  = fatudesp.vcambio-p
                tt-nfser.vconv-p    = fatudesp.vconv-p
                tt-nfser.vmoe-d     = fatudesp.vmoe-d
                tt-nfser.valor-d    = fatudesp.valor-d
                tt-nfser.vcambio-d  = fatudesp.vcambio-d
                tt-nfser.vconv-d    = fatudesp.vconv-d
                 .
             /**************
            disp
                tt-nfser.val-total
    ...
    then do on error undo:
        ...
        assign
            fatudesp.numerodi   = tt-nfser.numerodi
            ...
            fatudesp.vmoe-p = tt-nfser.vmoe-p
            fatudesp.valor-p = tt-nfser.valor-p  
            fatudesp.vcambio-p = tt-nfser.vcambio-p 
            fatudesp.vconv-p = tt-nfser.vconv-p 
            fatudesp.vmoe-d = tt-nfser.vmoe-d  
            fatudesp.valor-d = tt-nfser.valor-d 
            fatudesp.vcambio-d = tt-nfser.vcambio-d 
            fatudesp.vconv-d = tt-nfser.vconv-d  
                             .
        ...
        for each tt-titudesp where
            ...
            */
            ...
            then do:
                create titulo.
                buffer-copy tt-titudesp to titulo.
                ...
                assign
                    titulo.titvlcob = 
                        titulo.titvlcob + tt-titudesp.titvlcob
                    vtot-tit =  vtot-tit + tt-titudesp.titvlcob .    
                        . 
            ...
            then do:
                create tituctb.
                buffer-copy tt-titudesp to tituctb.
                tituctb.modcod = vmod-ctb.
            end.
            else  tituctb.titvlcob = 
                        tituctb.titvlcob + tt-titudesp.titvlcob  .
        ...
        then do:
            ...
            then do:
                run lan-contabil("EXTRA-CAIXA","228",lancactb.contadeb,
...
procedure gera-titulo:
    ...
    repeat:
        ...
        then do:
            find first btitudesp where btitudesp.empcod = wempre.empcod and
                                     ...
                                     btitudesp.titnum = vtitnum       and
                                     btitudesp.titpar = vtitpar       and
                                     btitudesp.titdtemi = vtitdtemi   and
                                     btitudesp.titbanpag = vsetcod    
                                     NO-LOCK no-error.
            if avail btitudesp
            then do:                         
        ...
        do i = 1 to vtitpar with frame f-par 9 down row 8
                    title " Parcelas " 
                     + trim(string(tt-nfser.qtd-par,">9")) 
                     + "  Valor liquido "
                     + trim(string(tt-nfser.val-liq,">>>,>>9.99")) + " "
                      column 25 width 55 overlay:
            if keyfunction(lastkey) = "END-ERROR"
            then leave.
            vdia = 0.
            display i format ">9" column-label "Par" with frame f-par .
            pause 0.
            vvenc = data-util.
            ...
            def var tit-dol as dec.
            def var cota-dol as dec decimals 6.
            def var tit-desp as dec.
            def var cota-desp as dec.
            def var vmoeda-p as char.
            def var vmoeda-d as char. 
            do :
                tit-dol = tt-nfser.valor-p / tt-nfser.qtd-par.
                cota-dol = tt-nfser.vcambio-p.
                vmoeda-p = tt-nfser.vmoe-p.
                vmoeda-d = tt-nfser.vmoe-d.
                /*if i = 1
                then assign
                        tit-desp = tt-nfser.valor-d
                        cota-desp = tt-nfser.vcambio-d.
                */
                vtitvlcob = (tit-dol * cota-dol) +
                    (tit-desp * cota-desp).
                disp with frame f-par. pause 0.
                
                update tit-dol format ">>>,>>9.99"
                       column-label "Valor ME"
                       cota-dol format ">>9.999999" column-label "Cotacao"
                       /*
                       tit-desp format ">>>,>>9.99"
                       column-label "Despesa"
                       cota-desp format ">>9.999999" column-label "Cotacao"
                       */
                       .
                vtitvlcob = (tit-dol * cota-dol) +
                            (tit-desp * cota-desp).
                       
                disp  vtitvlcob format ">>>,>>>,>>9.99"
                        column-label "Total R$"
                                with frame f-par  .
                vt = vt + vtitvlcob.
            ...
            do :
                ...
                assign tt-titudesp.exportado = yes
                   ...
                   tt-titudesp.titbanpag = vsetcod
                   tt-titudesp.vmoe-p = vmoeda-p
                   tt-titudesp.valor-p = tit-dol
                   tt-titudesp.vcambio-p = cota-dol
                   tt-titudesp.vconv-p = tit-dol * cota-dol
                   tt-titudesp.vmoe-d = vmoeda-d
                   tt-titudesp.valor-d = tit-desp
                   tt-titudesp.vcambio-d = cota-desp
                   tt-titudesp.vconv-d = tit-desp * cota-desp
                   ii = ii + 1.
    ...
    /*sresp = no.
    message "Deseja realmente sair?" update sresp.
 ...
 procedure exclui-titulo:
        ...
        message color red/with
        "PARA EXCLUIR ENTRAR EM CONTATO COM O SETOR DE CONTABILIDADE."
        VIEW-AS ALERT-BOX.
...
procedure gera-titulo-parcial:
    ...
    def var par-modcod like modal.modcod.
