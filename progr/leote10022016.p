 ...
 * 09/04/2014 - emiminada da tela o display dos campos Disp993 e Disp995     *
...
/*            produ.prodtcad  format "99/99/9999" label "Dt.Cad"*/
            fabri.fabcod label "Fornec." 
            fabri.fabfant no-label with frame f1.
                                  
    color display message produ.pronom vprocod
            with frame f1.
     
    disp produ.prodtcad  format "99/99/9999" label "Dt.Cad" colon 48
         produ.datexp format "99/99/9999"                 
            label "DatExp" colon 67 skip(1) with frame f1.
    ...
    */
    ...
    run reserv_ecom.p (input  produ.procod,
                       output vreserv_ecom).
    vdisponiv993 = vdisponiv993 - vreserv_ecom.
    
    if vdisponiv995 < 0 then vdisponiv995 = 0.
    if vdisponiv900 < 0 then vdisponiv900 = 0.
  ...
  (vestatual  - vestatual993 - /*vestatual995 -*/ vestatual988 - vestatual500 
     - vest-outros-dep - vestatual981 - vestatual900)
                             label "Estloja" format "->>>>9"  colon 8
            vestatual993      label "Est993"
            pedidos_993      label "Pedidos" format "->>>>9"
    ...
    def var vdisponivel like vdisponiv993.
    run corte_disponivel.p (input  produ.procod,
                            output vestoq_depos, 
                            output vreservas, 
                            output vdisponivel).
    pedidos_993 = vreservas.
    vdisponiv993 = vdisponivel.
    estoq-indisponivel = 0.
    estoq-disponivel = 0.
    if vdisponiv993 < 0
    then estoq-indisponivel = vdisponiv993 * -1.
    else estoq-disponivel   = vdisponiv993.
    display 
    ...
    */
    ...
    find first wpro where wpro.etbcod = 993
                     no-lock no-error.
                    
    assign vdisponiv993 =  vestatual993 - (if avail wpro then wpro.wreserva 
                                                        else 0).
