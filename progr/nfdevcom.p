...
repeat:
    ...
    run tt-devcom1.p(input forne.forcod,
                    input vetbcod).       
    ...
    for each w-movim where w-movim.marca = "*":
        ...
        if ((w-movim.movqtm * w-movim.movpc) *
                      (w-movim.movalipi / 100)) <> ?
        then vipi    = vipi + ((w-movim.movqtm * w-movim.movpc) *
                                 (w-movim.movalipi / 100)).
    ...
    if vprotot = ?
    then assign vprotot = 0.
    
    if vfrete = ?
    then assign vfrete = 0.
    
    if vdescpro = ?
    then assign vdescpro = 0.
    ...
    ***/
    ...
    if sresp
    then run manager_nfe.p (input "5202",
                            input ?,
                            output v-ok).
