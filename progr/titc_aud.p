...
repeat:
        ...
        do vdata1 = vdti to vdtf:
            ...
            for each plani where plani.movtdc = 12
                ...
                for each ctdevven where
                         ...
                         no-lock:
                if ctdevven.etbcod-ori = 0
                then next.
                find first bplani where 
                        bplani.etbcod = ctdevven.etbcod-ori and
                          bplani.placod = ctdevven.placod-ori and
                          bplani.movtdc = ctdevven.movtdc-ori
                          no-lock no-error.
                if avail bplani and
                    bplani.crecod = 2
                then do:      
                    for each bmovim where bmovim.etbcod = bplani.etbcod and
                        ...
                        find first movim where 
                           ...
                           movim.procod = bmovim.procod
                           no-lock no-error.
                        if avail movim
                        then do:
                            find first tt-dev where
                                   tt-dev.clicod = bplani.desti  and
                                   tt-dev.numero = bplani.numero
                                   no-lock no-error.
                            if not avail tt-dev
                            then do:
                                create tt-dev.
                                assign
                                tt-dev.clicod = bplani.desti 
                                tt-dev.numero = bplani.numero
                                tt-dev.numdev = plani.numero
                                tt-dev.pladat = bplani.pladat
                                tt-dev.datdev = plani.pladat.
                            
                            end.    
                            tt-dev.val-dev = tt-dev.val-dev +
    ...
    **/
    ...
    for each tt-titulo 
        ...
        then disp tt-titulo.etbcod column-label "Fil" with frame fd1.
        disp tt-titulo.tipo column-label "Tipo" 
             tt-titulo.titvlcob column-label "Valor"
             with frame fd1 no-box down.
        if tt-titulo.tipo = "1-EMISSAO"
        THEN DO:
            for each tt-venda where tt-venda.etbcod = tt-titulo.etbcod no-lock:
                disp tt-venda.vl-vista column-label "C-Vista"
                            format ">>,>>>,>>9.99"
                     tt-venda.vl-prazo column-label "C-Prazo"
                            format ">>,>>>,>>9.99"
                     tt-titulo.titvlcob - tt-venda.vl-prazo
                     column-label "CC-Dif"
                            format "->,>>>,>>9.99"
                     with frame fd1.
            end.
...
procedure p-registro.
    ...
    **/
    ...
    /*** teste relatório 
    */
    output to value(varqexp) append.
    if voper <> "B"
    then do:
    put unformat skip
     ...
     vhist format "x(250)"               /* 144-393  Histórico */ .
        ...
        THEN DO:
    put unformat skip
     tt-estab.etbcod format ">>9"           /* 01-03 */
     string(tt-dev.clicod) format "x(18)" /* 04-21 */
     "TIT "  format "x(5)"                /* 22-26 */
     " " format "X(5)"                    /* 27-31 */ 
     tt-dev.numero format "999999999999"  /* 32-43 */
     year(vdtoper) format "9999"          /* 44-51 */
     month(vdtoper) format "99"          
     day(vdtoper)   format "99"
     "C"     format "x(3)"             /* 52-54 */
     vtitvlpag              format "9999999999999.99" /* 55-70 */
     vsinal                 format "x(1)"             /* 71 */
     year(tt-dev.pladat)  format "9999" /* 72-79 */
     month(tt-dev.pladat) format "99"  
     day(tt-dev.pladat)   format "99"
     Vtitvlcob format "9999999999999.99"  /* 80-95 */
     year(tt-dev.pladat) format "9999"  /* 96-103 */
     month(tt-dev.pladat) format "99"  
     day(tt-dev.pladat) format "99"
     string(tt-dev.numero)     format "x(12)" /* 104-115 nro arquivamento */   
     " " format "x(28)"                  /* 116-143 cod.contabil */
     vhist format "x(250)"               /* 144-393  Histórico */ .
     END.
     ELSE DO:
     ...
     "TIT "  format "x(5)"                /* 22-26 */
     " " format "X(5)"                    /* 27-31 */ 
     tt-dev.numdev format "999999999999"  /* 32-43 */
     year(vdtoper) format "9999"          /* 44-51 */
     month(vdtoper) format "99"          
     day(vdtoper)   format "99"
     voper     format "x(3)"             /* 52-54 */
     vtitvlpag              format "9999999999999.99" /* 55-70 */
     vsinal                 format "x(1)"             /* 71 */
     year(tt-dev.datdev)  format "9999" /* 72-79 */
     month(tt-dev.datdev) format "99"  
     day(tt-dev.datdev)   format "99"
     Vtitvlcob format "9999999999999.99"  /* 80-95 */
     year(tt-dev.datdev) format "9999"  /* 96-103 */
     month(tt-dev.datdev) format "99"  
     day(tt-dev.datdev) format "99"
     string(tt-dev.numdev)        format "x(12)" /* 104-115 nro arquivamento */      " " format "x(28)"                  /* 116-143 cod.contabil */
     vhist format "x(250)"               /* 144-393  Histórico */ .
        END.
    end.  
    output close.
