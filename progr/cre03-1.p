...
def var vdtpagini as date format "99/99/9999".
def var vdtpagfim as date format "99/99/9999".
...
def var vsel-sit as char format "x(15)" extent 2
        init["Pendente","  Pago  "]
        .
...
repeat:
    ...
    disp vsel-sit with frame f-sel 1 down no-label.
    choose field vsel-sit with frame f-sel.
    vindex = frame-index.
    hide frame f-sel.
    disp vsel-sit[vindex] with frame f-sel1 1 down 
    ...
    then do on error undo:
        update vdtpagini label "Pagamento Inicial" colon 20
               vdtpagfim label "Final"
               with row 4 side-labels width 80 frame f2.
        if vdtpagini = ? or
           vdtpagfim = ? or
           vdtpagini > vdtpagfim
        then undo.   
...
then do:
    ...
    do vdata = vdtvenini to vdtvenfim:
        output stream stela to terminal.
            disp stream stela vdata label "Data"
                with frame fff side-label row 18
        ...
        for each estab where estab.etbcod >= vetbcod and
            ...
            disp stream stela estab.etbcod titulo.titnum
                            with frame fff side-label row 18
                                                    color white/cyan centered.
            pause 0.
            ...
            find first ctitulo use-index iclicod
                    where ctitulo.empcod = wempre.empcod and
                                     ctitulo.titnat = no            and
                                     ctitulo.modcod = "CRE"         and
                                     ctitulo.titsit = "LIB"         and
                                     ctitulo.clifor = titulo.clifor and
                                     ctitulo.titdtven > vdtvenfim
                                                       no-lock no-error.
    ...
    find first ctitulo use-index iclicod
            where ctitulo.empcod = wempre.empcod and
                             ...
                             ctitulo.titsit = "LIB"         and
                             ctitulo.clifor = titulo.clifor  and
                             ctitulo.titdtven > vdtvenfim no-lock no-error.
    if avail ctitulo
    then next.
    ...
    run val-celular.
        ...
        for each tt-titulo
            ...
            display
                    ...
                    tt-titulo.titdtpag    column-label "Dt.Pag"
    ...
    do vdata = vdtvenini to vdtvenfim:
        ...
        for each estab where estab.etbcod >= vetbcod and
            ...
            each titulo use-index titdtven where
                 ...
                 titulo.etbcod = ESTAB.etbcod and
                 titulo.titdtven = vdata and
                 /*titulo.titdtpag > vdtvenfim and*/
                 titulo.titdtpag >= vdtpagini and
                 titulo.titdtpag <= vdtpagfim and
                 titulo.titvlcob >= vlmax 
                 no-lock:
