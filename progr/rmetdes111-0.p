...
def shared var vindex-orca as int.
def shared var vindex-proj as int.
...
for each fin.modal where fin.modal.modcod <> "DEV"
    ...
    find first modclasf where
               modclasf.tipo = 1 and
               modclasf.modcod = fin.modal.modcod
               no-lock no-error.
    if not avail modclasf
    then next.
    if vindex-orca = 1
       and modclasf.tipclas = 1
    then.
    else if vindex-proj = 1
        and modclasf.tipclas = 2
        then.
        else if vindex-orca = 2
                and modclasf.tipclas = 1
                and modclasf.tipclas = 1
            then.
            else if vindex-orca = 3
                and modclasf.tipclas = 1
                and modclasf.tipclas = 2
                then.
                else if vindex-proj = 2
                    and modclasf.tipclas = 2
                    and modclasf.tipclas = 1
                    then.
                    else if vindex-proj = 3
                    and modclasf.tipclas = 2
                    and modclasf.tipclas = 2
                    then.
                    else next.
        ...
        then do:
            ...
            for each bwgru where bwgru.wset = wgru.wset and
                ...
                disp 
                    ...
                    val-sal   format "->>,>>>,>>9.99" column-label "Saldo" 
                    bwgru.apagar format "->>,>>>,>>9.99" column-label "A pagar"
                    /*
