...
then do:
        for each crm where
                 crm.etbcod = (if vetbcod <> 0
                               then vetbcod                 
                               else crm.etbcod) exclusive-lock use-index ch03crm:
            assign ict = ict + 1.
            disp crm.clicod label "Cliente"
                 ict        Label "RG"
                 with side-label  1 down
                 title "EXCLUSAO CRM"
                 col 25  row 10 frame f-a1.

            pause 0 no-message.

            find first crmprodutos where
                       crmprodutos.clicod = crm.clicod no-error.
            if avail crmprodutos
            then delete crmprodutos.
           
            find first crmclasses where
                       crmclasses.clicod = crm.clicod no-error.
            if avail crmclasses
            then delete crmclasses.
                    
            find first crmfabricantes where
                       crmfabricantes.clicod = crm.clicod no-error.
            if avail crmfabricantes
            then delete crmfabricantes.
...
for each medias where
      ...
      then do:
              ...
              /*message iqtd-ori view-as alert-box.  */
