...
def var real-venda as dec.
def var real-cp as dec.
def var seg-cp as dec.
...
def var vmgarantia as dec.
def var vmrfq as dec.
...
def temp-table wplani no-undo
    ...
    field   wminad   as dec
    field   wrinad   as dec
    field   wmgarantia   as dec
    field   wrgarantia   as dec
    field   wrqgarantia   as dec
    field   wmrfq        as dec
    field   wrrfq        as dec 
    field   wrqrfq        as dec
    index i1 wetbcod
...
def temp-table tt-metacomis  no-undo
    ...
    field emissao like plani.pladat
    field numero like plani.numero
    field procod like produ.procod
    field pronom like produ.pronom
    field plano like plani.pedcod
    field f0 as dec           format ">>>>>>>9.99"
    ...
    field g3m as dec           format "->>>>>>>9.99"
    field g3c as dec           format "->>>>>>>9.99"
    ...
    field g7 as dec           format "->>>>>>>9.99"
    field d7 as dec           format ">>>>>>>9.99"
    field b7 as dec           format ">>>>>>>9.99"
    field v7 as dec           format ">>>>>>>9.99"
    field q7 as dec           format ">>>>>>>9.99"
    field g8 as dec           format "->>>>>>>9.99"
    field d8 as dec           format ">>>>>>>9.99"
    field b8 as dec           format ">>>>>>>9.99"
    field v8 as dec           format ">>>>>>>9.99"
    field q8 as dec           format ">>>>>>>9.99"
    field g9 as dec           format "->>>>>>>9.99"
    field d9 as dec           format ">>>>>>>9.99"
    field b9 as dec           format ">>>>>>>9.99"
    field v9 as dec           format ">>>>>>>9.99"
    field q9 as dec           format ">>>>>>>9.99"
    ...
    field mgarantia as int
    field mcrepes as int
    field mrfq as int 
    ...
    field segcp as dec        format ">>>>>>>9.99"
...
def var vr-garantia as dec.
def var vrq-garantia as dec.
def var vr-rfq as dec.
def var vrq-rfq as dec.
...
do:
    ...
    if (vindex = 2 or vindex = 3 or vindex = 4) and
       vetbcod > 0
    then do:
        update vfuncod label "Codigo Admcom"
                    with frame f-func 1 down side-label title " Consultor ".
        if vfuncod > 0
        then do:
        ...
        update vfun-fol label " Codigo Folha"
            with frame f-func 1 down side-label.
        if vfun-fol <> ""
        then do:
            find first func where func.etbcod = vetbcod  and 
                            func.usercod = vfun-fol
                            no-lock no-error.
            if not avail func
            then do:
                bell.
        ...
        */
            ...
            for each plani where plani.movtdc = 5             and
                ...
                for each tt-movim no-lock:
                    ...
                    find clase where clase.clacod = produ.clacod
                                no-lock no-error.
                    if avail clase
                    then find gclase where gclase.clacod = clase.clasup
                                    no-lock no-error.
                    ...
                    then do:
                        find first tt-metacomis where
                           tt-metacomis.etbcod = plani.etbcod and
                           tt-metacomis.numero = plani.numero and
                           tt-metacomis.procod = produ.procod and
                           tt-metacomis.funcod = plani.vencod
                           no-error.
                    if not avail tt-metacomis
                    then do:
                        ...
                        assign
                            tt-metacomis.etbcod = plani.etbcod 
                            tt-metacomis.funcod = plani.vencod
                            tt-metacomis.numero = plani.numero
                            tt-metacomis.procod = produ.procod
                            tt-metacomis.pronom = produ.pronom
                            tt-metacomis.emissao = plani.pladat
                            tt-metacomis.plano = plani.pedcod
                            .
                    end. 
                    end.
                    ...
                    if avail clase and
                             clase.clasup = 801030100
                    then assign
                            tt-metacomis.v7  = tt-metacomis.v7 +
                                (if tt-movim.movtot <> ?
                                 then (tt-movim.movtot)
                                 else (tt-movim.movpc * tt-movim.movqtm)
                                ) 
                            tt-metacomis.q7 = tt-metacomis.q7 + tt-movim.movqtm
                            vr-garantia = vr-garantia +
                                (tt-movim.movpc * tt-movim.movqtm)
                            vrq-garantia = vrq-garantia + tt-movim.movqtm
                            .    
                    else if avail clase and
                             clase.clasup = 801030200
                    then assign
                            tt-metacomis.v8  = tt-metacomis.v8 + 
                                (if tt-movim.movtot <> ?
                                 then (tt-movim.movtot)
                                 else (tt-movim.movpc * tt-movim.movqtm)
                                ) 
                            tt-metacomis.q8  = tt-metacomis.q8 + 
                                                    tt-movim.movqtm
                            vr-rfq = vr-rfq + 
                                    (tt-movim.movpc * tt-movim.movqtm)
                            vrq-rfq = vrq-rfq + tt-movim.movqtm
                            .
                    else if (/*produ.procod = 559910 or TP36782004*/
                        produ.procod = 559911 or
                        produ.procod = 578790 or
                        produ.procod = 579359 /* or TP36782004
                    ...
                    then do:
                        ...
                        then acum-rcp = acum-rcp +
                                (if tt-movim.movtot <> ?
                                then tt-movim.movtot else tt-movim.movpc).
                        if produ.procod = 569131
                        then assign
                                tt-metacomis.segcp = tt-metacomis.segcp +
                                    (if tt-movim.movtot <> ?
                                 then tt-movim.movtot else tt-movim.movpc).
                                seg-cp = seg-cp + 
                                (if tt-movim.movtot <> ?
                    ...
                    then do:
                        ...
                        then
                        disp produ.pronom tt-movim.movtot plani.numero
                        plani.serie.
                        */
                        ...
                        if produ.catcod = 41 or
                           produ.catcod = 45
                        then tt-metacomis.g3c = tt-metacomis.g3c + 
                                   (if tt-movim.movtot <> ? and
                                       tt-movim.movtot > 
                                       (tt-movim.movpc * tt-movim.movqtm)
                                    then (tt-movim.movtot - 
                        ...
                        else tt-metacomis.g3m = tt-metacomis.g3m + 
                                   (if tt-movim.movtot <> ? and
                                       tt-movim.movtot > 
                                       (tt-movim.movpc * tt-movim.movqtm)
                                    then (tt-movim.movtot - 
                    ...
                    then do:
                        ...
                        then   
                        assign
                             tt-metacomis.g9 = tt-metacomis.g9 +
                                (if tt-movim.movpc <> ?
                    ...
                    then do:
                        ...
                        if produ.pronom matches "*frete*"
                           and produ.catcod = 31
                        then tt-metacomis.f0 = tt-metacomis.f0
                                + (tt-movim.movpc * tt-movim.movqtm).
                        else tt-metacomis.g0 = tt-metacomis.g0 +
                                (tt-movim.movpc * tt-movim.movqtm).
                    end.
        ...
        assign
                vtotalmeta = 0 vmeta-31 = 0 vende-31 = 0 
                        ...
                        mcrepes = 0 vmgarantia = 0
                        vmrfq = 0.
        run leitura-das-metas(input estab.etbcod,
                                  input vdtf,
                                  output vtotalmeta,
                                  output vmeta-31,
                                  output vende-31,
        ...
        assign
            ...
            wplani.wmgarantia = vmgarantia
            wplani.wrgarantia = vr-garantia
            wplani.wrqgarantia = vrq-garantia
            wplani.wmrfq  = vmrfq
            wplani.wrrfq  = vr-rfq
            wplani.wrqrfq  = vrq-rfq
            .
...
"      MOVEIS".
        ...
        for each wplani by wplani.wetbcod:
            ...
            then do:
                 ...
                 display wplani.wetbcod column-label "Fl"
                        ...
                        vetbcod column-label "Fl" format ">>9" 
                                    with no-box frame f-a down width 302.
            ...
            disp wplani.wseg-v(total) column-label "Valor!Seguro"
                 ...
                 wplani.wrgarantia(total) column-label "Realizado!Garantia"
                 wplani.wrqgarantia(total) column-label "Realizado!Q.Garantia"
                 wplani.wmgarantia(total) column-label "Meta!Garantia"
                 wplani.wrrfq(total) column-label "Realizado!RFQ"
                 wplani.wrqrfq(total) column-label "Realizado!Q.RFQ"
                 wplani.wmrfq(total) column-label "Meta!RFQ"
                 with frame f-a.
    ...
    then do:
        ...
        /*
        varquivo = "/admcom/relat/exporta_rh_" +
                        string(vdti,"99999999") + "_" + 
                        string(vdtf,"99999999") + ".csv" 
                        .
         */
...
Troca/Devol;Bonus;Meta Garantia;Real Garantia;Meta RFQ;Real RFQ;
G7;G8;G9;Meta CP;Real CP"
         skip.
        for each tt-metacomis :
            tt-metacomis.g7 = tt-metacomis.v7.
            tt-metacomis.g8 = tt-metacomis.v8.
            if tt-metacomis.G1 = 0 and
               tt-metacomis.G2 = 0 and
               tt-metacomis.G3 = 0 and
               tt-metacomis.G4 = 0 and
               tt-metacomis.G6 = 0 and
               tt-metacomis.G7 = 0 and
               tt-metacomis.G8 = 0 and
               tt-metacomis.G9 = 0
            then next.   
            assign
                vtotalmeta = 0 vmeta-31 = 0 vende-31 = 0 
                        ...
                        minadin = 0 rinadin = 0
                        minadincp = 0 rinadincp = 0
                        mcrepes = 0 vmgarantia = 0 vmrfq = 0.
            ...
            assign
                ...
                tt-metacomis.mgarantia = vmgarantia / vende-31
                tt-metacomis.mrfq      = vmrfq / vende-31
                tt-metacomis.mcrepes   = mcrepes / (vende-41 + vende-31)
                /*
                tt-metacomis.rvenda = tt-metacomis.rvenda -
            ...
            assign
                ...
                tt-metacomis.g7 = tt-metacomis.g7 -
                        (tt-metacomis.d7 + tt-metacomis.b7)
                tt-metacomis.g8 = tt-metacomis.g8 -
                        (tt-metacomis.d8 + tt-metacomis.b8)
                tt-metacomis.g9 = tt-metacomis.g9 -
                        (tt-metacomis.d9 + tt-metacomis.b9) 
            ...
            if tt-metacomis.g7 < 0 then tt-metacomis.g7 = 0.
            if tt-metacomis.g8 < 0 then tt-metacomis.g8 = 0.
            if tt-metacomis.g9 < 0 then tt-metacomis.g9 = 0.
            
            tt-metacomis.g1 = tt-metacomis.g1 + tt-metacomis.f0.
            
            real-venda = tt-metacomis.g1 + 
                         tt-metacomis.g2 +
                         tt-metacomis.g3 +
                         tt-metacomis.g6 +
                         tt-metacomis.d1 + 
            ...
            real-cp = tt-metacomis.segcp + g4 + g9.
            /*
            message g1 g2 g3 g6 d1 d2 d6 b1 b2 b6.
            pause.
            */             
            ...
            put unformatted 
                ...
                replace(string(tt-metacomis.minadcp),".",",") ";"
                 replace(string(real-venda,">>>>>9.99"),".",",") ";"
               ...
               replace(string(tt-metacomis.devolucao,">>>>>>>9.99"),".",",") ";"
                ...
                tt-metacomis.mgarantia format ">>>>>9"  ";"
                tt-metacomis.q7        format ">>>>>9"  ";"
                tt-metacomis.mrfq      format ">>>>>9" ";"
                tt-metacomis.q8        format ">>>>>9"  ";"
                replace(string(tt-metacomis.g7),".",",") ";"       
                replace(string(tt-metacomis.g8),".",",") ";"
                replace(string(tt-metacomis.g9),".",",") ";"      
                tt-metacomis.mcrepes   format ">>>>>9"   ";"
                replace(string(real-cp,">>>>>9.99"),".",",") 
                skip.
        end.
        output close.
        message color red/with
    ...
    then do:
        ...
        def var valorgarantia as dec.
        def var valorrfq as dec.
        def var tipovendedor as char.
        def var statusvendedor as char.
        def var statusseguro as char.
        def var statusgarantia as char.
        def var comissm1 as dec.
        def var comissm2 as dec.
        def var comissm3 as dec. 
        def var comissm4 as dec. 
        def var comissm5 as dec. 
        def var comissm6 as dec. 
        def var comissm7 as dec.
        def var comissm8 as dec.
        def var comissm9 as dec.
        def var comiscm1 as dec.
        def var comiscm2 as dec.
        def var comiscm3 as dec. 
        def var comiscm4 as dec. 
        def var comiscm5 as dec. 
        def var comiscm6 as dec. 
        def var comiscm7 as dec.
        def var comiscm8 as dec.
        def var comiscm9 as dec.
        ...
        for each tt-lj no-lock:
            ...
            assign
                vtotalmeta = 0 vmeta-31 = 0 vende-31 = 0 
                        ...
                        minadin = 0 rinadin = 0
                        minadincp = 0 rinadincp = 0
                        comissm1 = 0 comissm2 = 0 comissm3 = 0
                        comissm4 = 0 comissm5 = 0 comissm6 = 0
                        comissm7 = 0 comissm8 = 0 comissm9 = 0
                        mcrepes = 0 vmgarantia = 0 vmrfq = 0.
            ...
            for each tt-metacomis where
             (tt-metacomis.moveis > 0 or tt-metacomis.moda > 0):
                ...
                assign
                    tt-metacomis.rinadcp = rinadincp
                    tt-metacomis.minadcp = minadincp
                    tt-metacomis.mgarantia = vmgarantia
                    tt-metacomis.mrfq = vmrfq
                    .
                ...
                then do:
                    ...
                    if(tt-metacomis.mseguro < tt-metacomis.g51) 
                    then assign 
                            statusseguro = "SIM"
                    ...
                    if (tt-metacomis.mgarantia < tt-metacomis.q7) 
                    then valorgarantia = (G7 * (7.5 / 100)).            
                    else valorgarantia = (G7 * (5 / 100)).                    
                    if (tt-metacomis.mrfq < tt-metacomis.q8) 
                    then valorrfq = (G8 * (7.5 / 100)).               
                    else valorrfq = (G8 * (5 / 100)).                  

                    /*se for moveis G3 eh 1.3 se for moda G3 eh 2.5 */
                    if(tt-metacomis.moveis >= tt-metacomis.moda) 
                    then assign 
        
                        v-sem-meta = ((G1 + G2 + G3m + G4) * (1.3 / 100)) +
                                     (G3c * (3.0 / 100)) +
                            valorseguro + (G6 * (3.0 / 100)) /*2.5 para 3.0*/ 
                            + valorgarantia + valorrfq 
                            + (G9 * (1.3 / 100))

                        comissm1 = (G1 * (1.3 / 100))
                        comissm2 = (G2 * (1.3 / 100))
                        comissm3 = (G3m * (1.3 / 100)) + (G3c * (3.0 / 100))
                        comissm4 = (G4 * (1.3 / 100))   
                        comissm5 = valorseguro
                        comissm6 = (G6 * (3.0 / 100))  /*2.5 para 3.0*/
                        comissm7 = valorgarantia
                        comissm8 = valorrfq
                        comissm9 = (G9 * (1.3 / 100))
                        .  
                        ...
                        v-sem-meta = ((G1 + G2 + G3m + G4) * (1.3 / 100)) + 
                        (G3c * (3.0 / 100)) + valorseguro + (G6 * (3.0 / 100))
                        + valorgarantia + valorrfq
                        + (G9 * (1.3 / 100))   
                    ...
                    assign
                        v-com-meta = (G1 * (1.5 / 100)) + (G2 * (2.5 / 100)) +
                             ...
                             (G3c * (3.6 / 100)) +
                             (G4 * (3.5 / 100)) +
                             valorseguro + (G6 * (3.0 / 100)) 
                             + valorgarantia + valorrfq 
                             + (G9 * (1.5 / 100))
                    
                        comiscm1 = (G1 * (1.5 / 100))  
                        comiscm2 = (G2 * (2.5 / 100))  
                        comiscm3 = (G3m * (3.5 / 100)) + (G3c * (3.6 / 100))  
                        comiscm4 = (G4 * (3.5 / 100))  
                        comiscm5 = valorseguro
                        comiscm6 = (G6 * (3.0 / 100)) /*3.5*/
                        comiscm7 = valorgarantia
                        comiscm8 = valorrfq
                        comiscm9 = (G9 * (1.5 / 100))  
                        .
                    ...
                    then do:
                        ...
                        then.
                           ...
                           disp tt-metacomis.etbcod column-label "Fl."
                               ...
                               v-com-meta  column-label "Comissao!Bate metas"
                                v-sem-meta  
                                    column-label "Comissao!Nao Bate Metas"  
                                tt-metacomis.devol 
                                    column-label "Troca!Devolucao"
                                ...
                                tt-metacomis.g7 column-label "G7"
                                tt-metacomis.g8 column-label "G8"
                                tt-metacomis.g9 column-label "G9"
    ...
    then do:
        vetbi = vetbcod.
        varquivo = "/admcom/relat/comissaolj_jr_" +
                        string(vdti,"99999999") + "_" + 
                        string(vdtf,"99999999") + "." + string(time)
                        .
        {mdad_l.i
        ...
        for each tt-lj no-lock:
            ...
            for each tt-metacomis :
                ...
                vtotal = G1 + G2 + G3 + G4 + G5 + G6 + G7 + G8 + G9.
                    
                disp    tt-metacomis.etbcod column-label "Fil"
                            tt-metacomis.funcod  column-label "Func"
                            func.funnom when avail func  format "x(20)"
                            tt-metacomis.folha   column-label "Folha"
                            tt-metacomis.numero format ">>>>>>>>9"
                                                 column-label "Venda"
                            tt-metacomis.emissao column-label "Emissao"                                      tt-metacomis.plano   column-label "Plano"
                            tt-metacomis.pronom  column-label "Desc. Produto" 
                             format "x(20)" 
                            tt-metacomis.g1(total)     column-label "G1"
                            tt-metacomis.g2(total)     column-label "G2" 
                            tt-metacomis.g3(total)     column-label "G3"
                            tt-metacomis.g4(total)     column-label "G4"
                            tt-metacomis.g5(total)     column-label "G5"
                            tt-metacomis.g6(total)     column-label "G6"
                            tt-metacomis.g7(total)     column-label "G7"
                            tt-metacomis.g8(total)     column-label "G8"
                            tt-metacomis.g9(total)     column-label "G9"
                            vtotal         (total)     column-label "Total G"
                            with frame f-lj1 down width 240.
                   down with frame f-lj1.     
            end.
        end.
        output close.
        run visurel.p (input varquivo, input "").
...
procedure mercadologico:
    ...
    then do vi = 1 to 20:
        ...
        if valger3[vi - 10] > 0
        then do: 
            find clase where clase.clacod = int(valger3[vi - 10]) 
                                no-lock no-error.
            if avail clase
            then do:
                if clase.clagrau = 4
...
procedure leitura-das-metas:    
    ...
    def output parameter vmgarantia as dec.
    def output parameter vmrfq   as dec.
...
procedure devolucao-de-vendas:
    ...
    for each ctdevven where
         ...
         no-lock:
         /*
        if ctdevven.placod-ven <> ? then next.
         */
        ...
        then do:
            ...
            else valacrescimo = oplani.biss - valtotal.
        end.
        if  valacrescimo = ? or
            valacrescimo < 0
        then valacrescimo = 0.   
        ...
        for each dmovim where
            ...
            then do:           
                ...
                find first tt-metacomis where
                           tt-metacomis.etbcod = oplani.etbcod and
                           tt-metacomis.numero = oplani.numero and
                           tt-metacomis.procod = produ.procod  and
                           tt-metacomis.funcod = oplani.vencod
                           no-error.
                if not avail tt-metacomis
                then do:
                        ...
                        assign
                            tt-metacomis.etbcod = oplani.etbcod 
                            tt-metacomis.funcod = oplani.vencod
                            tt-metacomis.numero = oplani.numero
                            tt-metacomis.procod = produ.procod
                            tt-metacomis.pronom = produ.pronom
                            tt-metacomis.emissao = oplani.pladat
                            tt-metacomis.plano = oplani.pedcod
                            .
                end. 
                end.
