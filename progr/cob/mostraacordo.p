{admcab.i}
{setbrw.i}                                                                      
def var vmotivo-cancela as char.
...
def var esqcom1         as char format "x(15)" extent 5
    initial ["  CONSULTAR","  FILTRAR","  CANCELAR","" ].
...
form novacordo.id_acordo     column-label "Codigo!Acordo"
     ...
     with frame f-linha 8 down width 80 row 8 .
...
procedure aftselect:
    ...
    THEN DO:
        ...
        novacordo.qtd_parcelas
         ...
         novacordo.motivo1 no-label format "x(50)"
    ...
    *        run efetivar-novacordo.p(input recid(novacordo),
    ...
    THEN DO:
        ...
        THEN DO:
            ...
            then do on error undo:
                if setbcod = 999
                then repeat:
                    update vmotivo-cancela format "x(60)"
                 ...
                 then do on error undo:
                    ...
                    for each tit_novacao where
                        ...
                        :
                         tit_novacao.tipo =  "RENEGOCIACAO|CANCELADA|" +
                                        vmotivo-cancela.
    ...
    THEN DO:
        ...
        DO vi = 1 to 20:
            ...
            create tt-cont.
            assign
                tt-cont.contnum = int(entry(1,acha("CONTRATO" + string(vi), 
                novacordo.char1),";")).
                tt-cont.valor   = dec(entry(2,acha("CONTRATO" + string(vi), 
                novacordo.char1),";")).
        ...
        for each tt-cont :
            disp tt-cont with frame f-ttcont 11 down row 7
            title string(novacordo.id_acordo).
        end.    
        for each tit_acordo of novacordo no-lock.
            disp tit_acordo.titpar
                 tit_acordo.titdtven
                 tit_acordo.titvlcob (total)
                with frame f-acord 11 down column 35 row 7
                    title string(novacordo.id_acordo).
        end.    
        pause.
    END.
...
procedure motivo-cancelamento:

    def var v-mot as char extent 6 format "x(40)".
    assign
        v-mot[1] = "1. Alteração Valor da Entrada"
        v-mot[2] = "2. Alteração Valor da Parcela"
        v-mot[3] = "3. Alteração Quantidade de Parcelas"
        v-mot[4] = "4. Alteração Data do Pagamento"
        v-mot[5] = "5. Alteração Valor Total"
        v-mot[6] = "6. Desistir do cancelamento"
        .

    disp skip(1) v-mot skip(1)
    with frame f-mot 1 down no-label column 30 row 10 overlay
    title " motivos de cancelamento ".
    choose field v-mot with frame f-mot.

    vmotivo-cancela = v-mot[frame-index].
