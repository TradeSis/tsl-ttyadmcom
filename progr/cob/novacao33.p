...
def new shared temp-table tp-contrato like contrato
...
run cob/ajustanovacordo.p (input vclifor, output p-temacordo).

if p-temacordo
then do:
/*    run cob/mostraacordo.p(input vclifor, input "PENDENTE").
    return.
  */
...
repeat:
    ...
    for each tp-contrato:
        ...
        for each tp-titulo where
            ...
            if vdat-atr > tp-titulo.titdtven
            then vdat-atr = tp-titulo.titdtven.
        end.             
        
        vdia-atr = today - vdat-atr.
        
        if tp-contrato.atraso > 270 or
           tp-contrato.origem = "LEP"
        then tp-contrato.destino = "LEP".
        else tp-contrato.destino = "ADM".
    ...
    ac-ok = no.
    vjuro-ac = 0.
    run cob/novacordo-create.p(input vclifor, input 0, input "", input 0, input ?,input no, input "",output ac-ok).
    if ac-ok = no
    then do:
        undo.
    end.
...
procedure sel-contrato_21:
    ...
    then DO:
        assign
            ...
            esqcom1[3] = "Marca/Desm/Tudo"
            esqcom1[4] = "  Gera Acordo".
    end.
    else do:         
        assign
            esqcom1[2] = "Marca/Desmarca"
            esqcom1[3] = "Marca/Desm/Tudo"
            esqcom1[4] = "Acordo Loja"
            esqcom1[5] = "".
        par-valor = "".
        run le_tabini.p (setbcod, 0, 
                               "FEIRAO-NOME-LIMPO", OUTPUT par-valor) .
        
        par-feirao = par-valor = "SIM".
        if par-feirao
        then do:
            esqcom1[5] = "Acordo Feirao".
            esqhel1[5] = "Confirma Novacao por Feirao".
            vfeirao = yes.
        end.
    END.

    esqcom2[1] = "Ver parcelas".
    esqcom1[1] = if p-temacordo
                 then "Ver Acordo"
                 else "".
    ...
    */
    ...
    if esqcom1[esqpos1] = "  Gera Acordo" or
       esqcom1[esqpos1] = " Confirma " or
       esqcom1[esqpos1] = " Feirao"
    then do:
        find first tp-contrato where 
    ...
    if esqcom1[esqpos1] = "Acordo Loja"
    then do:
        find first tp-contrato where 
            tp-contrato.exportado no-error.
        if not avail tp-contrato
        ...
        run cob/acordo-filial.p(vclifor, output ac-ok).
    ...
    if esqcom1[esqpos1] = "Acordo Feirao"
    then do:
        find first tp-contrato where 
            tp-contrato.exportado no-error.
        if not avail tp-contrato
        ...
        run cob/acordo-feirao.p (vclifor, output ac-ok).
        a-seeid = -1.
