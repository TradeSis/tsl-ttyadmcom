...
def buffer etb-emite for forne.
...
do:
    DO:
        ...
        find etb-emite where etb-emite.forcod = tt-plani.emite no-lock.
        find cpforne where cpforne.forcod = etb-emite.forcod no-lock no-error.
        find mun-emite where mun-emite.cidnom = etb-emite.formunic and
        ...
        bairro-emite = etb-emite.forbairro.
        
        cep-emite = int(etb-emite.forcep). 
        def var ibge-uf-emite as char.
        def var ibge-uf-desti as char.
        find first tabaux where  tabaux.tabela = "codigo-ibge" and
                        tabaux.nome_campo = etb-desti.ufecod 
                        no-lock no-error.
        if not avail tabaux
        then do:
            message color red/with
            "Codigo do IBGE nao cadastrado para UF " etb-desti.ufecod 
            view-as alert-box.
            v-ok = no.
            return.
        end.  
        ibge-uf-desti = tabaux.valor_campo.
        ibge-uf-desti = tabaux.valor_campo.
        
        vemitecgc = etb-emite.forcgc.
        ...
        then do:
            ...
            then do:
                ...
                assign
                    ...
                    B01_IdeNFe.cuf    = if ibge-uf-emite <> ""
                                then int(ibge-uf-emite) else 43
                    B01_IdeNFe.cnf   = dec(
                ...
                if program-name(2) = "outsai.p"
                then B01_IdeNFe.natop  = opcom.opcnom.
                else if tt-plani.notobs[2] <> ""
                    then B01_IdeNFe.natop  = tt-plani.notobs[2].   
                    else B01_IdeNFe.natop  = opcom.opcnom. 
            end.
        end.
        else v-ok = yes.
        if v-ok = no
        ...
        then do:
            ...
            assign
                ...
                C01_Emit.xnome = etb-emite.fornom
                C01_Emit.xfant = etb-emite.fornom
                ...
                /*C01_Emit.cpf = ""*/   
                C01_Emit.xlgr  = etb-emite.forrua
                C01_Emit.nro   = string(etb-emite.fornum)
                ...
                */
                ...
                /*C01_Emit.cpais 
                C01_Emit.xpais   */
                C01_Emit.fone = dec(etb-emite.forfone).
        end.           
