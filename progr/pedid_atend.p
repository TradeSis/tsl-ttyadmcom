...
repeat:
    ...
    repeat with frame frame-a:
        ...
        then do.
            if pedid.vencod > 0
            then 
                find plani where plani.etbcod = pedid.vencod and
        ...
        display esqcom1
                with frame f-com1.
        if com.liped.PendMotivo <> ""
        then
            display com.liped.PendMotivo
                    com.liped.Lip_Status
                    with frame fmotivo row 20
                    no-box no-label color message.
        else hide frame fmotivo no-pause.
...
def var vdisponivel     as int format ">>>>>>".
def var vinddisponivel like vdisponivel.
...
find com.produ where com.produ.procod = com.liped.procod no-lock. 
        ...
        vdisponivel = 0.
        vinddisponivel = 0.
        if com.liped.Est_disponivel >= 0
        then vdisponivel    = com.liped.Est_disponivel.
        else vinddisponivel = com.liped.Est_disponivel * -1.
                                                    
        disp com.liped.procod   column-label "Codigo"
             com.produ.pronom   format "x(20)" column-label "Produto"
             com.liped.lipqtd   column-label "Pedido" format ">>>>9" 
             com.liped.lipent   column-label "Atend" format ">>>>9" 
             
             
             com.liped.Est_deposito @ vestoq_depos column-label "Depos"    
             com.liped.Est_reserv   @ vreservas    column-label "Pedidos"

             vdisponivel    column-label "Dispo"
             vinddisponivel column-label "Indisp"
             
             com.liped.Prioridade column-label "Prio" format "->>>>>"
                    when com.liped.lipqtd <> com.liped.lipent
