...
do on error undo with frame f-filtro side-label.
    ...
    */
    ...
    update vdtini colon 16 vdtfim skip.
...
for each estab where estab.etbcod < 300 no-lock.
...
then
    for each tt-seguro,
        ...
        find plani where plani.etbcod = vndseguro.etbcod
                   ...
                   no-lock no-error.
        if avail plani
        then find finan where finan.fincod = plani.pedcod no-lock no-error.
        */
        ...
        disp
            ...
            finan.finnom when avail finan format "x(15)"
            log-operacao
