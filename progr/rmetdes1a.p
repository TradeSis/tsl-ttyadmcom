...
def var vdtf as date initial 12/05/2009.*/
...
do vdt = vdti to vdtf :
    ...
    do vs = 1 to vset:
     ...
     for each metadesp where  metadesp.etbcod = setbcod and                              metadesp.setcod = int(entry(vs,vsetcod,",")) and
    ...
    for each wmodal:
        ...
        do vs1 = 1 to vset:
            ...
            find first wpag where 
                       ...
                       wpag.wset = int(entry(vs1,vsetcod,",")) /*fin.titulo.titbanpag*/ and
...
def var val-salact as dec.
def var val-salactg as dec.
def var vsgwmet as dec.
def var vsgwpag as dec.
def var vsgwrec as dec.
def var val-salacg as dec format "->>,>>>,>>9.99".
def var vtotgeral as dec.
def var vtotgeralm as dec.
...
for each wsetor:
    for each wgru where wgru.wset = wsetor.wsetcod and
        ...
        for each wpag where wpag.wgru = wgru.wmod and
            ...
            for each swpag where swpag.wgru = wgru.wmod and
                                 swpag.wset = wgru.wset no-lock:
        ...
        then do:
           ...
           disp    "Totais" @ modal.modnom
            ...
            for each bwgru where bwgru.wset = wgru.wset and
                ...
                find first tt-totgru where tt-totgru.wmod = bwgru.wmod no-lock no-error.
        if avail tt-totgru then do:
            assign val-salac = tt-totgru.tot
                   vtotgeralm = val-salac + vtotgeral.
        end.    
                ...
                disp 
                    ...
                    val-sal   format "->>,>>>,>>9.99" column-label "Saldo"     
                    val-salac format "->>,>>>,>>9.99" column-label "Saldo Acumulado Ano"              
                with frame f-disp3 down width 130.
    ...
    then do:
        ...
        for each wac-cla where wac-cla.wmod <> ""  and
            ...
            disp modal.modcod  column-label "Sigla" 
                 ...
                 val-sal  format "->>,>>>,>>9.99" column-label "Saldo" 
                 val-salac format "->>,>>>,>>9.99" column-label "Saldo Acumulado"
    ...
    for each wmodal:
        ...
        do vs3 = 1 to vset:
            ...
            find first swpag where 
                       ...
                       swpag.wset = int(entry(vs3,vsetcod,",")) /*fin.titulo.tit~ ~banpag*/ and
