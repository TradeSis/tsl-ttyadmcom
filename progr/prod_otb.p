...
def var esqcom1         as char format "x(12)" extent 5
    initial [" "," Alteracao "," Consulta ","  Procura  ","  "].
def var esqcom2         as char format "x(12)" extent 5
            initial [" "," ","","",""].
def var esqhel1         as char format "x(80)" extent 5
    initial [" Inclusao  de produ ",
             " Alteracao da produ ",
             " Exclusao  da produ ",
             " Consulta  da produ ",
             " Listagem  Geral de produ "].
...
repeat:
    ...
    repeat with frame frame-a:
        ...
        then do:
            form produ.procod at 6
                 produ.pronom format "x(50)"
                 produ.opentobuy  at 4
                ...
                then do with frame f-Lista:
                    update "Deseja Imprimir todas ou a selecionada "
                           ...
                           with frame f-lista row 15 centered color black/cyan
                                 no-label.
                    if sresp
                    then run lprodu.p (input 0).
                    else run lprodu.p (input produ.procod).
                    leave.
                end.
            end.
            else do:
...
then  
    if esqascend  
    then  
        find first produ where true
                                                no-lock no-error.
    else  
        find last produ  where true
                                                 no-lock no-error.
...
then  
    if esqascend  
    then  
        find next produ  where true
                                                no-lock no-error.
    else  
        find prev produ   where true
                                                no-lock no-error.
