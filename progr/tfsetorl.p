...
form esqcom1
    with frame f-com1
    row 6 no-box no-labels column 1.
form esqcom2
    with frame f-com2
        row screen-lines - 1 /*title " OPERACOES " */
        no-labels column 1 no-box centered.
...
repeat :
    ...
    else find banfin.titulo where recid(banfin.titulo) = recatu1 NO-LOCK.
    vinicio = no.
...
/*                run tipo-documento. */
            ...
            find last btitulo where btitulo.empcod = wempre.empcod
                ...
                ***/
                ...
                update vcobcod.
                ***/
                find banfin.cobra where cobra.cobcod = vcobcod NO-LOCK.
                /**
                ...
                ***/
                ...
                then do on error undo, retry:
                    ...
                    */          
                        ...
                        find lanaut where lanaut.etbcod = banfin.titulo.etbcod
                                      and lanaut.forcod = banfin.titulo.clifor
                                                no-lock no-error.
                        if avail lanaut
                        then do:
...
and banfin.titulo.titsit <> "PAG" NO-LOCK no-error.
            recatu1 = if avail banfin.titulo
                      then recid(banfin.titulo) else ?.
           leave.
       end. on f7 help.
...
NO-LOCK no-error.
        ...
        then do on error undo, retry on endkey undo, leave.
          ...
          then do:
            ...
            then do with frame ftit2:

            clear frame ftit2 all.
...
/***/           
                ...
                if vtotal > 1000
                then do:
                    message "Confirma o valor informado " + 
                                string(vtotal,">>,>>9.99")
                    + "?" update sresp.
                    ...
                    run message.p (input-output sresp,
                           input "Confirma o valor informado ?" +
                                 " ",
                                   input " !! ATENCAO !! ",
                                   input "    SIM",
                                   input "    NAO").
...
***/                                               
            ...
            then do on error undo with frame ftitulo:
                ...
                find current banfin.titulo exclusive.
        ...
        then vcampanha = acha("CAMPANHA",banfin.titulo.titobs[2]).
                ...
                find cobra where cobra.cobcod = banfin.titulo.cobcod NO-LOCK.
                ...
                then do with frame fbanco:
                    ...
                    find agenc of banco
                             where agenc.agecod = banfin.titulo.agecod NO-LOCK.
                    display agedesc.
                end.
                ...
                ***/        
                ...
                if  banfin.titulo.titvlcob <> vtitvlcob
                then do:
                   if  banfin.titulo.titvlcob < vtitvlcob
                   then do:
                    assign sresp = yes.
...
/*                        banfin.titulo.titdtemi*/
            ...
            then do on error undo:
                find current banfin.titulo.
                if banfin.titulo.titsit <> "BLO"
                then do:
                    message "Confirma o Bloqueio ?" update sresp.
                    if  sresp then do:
                    ...
                    if banfin.titulo.titsit = "BLO"
                    then do:
                        message "Confirma a Liberacao ? " update sresp.
                        if  sresp then do:
                            banfin.titulo.titsit = "LIB".
                            banfin.titulo.datexp = today.
                        end.
...
procedure gera-num-tit:
    ...
    def var vminuto as int.
    def var vsegundo as int. 
    vdata = today.
    vdia = day(vdata).
    vmes = month(vdata).
    vano = year(vdata).
    vhora = int(substr(string(time,"hh:mm:ss"),1,2)).
    vminuto = int(substr(string(time,"hh:mm:ss"),4,2)).
    vsegundo = int(substr(string(time,"hh:mm:ss"),7,2)).
    vtitnum = string(vdia,">9") + string(vmes,"99") + 
            substr(string(vano),3,2) +
        string(vhora + vminuto + vsegundo,"999").
...
procedure frame-a.
    ...
    else vmatricula = acha("FUNCIONARIO",banfin.titulo.titobs[1]).
