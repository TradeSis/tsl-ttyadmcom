...
for each estab where estab.etbcod >= 990 no-lock:
    create tt-estab.
...
repeat:
    ...
    ***/
    ...
    if opsys = "UNIX"
    then varquivo = "/admcom/relat/entsai." + STRING(time).
    else varquivo = "..\relat\entsai" + STRING(time).
    ...
    put skip "Vl.Custo (ENTRADAS):   "   totent skip 
        "Vl.Custo (SAIDAS):     "   totsai 
        skip.
