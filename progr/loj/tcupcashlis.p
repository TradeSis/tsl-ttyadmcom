...
def input param pacao       as char init "abertos".
def input param ctitle      as char.
def var pmarcar as log format "Sim/Nao".
...
*/
    ...
    form  
    cupomb2b.origemcriacao column-label "ori"
    cupomb2b.idCupom  
...
disp 
    pqtd  label "Quantidade"      format ">>>>>>>>9" colon 60        with frame ftot
            side-labels
            row screen-lines - 2
            width 80
            no-box.
...
def var porigem             like cupomb2b.origemCriacao.
...
repeat:
    ...
    then do:
        for each cupomb2b where 
                        cupomb2b.tipocupom = "CASHB" and
                                cupomb2b.datacriacao >= vdataGini and cupomb2b.dataCriacao <= vdataGfim  and

                cupomb2b.dataTransacao = ? 
        and (if petbcod <> 0 then cupomb2b.etbCriacao = petbcod  else true)
        and (if porigem <> "" then cupomb2b.origem     = porigem else true ) 
    ...
    then do:
        for each cupomb2b where
                        cupomb2b.tipocupom = "CASHB" and
                                cupomb2b.datacriacao >= vdataGini and cupomb2b.dataCriacao <= vdataGfim  and
         (if petbcod <> 0 then cupomb2b.etbCriacao = petbcod  else true)
        and (if porigem <> "" then cupomb2b.origem     = porigem else true ) 
    ...
    repeat with frame frame-a:
        ...
        if pacao = "NAOCSV" then esqcom1[1] = "". else esqcom1[1] = "filtra".
        /*
        def var vx as int.
        def var va as int.
        va = 1.
...
repeat with frame fcab
    ...
    update porigem label "origem"
        help "origem criacao - LOJ=filial BOL=boleto".
    ...
    */        
    ...
    then do:
        update vdataTini 
            with frame fdatat centered side-labels title "Data Uso".
       if vdataTini = ?
       then do:
...
put unformatted 
    "IDCupom;codigoCliente;nomeCliente;telefone;" +
    "valorDesconto;dataValidade;codigoLoja;dataCriacao"
    skip.
...
/*        and (if pcla2-cod <> 0 then cupomb2b.clacod = pcla2-cod  else true ) 
        and (if pcatcod <> 0 then cupomb2b.catcod = pcatcod  else true) */
         (if pdataValidade <> ? then cupomb2b.dataValidade <= pdataValidade else true) 
        and (if porigem <> "" then cupomb2b.origem     = porigem else true ) 
...
procedure pcsvimp.
def var vtelefone as char.

    find clien where clien.clicod = cupomb2b.clicod no-lock no-error.
    vtelefone = "".
    if avail clien then vtelefone = if clien.fax = ? then clien.fone else clien.fax.
    if vtelefone = ? then vtelefone = "".
