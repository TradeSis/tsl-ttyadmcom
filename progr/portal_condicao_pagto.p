...
define temp-table tt-valores no-undo serialize-name 'condicaoPagamento'
...
for each crepl no-lock:
    create tt-valores.
    assign tt-valores.codigo    = crepl.crecod
           tt-valores.descricao = fnremoveacento(crepl.crenom)
...
RUN PostRequest (INPUT 'produtos/integracaoCondicaoPagamento', 
                                 INPUT lo-arquivo).
