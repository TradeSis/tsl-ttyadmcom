...
def new shared var vEstab     as log  format "Sim/Nao"  init yes.
...
def new shared var vdtini      like titulo.titdtemi.
def new shared var vdtfin      like titulo.titdtemi.
...
do with 1 down side-label width 80 row 3 color blue/white:
   ...
   */
   update vdtini vdtfin with frame fopcoes.
   /*
...
message vdata.
  ...
  if setbcod = 999 then do:
        run conecta_d.p .
        run conscai_d.p. /* Processa Dragao */ 
        disconnect d.
  end.
  
  
  /* Principal */
def var ttotaux1 as dec.
def var ttotaux2 as dec.
def var ttotaux3 as dec.
...
for each tt-dados break by tt-dados.etbcod by tt-dados.data.

aux-vljuro = 0.
for each tt-caixa-anali where tt-caixa-anali.etbcod = tt-dados.etbcod
                          and tt-caixa-anali.data   = vdata
                          and tt-caixa-anali.tpreg  = 5 no-lock.
    aux-vljuro = aux-vljuro + tt-caixa-anali.vltrans.
    tt-dados.aux-vljuro = tt-dados.aux-vljuro + tt-caixa-anali.vltrans.
end.
...
/*
message tt-dados.vlvist tt-dados.vlentr tt-dados.vlpres tt-dados.aux-vljuro tt-dados.total-cheque-devolvido tt-dados.vldesc tt-dados.vdevolucao tt-dados.vtotglo tt-dados.vlnov view-as alert-box.

message vlvist vlentr vlpres aux-vljuro total_cheque_devolvido vldesc vdevolucao totglo vlnov view-as alert-box.
*/

    assign  ttotaux1 = tt-dados.vlvist 
                     + tt-dados.vlentr 
                     + tt-dados.vlpres 
                     + /*vljuro*/ tt-dados.aux-vljuro
                     + tt-dados.total-cheque-devolvido
                     - tt-dados.vldesc
                     - tt-dados.vdevolucao 
                     + tt-dados.vtotglo 
                     - tt-dados.vlnov.
     assign ttotaux2 =  /*tt-dados.vpag 
                     +*/ tt-dados.vchedia 
                     + tt-dados.vdeposito 
                     + tt-dados.vpredre 
                     + tt-dados.vl-pagar 
                     + /*vl-cart*/ tt-dados.aux-vl-cart.

     assign ttotaux3 = ttotaux2 - ttotaux1.
     
     disp tt-dados.etbcod 
          tt-dados.data
          ttotaux1 column-label "TOTAL PROCESSADO"  format "->>,>>,>>>,>>9.99"             ttotaux2 column-label "TOTAL NUMERARIO"  format "->>,>>,>>>,>>9.99"            ttotaux3 column-label "DIFERENCA"  format "->>,>>,>>>,>>9.99"             with frame frelat no-box WIDTH 70 down.
       
    if last-of(tt-dados.etbcod)
    then put skip(1).
...
/*
    ...
    if not vEstab
    then do:
         find wfEstab where wfEstab.Etbcod = tt-caixa-anali.Etbcod no-error.
...
for each estab /*where  ( if vetbcod > 0
    ...
    then do:
      ...
      */
      ...
      find first tt-dados where tt-dados.etbcod = estab.etbcod 
                            and tt-dados.data = vdata no-error.
      if not avail tt-dados
      then do:
        create tt-dados.
        tt-dados.etbcod = estab.etbcod.
        tt-dados.data = vdata.
      end.
      
      if avail deposito and vcaixa = 0
      then assign vdeposito = vdeposito + deposito.depdep      
                  tt-dados.vdeposito = tt-dados.vdeposito + deposito.depdep.
      ...
      for each chq where chq.datemi = vdata no-lock:
          ...
          then do:
                 ...
                 then assign vchedia = vchedia + chq.valor
                            tt-dados.vchedia = tt-dados.vchedia + chq.valor.
          end.
          if chq.data > vdata
          then do:
                assign v-gravou-ana = no.
                ...
                if v-gravou-ana then  assign vpredre = vpredre + chq.valor
                           tt-dados.vpredre = tt-dados.vpredre + chq.valor.
          end.
      end.
    ...
    for each contrato where contrato.dtinicial = vdata and
        ...
        then do :
                assign  ct-praz = ct-praz + 1
                        tt-dados.ct-praz = ct-praz + 1
                        vlpraz  = vlpraz + contrato.vltotal
                        tt-dados.vlpraz = tt-dados.vlpraz + contrato.vltotal.
                /* Val.Prazo (Contratos) */
    ...
    for each titulo  
            ...
            then do:
                qtd-cart = qtd-cart + 1.
                tt-dados.qtd-cart = tt-dados.qtd-cart + 1.
                vl-cart = vl-cart + titulo.titvlpag.
                tt-dados.vl-cart = tt-dados.vl-cart + titulo.titvlpag.
                ...
                tt-dados.aux-vl-cart = tt-dados.aux-vl-cart + titulo.titvlpag.
                ...
                for each titpag where
                    ...
                    then do:
                        ...
                        tt-dados.vl-cart = tt-dados.vl-cart + titpag.titvlpag.
                        aux-vl-cart = aux-vl-cart + titpag.titvlpag.
                 tt-dados.aux-vl-cart = tt-dados.aux-vl-cart + titpag.titvlpag.
    ...
    for each caixa where caixa.etbcod = estab.etbcod no-lock:
        ...
        for each titulo where titulo.empcod = 19
            ...
            then do:
                    ...
                    tt-dados.total-cheque-devolvido =
                    tt-dados.total-cheque-devolvido + titulo.titvlpag
                    tt-dados.conta-cheque-devolvido =
                    tt-dados.conta-cheque-devolvido + 1.
    ...
    for each titulo where titulo.etbcobra = estab.etbcod and
            ...
            then do:
                for each titpag where
                    ...
                    then do:
                        ...
                        then assign vlpagcartao = vlpagcartao + titpag.titvlpag
                                    tt-dados.vlpagcartao =
                                    tt-dados.vlpagcartao + titpag.titvlpag.
                        else do:
                            vlpres = vlpres +  titpag.titvlpag.
                            tt-dados.vlpres = tt-dados.vlpres + titpag.titvlpag.
                        ...
                        then assign vljurpre = vljurpre + titulo.titjuro
                                    tt-dados.vljurpre = tt-dados.vljurpre + titulo.titjuro.
                ...
                then assign val-pago = titulo.titvlpag
                            tt-dados.val-pago = titulo.titvlpag.
                else assign val-pago = titulo.titvlcob
                            tt-dados.val-pago = titulo.titvlcob.
                ...
                then do:
                    ...
                    then do:
                        ...
                        tt-dados.vlpagcartao = tt-dados.vlpagcartao + val-pago.
                    end.
                    else do:
                        vlpres = vlpres + val-pago.
                        tt-dados.vlpres = tt-dados.vlpres + val-pago.
                        run Pi-Cria-Anali(input "titulo", input 4, 
            ...
            then do:
                    assign vlnov  = vlnov + titulo.titvlcob
                      tt-dados.vlnov = tt-dados.vlnov + titulo.titvlcob
                           ct-nov = ct-nov + 1
                           tt-dados.ct-nov = tt-dados.ct-nov + 1.
        ...
        for each titulo where  titulo.etbcobra = estab.etbcod and
            ...
            then do:
                assign ct-entr = ct-entr + 1
                  tt-dados.ct-entr = tt-dados.ct-entr + 1
                       vlentr  = vlentr  + titulo.titvlcob
                       tt-dados.vlentr = tt-dados.vlentr + titulo.titvlcob.
            ...
            ***/
            ...
            then do:
                ...
                assign vljuro = vljuro + titulo.titjuro
                       tt-dados.vljuro = tt-dados.vljuro + titulo.titjuro
                       vldesc = vldesc + titulo.titdesc
                       tt-dados.vldesc = tt-dados.vldesc + titulo.titdesc.
                p-juro = 0.
                if titulo.titjuro > 0
                then
            ...
            then do:
                if bmoeda.moetit = yes
                then assign ct-pagcartao = ct-pagcartao + 
                                          (if titulo.titvlcob > 0
                                           then 1 else 0)
                     tt-dados.ct-pagcartao = tt-dados.ct-pagcartao +
                                             (if titulo.titvlcob > 0
                                             then 1 else 0).
                else assign
                        ct-pres = ct-pres + (if titulo.titvlcob > 0
                                          then 1 else 0)
                        tt-dados.ct-pres = tt-dados.ct-pres +
                                           (if titulo.titvlcob > 0
                                            then 1 else 0).
            end.
            else assign
                    ...
                    tt-dados.ct-pres = tt-dados.ct-pres + (if titulo.titvlcob > 0 then 1 else 0).
                                
            ct-juro = ct-juro + if titulo.titjuro > 0
                                then 1
                                else 0.
            tt-dados.ct-juro = tt-dados.ct-juro + if titulo.titjuro > 0
                                                  then 1 
                                                  else 0.
            ct-desc = ct-desc + if titulo.titdesc > 0
                                then 1
                                else 0.
            tt-dados.ct-desc = tt-dados.ct-desc + if titulo.titdesc > 0
                                                  then 1
                                                   else 0.
         end.
 end.
            ...
            then do:
                ...
                then do:
                    ...
                    then assign vnumtra = vnumtra + plani.protot
                                                  /* + plani.frete */
                                                  + plani.acfprod
                                tt-dados.numtra = tt-dados.numtra
                                                + plani.protot + plani.acfprod.
                end.

                if plani.crecod = 1
                then do:
                    assign ct-vist  = ct-vist + 1
                            tt-dados.ct-vist = tt-dados.ct-vist + 1
                            ...
                            vlvist = vlvist + (plani.protot /* + plani.frete */
                                            + plani.acfprod - plani.descprod)
                                            - plani.vlserv.
                            tt-dados.vlvist = tt-dados.vlvist
                    ...
                    tt-dados.vlvist = tt-dados.vlvist - vvalor-cartpre.
                    vlauxt = vlauxt - vvalor-cartpre.
                    tt-dados.vlauxt = tt-dados.vlauxt - vvalor-cartpre.
                ...
                then do:
                        ...
                        tt-dados.vlpraz = tt-dados.vlpraz - plani.vlserv.
                        /* Val.Prazo (Contratos) */
                ...
                then do:
                     assign ct-dev = ct-dev + 1
                            tt-dados.ct-dev = tt-dados.ct-dev + 1
                            vldev = vldev + plani.vlserv
                            tt-dados.vldev = tt-dados.vldev + plani.vlserv.
                end.
            end.
        end.
 end.
...
/* tt-dados.qtd-pagar = 0.*/
 ...
 for each titluc where titluc.etbcobra = estab.etbcod and
          ...
          tt-dados.vl-pagar = tt-dados.vl-pagar + titluc.titvlpag.
          qtd-pagar = qtd-pagar + 1.
          tt-dados.qtd-pagar = tt-dados.qtd-pagar + 1.
