...
def temp-table tt-monmov
    ...
    field emite   like adm.monmov.emite
...
repeat:
    ...
    do on error undo:
        ...
        update vmoncod at 1 label "Montagem......."
            with frame f-dados.

        if vmoncod > 0
        then do:
        find adm.montagem where
             adm.montagem.moncod = vmoncod
             no-lock no-error.
        if not avail adm.montagem
        then do:
             bell.
             message color red/with
             "Tipo de Montagem não cadatrado."
             view-as alert-box.
             undo.
        end.
        v-forcod = int(acha("FORNECEDOR",adm.montagem.aux)).
        disp adm.montagem.monnom no-label format "x(20)" 
                    with frame f-dados.     
        disp v-forcod label "Fornecedor" with frame f-dados.
        end.
        else disp "TODAS" @ adm.montagem.monnom with frame f-dados.
    end.
            ...
            do vdata = vdata1 to vdata2:
                for each adm.monmov where adm.monmov.etbcod = estab.etbcod
                    ...
                    assign tt-monmov.etbcod  = adm.MonMov.EtbCod 
                           ...
                           tt-monmov.emite   = adm.monmov.emite
    ...
    then do:
        ...
        then do:
            ...
            then do:
                bell.
                message color red/with
                "Fornecedor não cadastrado para tipo de montagem "
                vmoncod
                view-as alert-box.
                next.
            end.
    ...
    then do:
        for each tt-monmov break by tt-monmov.etbcod
            ...
            disp space(6)             
                 ...
                 tt-monmov.emite  column-label "FilNF" format ">>9" 
                 tt-monmov.montcod column-label "NumMont" format ">>>>9"
...
procedure cria-desp:
    ...
    find first foraut where 
               foraut.forcod = v-forcod no-lock. 
    create titluc. 
