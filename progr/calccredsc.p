...
def var vtamanho as int.
    ...
    for each credscore where credscore.desc-campo = "ESTABCLI"
                       ...
                       by credscore.vl-ini.
/***                       
        if int(substr(string(clien.clicod),length(string(clien.clicod)) - 2, 3)) = int(credscore.campo)
***/
...
vguardacampo = ?.
    ...
    /*
message vcontapar vnumparcpg vtotalpar vvalparcpg view-as alert-box.
*/
...
find first fin.titulo where fin.titulo.titnat = no
                    and fin.titulo.clifor = clien.clicod
                    and fin.titulo.titdtpag = ?
                    and fin.titulo.titsit  = "LIB"
                    and fin.titulo.titpar >= 31
...
for each fin.titulo where fin.titulo.titnat = no
                      and fin.titulo.clifor = clien.clicod
                      and fin.titulo.titdtpag = ?
                      and fin.titulo.titsit  = "LIB" no-lock use-index iclicod.
