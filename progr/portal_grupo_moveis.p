...
define temp-table tt-valores no-undo serialize-name "grupoMixMoveis"
    field codigo    as integer   serialize-name "codigo"
    field descricao as character serialize-name "descricao"
    index idx01 codigo descricao.
    
define dataset bisteka for tt-valores.

for each tabmix no-lock:
    if not can-find(first tt-valores
                    where tt-valores.codigo    = tabmix.codmix
                      and tt-valores.descricao = tabmix.descmix) then do:
        create tt-valores.
        assign tt-valores.codigo    = tabmix.codmix
               tt-valores.descricao = tabmix.descmix.
    end.
end.
...
RUN PostRequest (INPUT 'produtos/integracaoGrupoMixMoveis', 
                                 INPUT lo-arquivo).
