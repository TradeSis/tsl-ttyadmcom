...
then return.*/
update vclacod at 1 with frame ff1.
find clase where clase.clacod = vclacod no-lock.
disp clase.clanom no-label with frame ff1.
update vvalor at 1 label "Preco de referencia"
        with frame ff1.
...
for each plani where plani.movtdc = 5 and
                     ...
                     no-lock.
    disp plani.pladat plani.numero format ">>>>>>>>9" with frame f-pro.
    pause 0.
    ...
    for each movim where movim.etbcod = plani.etbcod and
        ...
        if produ.clacod = vclacod and movim.movpc = vvalor
        then do:
            tt-promo.valor = tt-promo.valor + 
                        (movim.movqtm * movim.movpc).
        end.
    end.
end.     
end.         
for each tt-promo  where tt-promo.valor = 0:
       delete tt-promo.
end.       
def var vi as dec.
/*
for each tt-promo:
    ...
    /*tt-promo.premiov = tt-promo.premiov / vi.
    tt-promo.premiog = tt-promo.premiog / vi.
    */
end.*/
bl-princ:
repeat:
...
procedure relatorio:
    ...
    if opsys = "UNIX"
    then varquivo = "/admcom/relat/rbon" + string(day(today),"99") 
                                       + string(month(today),"99") 
                                       + string(year(today),"9999").
    else varquivo = "l:\relat\rbon" + string(day(today),"99") 
                                       + string(month(today),"99") 
                                       + string(year(today),"9999").
    ...
    {mdadmcab.i &Saida     = "value(varquivo)"   
                ...
                &Tit-Rel   = """VENDA RECARGA VIVO 26,00""" 
                &Width     = "100"
                &Form      = "frame f-cabcab"}
    disp with frame ff1.
