...
then do:
    find cybacparcela where 
            cybacparcela.idacordo = int(entry(1,banbolorigem.dadosOrigem)) and
            cybacparcela.parcela  = int(entry(3,banbolorigem.dadosOrigem))
        no-lock no-error.
    if not avail cybacparcela
    then return.
    find cybacordo of cybacparcela no-lock no-error.
    ...
    then do on error undo:
        ...
        then do:
            ...
            **/
            ...
            titulo.titobs[2] = "PROMESSA=" + string(cybacordo.IDACORDO).
          ...
          ASSIGN
            ...
            pdvdoc.Desconto_Tarifa   = 0
            pdvdoc.Valor_Encargo     = 0
            pdvdoc.hispaddesc        = "BAIXA DE BOLETO ACORDO CSL " + string(cybacordo.idacordo).
...
if banbolOrigem.tabelaOrigem = "titulo" and
