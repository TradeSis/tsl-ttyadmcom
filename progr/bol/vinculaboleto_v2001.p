...
then do on error undo:
    ...
    then do:
        for each cybacparcela where
            cybacparcela.idacordo = int(entry(1,par-dadosOrigem)) and
            cybacparcela.contnum  = int(entry(2,par-dadosOrigem)) /*and
            cybacparcela.parcela  = int(entry(3,par-dadosOrigem))   */
              no-lock .
            find cybacordo of cybacparcela no-error.
            if avail cybacordo
            then do:
                ...
                ASSIGN
                    ...
                    banBolOrigem.DadosOrigem  = par-DadosOrigem + "," + string(cybacparcela.parcela) .
