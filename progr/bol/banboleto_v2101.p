...
def var par-banco like banco.numban.
...
def var par-vencimento as log format "Sim/Nao" label "Filtra Vencimento".
    ...
    form
       /** ttboleto.marca  format " / " column-label "" **/
        banco.numban
        banboleto.impnossonumero format "x(18)"
...
procedure montatt.
    ...
    then do:
        for each banboleto where 
            banboleto.bancod    = banco.bancod and
...
repeat.
    ...
    find banboleto where 
            banboleto.bancod      = banco.bancod and
            banboleto.nossonumero = par-nossonumero 
        no-lock no-error.
    if par-nossonumero <> 0
    then do:
