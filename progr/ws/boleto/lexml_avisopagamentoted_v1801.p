...
def shared temp-table AvisoPagamentoTedEntrada
    field codigo_cpfcnpj as char
    field bancod as char
    field idted as char.


create AvisoPagamentoTedEntrada.
...
/**        message loop hc:subtype vh:name. **/
        ...
        then do:
            if vh:name = "AvisoPagamentoTedEntrada"
            then do:
                vtabela = "AvisoPagamentoTedEntrada".
            end.
            if vh:name = "Parcelas"
            then do:
                vtabela = "Parcelas".
        ...
        then do:
            ...
            if vtabela = "AvisoPagamentoTedEntrada"
            then do:
                case vh:name:
                when "codigo_cpfcnpj"        
                    then AvisoPagamentoTedEntrada.codigo_cpfcnpj = hc:node-value.
                when "banco"
                    then AvisoPagamentoTedEntrada.banco = hc:node-value.
                when "idted"       
                    then AvisoPagamentoTedEntrada.idted = hc:node-value.
                end case.
            end.
            if vtabela = "Parcelas"
            then do: 
