...
procedure cal-avp:
    ...
    message vdatref. pause.
    
    for each wclien where wclien.clicod > 49 no-lock,
        each GClasOPC use-index indx7 where
             GClasOPC.dtvisao = vdatref and
             GClasOPC.tpvisao = "GERENCIAL" and
             GClasOPC.clifor = wclien.clicod and
             GClasOPC.situacao = "" 
             no-lock:
        /*GClasOPC.valor-avp = 0.
          */   

        if GClasOPC.modcod <> "CRE" then next.
        
        /*if GClasOPC.cobcod = 10 then next.
        */
         
        for each titulo where titulo.empcod = 19 and
                          titulo.titnat = no and
                          titulo.modcod = GClasOPC.modcod and
                          titulo.etbcod = GClasOPC.etbcod and
        ...
        then do:
            ...
            then do:
                ...
                assign
                    ...
                    /*GClasOPC.valor-avp = GClasOPC.valor-avp +
                            (titulo.titvlcob / vb)
                    */
                    .
        ...
        else v-vencido = v-vencido + titulo.titvlcob.
