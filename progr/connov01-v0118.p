    ...
    */
    ...
    then do:
        ...
        for each tt-data use-index i1:
            tt-data.japlic = tt-data.vtotal - tt-data.origem.
    ...
    for each tt-cont where tt-cont.clicod > 0
            by tt-cont.dtinicial
                       by tt-cont.clicod :
        
        find clien where clien.clicod = tt-cont.clicod no-lock no-error.
        ...
        then do:
         ...
         for each titulo where titulo.empcod = 19 and
                          ...
                          no-lock:
            if titulo.tpcontrato <> "" /*** vparcela = 0 and
               titulo.titpar > 31***/
...
procedure sel-contrato.
    ...
    for each tt-modalidade-selec,
        each contrato where contrato.datexp = vdata
    ...
    for each tt-modalidade-selec,
        ...
        find first btitulo where 
                     ...
                     no-lock no-error.
        if avail btitulo
        then vdtinclu = btitulo.titdtemi.
        else vdtinclu = titulo.titdtemi.
