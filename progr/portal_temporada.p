...
define temp-table tt-valores no-undo serialize-name 'temporada'
...
for each temporada no-lock:
    create tt-valores.
    assign tt-valores.codigo    = temporada.temp-cod
           tt-valores.descricao = fnremoveacento(temporada.tempnom)
           tt-valores.cod-pai   = 1.
end.

/*dataset bisteka:write-json("file","/home/kbase/bkps/produtos/temporada.txt",true).*/
...
RUN PostRequest (INPUT 'produtos/integracaoTemporada', 
                 INPUT lo-arquivo).
