...
def var vclacod      like produ.clacod label "Classe".
def var ex-clacod    like produ.clacod label "Descarta Classe".
...
update vclacod colon 15 
  help "Informe a classe para geração. '0' = Todas"  with frame f-for.
if vclacod > 0
then do:
     find clase where clase.clacod = vclacod no-lock no-error.
     disp clase.clanom no-label with frame f-for.
     end.
else disp "Todas" @ clase.clanom with frame f-for. 
    
update ex-clacod label "Exclui Classe" colon 15 
   help "Informe a classe que sera descartadas. '0' = Todas" with frame f-for.
if ex-clacod > 0
then do:
     find bclase where bclase.clacod = ex-clacod no-lock no-error.
     disp bclase.clanom no-label with frame f-for.
     end.
else disp "Todas" @ bclase.clanom with frame f-for.      
...
for each produ where produ.catcod = vcatcod and
                     produ.fabcod = vfabcod no-lock
                                            break by produ.fabcod
                                                  by produ.pronom.
     if vclacod <> 0 and produ.clacod <> vclacod 
     then next. 
     
     if ex-clacod <> 0 and produ.clacod = ex-clacod 
     then next.
     
     if ex-clacod = 0 and produ.clacod <> vclacod
     then next.
