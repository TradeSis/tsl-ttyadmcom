...
do on error undo, return:
    ...
    if vdti = ? or vdtf = ?
    then undo.        
    update data_arq validate(data_arq <> ?,"") label "Data do Dia" /*at 01*/
                with frame f1 width 80 side-label overlay
                color with/cyan row 3.
end.
if vdti = ? or vdtf = ? or data_arq = ?
then do:
    return.
end.
...
repeat:                   
    view frame f1 .
    disp vdti vdtf data_arq with frame f1.
    pause 0.
   ...
   /****/
    ...
    if opsys = "UNIX"
    then varq = "/admcom/import/leitora/" + 
           string(year(data_arq),"9999") +
           string(month(data_arq),"99")  + 
           string(day(data_arq),"99") + ".txt".
    else varq = "..~\import~\leitora~\" + 
           string(year(data_arq),"9999") +
           string(month(data_arq),"99")  + 
           string(day(data_arq),"99") + ".txt".
        ...
        repeat:
            ...
            then do:
                ...
                then do:
                    ...
                    assign tt-chq.rec = recid(chq)
                           tt-chq.cmc7 = vreg
                           tt-chq.vmarca = "LI" /*LIDO*/.                                        end. 
            end.    
            else do:
                   
                   /****** antonio - novo cmc7 invalido *******/
                   find first tt-dif where 
                               tt-dif.banco   = int(vbanco)          and
                               tt-dif.agencia = int(vagencia)        and
                               tt-dif.conta   = vconta               and  
...
procedure procura-che:
    ...
    def var v-sobre-poe as logical format "Sim/Nao".
    ...
    {sklcls.i
        ...
        &naoexite1 = " bell.
                       message color red/with
                            ...
                            view-as alert-box .
                      " 
        /* Antonio - Sugerir Cmc7 Lido para Alterar */
        &aftselect1 = "
                        ...
                        then do:
                            ...
                            if v-sobre-poe = yes
                            then assign chq.agencia = p-agencia
                                        chq.conta   = p-conta
                                        chq.numero  = p-numero.           
                            else     
                            update  chq.agencia
