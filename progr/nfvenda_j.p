...
form produ.procod
     ...
     w-movim.movcsticms column-label "CST" format "xxx"
     w-movim.movalicms column-label "ICMS"
     w-movim.subtotal format ">>,>>9.99" column-label "Total"
     with frame f-produ1 row 8 10 down overlay
...
form
    ...
    with frame f1 side-label width 80 row 4 color white/cyan no-box.
...
repeat:
    ...
    if sresp
    then do:
        if tt-plani.opccod = 5502
        then run manager_nfe.p (input "5502_j",
                                input ?,
                                output vok).
        else run manager_nfe.p (input "5102_j",
                                input ?,
                                output vok).
    end.
end.
