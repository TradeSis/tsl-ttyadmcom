...
l1: repeat:
    ...
    {sklcls.i
        &file = pedaxnfv
        &cfield = pedaxnfv.etbcod
        &noncharacter = /*
        &ofield = "  
              pedaxnfv.etbcod column-label ""Fil"" format "">>9""
              pedaxnfv.procod column-label ""Produto""
              pedaxnfv.movqtm column-label ""Venda""        format "">>>9""
              pedaxnfv.qtdmix   column-label ""Mix""            format "">>>9""
                        when pedaxnfv.qtdmix <> 0
              pedaxnfv.estloja  column-label ""Estoque""    format ""->>>9""
                        when pedaxnfv.estloja <> 0
              pedaxnfv.qtdpedido  column-label ""Pedido""    format "">>>9""
                        when pedaxnfv.qtdpedido <> 0
              pedaxnfv.sugerido   column-label ""Suge!rido"" format "">>>9""
                        when pedaxnfv.sugerido <> 0
              pedaxnfv.ajustemin column-label ""Ajuste!Minimo""  format "">>>9""
                        when pedaxnfv.ajustemin <> 0
              pedaxnfv.ajustemix  column-label ""Ajuste!Mix""    format "">>>9""
                        when pedaxnfv.ajustemix <> 0
              pedaxnfv.entregaoutf column-label ""Entrega!OutFil"" 
                        format "">>>9"" when pedaxnfv.entregaoutf <> 0
              pedaxnfv.qtdconjunto   column-label ""Con!junto"" format "">>>9""
                        when pedaxnfv.qtdconjunto <> 0
              pedaxnfv.mixdifer column-label ""Mix!Difer""  format ""Sim/Nao""
    "
    &where = "
                (if vetbcod > 0 
                then pedaxnfv.etbcod = vetbcod else true) and
                (if vprocod > 0
                 then pedaxnfv.procod = vprocod else true) and
                 pedaxnfv.pladat /*datgera*/ = vdata
                 no-lock
                 "
    &naoexiste1 = " bell.
    ...
    &form = " frame ff-1 12 down title ""  ""  + string(vdata,""99/99/9999"") + ""  "" width 80 "
    }
...
procedure aftselect:
    ...
    def var vsituacao as char format "x(30)".
    def buffer bprodu for produ.
    clear frame f-linha1 all.
    if esqcom1[esqpos1] = "Consulta"
    THEN DO:
        if pedaxnfv.situacao = "ESP"
        then vsituacao = "VENDA PEDIDO ESPECIAL".
        else vsituacao = "VENDA PEDIDO AUTOMATICO".
        find produ where produ.procod = pedaxnfv.procod no-lock.
        if pedaxnfv.prosub > 0
        then find bprodu where 
                  bprodu.procod = pedaxnfv.prosub no-lock no-error.
        
        disp produ.procod    label "Produto"
             produ.pronom    label "Descricao"
             pedaxnfv.numero    label "Venda"  format ">>>>>>>>9"    
             pedaxnfv.pladat    label "Data Venda"
             pedaxnfv.pednum         label "Pedido"
             bprodu.procod  label "Substituto" when avail bprodu
             bprodu.pronom  label "Descrição"  when avail bprodu
             pedaxnfv.datgera label "Processamento"
             string(pedaxnfv.horgera,"hh:mm:ss") label "Hora Proc "
             vsituacao label "Situacao Venda"
    ...
    THEN DO:
        for each liped where liped.etbcod = pedaxnfv.etbcod and
                liped.procod = pedaxnfv.procod  and
                liped.lipsit <> "l" and
                liped.lipsit <> "C" and
                liped.lipsit <> "R"
                no-lock:
        ...
        for each movim where movim.etbcod = plani.etbcod and
            ...
            then do:
                ...
                if (not avail estoq or estoq.estatual <= 0)
                    and (not avail bestoq or bestoq.estatual <= 0)
                then next.                 
            end.
