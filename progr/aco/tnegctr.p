...
/*            help "Pressione L para Listar" */
        ...
        then do:
            ...
            then do:
                ...
                for each bttcontrato where bttcontrato.negcod = par-negcod.
...
if par-tipo = "pri" 
then do:
        find first ttcontrato  where ttcontrato.negcod = par-negcod
                no-lock no-error.
end.    
                                             
if par-tipo = "seg" or par-tipo = "down" 
then do:
        find next ttcontrato  where  ttcontrato.negcod = par-negcod
                no-lock no-error.
end.    
             
if par-tipo = "up" 
then do:
        find prev ttcontrato  where  ttcontrato.negcod = par-negcod
                no-lock no-error.
...
procedure ajustanegociacao.
        
        find current ttnegociacao.
        ttnegociacao.vlr_selecionado = 0.
        ttnegociacao.vlr_selaberto   = 0.
        ttnegociacao.qtd_selecionado = 0.
        
        for each cttcontrato where cttcontrato.negcod = ttnegociacao.negcod.    
            find contrato where contrato.contnum = cttcontrato.contnum no-lock. 
                if cttcontrato.marca
                then do:
                    ttnegociacao.vlr_selecionado = ttnegociacao.vlr_selecionado + cttcontrato.vlr_divida. 
                    ttnegociacao.vlr_selaberto   = ttnegociacao.vlr_selaberto + cttcontrato.vlr_aberto. 
