...
varquivo = "/admcom/relat/ltbradescoretrel" + string(time).
...
for each lotcretit of lotcre no-lock.
    ...
    if lotcretit.spcretorno <> ""  and
       lotcretit.spcretorno <> "BD" 
    then do:
    ...
    if (lotcretit.spcretorno = "BD" or
       lotcretit.spcretorno = "") and
       lotcretit.lottitpag  <> ?
    then vok = vok + 1.
    else verro = verro + 1.
    ...
    disp titulo.clifor
         ...
         lotcretit.lottitpag
         with frame f-lin down no-box width 175.
end.
put skip(1) "Corretos = " vok " Com erro = " verro skip(1).

for each tt-erro where tt-erro.mensagem <> "" no-lock.
