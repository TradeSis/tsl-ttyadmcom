...
run hnc/hcsimulamov.p (string(20)).
   ...
   numero_cupom         = string(vseq). 
   codigo_cliente       = string(tt-titdev.clifor). 
   valor_total_venda    = replace(string(tt-titdev.titvlcob),".",",").
...
for each tt-devolver.
    ...
    then next.
            ...
            valor_unitario_produto = replace(string(tt-devolver.movpc),".",",") . 
...
then do:
     hide message no-pause.
     message "Gravando P2K...".
     
     run hnc/wsorap2k.p ("insertTransacao", output vretorno).
 
     hide message no-pause.
     message "Retorno P2K..." vretorno.
