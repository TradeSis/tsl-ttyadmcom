...
def temp-table tt-plaent
    ...
    field valcob like plani.platot
    index i1 dtinclu
    index i2 titdtven
      .
     
update vetbcod like estab.etbcod at 7 label "Filial" with side-label width 80.
find estab where estab.etbcod = vetbcod no-lock.
disp estab.etbnom no-label.

def var vcatcod like produ.catcod.

update vcatcod at 1 label "Departamento"
...
disp "Todos os departamentos" @ categoria.catnom.     
...
update vdti  at 6 format "99/99/9999" label "Periodo"
...
def var vcatmovim like produ.catcod.
for each plani where plani.etbcod = vetbcod and
    ...
    tt-plaent.valcob = tt-plaent.valcob + titulo.titvlcob.
    end.    
end. 
...
for each tt-plaent where tt-plaent.numero > 0 no-lock
    ...
    disp tt-plaent.numero   column-label "Numero NF"  format ">>>>>>>>9"
         ...
         tt-plaent.platot(total) column-label "Total NF" format ">>,>>>,>>9.99"
         tt-plaent.valcob(total) column-label "Total!Cobrado" 
                        format ">>,>>>,>>9.99"
         with width 180
         .
...
UACAO;VENCIMENTO;DIASVENCIMENTO;TOTAL NF;TOTAL COBRADO"
SKIP.
