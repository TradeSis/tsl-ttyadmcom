...
def temp-table tt-plani
    ...
    index idx-tt01 EtbCod PlaCod Serie.
...
define buffer bf-movim for movim.
define buffer bf-produ for produ.
                    ...
                    &Otherkeys = " 
                                 ...
                                 THEN DO:
                                   ...
                                   v-titulo = string(setor.setcod) + 
                                             ' - ' + setor.setnom.
                                   run perf_brw.p (input v-titulo). next l95.
                                 END.   "
                    ...
                    &Otherkeys = "  IF KEYFUNCTION(LASTKEY) = ""CUT""
                                 ...
                                 THEN DO:
                                   ...
                                   for each ttnivel1 where
                                       ...
                                       then do:
                                           v-ttger = 0.
                                          for each bttnivel1 where 
                                          bttnivel1.etbcod = ttnivel1.etbcod                                       and bttnivel1.clasup = p-setor :
                                        v-ttger = v-ttger + bttnivel1.platot.
                                            end.
                    ...
                    &Otherkeys = " 
                                 ...
                                 THEN DO:
                                   ...
                                   for each ttnivel2 where
                                       ...
                                       then do:
                                           v-ttger = 0.
                                           for each bttnivel2 where 
                                            bttnivel2.etbcod = ttnivel2.etbcod 
                                       and bttnivel2.clasup = p-clase1 :
                                        v-ttger = v-ttger + bttnivel2.platot.
                                            end.  
                                           create tresu.
...
/****/                
                ...
                repeat :
                    ...
                    &Otherkeys = " 
                                 ...
                                 THEN DO:
                                   ...
                                   for each ttclase where
                                       ...
                                       then do:
                                           v-ttger = 0.
                                           for each bttclase where 
                                           bttclase.etbcod = ttclase.etbcod                                       and bttclase.clasup = p-clase2 :
                                        v-ttger = v-totger + ttclase.platot.
                                                end.    
                                           create tresu.
                    ...
                    repeat :
                        ...
                        run disp-ttsclase-convgen_.p(input p-clase,
                                                     input p-loja,
                                                     input v-titscla,
                                                     output p-sclase
                                                     ).
                                   ...
                                   for each ttsclase where
                                       ...
                                       then do:
                                           ...
                                           for each bttsclase where 
                                                    bttsclase.etbcod = ttsclase.etbcod                                              and bttsclase.clasup = p-clase :
    ...
    each plani where plani.movtdc = tipmov.movtdc
            ...
            */
            ...
            if not can-find(first tt-plani
                            where tt-plani.EtbCod = plani.EtbCod
                              and tt-plani.PlaCod = plani.PlaCod
                              and tt-plani.Serie  = plani.Serie)
            then do:
                    ...
                    for each bf-movim where bf-movim.etbcod = plani.etbcod and
                                            bf-movim.placod = plani.placod and
                                            bf-movim.movtdc = plani.movtdc and
                                            bf-movim.movdat = plani.pladat
                                                            no-lock:
                     find first bf-produ where bf-produ.procod = bf-movim.procod
                                                        no-lock no-error.
                        if avail bf-produ
                        then do:
                            if produ.catcod <> 31
                            then if bf-produ.catcod <> produ.catcod
                                  then vignora = yes.
                            if produ.catcod = 31
                            then if bf-produ.catcod = produ.catcod or
                                     bf-produ.procod = 88888
                    ...
                    then do:
                        ...
                        then assign val_acr = plani.biss - (plani.platot - plani.vlserv).
            ...
            */
            ...
            if plani.biss > (plani.platot - plani.vlserv)
            then assign val_acr = val_biss - (val_platot - val_dev).
            else val_acr = ((movim.movpc * movim.movqtm) / plani.platot) *
                                             plani.acfprod.
                                                 
            val_des =  ((movim.movpc * movim.movqtm) / plani.platot) *
                                             plani.descprod.
              
            v-valor = val_platot - val_des + val_acr.
...
/******/
            ...
            for each movim use-index datsai where movim.procod = produ.procod
                ...
                */
                ...
                val_biss = ((movim.movpc * movim.movqtm) / plani.platot) *                                                      plani.biss.                     
                val_platot = ((movim.movpc * movim.movqtm) / plani.platot) *                                                    plani.platot.                   
                if plani.biss > (plani.platot - plani.vlserv)                                   then assign val_acr = val_biss - (val_platot - val_dev).                        else val_acr = ((movim.movpc * movim.movqtm) / plani.platot) *                                                  plani.acfprod.                  
               ...
               for each ttclase where ttclase.etbcod = p-loja
                  ...
                  for each ttsclase where ttsclase.etbcod = p-loja
                     ...
                     for each ttprodu where ttprodu.etbcod = p-loja
                        ...
                        find   produ where 
                                produ.procod = ttprodu.procod no-lock.
                        find first  estoq where (estoq.etbcod < 900 and
                                    {conv_driigual.i estoq.etbcod}) 
        ...
        l6: 
            repeat : 
                ...
                for each ttaux-produ:
                     ...
                     find first estoq where /*estoq.etbcod < 900
                                 and*/  estoq.etbcod = ttaux-produ.etbcod
/*{conv_igual.i estoq.etbcod} */
                                 and estoq.procod = produ.procod 
                                     no-lock no-error.
                        if not avail estoq
                        then find first estoq where estoq.procod = produ.procod
                        no-lock.                                     
                     find first tt-preco where
                                 tt-preco.preco = estoq.estvenda
                                 no-error.
