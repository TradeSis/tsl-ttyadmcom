...
repeat with frame f2:
    for each {1} where {6} by {3} with frame f2 i = 1 to 13: 
        array[i] = string(pronom,"x(52)") + string(procod,">>>>>>99").
    ...
    prompt-for campo with frame f1 editing:
        ...
        if lastkey = keycode("PAGE-DOWN")
            ...
            if array[13] <> ""
                ...
                for each {1} where ({6}) and {3} > array[13] by {3} j = 1 to 13:
                    array[j] = string(pronom,"x(52)") + string(procod,">>>>>>99").
                ...
                if j > 1
                  then do:
                    ...
                    for each {1} where ({6}) and {3} > array[1] by {3}
                                 j = 2 to 13:
                        array[j] = 
                            string(pronom,"x(52)") + string(procod,">>>>>>99").
        ...
        then do:
            ...
            then i = i - 1.
                ...
                for each {1} where ({6}) and string(string(pronom,"x(52)") + 
                                                   string(procod,">>>>>>99"))
                                        < array[1] by {3} 
                                                descending j = 1 to 1:
                                                
                    aux = string(pronom,"x(52)") + string(procod,">>>>>>99").
        ...
        if input campo >= array[1] and input campo <= array[13]
            ...
            if available {1}
                then do:
                array = "".
                array[1] = string(pronom,"x(52)") + string(procod,">>>>>>99").
