...
def input parameter vcatcod as integer.
def output parameter varquivo as char.

/*
def var vcatcod as integer initial 31.
    ...
    ********************************************************/
    ...
    for each estab where if vetbcod = 0
        ...
        for each movim where movim.etbcod = plani.etbcod and
            ...
            find first adicla where adicla.clacod = produ.clacod
                        no-lock no-error.
            if not avail adicla
            then next.
            ...
            for each bmovim where bmovim.etbcod = plani.etbcod and
                ...
                find badicla where badicla.clacod = adicla.clacod and
                                   badicla.codadi = bprodu.clacod 
                                            no-lock no-error.
                if not avail badicla
                then next.
                ...
                find first tt-cla where tt-cla.vencod = plani.vencod  and
                                        ...
                                        tt-cla.clacod = adicla.clacod no-error. 
    ...
    if opsys = "UNIX"
    then varquivo = "/admcom/relat-auto/adic." + string(time) + ".txt".
    else varquivo = "l:~\relat-auto~\adic." + string(time) + ".txt".
