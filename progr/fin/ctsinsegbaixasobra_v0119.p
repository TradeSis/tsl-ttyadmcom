...
find pdvmov where recid(pdvmov) = par-recid-pdvmov no-lock no-error.

if not avail pdvmov
then do:
    message "nao dispnivel".
    pause.
    return.
end.
def buffer bpdvdoc for pdvdoc.

/*p-arquivo = "PAGAMENTOS240619".
*/
...
def shared temp-table tt-segpag like tbsegpag
    field saldo_aberto as dec
    field sit as char .
/**
for each tbsegpag where tbsegpag.segurado = p-arquivo no-lock:
    disp tbsegpag.
    if tbsegpag.valor_indenizar_cliente > 0
    then do:
        create tt-segpag.
        buffer-copy tbsegpag to tt-segpag.
        for each titulo where
                 titulo.clifor = tbsegpag.cliente and
                 titulo.titnat = no and
                 titulo.titsit = "LIB"
                 no-lock.
            saldo_aberto = saldo_aberto + titulo.titvlcob.
        end.         
    end.
end.  
**/
...
for each tt-segpag.
        for each titulo where
                 titulo.clifor = tt-segpag.cliente and
                 titulo.titnat = no and
                 titulo.titsit = "LIB"
                 no-lock.
            saldo_aberto = saldo_aberto + titulo.titvlcob.
...
def var esqcom1         as char format "x(15)" extent 5
    initial ["  Consulta","","  Baixa ","  Baixa Geral",""].
...
form 
     tt-segpag.certificado format "x(21)"
     tt-segpag.cpf         format "x(13)"
     tt-segpag.valor_indenizar_cliente  column-label "Valor Sobra"
     tt-segpag.saldo_aberto 
...
l1: repeat:
    ...
    */
    {sklclstb.i  
        &color = with/cyan
        &file = tt-segpag  
        &cfield = tt-segpag.certificado
        &noncharacter = /*                          */
        &ofield = " 
                    tt-segpag.cpf
                    tt-segpag.valor_indenizar_cliente
                    tt-segpag.saldo_aberto
                    "  
        &aftfnd1 = " "
        &where  = " true "
...
procedure aftselect:
    ...
    THEN DO:
        ...
        run baixa-contrato(input recid(tt-segpag)).
    ...
    THEN DO on error undo:
            disp tt-segpag.certificado format "x(21)"
                 tt-segpag.CPF format "x(13)"
                 tt-segpag.Filial
                 tt-segpag.Cliente 
                 tt-segpag.Contrato
                 tt-segpag.Data_programado
                 tt-segpag.Valor_pagar
                 tt-segpag.Valor_pago
                 tt-segpag.valor_sobra
                 tt-segpag.valor_baixa_sobra label "Baixa_sobra"
                 tt-segpag.valor_indenizar_cliente label "Indenizar"
                 tt-segpag.saldo_aberto
                with frame f-con overlay 1 column
                column 24 row 4 title " Consulta " width 50.
...
procedure baixa-contrato:
    ...
    for each tt-segpag where
             (if p-recid = ? then true else recid(tt-segpag) = p-recid)
              and
             tt-segpag.valor_indenizar_cliente > 0
             /*and tt-segpag.saldo_aberto > 0 */
             and tt-segpag.sit = "".
        disp tt-segpag.certificado
             tt-segpag.cpf
             tt-segpag.saldo_aberto
             tt-segpag.valor_indenizar_cliente
             tt-segpag.saldo_aberto - tt-segpag.valor_indenizar_cliente
             with frame f-saldo1 width 80
                     title "Saldo insuficiente"
             .   
        pause 0.

        run sel-contrato-baixar(tt-segpag.cpf). 
    end.    
    ...
    ***/
    ...
    find first tt-contsel no-error.
    if not avail tt-contsel then return.
    sresp = no.    
    ...
    then do:
        for each tt-segpag where
            ...
            find first clien where clien.ciccgc = replace(tt-segpag.cpf,"-","")
                    no-lock no-error.
            if not avail clien 
            then next.
            find tbsegpag where
                 tbsegpag.certificado = tt-segpag.certificado
                 no-error.
            if avail tbsegpag
            then do:     
                    
                    assign
                        val-baixar = tbsegpag.valor_indenizar_cliente
                        val-baixado = 0.
                    ...
                    ******/
                        ...
                        for each    tt-contsel where 
                            ...
                            for each tt-titsel where
                                ...
                                find tt-titulo where 
                                        tt-titulo.titnat = tt-titsel.titnat and
                                        tt-titulo.titnum = tt-titsel.titnum and
                                        tt-titulo.titpar = tt-titsel.titpar
                    ...
                    assign
                        ...
                        tt-segpag.valor_pago = tbsegpag.valor_pago
                        tt-segpag.valor_baixa_sobra =
                                tbsegpag.valor_baixa_sobra
                        tt-segpag.valor_indenizar_cliente =
                                tbsegpag.valor_indenizar_cliente
                        .

            end. 
            tt-segpag.sit = "PROCESSADO".                                    
        end.
        message color red/with
        "Baixa finalizada."
        view-as alert-box.
                ...
                then do:
                    ...
                    for each titulo where 
                        ...
                        then do: 
                            tt-arqimp.saldo-devedor =
                                tt-arqimp.saldo-devedor + titulo.titvlcob.
                            find tt-titulo of titulo no-lock no-error.
                            if not avail tt-titulo
                            then do:
                                create tt-titulo.
                               buffer-copy titulo to tt-titulo.
...
procedure sel-contrato-baixar:
    ...
    find clien where clien.ciccgc = replace(par-cpf,"-","") no-lock no-error.
    if avail clien
    then do:
        for each contrato where contrato.clicod = clien.clicod no-lock:
        ...
        form with frame f-sel 9 down row 8 overlay
            title " Falta " + string(tt-segpag.valor_indenizar_cliente)
          width 80      .
        assign
            a-seeid = -1
            a-recid = -1
    ...
    for each tt-titulo where 
        ...
        then do:
            assign
                ...
                tt-titulo.cxacod   = 99
                tt-titulo.titdtpag = date(tt-segpag.data_programado)
            ...
            then do on error undo:
                ...
                pdvdoc.valor    = tt-titulo.titvlpag.
                pdvdoc.titvlcob = tt-titulo.titvlpag.
                ...
                assign
                   ...
                   tt-segpag.saldo_aberto = 
                        tt-segpag.saldo_aberto - tt-titulo.titvlpag
                    titulo.titobs[2] = titulo.titobs[2] +
                                 "|BAIXA-SINISTRO=" + tt-segpag.certificado +
                                 "|".

                    run fin/baixatitulo.p(recid(pdvdoc),recid(titulo)).

                val-baixado = val-baixado + titulo.titvlpag.
             
                find tbsegpagsobra where
                    tbsegpagsobra.certificado = tbsegpag.certificado and
                    tbsegpagsobra.cliente     = titulo.clifor and
                    tbsegpagsobra.contrato    = int(titulo.titnum)
                    no-error.
                if not avail tbsegpagsobra
                then do:
                    create tbsegpagsobra.
                    assign
                        tbsegpagsobra.certificado = tbsegpag.certificado
                        tbsegpagsobra.cliente     = titulo.clifor
                        tbsegpagsobra.contrato    = int(titulo.titnum)
                        tbsegpagsobra.filial      = tbsegpag.filial
                        tbsegpagsobra.segurado    = tbsegpag.segurado
                        tbsegpagsobra.CPF         = tbsegpag.CPF
                        tbsegpagsobra.data_programado = 
                            tbsegpag.data_programado
                        .
                end.
                tbsegpagsobra.valor_baixa_contrato =
                    tbsegpagsobra.valor_baixa_contrato + titulo.titvlpag.
