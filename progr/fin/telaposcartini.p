...
def new shared var vdtfin as date format "99/99/9999" label "Ate".             def new shared var vreferencia as log format "Referencia/Vencimento".
def new shared temp-table tt-modalidade-selec 
    field modcod as char.
def new shared var vmod-sel as char.
def new shared var vquarter  as log format "Sim/Nao".
form creferencia
     vdtini
     vdtfin 
...
creferencia = if vreferencia   then "Referencia" else
              if not vquarter  then "Vencimento" else
                                    "Quarter".
disp creferencia
    ...
    if vreferencia
    then run fin/telaposcartref.p ("Geral","","geral",?,?,?,?,?,?).
    else run fin/telaposcartper.p ("Geral","","geral",?,?,?,?,?,?).
