/*
#1 04/19 - Projeto ICMS Efetivo - tabela movimimp
*/
{admcab.i}
...
then do:
    if plani.movtdc <> 11 and
       ...
       plani.movtdc <> 48
    then do:
        message "Estorno nao disponivel para tipo " plani.movtdc 
             view-as alert-box.
        return.     
    end.   
    ...
    if plani.movtdc = 12
    then tt-plani.movtdc = 59.
    if plani.movtdc = 13
    then tt-plani.movtdc = 60.
    if plani.movtdc = 31
    then tt-plani.movtdc = 65.
    if plani.movtdc = 24
    then tt-plani.movtdc = 61.
    if plani.movtdc = 16
    then tt-plani.movtdc = 62.
    if plani.movtdc = 61
    then tt-plani.movtdc = 66.
    if plani.movtdc = 53
    then tt-plani.movtdc = 68.
    if plani.movtdc = 27
    then tt-plani.movtdc = 67.
    if plani.movtdc = 11 or
       plani.movtdc = 26 or
       plani.movtdc = 75 or
       plani.movtdc = 80
    then tt-plani.movtdc = 11.
    if plani.movtdc = 73
    then tt-plani.movtdc = 78.               
    if plani.movtdc = 6
    then tt-plani.movtdc = 79.

    /*
    if substring(string(tt-plani.opccod),1,1) = "5"
    then tt-plani.opccod = int("1" + substring(string(tt-plani.opccod),2,3)).
    if substring(string(tt-plani.opccod),1,1) = "6"
    then tt-plani.opccod = int("2" + substring(string(tt-plani.opccod),2,3)).
    */
    
    if substring(string(tt-plani.opccod),1,1) = "5"
    then tt-plani.opccod = 1949.
    if substring(string(tt-plani.opccod),1,1) = "6"
    then tt-plani.opccod = 2949.
    
    tt-plani.hiccod = int(tt-plani.opccod).
    ...
    for each movim where movim.etbcod = plani.etbcod and
        ...
        assign
            ...
            tt-movim.opfcod = tt-plani.hiccod
            tt-movim.movtdc = tt-plani.movtdc
            tt-movim.movcsticms = "90".
    end.

    for each movimimp where movimimp.etbcod = plani.etbcod
                        and movimimp.placod = plani.placod
                      no-lock.
        create tt-movimimp.
        buffer-copy movimimp to tt-movimimp.
        assign
            tt-movimimp.placod = ?.
    end.        
    ...
    find first tt-movim where tt-movim.etbcod = tt-plani.etbcod and
                        ...
                        no-lock no-error.
    if avail tt-movim
    then run manager_nfe.p (input "ESTORNO", input ?, output p-ok).
    else message "NF sem itens" view-as alert-box.
