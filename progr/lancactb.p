...
form lancactb.modcod       column-label "Mod" format "x(3)"
     modal.modnom no-label
     lancactb.forcod       column-label "Fornec" format ">>>>>>>9"
...
l1: repeat:
    ...
    {sklclstb.i  
        &color = with/cyan
        &file = lancactb  
        &cfield = lancactb.modcod
        &noncharacter = /* 
        &ofield = " lancactb.modcod
                    modal.modnom when avail modal
                    lancactb.forcod when lancactb.forcod > 0
                    lancactb.contadeb
                    lancactb.contacre
                    lancactb.int1
                    lancactb.int2  
                    "  
        &aftfnd1 = " find modal where modal.modcod = lancactb.modcod
                        no-lock no-error. "
        &where  = " lancactb.id = 0 "
        &aftselect1 = " run aftselect.
                        a-seeid = -1.
                        if esqcom2[esqpos2] = ""Conta/Classe""
                        then do:
                            leave l1.
                        end.
                        else next keys-loop. "
...
procedure aftselect:
    ...
    THEN DO on error undo with frame f-inclui centered row 10 1 down
        ...
        assign
            ...
            .
        do on error undo:
            update tt-lancactb.modcod at 4.
            find modal where 
                 modal.modcod = tt-lancactb.modcod no-lock no-error.
            if not avail modal
            then do:
               bell.
    ...
    THEN DO on error undo with frame f-altera centered row 10 1 down
        ...
        do on error undo:
            ...
            disp modal.modnom no-label.
            disp tt-lancactb.forcod when tt-lancactb.forcod > 0.
