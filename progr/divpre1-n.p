...
def var vdec-dif-per as decimal.

def var vpromo2         as logical.
def var vprecoliberado2 as logical.
...
repeat:    
    ...
    for each estab where estab.etbcod = (if vetbcod <> 0
        ...
        do vdata = vdti to vdtf:
            if vcatcod = 41
            then run divpre-v.p(input estab.etbcod,
                         input vdata).
    ...
    for each estab where estab.etbcod = (if vetbcod <> 0
        ...
        do vdata = vdti to vdtf:
            for each divpre where divpre.etbcod = estab.etbcod and
                                  divpre.divdat = vdata  and
                                  divpre.divjus <> "DESC. SUPERVISOR" 
                                   no-lock:
                ...
                if avail plani and plani.usercod <> ""
                then next.
                ...
                if avail tt-livre
                then do:
                    if divpre.preven = divpre.prefil /*premat*/
                    then next.
                ...
                then do:
                    vprecoliberado = no.
                    vpromo = no.
                    run leplapromo3.p(input recid(plani), 
                                     input divpre.procod,
                                     output vpromo,
                ...
                /* Descarta QUALQUER produto com desconto superior a 20% */
                /*
                if divpre.procod = 420394
                then message ((divpre.premat * 20 / 100)
                     >= (divpre.premat - divpre.preven)) 
                     divpre.premat divpre.preven
                       (divpre.premat * 20 / 100)
                       (divpre.premat - divpre.preven)
                       view-as alert-box.
                */
                /* Aqui o teste estava invertido... */
                                           
                
                if ((divpre.prefil /*premat*/ - divpre.preven)
                     >= (divpre.prefil /*premat*/ * 20 / 100) )
                     and vcatcod = 31
                then next.
                ...
                then do:
                    if ((divpre.prefil /*premat*/ * 20 / 100)
                           >= (divpre.prefil /*premat*/ - divpre.preven))
                    then next.
                end.
                else if substring(produ.pronom,1,1) = "*"
                then do:
                    if ((divpre.prefil /*premat*/ * 10 / 100)
                           >= (divpre.prefil /*premat*/ - divpre.preven))
                    then next.
                end.
                ...
                */
                ...
                disp estab.etbcod format ">>>9"
                     produ.procod format ">>>>>>>9"
                      ...
                      title "Processando Divergencias".
                pause 0.
                ...
                if (divpre.preven - divpre.prefil /*premat*/) < 0
                then vtot = vtot + 
                (((divpre.preven - divpre.prefil) * vmovqtm) * -1).
...
repeat:
    ...
    vdif =  (divpre.preven - divpre.prefil) * vmovqtm. 
    ...
    assign vdec-dif-per = (((divpre.prefil /*premat*/ - divpre.preven) * 100) / divpre.prefil /*premat*/).          
    ...
    display
        ...
        vdec-dif-per column-label "Dif!Per" format ">>9.9%"
    ...
    repeat:
        ...
        vdif =  (divpre.preven - divpre.prefil /*premat*/). 
        
        display divpre.divjus label "Justificativa" with frame f-jus.
        display divpre.divobs with frame f-obs.


        assign vdec-dif-per = (((divpre.prefil /*premat*/ - divpre.preven) * 100) /
        divpre.prefil /*premat*/).
    ...
    repeat with frame frame-a:
        ...
        hide frame frame-a no-pause.
          ...
          then do:
            ...
            then do:
                if opsys = "UNIX"
                then varquivo = "/admcom/relat/r-desc." + string(time).
            ...
            then do:
                /*
                run confp1.p(input /*estab.etbcod*/ divpre.etbcod /*vetbcod*/,
                             input vdti,
                             input vdtf,
                             input vpercdiver,
                             input vcatcod).
...
procedure nfvenco_p:
    ...
    */
    ...
    for each movim where movim.etbcod = plani.etbcod and
        ...
        disp movim.procod
             produ.pronom format "x(24)" when avail produ
             movim.movqtm (total)
             movim.movpc
...
procedure p-altpreco3per:
    ...
    percentual-altp = 0.
    percentual-altp = 
            (((divpre.preven - divpre.prefil /*premat*/) / divpre.prefil /*premat*/ ) * 100).
    ...
    /* Sol. 34766 - Produtos com * voltam a aparecer no relatório - Laureano
    if produ.pronom begins "*" and
        sfuncod <> 7
    then p-diver = no.
    */
end procedure.
...
procedure p-listagem:
    ...
    if opsys = "UNIX"
    then varquivo2 = "/admcom/relat/divpre1." + string(time).
    else varquivo2 = "l:~\relat~\divpre1." + string(time).
    
    {mdadmcab.i &Saida     = "value(varquivo2)"
                &Page-Size = "64"
                &Cond-Var  = "80"
                &Page-Line = "66"
                &Nom-Rel   = ""DIVPRE1""
                &Nom-Sis   = """SISTEMA CREDIARIO"""
                &Tit-Rel   = """RELATORIO DE DIVERGENCIA DE PRECOS """
                &Width     = "130"
                &Form      = "frame f-cabcab"}
    
        disp with frame f-dados.

    for each tt-divpre no-lock:
    
        assign vdec-dif-per = 0.

        find divpre where recid(divpre) = tt-divpre.rec no-error.
        
        if not avail divpre then next.
         ...
         */                           
         ...
         assign vdec-dif-per = (((divpre.prefil /* premat*/
                                   - divpre.preven) * 100) / divpre.prefil /*premat*/).                  
         ...
         display
          ...
          divpre.prefil column-label "Preco!Filial"  format ">,>>9.9" 
            ...
            vdif column-label "Difer.!Preco"  format "->>9.99"
            vdec-dif-per column-label "Dif!Per" format ">>9.9"
              with frame f-relat 10 down centered 
                title trim(vetbnom + " -  Dia: " + 
                           string(vdti,"99/99/9999") + " - " + 
                           string(vdtf,"99/99/9999")).
    ...
    message "Custom - Caminho do arquivo: " varquivo2.
    pause.

    if opsys = "UNIX"
    then run visurel.p (input varquivo2, input "").
    /*
    else {mrod.i}.
    */
