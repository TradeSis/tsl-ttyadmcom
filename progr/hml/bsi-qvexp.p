/* #1 20.05.19 - helio.neto - inclusao dos campos do movimaux, por linha da nota, DESC-CAMPANHA;BONUSCRM;BONUSNRO;DESCONTO
                              estes campos já sao calculados no integrador bsip2k 
#2 28.01.20 - mudanca periodo por horario
*/
...
def var p2kDESC-CAMPANHA    as dec.
def var p2kBONUSCRM         as dec.
def var p2kBONUSNRO         as char.
def var p2kDESCONTO         as dec.

def var vtippes as char format "x".
...
form
    ...
    then do:
        ...
        /* #2 */
        if time <= 25200 /* 07:00hs */
        then vdtini = today - 3.
        else vdtini = today. 

        if today = 11/16/2014
        then vdirsaida = "/admcom/lebesintel/carga_crm/".
        else vdirsaida = "/admcom/lebesintel/hml/".
...
unix silent(sudo chmod 777 /admcom/lebesintel/hml/*.csv).
...
procedure p-gera-arquivos:
    ...
    */
    ...
    put stream movim unformatted
        ...
        "LucroBruto;ValorVendaAnunciado;StatusCombo;TipPes;"
        "DESC-CAMPANHA;BONUSCRM;BONUSNRO;DESCONTO;" /* #1 */
           skip. 
    ...
    */
    ...
    for each tt-movim use-index idx04,
        ...
        */
        ...
        when "V" then do:
            ...
            if produ.catcod = 71 /* EMPRESTIMOS */
            then do:
                /* Calculo do Custo medio Liquido */
                if produ.procod = 8000 or produ.procod = 8001
                then do:
                    /*CREDITO PESSOAL*/
                    tt-movim.movctm = tt-movim.movtot.
                end.
                else do:
                    /*SEGURO PESSOAL*/ 
                    tt-movim.movctm = tt-movim.movtot * 0.30.
                end.
        ...
        find first estoq where estoq.procod = tt-movim.procod
                and estoq.etbcod = tt-movim.etbcod 
                no-lock no-error. 
        ...
        */  
        ...
        /* 26.02.19 helio.neto*/               
        find clien where clien.clicod = tt-movim.clicod no-lock no-error.
        vtippes = if avail clien
        ...
        p2kDESC-CAMPANHA = 0.   
        find first movimaux where 
                movimaux.etbcod = tt-movim.etbcod and 
                movimaux.placod = tt-movim.placod and 
                movimaux.procod = tt-movim.procod and 
                movimaux.nome_campo = "DESC-CAMPANHA"
                no-lock no-error.
        if avail movimaux
        then p2kDESC-CAMPANHA = dec(movimaux.valor_campo).         
        if p2kDESC-CAMPANHA = ? then p2kDESC-CAMPANHA = 0.
        ...
        find first movimaux where 
                ...
                movimaux.placod = tt-movim.placod and 
                movimaux.procod = tt-movim.procod and 
                movimaux.nome_campo = "BONUSCRM"
                no-lock no-error.
        if avail movimaux
        then p2kBONUSCRM = dec(movimaux.valor_campo).         
        if p2kBONUSCRM = ? then p2kBONUSCRM = 0.
        ...
        find first movimaux where 
                ...
                movimaux.placod = tt-movim.placod and 
                movimaux.procod = tt-movim.procod and 
                movimaux.nome_campo = "BONUSNRO"
                no-lock no-error.
        if avail movimaux
        then p2kBONUSNRO = movimaux.valor_campo.         
        if p2kBONUSNRO = ? then p2kBONUSNRO = "".
        ...
        find first movimaux where 
                ...
                movimaux.placod = tt-movim.placod and 
                movimaux.procod = tt-movim.procod and 
                movimaux.nome_campo = "DESCONTO"
                no-lock no-error.
        if avail movimaux
        then p2kDESCONTO = dec(movimaux.valor_campo).         
        if p2kDESCONTO = ? then p2kDESCONTO = 0.
...
output to "/admcom/lebesintel/hml/bi_canceladas.csv".                              
