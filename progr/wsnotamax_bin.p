...
/**    put-string(vb, 1) = vretorno.**/
    ...
    varq = "/admcom/mdfe/mdfe-xml/wsnotamax-RET." + vcmetodo + 
                        "." + vcLOG + ".xml".    
    copy-lob vretorno to file varq.

        
            create tt-xmlretorno.
            tt-xmlretorno.tag   = "binario_pdf". /* nomes das tags */
            tt-xmlretorno.valor = varq.
...
procedure obtemnode.
    ...
    def var bin-conteudo as longchar.
    message "p-root" 1 vh:name vh:type vh:num-children.
    pause.        
    create x-noderef hc.
    /* A partir daqui monta o retorno */
    do loop = 1 to vh:num-children: /*faz o loop até o numero total de filhos*/
        message p-root "filhos" loop.
        pause.
