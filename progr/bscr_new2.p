...
def var dtfim as date format "99/99/9999" initial today.
 ...
 update dtini colon 15 label "Cliente Novos de" 
               dtfim label "ate".
       

varquivo2 = "/admcom/import/cdlpoa/NEW-cscore-"
+ string(month(dtini))
+ string(year(dtini))
+ string(month(dtfim))
+ string(year(dtfim))
...
pause 0.
 ...
 find first titulo use-index iclicod where
             ...
             if   temcadastro = 1  and temcomportamento = 1 then do:
...
diasdeatraso = 0.
   ...
   ( titulo.modcod = "CP1" OR titulo.modcod = "CP0" 
) and titdtven >=  dtini  and titdtven <=  dtfuturo
