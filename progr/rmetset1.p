...
if opsys = "UNIX"
then varquivo = "/admcom/relat/relat_metas_setor_" + string(time).
else varquivo = "l:\relat\relat_metas_setor_" + string(time).
...
for each tt-tbmeta where tt-tbmeta.etbcod = estab.etbcod no-lock,
    ...
    assign vcha-filial-aux = "Filial " + string(tt-tbmeta.etbcod)
                                       + " - "
                                       + string(estab.munic) .                         
    ...
    display vcha-filial-aux       format "x(30)"     column-label ""
            vcha-clase-aux        format "x(42)"     column-label ""
