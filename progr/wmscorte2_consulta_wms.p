...
def var dcb-cod as int format ">>>>>9".
        ...
        then do:
            ...
            then do:
                ...
                then do with frame f-liped on error undo.
                    ...
                    pause 0.
                    run reservpro-wms.p (input "PROCOD=" + string(produ.procod) +
                                     "|" + 
                                     "RECID_LIPED=" + string(recid(liped)) ).
                    recatu1 = recid(liped).
                    leave.
                end.
                if esqcom1[esqpos1] = " Gerado "
                then do:
                    run pendente-gerado.
                end.
...
procedure pendente-gerado:
    find first pedpend where 
                               pedpend.etbcod-ori = liped.etbcod and
                               pedpend.pedtdc-ori = liped.pedtdc and
                               pedpend.pednum-ori = liped.pednum
                               no-lock no-error.
                    if avail pedpend
                    then do.
                        find first pedid_pend where
                                   pedid_pend.pednum = pedpend.pednum-des and
                                   pedid_pend.pedtdc = pedpend.pedtdc-des and
                                   pedid_pend.etbcod = pedpend.etbcod-des
                                   no-lock no-error.
                        if avail pedid_pend
                        then           
                        find liped_pend where 
                             liped_pend.etbcod = pedpend.etbcod-des and
                                  liped_pend.pedtdc = pedpend.pedtdc-des and
                                  liped_pend.pednum = pedpend.pednum-des and
                                  liped_pend.procod = liped.procod
                                  no-lock no-error.
                        if avail liped_pend
                        then do: 
                            find tt-reservas where 
                            tt-reservas.rec_liped = recid(liped_pend) 
                            no-error.
...
tot-pedido   = liped_pend.lipqtd.
tot-atendido = tt-reservas.atende.
    ...
    display 
            ...
            tbgenerica.tgint / 10 format ">>9" label "Prioridade"  colon 15
                              when tgint  <> 9999
            with frame frame-pendente1 centered color white/red row 5
                                  overlay side-label.
    end.
    end.
end procedure.
