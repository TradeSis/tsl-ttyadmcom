...
if sresp then do:
    if opsys = "UNIX" then do:
        vdir = "/admcom/zebra/04/".
        varquivo = vdir + "eti-04.bat".

        unix silent value("rm -f " + varquivo).
        unix silent value("rm -f " + vdir + "c*.*").

        unix silent value("rm -f " + vdir + "04.zip").
        unix silent value("rm -f " + vdir + "04.ZIP").
        ...
        put unformatted "zip -q " vdir 
                "04.zip c*.* eti-04.bat *.GRF PKUNZIP.EXE UZIPA-01.BAT".
        ...
        varquivo = "l:~\zebra~\04~\eti-04.bat".
    
        if search(varquivo) <> ?
        then do:
            dos silent del l:~\zebra~\04~\eti-04.bat.
            dos silent del l:~\zebra~\04~\c*.* .
        end.        
    end.
    ...
    for each tt-etiq:
        run leb-etiq-imprime04.p (input tt-etiq.rec,
                                input tt-etiq.qtd,
    ...
    if opsys = "UNIX" then unix silent value("/admcom/zebra/04/zipa.sh").
end.
