...
def temp-table tt-exp
    ...
    field modctb like fin.modal.modcod
...
repeat with column 50 side-labels 1 down width 31 row 4 frame f1:
    ...
    repeat:
       ...
       for each wfor:
        ...
        if opsys = "UNIX"
        then varquivo = "/admcom/relat/rpagmodctb.csv".
        else varquivo = "l:\admcom\relat\rpagmodctb.csv".
...
procedure fin-titudesp:
    ...
    for each titudesp where titudesp.empcod = wempre.empcod and
            ...
            find first fatudesp where
                       ...
                       fatudesp.clicod = titudesp.clifor
                       no-lock no-error.
            if not avail fatudesp then next.           
