...
form
    ...
    /*vciccgc        colon 15*/
    /*clien.clfnom  no-label format "x(20)"*/
    /*clien.ciccgc  no-label*/
    vnumero   colon 15
    vserie
    vmovtdc 
    tipmov.movtnom format "x(15)"
...
form
    plani.protot    colon 20
    plani.bicms     colon 10
    plani.icms      colon 30
    plani.bsubst    colon 10 label "B. Subst."
    plani.icmssubst colon 40
...
repeat:
    ...
    vmovtdc = 13.
    update vmovtdc column-label "Tipo NF"
            with frame f1.
    ...
    /**
    update plani.pladat with frame f1.
    for each movim where movim.etbcod = plani.etbcod and
        ...
        find produ where produ.procod = movim.procod no-lock.
         ...
         update movim.movqtm format ">,>>9.99" column-label "Qtd"
                movim.movpc  format ">,>>9.99" column-label "Custo"
                 with frame f-produ2.
                 
         disp  ((movim.movpc * movim.movqtm) -
     ...
     update plani.protot
            ...
            plani.descpro
            plani.acfprod
            plani.platot with frame f2 no-validate.

     update plani.notobs[1] format "x(50)"
            plani.notobs[2] format "x(50)"
            plani.notobs[3] format "x(50)"
            with frame f3 width 60 no-label.
     **/
     message "Confirma Reimpressao da Nota Fiscal? " update sresp.
     if sresp
     then do:
