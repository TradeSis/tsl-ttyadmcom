...
/* #3 Helio 04.04.18 - Versionamento com Regra definida 
   ...
   #4 helio.neto 14.12.2018 - Nova Marcacao de Feirao, com nova Coluna, Mas pertencendo a Carteira 
...
def temp-table wtit no-undo /* #1 */
    ...
    index i1 is unique primary wetb asc.
                  
def temp-table wtitfeirao no-undo /* #1 */
    field wetb like titulo.etbcod
    ...
    field feinov    like titulo.titvlcob
    field wpar   like titulo.titpar format ">>>>>>9"
    index i1 is unique primary wetb asc anoemi asc.
                ...
                for each tt-modalidade-selec no-lock,
                    ...
                    then do:
                        ...
                        then do:
                            ...
                            then wtit.feinov  = wtit.feinov + titulo.titvlcob.
                            else wtit.nov  = wtit.nov + titulo.titvlcob.
                        end.   
                        else
                    ...
                    if acha("FEIRAO-NOVO",titulo.titobs[1]) = "SIM" or 
                       ...
                       titulo.tpcontrato = "N"
                    then do:
                        find first wtitfeirao where
                            wtitfeirao.wetb   = wtit.wetb and
                            wtitfeirao.anoemi = int(string(year (titulo.titdtemi),"9999")) 
                                   no-error.
                        if not avail wtitfeirao
                        then do:
                            create wtitfeirao.         
                            wtitfeirao.wetb   = wtit.wetb.
                            wtitfeirao.anoemi = int(string(year (titulo.titdtemi),"9999")). 
                        end.
                        wtitfeirao.wpar   = wtitfeirao.wpar   + 1.
                        
                        if acha("FEIRAO-NOME-LIMPO",titulo.titobs[1]) = "SIM"
                        then wtitfeirao.fei = wtitfeirao.fei + titulo.titvlcob.
                        else
                        if acha("FEIRAO-NOVO",titulo.titobs[1]) = "SIM"
                        then wtitfeirao.feinov = wtitfeirao.feinov + titulo.titvlcob.
                        else wtitfeirao.nov    = wtitfeirao.nov    + titulo.titvlcob.
...
procedure relatorio:
    ...
    for each wtit no-lock use-index i1:
        wcrenov = wtit.nov + wtit.feinov + wtit.cre. /* #1 */
        disp wtit.wetb column-label "Filial"
             wtit.wvalor(total) column-label "(1+2+3+4+5)!Vl.Total"
             wtit.wpar(total) column-label "Tot.Par"
              /* #1 */
             wfeirao  format "x(4)" column-label "Ano"
              
             wtit.fei (total) column-label "(1)!Feirao"
             wtit.lp  (total) column-label "(2)!LP" 
             wtit.nov (total) column-label "(3)!Novacao"
             wtit.feinov (total) column-label "(4)!Feirao!Carteira"
             wtit.cre (total) column-label "(5)!Venda"
             wcrenov (total) column-label "(3+4+5)!Carteira"
             /* #1 */
             with frame f2 down width 160.
        find first wtitfeirao
            where wtitfeirao.wetb = wtit.wetb
            no-lock no-error.
        if avail wtitfeirao
        then do:
            
            for each wtitfeirao where wtitfeirao.wetb = wtit.wetb no-lock.
                down with frame f2.
                disp
                    wtitfeirao.anoemi @ wfeirao
                    wtitfeirao.wpar   @ wtit.wpar
                    wtitfeirao.fei    @ wtit.fei
                    wtitfeirao.nov    @ wtit.nov
                    wtitfeirao.feinov    @ wtit.feinov
                    with frame f2.
            end.
        end.     
    end.            
