...
def temp-table tt-titulo like fin.titulo
    index i1 titsit titdtpag descending.
...
for each fin.titulo where fin.titulo.clifor = clien.clicod no-lock:
    ...
    then next.
    if fin.titulo.titnat = yes then next.
    create tt-titulo.
...
/***
find clien where clien.clicod = 7570.
def var rec-clien as recid.
rec-clien = recid(clien).
def var vcalclimite as dec.
def var par-dias as int.
***/
...
*/
    ...
    for each credscore where credscore.desc-campo = "ESTABCLI"
                       ...
                       by credscore.vl-ini.
        if int(substr(string(clien.clicod),length(string(clien.clicod)) - 1, 2)) = int(credscore.campo)
        then do:                                                            
            run le-credscore.                                               
            leave.                                                          
        end.                                                                
...
/*message "percrenda" vcalclimite view-as alert-box.*/
...
vguardacampo = ?.
    ...
    then assign vnumparcpg = credscore.vl-ini
                vguardacampo = credscore.campo.                                     else assign vnumparcpg = 0
                vguardacampo = ?.
    ...
    /* Valor Total das Parcelas Pagas */                                            find first credscore where credscore.campo = "VALPARCPG"                             no-lock no-error.                                                   ~        if avail credscore                          
    then assign vvalparcpg = credscore.vl-ini
                vguardacampo = credscore.campo.                                ~     else assign vvalparcpg = 0
                vguardacampo = ?.    
    ...
    then do:
        ...
        vcontapar = 0.
        vtotalpar = 0.
        for each tt-titulo where tt-titulo.titsit = "PAG"
                          and fin.titulo.titdtpag <> ?
                          :
            vcontapar = vcontapar + 1.
            vtotalpar = vtotalpar + fin.titulo.titvlpag.
        end.                                                                       end.
...
repeat qtdpos = 1 to 4:
    ...
    find first tt-dados where tt-dados.parametro = cpfis.rcomnome[qtdpos]              no-lock no-error.
    if not avail tt-dados
    then do:
        create tt-dados.
...
/* Procura se o cliente tem novacao */
/*if not connected("d")
then run conecta_d.p.
*/

def buffer btitulo for dragao.titulo.
def buffer ctitulo for fin.titulo.
def var vparcel as int init 0.
def var vvaltot as dec init 0.
def var vnovatras as log.
 
vnovatras = no.
for each dragao.titulo where dragao.titulo.titnat = no
                         and dragao.titulo.clifor = clien.clicod
                         and dragao.titulo.titdtpag = ?
                         and dragao.titulo.titsit  = "LIB"
                         and dragao.titulo.titpar >= 31
                             no-lock.
...
if vnovatras = no
then do:
for each tt-titulo where tt-titulo.titsit  = "LIB"
                      and tt-titulo.titdtpag = ?
                      and tt-titulo.titpar >= 31
                             no-lock.
...
release dragao.titulo.
release fin.titulo.
vguardaval = vcalclimite.
find first dragao.titulo where dragao.titulo.titnat = no
                    and dragao.titulo.clifor = clien.clicod
                    and dragao.titulo.titdtpag = ?
                    and dragao.titulo.titsit  = "LIB"
                    and dragao.titulo.titpar >= 31
                          no-lock no-error.
if avail dragao.titulo
then do:
    assign vcalclimite = 0.

/***
    for each btitulo where btitulo.titnat = no
                       and btitulo.clifor = clien.clicod
                       and btitulo.titdtpag <> ?
                       and btitulo.titsit = "PAG"
                       and btitulo.titpar >= 31
                       and btitulo.titnum = dragao.titulo.titnum
                           no-lock.
        vparcel = vparcel + 1.
        if vparcel >= 5
        then leave.
...
***/    
    ...
    find first tt-titulo where tt-titulo.titsit  = "LIB"
                    and tt-titulo.titdtpag = ?
                    and tt-titulo.titpar >= 31
                          no-lock no-error.
    if avail tt-titulo
    then do:
        ...
        /***
        for each ctitulo where ctitulo.titnat = no
                       and ctitulo.clifor = clien.clicod
                       and ctitulo.titdtpag <> ?
                       and ctitulo.titsit = "PAG"
                       and ctitulo.titpar >= 31
                       and ctitulo.titnum = fin.titulo.titnum
                           no-lock.
            vparcel = vparcel + 1.
            if vparcel >= 5
            then leave.
...
vmaioratraso = ?.
for each tt-titulo where  tt-titulo.titdtpag = ?
                      and tt-titulo.titsit  = "LIB" no-lock.
    saldo-aberto = saldo-aberto + tt-titulo.titvlcob.
    if vmaioratraso = ? or
       vmaioratraso < (today - tt-titulo.titdtven)
    then vmaioratraso = (today - tt-titulo.titdtven).
    else next.
end.
for each dragao.titulo where dragao.titulo.titnat = no
                         and dragao.titulo.clifor = clien.clicod
    ...
    saldo-aberto = saldo-aberto + dragao.titulo.titvlcob.
    if vmaioratraso = ? or
       vmaioratraso < (today - dragao.titulo.titdtven)
    then vmaioratraso = (today - dragao.titulo.titdtven).
    else next.
end.
...
vcontrpar = 0.   
vmaioratraso = 0.                                                            
for each tt-titulo where tt-titulo.titdtpag <> ?  and
                         tt-titulo.titsit = "PAG":

     if vatraso = 0 or ((today - tt-titulo.titdtven) > 0 and
                         vatraso < (today - tt-titulo.titdtven))
     then vatraso = (today - tt-titulo.titdtven).
     if vcontrpar < vnumparcpg
     then do:
        vcontrpar = vcontrpar + 1.                                      
        vmaioratraso = vmaioratraso + (tt-titulo.titdtpag -
                                tt-titulo.titdtven).
     end.
     vsomapar = vsomapar + 1.
...
find first credscore where credscore.desc-campo = "PC PAGAS"
                       and credscore.int2 = vsomapar
                           no-lock no-error.
if not avail credscore
then find last credscore where credscore.desc-campo = "PC PAGAS"
                            and credscore.int2 <= vsomapar
                                no-lock no-error.
if avail credscore
then do:
...
then do:
    ...
    then do:
        create tt-dados.
        tt-dados.parametro =  
        "Cliente com " + string(vcontapar) + " parcelas pagas 
        (<30 parcelas pagas zera limite)".
    end.
    tt-dados.vcalclim = vcalclimite.
    find last btt-dados no-lock no-error.
    if not avail btt-dados
