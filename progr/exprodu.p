...
find bfinan where finan.fincod = 43 no-lock no-error.
find cfinan where finan.fincod = 87 no-lock no-error.
...
for each produ where produ.procod = vproduto no-lock:
    ...
    for each estoq where estoq.procod = produ.procod no-lock:
        ...
        assign
            p42 = ((estoq.estvenda * finan.finfat) * 20)
            p43 = ((estoq.estvenda * bfinan.finfat) * 20)
            p87 = ((estoq.estvenda * cfinan.finfat) * 20).
