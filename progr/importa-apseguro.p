...
if sresp then
for each tt-cap:
    if tt-cap.valor[1] begins "Produto" 
    ...
    then do:
        ...
        then do:
            ...
            assign
                ...
                apseguro.codigo_estipulante = tt-cap.valor[4]
                apseguro.estipulante = tt-cap.valor[5]
                apseguro.segurado = tt-cap.valor[6]
                apseguro.cpf = tt-cap.valor[7]
                apseguro.processamento = date(tt-cap.valor[8])
                apseguro.inicio_vigencia = date(tt-cap.valor[9])
                apseguro.termino_vigencia = date(tt-cap.valor[10])
                apseguro.vencimento = date(tt-cap.valor[11])
                apseguro.parcela = tt-cap.valor[12]
                apseguro.premio_total = dec(tt-cap.valor[13])
                apseguro.premio_liquido = dec(tt-cap.valor[14])
                apseguro.iof = dec(tt-cap.valor[15])
                apseguro.prolabore = dec(tt-cap.valor[16])
                apseguro.comissao  = dec(tt-cap.valor[17])
                apseguro.lote = tt-cap.valor[18]
