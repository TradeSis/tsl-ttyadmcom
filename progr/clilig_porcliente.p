...
/*disp par-clicod string(time,"HH:MM:SS") with no-label.*/
...
do on error undo.
    find clilig where clilig.clicod = clien.clicod no-error.  
    if avail clilig
    then do.
...
procedure cria-clilig.
    ...
    def var p-tot  as int.
    p-tot = 0.
    for each btitulo use-index iclicod where btitulo.clifor  = p-clifor no-lock.
        if btitulo.empcod  = 19     and
    ...
    for each btitulo use-index iclicod where btitulo.clifor  = p-clifor no-lock.
            ...
            then do:
                ASSIGN clilig.clicod    = btitulo.clifor
                       ...
                       clilig.dtven     = btitulo.titdtven
                                         /*min(clilig.dtven,btitulo.titdtven)*/                        clilig.int1      = btitulo.titpar
        ...
        if p-cont = p-tot and clilig.char3 = ""
        then do:
