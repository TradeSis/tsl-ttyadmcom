...
for each mvcusto where 
                   ...
                   no-lock by procod by dativig by horivig.
    ...
    disp 
         ...
         mvcusto.valctotransf column-label "C.Transf"    format ">>>,>>9.99"
         mvcusto.custo1       column-label "C.M.Ant"   format ">>>,>>9.99"
         mvcusto.valctomedio  column-label "C.M.Atu"   format ">>>,>>9.99"
    ...
    then
        disp plani.etbcod
            ...
            plani.ufdes format "x(50)" label "Chave"
...
def var vfcpst as dec.
def var vstsolidario as dec.
...
vstsolidario = dec(acha("valstsol",mvcusto.char2)).
vfcpst = dec(acha("valstfcp",mvcusto.char2)).
...
then*/ do:
    ...
    then do:
        ...
        if vstsolidario > 0
        then vmais = vmais + " + " + 
        trim(string(vstsolidario,">>>,>>9.99")).
        if vfcpst > 0
        then vmais = vmais + " + " +
        trim(string(vfcpst,">>>,>>9.99")).
    end.
    else do:
        if vsubst > 0
        then vmais = vmais + " + " +
...
vmenos = "(".
if vsubst = 0
then vmenos = vmenos + trim(string(vicms,">>>,>>9.99")) + " + " .
if vmovdes > 0
