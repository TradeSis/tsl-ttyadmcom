...
if p_mail = "" then return.

if (not p_mail matches("*.*"))
then do:
    message "< Erro no formato de e-mail falta '.' >" view-as alert-box.
    assign p_retorno = no.
    return.
end.    
if (not p_mail matches("*@*")) 
...
then do:
    message "< Erro na estrutura de e-mail ou falta '@' >" view-as alert-box.
    assign p_retorno = no.
    return.
end.
...
if v-ok-dom = no
then do:
    message string("< Erro no Dominio de e-mail :  ." + v-dom-par + " >")
            view-as alert-box.
    assign p_retorno = no.
    return.
end.
...
do i_ct = 1 to length(p_mail):
    ...
    then do:
          message "< " substring(p_mail,i_ct,1) " >" 
                  "Caracter invalido no e-mail, favor verificar! " skip
                  "somente permite composicao: ('a' a 'z' '0' a '9' e '_ @ .')"
                  skip "Verifique e corrija possivel erro!"
                  view-as alert-box.
          assign p_retorno = no.
    end.
end.       
