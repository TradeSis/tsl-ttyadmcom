...
for each d.titulo where d.titulo.empcod = 19                        and 
                        d.titulo.titnum = string(cyber_contrato.contnum)  and 
                        d.titulo.titnat = no                        and 
                        d.titulo.clifor = cyber_contrato.clicod     and 
                        d.titulo.modcod = "CRE"                     and 
                        d.titulo.etbcod = cyber_contrato.etbcod     and
                        d.titulo.titsit = "LIB"
                      no-lock by d.titulo.titdtven desc.
    if d.titulo.titdtven < today 
    then  
        assign vvalor_atraso  = vvalor_atraso  + d.titulo.titvlcob
               vvalor_vencido = vvalor_vencido + d.titulo.titvlcob
    ...
    vvalor_total_divida = vvalor_total_divida + d.titulo.titvlcob.
    vdata_vencimento_contrato = formatadata(d.titulo.titdtven).
    run juro_titulo.p (input  d.titulo.titdtven, d.titulo.titvlcob,
                       output par-juros).
    vvalor_juros = vvalor_juros + par-juros.
    vqtd_parcelas = vqtd_parcelas + 1. 
...
for each d.titulo where d.titulo.empcod = 19
                  ...
                  no-lock by d.titulo.titdtpag .
    vdata_ultimo_pagamento  = formatadata(d.titulo.titdtpag).
    vvalor_ultimo_pagamento = d.titulo.titvlpag.
    vqtd_parcelas_pagas = vqtd_parcelas_pagas + 1.
    vqtd_parcelas = vqtd_parcelas + 1.
