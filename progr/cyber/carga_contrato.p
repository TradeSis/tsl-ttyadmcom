...
/*  1.  Carga Contratos         - carga_AAAAMMDD_hhmmss.txt   */
{admcab.i}
...
for each lotcrecontrato of lotcre no-lock.
    ...
    find fin.contnf where contnf.etbcod  = cyber_contrato.etbcod and
                          contnf.contnum = cyber_contrato.contnum
                    no-lock no-error.
    if avail contnf
    then do.
...
put unformatted    
    ...
    cyber_contrato.etbcod                    format "9999"    /* 3044       */
    formatadata(cyber_contrato.dtinicial)    format "x(8)"    /* 3048       */
