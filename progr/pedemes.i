...
def var vmodal      as log format "Sim/Nao".
...
def var vmesext         as char format "x(10)"  extent 12
    ...
    form
        ...
        space(5) vmesext[12] space(5) skip
         with frame fmes
         title string(vreferencia,"   Referencia   /   Vencimento   ").  
        display "Informe o Mes e Ano / Estabelecimento " at 23
            with frame fdad row 4 no-box width 81 color message.

    update vreferencia colon 16 format "Referencia/Vencimento   " 
       label "Data Venc/Ref ?"     
    with frame fano. 
    vquarter = no.
    if vreferencia = no
    then update vquarter label "Visao Quarter?"
            with frame fano. 
...
if vreferencia
then disp "" @ vdtini with frame fano.
disp
    vdtini when vreferencia = no no-labels
    vdtfin no-labels
        with frame fano.
