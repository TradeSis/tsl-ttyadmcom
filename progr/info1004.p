...
find first tbcntgen where tbcntgen.etbcod = 1004 no-lock no-error.
...
varquivo = "/admcom/relat/informativo1004M.txt".
...
if venvmail = yes
then do:
    run /admcom/progr/envia_info.p("1004",varquivo).
end.

pause 5 no-message.

varquivo = "/admcom/relat/informativo1004C.txt".
output to value(varquivo). 
put unformatted
"PRODUTOS COM ALTERACAO DE PRECO NAS ULTIMAS 24 HORAS<BR>" SKIP
"DAS 12:00HS DO DIA " TODAY - 1 " ATE AS 12:00HS DO DIA " TODAY "<BR>" SKIP
