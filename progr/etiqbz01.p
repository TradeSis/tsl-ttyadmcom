    ...
    */
    ...
    then do:
      for each wfetq break by linha:
        ...
        assign
            substring(wfetq.comando,vpos) = if wfetq.linha = 3
                                            ...
                                            then string(produ.procod,"999999")
                                                    ...
                                                    wfetq.linha = 20
                                            then string(produ.pronom,"x(25)")
                                            else if wfetq.linha = 28 or
                                                    wfetq.linha = 29
                                            then                           substr(string(produ.pronom,"x(60)"),26,25)
                                            else if wfetq.linha = 11 or
                                                    wfetq.linha = 23
        ...
        if opsys = "UNIX"
        then varquivo = "../free/etqbz" + string(produ.procod).
        else varquivo = "..~\free~\etqbz" + string(produ.procod).
...
substr(string(produ.pronom,"x(60)"),26,25)
        ...
        if opsys = "UNIX"
        then varquivo = "../free/etqbzs" + string(produ.procod).
        else varquivo = "..~\free~\etqbzs" + string(produ.procod).
        output to value(varquivo).
            for each wfetq break by linha.
                put unformatted wfetq.comando skip.
            end.
...
then do:
        ...
        then do:        
                if vdtfliped <> ?
                then do:
                        assign vdtped = substring(string(month(vdtfliped)),3,2)
                                      + string(year(vdtfliped),"99").
                end.
        end. 
        if vdtfpedid = ? and
           vdtfliped = ?
        then do:
                assign vdtped = substring(string(month(today)),3,2) 
                                + string(year(today),"99").                           end.            
end.
