...
def stream sarq.
def var vcod like lfcad.codigo.
    ...
    for each fiscal where fiscal.movtdc = 4       and
        ...
        then do:
            find first lfcad where lfcad.cgc = vcgc and
                                   lfcad.empcod = 995 no-error.
            if not avail lfcad or vcgc = ""
            then do:
                find first lfcad where lfcad.cgc    = vcgc and
                                       lfcad.empcod = 122 no-error.

                if not avail lfcad or vcgc = ""
                then do:
                    find first wforne where wforne.wforcod = forne.forcod 
                                no-error.
                    if not avail wforne
                    then create wforne.
...
do vdt = vdti to vdtf:
    ...
    for each fiscal where fiscal.movtdc = 4       and
        ...
        if avail lfcad
        then vcod = lfcad.codigo.
        else */  vcod = int(substring(string(forne.forcod),1,5)).
