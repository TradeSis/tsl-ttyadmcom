...
def temp-table tt-ent
    ...
    field valor as dec
    field plaori like plani.numero
...
for each estab where ( if vetbcod > 0
    ...
    for each ctdevven where ctdevven.etbcod = estab.etbcod and
        ...
        assign ventrada = 0 vsaida = 0 vorigem = 0.
        */
        for each tt-procod . delete tt-procod. end.
        ...
        for each movim where /*movim.movtdc = plani.movtdc and*/
            ...
            assign
                ...
                .
              if avail cplani
              then   tt-ent.plaori = cplani.numero .
              else   tt-ent.plaori = 0.
                .
             create tt-procod.
             tt-procod.procod = produ.procod.   
        end.
        if avail bplani
        then               
        ...
        for each cmovim where /*cmovim.movtdc = cplani.movtdc and */
            ...
            if produ.catcod <> 31
            then next.
            find first tt-procod where
                    tt-procod.procod = produ.procod no-error.
            if not avail tt-procod then next.        
...
for each tt-ent:
    find estab where estab.etbcod = tt-ent.etbcod no-lock.
    find first tt-filial where tt-filial.etbcod = estab.etbcod no-error.
    if not avail tt-filial
    then do:
            create tt-filial.
    ...
    tt-filial.valent = tt-filial.valent + tt-ent.valor.

    find produ where produ.procod = tt-ent.procod no-lock.
    find first tt-sai where
                tt-sai.etbcod = tt-ent.etbcod and
                tt-sai.clifor = tt-ent.clifor and
                tt-sai.pladat = tt-ent.pladat 
                no-error.
    if not avail tt-sai
    then do:
        tt-filial.valent = tt-filial.valent - tt-ent.valor.
        ...
        find bprodu where bprodu.procod = tt-sai.procod no-lock.
        if produ.catcod <> bprodu.catcod or
           produ.clacod <> bprodu.clacod
        then do:
            tt-filial.valent = tt-filial.valent - tt-ent.valor.
            delete tt-ent.   
        end.
        tt-filial.valsai = tt-filial.valsai + tt-sai.valor.
    end.         
end.                    
...
form tt-ent.pladat     column-label "Emissao" format "99/99/9999"
     ...
     tt-ent.plaori     column-label "NFOri" format ">>>>>>9"
     ...
     with frame f-filial1 width 125 down
     .
...
if opsys = "UNIX"
then varquivo = "../relat/ctdevmov" + string(vetbcod,"999").
else varquivo = "l:~\relat~\ctdevmov" + string(vetbcod,"999").
{mdadmcab.i
...
then do:
     for each tt-filial where valent <> 0 :
        ...
        tt-filial.valpct = (tt-filial.valent / tt-filial.valsai) * 100. 
    ...
    t1valpct = (t1valent / t1valsai) * 100.
    ...
    for each tt-ent break by tt-ent.etbcod 
        ...
        then do:
            for each tt-sai where tt-sai.etbcod = tt-ent.etbcod and   
                ...
                disp tt-sai.procod @ tt-ent.procod
                     produ.pronom
