...
if opsys = "UNIX"
then vdirarq = "/admcom/import/etiquetaslote/".
...
for each tt-lj no-lock,
    ...
    each estoq where estoq.etbcod = vetbcod                                   
                     no-lock,
...
for each tt-lj no-lock:
    ...
    for each tt-preco where
        ...
        then do:
            vpag = vpag + 1.
            if opsys = "UNIX"
            then varquivo =  "/admcom/import/etiquetaslote/" + string(tt-lj.etb~cod,"999") 
                            + "_" + string(vpag) + ".txt" .
    ...
    for each tt-preco where
        ...
        then do:
            vpag = vpag + 1.
            if opsys = "UNIX"
            then varquivo =  "/admcom/import/etiquetaslote/" + string(tt-lj.etbcod,"999") 
                            + "_" + string(vpag) + ".txt" .
            else varquivo = "l:~\relat~\" + string(tt-lj.etbcod,"999") 
                        + "_" + string(vpag) + ".txt" .
