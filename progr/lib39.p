...
def temp-table wmodal-i like modal.
def temp-table wmodal-e like modal.
...
repeat with column 50 side-labels 1 down width 31 row 4 frame f1:
    ...
    repeat:
        ...
        if wmodcod = ""
        then do:
            run modalidade-incluir.
            run modalidade-excluir.
        ...
        find first wmodal-i where wmodal-i.modcod <> "" no-error.
        if avail wmodal-i
        then do:
            for each wmodal:
                delete wmodal.
            ...
            for each wmodal-i:
                create wmodal.
                buffer-copy wmodal-i to wmodal.
            end.    
        end.
        for each wmodal-e where wmodal-e.modcod <> "":
            find first wmodal where
                       wmodal.modcod = wmodal-e.modcod
                       no-error.
            if avail wmodal
            then delete wmodal.            
        end.
        ...
        for each wmodal where wmodal.modcod <> "",
...
procedure modalidade-incluir:
    repeat with frame f-e column 15 title " Modalidade incluir "
                    row 8:
        prompt-for wmodal-i.modcod. 
        if input wmodal-i.modcod = ""
        then leave.
        find modal where modal.modcod = input wmodal-i.modcod 
                    no-lock no-error.
        if avail modal 
        then do:
            create wmodal-i.
            buffer-copy modal to wmodal-i.
            disp wmodal-i.modnom.
        end.
    end.
end procedure.
procedure modalidade-excluir:
    repeat with frame f-e column 45 title " Modalidade excluir "
                row 8:
        prompt-for wmodal-e.modcod. 
        if input wmodal-e.modcod = ""
        then leave.
        find modal where modal.modcod = input wmodal-e.modcod 
                    no-lock no-error.
        if avail modal 
        then do:
            create wmodal-e.
            buffer-copy modal to wmodal-e.
            disp wmodal-e.modnom.
        end.
    end.
end procedure.
