...
for each wgru:
    find first wacu where
               ...
               wacu.wmod = wgru.wmod and
               wacu.wano = wgru.wano and
               wacu.wmes = wgru.wmes no-error.
    if not avail wacu
    then do:
        create wacu.
        wacu.wset = wgru.wset.
...
form with frame f-fdisp1.       
def var varquivo as char.
        
def var varq as char.

    if opsys = "UNIX"
    then varq = "/admcom/relat/rmetdes_emissao_sintetico_" 
         + string(time)
         + ".csv".
    else varq = "l:\relat\rmetdes_emissao_analitico_"
         + string(time)
         + ".csv".
    ...
    for each wgru where wgru.wset = wsetor.wsetcod and
        ...
        for each wpag where wpag.wgru = wgru.wmod and
         ...
         /* vval = val-sal - vtot + wpag.wdeb. */
            ...
            disp 
                 setaut.setcod label "Setor"               
                 setaut.setnom no-label format "x(20)"     
                 modal.modcod   column-label "Sigla" 
        ...
        then do:
            ...
            assign vval = tt-totgru.tot
                ...
                disp 
                   setaut.setcod label "Set"                       
                   setaut.setnom label "Setor" format "x(20)"      
        ...
        ****/
        ...
        for each wacu where wacu.wset = wsetor.wsetcod:
            ...
            disp
                ...
                with frame f-disp11 down width 140.
            down with frame f-disp11. 
...
put 
  "Ano;Mes;Setor;;Grupo;;Limite;Realizado;Creditos;Debitos;Saldo"
  skip.
        ...
        */
        ...
        for each wacu where wacu.wset = wsetor.wsetcod
            ...
            put unformatted 
                ...
                format "x(20)"
                ";" 
                replace(string(wacu.wmet,"->>>>>>>9.99"),".",",")
                ";"
                replace(string(wacu.wpag,"->>>>>>>9.99"),".",",")
                ";"
                replace(string(wacu.wrec,"->>>>>>>9.99"),".",",")
                ";"
                replace(string(wacu.wdeb,"->>>>>>>9.99"),".",",")
                ";"
