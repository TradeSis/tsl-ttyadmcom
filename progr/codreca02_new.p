...
def stream splanilha.                                                          def buffer bestab for estab.
        ...
        for each bestab 
           ...
           /* do vdt = vdatini to vdatfin: */
              ...
              for each plani where plani.movtdc = 5 and
                  ...
                  if not avail tt-cli
                  then do:
                        find first rfvcli where rfvcli.clicod = plani.desti 
                            no-lock no-error.
                  
                        if not avail rfvcli then next.
                        run Pi-Monta-Cliente.
                  end.
                  disp "Processando : " plani.pladat no-label bestab.etbcod no-label
                      plani.desti no-label
...
def var v-tot-vend-lim1 as dec.
def var v-tot-vend-lim2 as dec.
def var v-tot-loja-lim1 as dec.
def var v-tot-loja-lim2 as dec.
def var v-tot-geral-lim1 as dec.
def var v-tot-geral-lim2 as dec.
...
for each tt-loja ,
    ...
    each tt-cli where tt-cli.clicod = tt-ven.clicod
      ...
      if v-ana-sin = 1
      then
      disp tt-ven.clicod  column-label "Cliente"
      ...
      then do:
           put  stream splanilha
                ...
                tt-ven.clicod ";"
                clien.clinom ";".
                if tt-cli.limdis1 <> ? then put tt-cli.limdis1.
                put ";".
                if tt-cli.limdis2 <> ? then put tt-cli.limdis2.
                put ";".
                if tt-cli.percen <> ?  then put tt-cli.percen.
                put ";"
                tt-ven.qtdcom-ca ";"
                tt-ven.vlcom-ca  ";" 
                tt-ven.qtdcom-sc ";"
      ...
      then do:
          ...
          disp "" @ tt-ven.clicod
               ( if v-ana-sin = 1 then "    Total do Vendedor => " 
               else tt-ven.vennom ) @ clien.clinom
      ...
      then do:
          ...
          disp "" @ tt-ven.clicod
           ...
           with frame fmostra width 158 down.
...
disp "" @ tt-ven.clicod
               "Total Geral do Periodo => " @ clien.clinom
               v-tot-geral-lim1 @ tt-cli.limdis1
               v-tot-geral-lim2 @ tt-cli.limdis2
               v-aux-percen    @ tt-cli.percen
               v-totq-geral-ca  @ tt-ven.qtdcom-ca
               v-totv-geral-ca  @ tt-ven.vlcom-ca  
               v-totq-geral-sc  @ tt-ven.qtdcom-sc
               v-totv-geral-sc  @ tt-ven.vlcom-sc  
               v-totq-geral-ger @ tt-ven.qtdcom-ger
               v-totv-geral-ger @ tt-ven.vlcom-ger  
          with frame fmostraf width 100 down.
...
Procedure Pi-Monta-Cliente.

find first tbcartao where 
                        tbcartao.codoper = 999 and
                        tbcartao.clicod = rfvcli.clicod and
                        (tbcartao.situacao = "E" or
                        tbcartao.situacao = "L")
                        no-lock no-error.
create tt-cli.
assign tt-cli.etbcod = rfvcli.etbcod
       tt-cli.clicod = rfvcli.clicod
       tt-cli.limdis1 = ?
       tt-cli.limdis2 = ?.

find first tt-loja where tt-loja.etbcod = rfvcli.etbcod 
                        no-error.
...
then do: 
         find first estab where estab.etbcod = tt-loja.etbcod
                       no-lock no-error.
         create tt-loja.
         assign tt-loja.etbcod = rfvcli.etbcod
                tt-loja.etbnom = estab.etbnom 
                               when avail estab.
end.

if avail tbcartao
then tt-loja.qtdcar  = tt-loja.qtdcar + 1.
