    ...
    repeat with frame frame-a:
        ...
        then do:
            ...
            then do with frame fbusca
                ...
                find first Prof_1701 of dtrecebimento 
                            where 
                            Prof_1701.PRODUCT_ID = vbusca and
                            int(Prof_1701.store_id) > 0 
        ...
        then do:
            ...
            then do:
                ...
                if esqcom1[esqpos1] = " Marca " and Prof_1701.dtinte = ?
                then do with frame f-Prof_1701 on error undo.
                    find Prof_1701 where recid(Prof_1701) = recatu1.
                    Prof_1701.marca = not Prof_1701.marca.
                    message "Para todos os registros do mesmo produto ?"
                            update sresp.
                    if sresp 
                    then do.
                        for each bProf_1701 of dtrecebimento where
                                 bProf_1701.PRODUCT_ID = Prof_1701.PRODUCT_ID.
                            bProf_1701.marca = Prof_1701.marca.
                        end.
                    end.
                ...
                then do on error undo.
                    sresp = no.
                    message "Confirma a Integração ?"
                    update sresp.
                    if sresp
                    then run itim/integra_1701.p (DtRecebimento.dtreceb).
                end.
...
vPRICE_TYPE =  if Prof_1701.PRICE_TYPE = "R" 
               ...
               if Prof_1701.PRICE_TYPE = "C"
               then "Saldo"
               else
               if Prof_1701.PRICE_TYPE = "P"
               then "Promocional"
               else "".
display
    prof_1701.marca no-label 
    Prof_1701.PRICE_TYPE no-label
    vPRICE_TYPE             column-label "TYPE"   format "x(3)"
...
then  
    if esqascend  
    then  
        find first Prof_1701 where of dtrecebimento
                                                no-lock no-error.
    else  
        find last Prof_1701  where of dtrecebimento
                                                 no-lock no-error.
...
then  
    if esqascend  
    then  
        find next Prof_1701  where of dtrecebimento
                                                no-lock no-error.
    else  
        find prev Prof_1701   where of dtrecebimento
                                                no-lock no-error.
