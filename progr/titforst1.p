...
procedure rateio-cria-titulo:
    ...
    do on error undo with frame frtit2 centered side-label 1 column:
        ...
        sresp = no.
        message "Deseja alterar valores do rateio?" update sresp.
        
        for each tt-lj no-lock:
            create fin.titulo.
            assign 
                fin.titulo.exportado = yes
                fin.titulo.empcod = wempre.empcod
                fin.titulo.titsit = "LIB"
                fin.titulo.titnat = vtitnat
                fin.titulo.modcod = vmodcod
                fin.titulo.etbcod = tt-lj.etbcod
                fin.titulo.datexp = today
                fin.titulo.clifor = vclifor
                fin.titulo.titnum = vtitnum
                fin.titulo.titpar  = 1
                fin.titulo.titdtemi = vtitdtemi
                fin.titulo.titdtven = vvenc
                fin.titulo.titvlcob = vtotal / vqtd-lj
                fin.titulo.titbanpag = vsetcod
                fin.titulo.titobs[1] = vtitobs[1]
                fin.titulo.titobs[2] = vtitobs[2].

            disp fin.titulo.etbcod
                 fin.titulo.titnum
                 fin.titulo.titvlcob
                 with frame f-rateio down.
            pause 0.     
            if sresp
            then  update fin.titulo.titvlcob with frame f-rateio.
            ...
            then assign vlancod = 100
                        ...
                        find last blancxa where 
                                     ...
                                     blancxa.etbcod = fin.titulo.etbcod and
                                     blancxa.lantip = "C"
                                             no-lock no-error.
                        if avail blancxa
                        then assign vlancod = blancxa.lancod
                        ...
                        find lanaut where 
                                lanaut.etbcod = fin.titulo.etbcod and
                                lanaut.forcod = fin.titulo.clifor
                                                no-lock no-error.
                        if avail lanaut  
                        then do:  
                            assign vlanhis = lanaut.lanhis 
                    ...
                    find lanaut where lanaut.etbcod = fin.titulo.etbcod and
                                      lanaut.forcod = fin.titulo.clifor
                                                no-lock no-error.
                    if avail lanaut 
                    then do: 
                    ...
                    if vlanhis = 6
                    then vcompl = "".
            fin.titulo.vencod = vlancod.
            fin.titulo.titnumger = vcompl.
            fin.titulo.titparger = vlanhis. 
        end.
    end.
end procedure.
...
procedure inclusao.
    ...
    form with frame f-par down centered row 7 overlay  color message.

    def var vdata     as date.
    def var vdtini    as date.
    def var vdtfin    as date.
    def var vtotmeta  as dec.
    ...
    do with frame ftit2.
                ...
                then do:
                    message color red/with
                        "Inclusao nao permitida para tipo de documanto."
                        view-as alert-box.
                    undo, retry.    
                end.
                **/
        ...
        do i = 1 to vtitpar:
            ...
            /***update vdia column-label "Dias" with frame f-par.***/
            vvenc = /*vtitdtemi + vdia*/ today + 3.
            update vvenc validate(vvenc >= today + 3,"Data invalida")
                                                with frame f-par.
            ...
            do on error undo.
                ...
                for each metadesp where metadesp.etbcod = estab.etbcod
                                  ...
                                  no-lock.
                    vtotmeta = vtotmeta + metadesp.metval.
                end.
                end.
                ...
                then do.
                    ...
                    for each btitulo where btitulo.empcod = wempre.empcod
                                       ...
                                       and btitulo.modcod = modal.modcod
                                       and btitulo.etbcod = vetbcod
                                       and btitulo.titdtven = vdata

                                       and btitulo.titbanpag = vsetcod
                                       and btitulo.evecod <> 9
                                     no-lock.  
                        vtotmodal = vtotmodal + btitulo.titvlcob.
                    end.
                    end.
                    end.
                    if vtotmodal + vtitvlcob > vtotmeta * 0.9
                    then do.
                        vsenha = "".
                        message "Meta atingida em mais de 90%:" 
                                vtotmodal + vtitvlcob "/" vtotmeta
                    ...
                    assign titulo.exportado = yes
                           ...
                           titulo.titvlcob = vtitvlcob
                                        /***(vtotal - vt) / (vtitpar - ii)***/
