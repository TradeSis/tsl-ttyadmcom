...
def buffer c01 for a01_infnfe.
    ...
    run chama-wsnfe-integra.p(input plani.etbcod,
                       input plani.numero,
                       input "NotaMax",
                       input vmetodo,
                       input varquivo,
...
then do on error undo:
    ...
    then do:
        ...
        if not avail b01
        then do:
            find c01 where c01.etbcod = plani.etbcod and
                           c01.placod = plani.placod
                           no-lock no-error.
            if not avail c01
            then do on error undo:
