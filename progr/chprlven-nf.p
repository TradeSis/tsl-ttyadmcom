...
def var vnumven as int format ">>>>>>>>9".
...
repeat:
    ...
    if vetbcod <> 0
    then update vnumven at 1 label "Numero venda" with frame fopcoes.
    
    if vnumven = 0
    then do:
    update vdatini at 1 label "Data Inicial"
    ...
    disp skip(3)
        ...
        for each plani where
            ...
            vetbutil = plani.etbcod.
            vdiferenca = plani.platot.
            for each titulo where titulo.empcod = 19
                              and titulo.titnat = yes
                              and titulo.modcod = "CHP"
                              and titulo.etbcod = 999
                              and titulo.clifor = 110165
                              and titulo.etbcobra = plani.etbcod
                              and titulo.titdtpag = plani.pladat
                              and titulo.cxacod = plani.cxacod
                              and titulo.evecod = plani.vencod
                              no-lock:
                vetbvenda = ?.
                vplavenda = ?.
                ...
                vetbvenda = int(acha("ETBCODVENDACHP",titulo.titobs[1])).
                vplavenda = int(acha("PLACODVENDACHP",titulo.titobs[1])).
                vdiferenca = vdiferenca - titulo.titvlcob.
                find bplani where bplani.etbcod = vetbvenda
                          and bplani.placod = vplavenda 
                          and bplani.movtdc = 5 no-lock no-error.
 
                disp titulo.titnum  column-label "Numero!Cartao"
                     bplani.etbcod when avail plani column-label "Fil!Ven"
                    titulo.titdtven     column-label "Data!Ativacao"
                    bplani.vencod when avail plani column-label "Vend!Venda"
                    bplani.numero when avail plani column-label "Numero!Venda"
                                format ">>>>>>>9"
                    titulo.titvlcob    format ">>>>,>>9.99"
                                column-label "Valor!Cartao"
                    vcatutil      format ">>9"     column-label "Set!Util"
                    vetbutil when vetbutil <> ?   column-label "Fil!Util"
                    titulo.titdtpag               column-label "Data!Util"
                    plani.vencod                 column-label "Vend!Util"
                    plani.numero                  column-label "Numero!Util"
                            format ">>>>>>>>9"
                    plani.platot when avail plani column-label "Valor!Util"
                            format ">>>>,>>9.99"
                    vdiferenca  when avail bplani  column-label "Diferenca"
                    with frame frame-a1 10 down centered
                                   color white/red row 5 width 130.
            end.
        end.
    end.
