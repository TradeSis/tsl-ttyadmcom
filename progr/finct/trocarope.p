...
def var vagrupardia as log format "Sim/Nao" init yes.

message "Agrupar por dia?" update vagrupardia.
...
/*    field cobcodSAIDA   like ctbtrocart.cobcodSAIDA */
    ...
    field qtdTit    as int format ">>>>>>>9" column-label "qtd Tit"
...
def temp-table ttcontrato no-undo
    ...
    index x is unique primary contnum asc dtinc asc cobcodENTRADA asc  cobcodSAIDA asc.
...
for each ctbtrocart where ctbtrocart.dtinc >= vdtini and ctbtrocart.dtinc <= vdtfin no-lock.
    vi = vi + 1.
    if vi mod 1000 = 0 then do:
        hide message no-pause.
        message color normal "fazendo calculos... aguarde..." vi .
    end.
            
    find contrato of ctbtrocart no-lock no-error.
    if not avail contrato then next.
    
    find first tttrocactr where         
        tttrocactr.dtinc        = (if vagrupardia then ctbtrocart.dtinc else ?) and
...
/*    tttrocactr.dtrefsaida   = ?.*/
    ...
    then do:
        ...
        ttcontrato.dtinc        = (if vagrupardia then ctbtrocart.dtinc else ?).
...
/*    ttcontrato.dtrefsaida   = ctbtrocart.dtrefSAIDA.*/
    ...
    then do:
        ...
        hide message no-pause.
        message color normal "selecionados... " vi .
...
/*                                   string(tttrocactr.cobcodsaida) + "_" + */
                                   ...
                                   (if vagrupardia then string(tttrocactr.dtinc,"99999999") else "") + "_" +
