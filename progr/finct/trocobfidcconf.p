...
do on error undo with frame f-filtro side-label centered
    ...
    varqout = "/admcom/tmp/ctb/VendaCarteira-Drebes-Fidc/" + "vendafidc_" + 
...
for each tt-titulo no-lock.
    ...
    then do:
        ...
        run finct/trocacart.p (vcobcoddes, /* 09022023 helio ID 155965 - motivo*/
                               ...
                               int(titulo.titnum),
                               titulo.titpar,
                               "Troca Carteira de " +  string(vcobcodori) + "->" + string(vcobcoddes) + " - lote fidc " + string(vlote)
                                    + " (PROGRAMA:finct/trocobfidcconf.p) "
                                ). /* 09022023 */
