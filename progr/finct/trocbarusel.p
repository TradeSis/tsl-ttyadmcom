...
def new SHARED temp-table tt-carteira-selec
...
def shared temp-table ttfiltros no-undo
    ...
    field ctr_acrescimo  as log format "Sim/Nao" init no
...
update
    ...
    ttfiltros.ctr_acrescimo   label "Filtrar Contrato com Acréscimo" colon 35    
    with frame fcab.
update    
    ttfiltros.vlf_acrescimo   label "valor Minimo de Acrescimo" colon 35    
            when ttfiltros.ctr_acrescimo
    ...
    run selec-cobra.p.
    ttfiltros.carteiras = "".
    for each tt-carteira-selec.
        ttfiltros.carteiras = ttfiltros.carteiras + (if ttfiltros.carteiras = "" then "" else ",") + string(tt-carteira-selec.cobcod).  
    end.    
    disp ttfiltros.carteiras colon 35 label "carteiras"
        with frame fcab.
...
then for each titulo where titnat = no and titdtpag = ? and titulo.titdtven > today - ttfiltros.diasatraso 
                /**and cobcod = vcobcodori**/ no-lock.
    ...
    if titulo.cessaobaru then next.
    ...
    then.
        ...
        /* testa carteiras */
            if titulo.cobcod = if ttfiltros.carteiras = ""  
                                 then titulo.cobcod
                                 else if  lookup(string(titulo.cobcod),ttfiltros.carteiras) = 0
                                      then ?
                                      else titulo.cobcod
            then.
            else next.
    ...
    if avail ttcontrato and (vsel = 1 or (vsel < 10 and vsel mod 10 = 0) or (vsel > 100 and vsel mod 100  = 0))
    then do:
        hide message no-pause.
...
do on error undo:
    ...
    if ttfiltros.ctr_acrescimo and ttcontrato.vlf_acrescimo = 0
    then ttcontrato.marca = no.   
