...
define temp-table tt-valores no-undo serialize-name 'cor'
...
for each cor no-lock:
    create tt-valores.
    assign tt-valores.codigo    = cor.corcod
           tt-valores.descricao = fnremoveacento(cor.cornom)
...
RUN PostRequest (INPUT 'produtos/integracaoCor', 
                 INPUT lo-arquivo).
