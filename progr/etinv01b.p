...
if opsys = "UNIX"
then do:
    output to /admcom/zebra/01/eti-01.bat append.
    ...
    */
    ...
    then do:
      for each wfetq break by linha:
        ...
        if opsys = "UNIX"
        then varquivo = "/admcom/zebra/01/c" + string(produ.procod)
    ...
    then do:
      for each wfetq break by linha:
        ...
        if opsys = "UNIX"
        then varquivo = "/admcom/zebra/01/cr" + string(produ.procod) + 
                            "." + trim(string(par-taman,"x(3)")).
...
def var vprocod    like produ.procod.
    ...
    then do:
        ...
        then
                if vdtfliped <> ?
                then assign vdtped = substring(string(year(vdtfliped)),3,2)
