...
do on error undo:
    ...
    for each estab where estab.etbcod < 189
             no-lock:
...
varquivo = "/admcom/relat/limites." + string(time).
    ...
    for each tbcartao where 
        ...
        *****/
        ...
        if not avail tt-cli then do:
            ...
            create  tt-cli.
            assign  tt-cli.clicod = tbcartao.clicod no-error.
            assign  tt-cli.rec_as = recid(clien) no-error.
            assign  tt-cli.clinom = clien.clinom no-error.
            assign  tt-cli.filial = vfilcli no-error.
...
if vcsv = no then leave.
    ...
    if opsys = "UNIX"
    then varquivo = "../relat/limites" + string(time) + ".csv".
