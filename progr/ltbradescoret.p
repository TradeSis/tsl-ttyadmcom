...
def temp-table tt-arquivo
    field i-registro as int
    field t-cpfcnpj  as int
    field n-cpfcnpj  as char
    field n-cpfcnpjaux as char
...
varq = "PG" + string(day(vdt-arq) - 1,"99") + string(month(vdt-arq),"99").
...
for each lotcretit of lotcre no-lock:
    ...
    /****
    message forne.fornom
    aux-cgccpf
    titulo2.dec1
    titulo2.titnum
    titulo2.dec1
    titulo.titvlcob + titulo.titjuro -
                                            titulo.titvldes
    ...
    ****/
    ...
    for each tt-arquivo use-index i1 where
        ...
        /*message tt-arquivo.v-pagamento. pause.
        */
    ...
    for each tt-arquivo use-index i1 where
               tt-arquivo.n-cpfcnpjaux   = string(aux-cgccpf) and
               (if titulo2.dec1 = 0
                then tt-arquivo.n-documento = titulo2.titnum
                else tt-arquivo.n-pagamento = string(titulo2.dec1)) and
               tt-arquivo.v-pagamento = titulo.titvlcob + titulo.titjuro -
...
for each agtitbra where
        ...
        for each tt-arquivo use-index i1 where
               tt-arquivo.n-cpfcnpjaux   = agtitbra.cgccpf and
               /*tt-arquivo.n-documento = "1" and*/
               tt-arquivo.d-vencimento = agtitbra.titdtven and
               tt-arquivo.v-pagamento = agtitbra.valor-pago
               : 
...
for each agtitfor where
        ...
        for each tt-arquivo use-index i1 where
               tt-arquivo.n-cpfcnpjaux   = agtitfor.cgccpf and
               /*tt-arquivo.n-documento = "1" and*/
               tt-arquivo.d-vencimento = agtitfor.titdtven and
               tt-arquivo.v-pagamento = agtitfor.valor-pago
               : 
...
procedure processa-retorno:
    ...
    for each lotcretit of lotcre :
        ...
        /*
        for each tt-arquivo where tt-arquivo.n-cpfcnpj   = string(aux-cgccpf)
        .
        disp tt-arquivo.n-documento tt-arquivo.v-pagamento
        tt-arquivo.t-retorno sl-arquivo.t-retorno
        tt-arquivo.marca.
        pause.
        end.
        pause.
        */
        ...
        find first tt-arquivo use-index i1 where
               ...
               tt-arquivo.marca = "*"
               no-error.                        
        if not avail tt-arquivo 
        then find first tt-arquivo use-index i1 where
                tt-arquivo.n-cpfcnpjaux   = string(aux-cgccpf) and
                (if titulo2.dec1 = 0
                then tt-arquivo.n-documento = titulo2.titnum
                else tt-arquivo.n-pagamento = string(titulo2.dec1)) and
        ...
        if not avail tt-arquivo
        then next. 
        /*message tt-arquivo.n-cpfcnpj tt-arquivo.n-cpfcnpjaux. pause. 
          */
...
procedure importa-dados-arquivo-retorno:
    ...
    repeat:
        ...
        if int(substr(vlinha,1,1)) = 1
        then do:
        create tt-arquivo.
        assign
            tt-arquivo.i-registro       = int(substr(vlinha,1,1))
            tt-arquivo.t-cpfcnpj        = int(substr(vlinha,2,1))
            tt-arquivo.n-cpfcnpj        = 
                if tt-arquivo.t-cpfcnpj = 2
                then substr(vlinha,4,14)
                else substr(vlinha,3,9) + substr(vlinha,16,2)
            tt-arquivo.n-cpfcnpjaux     = (if substr(vlinha,251,1) = "1"
                                          then substr(vlinha,400,14)
                                          else "")
            tt-arquivo.n-pagamento      = substr(vlinha,120,16)
            tt-arquivo.n-documento      = string(int(substr(vlinha,253,10)))
            tt-arquivo.d-vencimento     = if int(substr(vlinha,170,2)) <> 0
                                          then date(int(substr(vlinha,170,2)),
                                               int(substr(vlinha,172,2)),
                                               int(substr(vlinha,166,4)))
                                          else ?
            tt-arquivo.d-emissao        = if int(substr(vlinha,178,2)) <> 0
                                          then date(int(substr(vlinha,178,2)),
                                               int(substr(vlinha,180,2)),
                                               int(substr(vlinha,174,4)))
                                          else ?     
            tt-arquivo.d-pagamento      = if int(substr(vlinha,270,2)) <> 0
                                          then date(int(substr(vlinha,270,2)),
                                               int(substr(vlinha,272,2)),
                                               int(substr(vlinha,266,4)))
                                          else ?
            tt-arquivo.t-retorno        = substr(vlinha,279,2)
            tt-arquivo.v-documento      = dec(int(substr(vlinha,195,10)) / 100)
            tt-arquivo.v-pagamento      = dec(int(substr(vlinha,205,15)) / 100)
...
procedure processa-pagamento:
    ...
    find first formpag where
               ...
               no-lock.
    message titulo.clifor titulo.titnum titulo.titsit. pause.           
