...
def var vendereco       like clien.endereco format "x(60)" extent 0.
...
find clien where clien.clicod = tt-clien.clicod no-lock.
    ...
    for each titulo where titulo.titnat = no and
                          ...
                          titulo.titdtven < vtoday and
                          (if setbcod <> 999
                          then titulo.etbcod = setbcod
                          else true)
                    no-lock.
    ...
    vdataprom = ?.
    find last bclitel where bclitel.clicod = vclicod no-lock no-error.
    vdataprom = if avail bclitel then bclitel.dtpagcont else ?.
    for each bclitel where bclitel.clicod = vclicod
                      and bclitel.dtpagcont  <> ? no-lock.
         
         vdataprom = bclitel.dtpagcont.  
    end.                        
    if vdataprom <> ? and vdataprom > today
    then next.
    
                        find tt-clien where tt-clien.clicod = vclicod no-lock
                                                    no-error.
                        if not avail tt-clien
                        then do.
                            create tt-clien.                                                       tt-clien.clicod = vclicod.
                        end.
                        recatu1 = recid(tt-clien).
                        leave.
                    end.
                ...
                then do on error undo:
                   ...
                   message clien.clicod vetbcod view-as alert-box.
                   run bsfqtitag.p (clien.clicod,vetbcod).
                   pause 0.
                   view frame frame-a.
                   view frame f-com1.
                   view frame telobs.
...
/*                             cpfis.estado-fone label "Tipo"*/
                        ...
                        then do:
                            find tt-clien where recid(tt-clien) = recatu1.
                            recatu1 = ?.
                            /*
...
then do.
    ...
    find last bclitel where bclitel.clicod = vclicod no-lock no-error.
    vdataprom = if avail bclitel then bclitel.dtpagcont else ?.    
    for each bclitel where bclitel.clicod = clilig.clicod
                      and bclitel.dtpagcont  <> ? no-lock.
...
then do.
        ...
        */
        ...
        then do.
            ...
            then .
               ...
               find estab where estab.etbcod = clitel.etbcobra no-lock no-error.
               find  estrgcobra of estab no-lock.
               if vrgccod <> estrgcobra.rgccod
               then next.
            end.
    ...
    find last bclitel where bclitel.clicod = vclicod no-lock no-error.
    vdataprom = if avail bclitel then bclitel.dtpagcont else ?.    
       for each bclitel where bclitel.clicod = clitel.clicod
                      and bclitel.dtpagcont  <> ? no-lock.
    ...
    vdivida = 0.
    for each titulo where titulo.titnat = no and
                          titulo.clifor = clitel.clicod and
                          titulo.modcod  = "CRE" and
                          titulo.titsit = "LIB" and
                          titulo.titdtven < vtoday and
                          (if setbcod <> 999
        ...
        find clilig where clilig.clicod = clitel.clicod no-lock no-error. 
...
for each clilig where clilig.dtproxlig = vtoday no-lock.
    ...
    vdataprom = ?.
    find last bclitel where bclitel.clicod = vclicod no-lock no-error.
    vdataprom = if avail bclitel then bclitel.dtpagcont.    
    for each bclitel where bclitel.clicod = clilig.clicod
                      and bclitel.dtpagcont  <> ? no-lock.
...
vtitle = ctipos  + "QDT CLIENTES = " + string(vqtd).
 ...
 procedure frame-a.
    ...
    for each titulo where titulo.titnat = no and
                    ...
                    no-lock.
        vdivida = vdivida + titulo.titvlcob - titulo.titvlpag.
        if vdtven > titulo.titdtven or vdtven = ?
        then vdtven = titulo.titdtven.
    end.
    
    vnumdias = vtoday - /*tt-clien.titdtven*/ vdtven .

    find last clitel where clitel.clicod = tt-clien.clicod /*and
               clitel.teldat > tt-clien.dtven and
              (clitel.dtpagcont < vtoday or clitel.dtpagcont = ?)*/
                 no-lock no-error.
...
/*    def buffer bclitel for clitel.*/
    for each bclitel of tt-clien where bclitel.dtpagcont  <> ? no-lock.
         
         vdataprom = bclitel.dtpagcont.  
    end.
                
    find func where func.etbcod = clitel.etbcobra and
                    func.funcod = clitel.funcod no-lock no-error.
...
vdtven = ?.
    ...
    vnumdias = vtoday - /*tt-clien.titdtven*/ vdtven.
    display
       tt-clien.clicod column-label "Conta"
...
procedure leitura-titulos.
    ...
    then next.
            ...
            then do:
                ...
                assign
                    ...
                    tt-clien.titdtven = /*vtoday*/ vdtven.
                vqtd = vqtd + 1.
            end.                    
        end.
    end. 
        ...
        for each titulo /*use-index por-dtpag-uo-modal*/
            ...
            for each btitulo where
                ...
                btitulo.titdtpag = ? and
                          (if setbcod <> 999
                          then btitulo.etbcod = setbcod
                          else true)
                no-lock.
                vmax = max(vmax,vtoday - titulo.titdtven).
                if vdtven > btitulo.titdtven or vdtven = ?
                    then vdtven = btitulo.titdtven.
            end.
            ...
            then
                for each xtitulo where xtitulo.titnat   = no              and
                                      ...
                                      (xtitulo.modcod   = "CLI" or
                                       xtitulo.modcod   = "GE") and
                          (if setbcod <> 999
                          then xtitulo.etbcod = setbcod
                          else true)
                                        no-lock.
    ...
    vdivida = 0.
    for each xtitulo where xtitulo.titnat = no and
                          xtitulo.clifor = titulo.clicod and
                          xtitulo.modcod  = "CRE" and
                          xtitulo.titsit = "LIB" and
                          xtitulo.titdtven < vtoday and
                          (if setbcod <> 999
                          then xtitulo.etbcod = setbcod
                          else true)
                    no-lock.
        vdivida = vdivida + xtitulo.titvlcob - xtitulo.titvlpag.
    end.           
    if vdivida = 0
    then next.
                find first tt-clien where
