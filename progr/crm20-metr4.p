...
def var vlog-descarta  as logical.
...
for each acao-cli where 
         ...
         first pedid where pedid.pedsit  = yes
                       and pedid.clfcod  = acao-cli.clicod
                       and pedid.pedtdc  = 21
                       and pedid.sitped  = "F"
                       and pedid.peddti >= acao.dtini
                       and pedid.peddti <= acao.dtfin no-lock,
        ...
        for each movim where movim.etbcod = plani.etbcod
            ...
            first liped where liped.etbcod = pedid.etbcod
                          and liped.pedtdc = pedid.pedtdc
                          and liped.pednum = pedid.pednum
                          and liped.procod = produ.clacod no-lock:
        
            assign vlog-descarta = no.
    ...
    if not selfil then do:
        ...
        for each movim where movim.etbcod = plani.etbcod
            ...
            first liped where liped.etbcod = pedid.etbcod
                          and liped.pedtdc = pedid.pedtdc
                          and liped.pednum = pedid.pednum
                          and liped.procod = movim.procod no-lock:
...
procedure p-excel:
    ...
    if opsys = "UNIX"
    then assign
            varquivo = "/admcom/relat/crm20-conacao.xls".
    else assign
            varquivo = "l:\relat\crm20-conacao.xls".
