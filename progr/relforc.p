...
for each produ where produ.catcod = 41 and
    ...
    then do:
        ...
        if month(movim.movdat) = month(today) and
           year(movim.movdat)  = year(today)
    ...
    put skip
        produ.pronom format "x(34)" space(1)
        produ.procod space(1)
        estoq.estvenda format ">,>>9.99" space(1)
        estoq.estdtven format "99/99/99" space(1)
        vtotest format "->>,>>9" space(1)
        vmargen space(1)
        vultcomp format "99/99/99" space(1)
