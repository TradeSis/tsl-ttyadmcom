...
choose field v-sel[5] v-sel[4] v-sel[6]with frame f-sel.
...
if vano = 2010
then
varquivo = "/admcom/financeira/CTB2010/CANCELAMENTOS CONTRATOS/CANCELADOS"
            + vnomes[vmes] + string(vano,"9999")
            + ".csv".
else if vano = 2011
then varquivo = 
     "/admcom/financeira/CTB2011/cancelamentos/cancelamentos "
        + lc(vnomes1[vmes]) + string(vano,"9999") + ".csv".
else if vano = 2012
then varquivo = 
     "/admcom/financeira/CTB2012/Cancelamentos/cancelamentos"
        + lc(vnomes[vmes]) + string(vano,"9999") + ".csv".
else if vano = 2013
then varquivo =
    "/admcom/financeira/CTB2013/cancelamentos/cancelamentos"
    + lc(vnomes[vmes]) + string(vano,"9999") + ".csv".
else if vano = 2014
then varquivo =
     "/admcom/financeira/CTB2014/cancelamentos/cancelamentos"
     + lc(vnomes[vmes]) + string(vano,"9999") + ".csv".
else varquivo =
     "/admcom/financeira/CTB" + string(vano,"9999") + 
...
then do:
    ...
    for each tt-estpg:
        ...
        then do on error undo:
            ...
            assign
                financeirace.contnum = int(tt-estpg.val[2])
                financeirace.datcan = date(tt-estpg.val[8])
            .
        end.
        do on error undo:
        assign
        ...
        create ContFinan.
        assign
        ContFinan.tipo_movimento = "CANCELAMENTO"
        ContFinan.numero_contrato = int(tt-estpg.val[2])
        ContFinan.data_emissao = date(tt-estpg.val[3])
        ContFinan.data_vencimento = date(tt-estpg.val[4])
        ContFinan.data_movimento = date(tt-estpg.val[9])
        ContFinan.valor_renda = dec(tt-estpg.val[13])
        ContFinan.valor_comercial = dec(tt-estpg.val[14])
        ContFinan.valor_entrada = dec(tt-estpg.val[15])
        ContFinan.qtd_parcelas = int(tt-estpg.val[16])
        ContFinan.valor_IOF = dec(tt-estpg.val[17])
        ContFinan.valor_principal = dec(tt-estpg.val[19])
        ContFinan.moeda = tt-estpg.val[20]
        ContFinan.produto = tt-estpg.val[22]
        ContFinan.agencia = tt-estpg.val[23]
        ContFinan.grupo = tt-estpg.val[24]
...
if vano = 2010
then
varquivo = "/admcom/financeira/CTB2010/ESTORNOS RECEBIMENTOS/ESTORNOS"
            + vnomes[vmes] + string(vano,"9999") + ".csv".
else if vano = 2011
then varquivo = 
     "/admcom/financeira/CTB2011/estornos/estornos "
        + lc(vnomes1[vmes]) + string(vano,"9999") + ".csv".
else if vano = 2012
then varquivo = 
     "/admcom/financeira/CTB2012/Estornos/estornos"
        + (vnomes[vmes]) + string(vano,"9999") + ".csv".
else if vano = 2013
then varquivo =
    "/admcom/financeira/CTB2013/estornos/estornos"
    + lc(vnomes[vmes]) + string(vano,"9999") + ".csv".
else if vano = 2014
then varquivo =
    "/admcom/financeira/CTB2014/estornos/estornos"
    + lc(vnomes[vmes]) + string(vano,"9999") + ".csv".
else varquivo =
...
for each tt-estpg:
    ...
    find first financeirace where financeirace.contnum = int(tt-estpg.val[4])
        no-error.
    if not avail financeirace
    then do:
        create financeirace.
    ...
    find fin.contrato where contrato.contnum = int(tt-estpg.val[4]) 
            no-lock no-error.
    ...
    create ContFinan.
    assign
        ContFinan.tipo_movimento = "ESTORNO"
        ...
        ContFinan.data_movimento = date(tt-estpg.val[1])
        ...
        ContFinan.valor_iocatraso = dec(tt-estpg.val[15])
        ContFinan.valor_outros = dec(tt-estpg.val[16])
        ContFinan.valor_descontoautomatico = dec(tt-estpg.val[17])
        ContFinan.valor_receita = dec(tt-estpg.val[19])
        ContFinan.valor_creditos = dec(tt-estpg.val[20])
        ContFinan.produto = tt-estpg.val[25]
        ContFinan.evento = int(tt-estpg.val[6])
        ContFinan.agencia = tt-estpg.val[3]
        ContFinan.grupo = tt-estpg.val[21]
        ContFinan.nome = tt-estpg.val[30]
        .
      */
end.
end.
