...
find tipmov where tipmov.movtdc = 33 no-lock.
find first opcom where opcom.movtdc = 33 no-lock no-error.
if avail opcom
...
repeat:
    ...
    */
    ...
    for each w-movim:
        ...
        ASSIGN tt-movim.movtdc = tt-plani.movtdc
           ...
           if tt-movim.MovAlICMS > 0
              then assign                                                                       tt-movim.movbicms = tt-movim.movpc * tt-movim.movqtm
    ...
    if sresp
    then do:
        update tt-plani.notobs
        with frame f-obs 1 down centered row 16
        no-label title "Informações Adicionais"
        overlay.
        
        run manager_nfe.p (input "5551_j",
                           input ?,
                           output vok).
