...
def temp-table tt-pedid like pedid.
    ...
    form
        tt-pedid.pedobs[1] at 1
        tt-pedid.pedobs[2] at 1
        tt-pedid.pedobs[3] at 1
        tt-pedid.pedobs[4] at 1
        tt-pedid.pedobs[5] at 1 with frame fobs color white/cyan overlay row 9
...
do on error undo:
    ...
    then do:
        ...
        if not avail estab
        then undo.
        else disp estab.etbnom no-label with frame f-data.
    end.
    else disp "Todos" @ estab.etbnom with frame f-data.
end.    
...
do on error undo:
    ...
    update skip
        ...
        vsitped label "Situacao"
        help "Informe a Situacao ou deixe em branco para Todos"
        with frame f-data width 80 side-labels 1 down 
                          title " DATA PARA CONSULTA " . 

    if vdata1 = ? or vdata2 = ? or
       (vdata1 > vdata2)
    then undo.
...
then do:
    do vdata = vdata1 to vdata2:
       for each pedid where pedid.pedtdc = 3
                        and pedid.etbcod = vetbcod
                        and pedid.peddat = vdata no-lock:

           if vsitped <> ""
           then if pedid.sitped <> vsitped
                then next.
                
           find tt-pedid where tt-pedid.etbcod = pedid.etbcod
                           and tt-pedid.pedtdc = pedid.pedtdc
                           and tt-pedid.pednum = pedid.pednum no-error.
           if not avail tt-pedid 
           then do:
                create tt-pedid.
                {tt-pedid.i tt-pedid pedid}.
           end.                             
       end.
    end.
end.
...
repeat:
    ...
    if recatu1 = ?
    then find first tt-pedid where tt-pedid.pedtdc = vpedtdc 
                             /*  and tt-pedid.peddat = vdata*/ no-lock no-error.
    else
    ...
    repeat:
        ...
        find next tt-pedid where tt-pedid.pedtdc = vpedtdc /*
                              and tt-pedid.peddat = vdata*/ no-lock.
        if not available tt-pedid
        then leave.
    ...
    repeat with frame frame-a:
        ...
        prompt-for tt-pedid.pednum.
            find last tt-pedid where tt-pedid.pedtdc = vpedtdc 
                                 and tt-pedid.pednum = input tt-pedid.pednum
                               /*  and tt-pedid.peddat = vdata */
                            no-lock no-error.
            if avail tt-pedid
        ...
        then do:
            find prev tt-pedid where tt-pedid.pedtdc = vpedtdc 
                               /*and tt-pedid.peddat = vdata*/ no-lock no-error.
            if not avail tt-pedid
            then next.
        ...
        hide frame  frame-a no-pause.
          ...
          then do:
            ...
            then do:
                ...
                for each liped of tt-pedid no-lock.
                    ...
                    disp liped.procod
                         ...
                         liped.lipsep column-label "Qtd.S" format ">>>9"
            ...
            then do with frame f-Lista overlay row 9 1 column centered.
                ...
                &Tit-Rel   = """PEDIDO N. "" + string(tt-pedid.pednum) "
                &Width     = "110"
                &Form      = "frame f-cabcab"}
                ...
                for each liped where liped.pedtdc = tt-pedid.pedtdc and
                                     ...
                                     liped.pednum = tt-pedid.pednum no-lock.
