...
repeat:
    ...
    find lancxa where recid(lancxa) = tt-lan.reclan no-lock no-error.
    ...
    display lancxa.livre2 no-label format "x(01)"
            ...
            lancxa.lancod column-label "Debito"
            lancxa.cxacod column-label "Credito"            
                        /*
    ...
    repeat with frame frame-a:
        ...
        hide frame frame-a no-pause.
          ...
          then do:
            ...
            then do on error undo with frame f-altera overlay row 6 centered.
                ...
                display extra-des no-label
                        ...
                        lancxa.titnum at 3
                        lancxa.forcod at 2
                        lancxa.etbcod at 6 label "Filial"
                        lancxa.lantip at 8
                ...
                find current lancxa no-lock.       
            end.

            if esqcom1[esqpos1] = "Consulta"
            then do with frame f-consulta overlay row 6 centered.
            
                find lancxa where recid(lancxa) = tt-lan.reclan 
                        no-lock no-error.
                find tablan where tablan.lancod = lancxa.lancod
                                no-lock no-error.
...
procedure filtro:
    /*def var vfiltro as char extent 3  format "x(15)"
        init["GERAL","FORNECEDOR","FILIAL"].
    disp vfiltro with frame f-filtro 1 down centered no-label.
    choose field vfiltro with frame f-filtro.
    */
    ...
    update vforcod label "Fornecedor"
           ...
           with frame f-filtro 1 down side-label centered
           row 7.
    if vforcod  > 0
    then do:
    ...
    else if vforcod > 0 and vetbcod = 0
    then do:
        for each tt-lan. delete tt-lan. end.
        do vdata = vdti to vdtf:
            for each lancxa where /*lancxa.cxacod <> 0 and*/ 
