...
repeat:
    ...
    */
    update qtd-cont to 36 label "Quantidade de contratos"
           help "Informe a quntidade de contratos por filial" 
           val-cont to 36  label "Valor minimo do contrato"
           help "Informe o valor minimo do contrato"
           with frame f1.
           
    do on error undo:
        update vclicod to 36 label "Codigo do Cliente" with frame f1.
    ...
    do on error undo:
        update vfincod to 29 label "Vendas no Plano"
            with frame f1.
        if vfincod > 0
        then do:
            find finan where finan.finco = vfincod no-lock no-error.
            if not avail finan
            then do:
                message color red/with
            ...
            disp finan.finnom no-label with frame f1.
        end.
    end.
 
    v99 = no.
    update v99 to 29 label "Com Substituicao"
        with frame f1.
    v00 = no.
    update v00 to 29 label "Sem Substituicao"
        with frame f1.
    ...
    vpctacr = 0.
    update vpctacr to 34 label "Percentual de acrescimo"
            format ">>9.99 %" with frame f1.
        ...
        for each contrato where contrato.etbcod = tt-estab.etbcod 
            ...
            then do:
                ...
                for each movim where movim.etbcod = plani.etbcod and
                    ...
                    if (v99 and produ.proipiper = 99) 
                    then do:
                        vretornar = no.
                        leave.
                    end.
                    else if v00 and produ.proipiper = 99
                    then do:
                        vretornar = yes.
                        leave.
                    end.
...
put unformat skip
    ...
    (if clien.estciv > 0 and clien.estciv < 6
     ...
     substr(string(clien.natur),3,20) format "x(20)"       /*  150 - 169 */
...
procedure p-registro-05.
    ...
    put unformat skip "05"  /* 01  - 02  TIPO  FIXO  05*/
        ...
        clien.cep[2]        /* 165 - 172 */    format "x(8)"
        vaux                /* 173 - 193 */    format "x(19)" 
        /*DDD
        clien.protel[1]    /* 177 - 188 */    format "x(12)"
        ""                  /* 189 - 193 */    format "x(5)"
        */
        cpclien.var-int4    /* 194 - 197 */   format "9999"
        ""                  /* 198 - 227 CARGO*/ format "x(40)"
        f-troca(string(clien.prodta[1])) /* 228 - 235 */ format "99999999"
        vrenda * 100 format "999999999999" /* 236 - 252 */
        ""                  /* 253 - 269 OUTROS RENDIM*/
        ""                  /* 270 - 299 DESCRIÇÃO OUTROS RENDIMENTOS*/
