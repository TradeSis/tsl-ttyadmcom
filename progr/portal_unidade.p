...
define temp-table tt-valores no-undo serialize-name 'unidadeCompra'
    field codigo    as character serialize-name 'codigo'
...
for each unida no-lock:
    create tt-valores.
    assign tt-valores.codigo    = unida.unicod
           tt-valores.descricao = fnremoveacento(unida.uninom)
...
RUN PostRequest (INPUT 'produtos/integracaoUnidadeCompra', 
                 INPUT lo-arquivo).
