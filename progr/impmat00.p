...
do i = vetbi to vetbf:
    ...
    IF v-etbcod = 44 OR v-etbcod = 47 OR v-etbcod = 48 OR
       v-etbcod = 19 or v-etbcod = 46 or v-etbcod = 50 or v-etbcod = 49 
    THEN DO :
        RUN serbema.p ( INPUT v-dtini , INPUT v-dtfin , input i ).
    END.
    ELSE DO :
        {serial00.i}.
    END.
    ...
    if search("..\import\" + string(i,"99") + "\glopre.d") <> ?
    then do:
        pause 0.
        input from value("..\import\" + string(i,"99") + "\glopre.d") no-echo.
        repeat:
            for each tt-glopre:
                delete tt-glopre.
            end.
    ...
    repeat with frame ftitulo:
        ...
        then do transaction:
            ...
            if titulo.etbcobra <> titulo.etbcod 
            then do:
                find first tto where tto.etbcod = titulo.etbcod
                               no-error.
                if avail tto
                then do:
    ...
    def temp-table tt-controle
        field contcli as int  
        field contcon as int 
        field conttit as int 
        field contpro as int 
        field contest as int 
        field contpla as int 
        field contmov as int 
        field dtini   as date 
        field dtfin   as date  
        field dtexp   as date 
        field hrexp   as int.
    ...
    /***   controle ********/
    if search("..\import\" + string(i,"99") + "\controle.d") <> ?
    then do:
        pause 0.
        input from value("..\import\" + string(i,"99") + "\controle.d") no-echo.
        repeat:
            ...
            find cpd-controle where 
                 cpd-controle.etbcod  = i and 
                 cpd-controle.dtexp   = tt-controle.dtexp and
                 cpd-controle.hrexp   = tt-controle.hrexp  no-error.
            do transaction:
                if  not available cpd-controle
                then do:
                    create cpd-controle.
                    ASSIGN cpd-controle.etbcod = i
                           cpd-controle.dtexp  = tt-controle.dtexp
                           cpd-controle.hrexp  = tt-controle.hrexp.
                end.
                assign Cpd-controle.iDtFim    = tt-controle.dtfin
                       Cpd-controle.iDtIni    = tt-controle.dtini
                       Cpd-controle.icontcli  = tt-controle.contcli
                       Cpd-controle.icontcon  = tt-controle.contcon
                       Cpd-controle.icontest  = tt-controle.contest
                       Cpd-controle.icontmov  = tt-controle.contmov
                       Cpd-controle.icontpla  = tt-controle.contpla
                       Cpd-controle.icontpro  = tt-controle.contpro
                       Cpd-controle.iconttit  = tt-controle.conttit
