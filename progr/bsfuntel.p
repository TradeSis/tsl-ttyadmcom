...
def temp-table tt-liga 
    ...
    field clfnom like clien.clinom  format "x(31)"
...
def temp-table tt-sinte
    ...
    index funcod is primary unique etbcod  asc
                                   funcod  asc
                                   codcont asc
...
repeat:
    ...
    def temp-table tt-func
        ...
        index funape    is primary unique etbcod apelido asc
        index funcod    etbcod asc
                        codigo asc.
    for each tt-liga .
        delete tt-liga .
    end.
    ...
    then do:
        for each clitel where clitel.teldat >= vdatlig and
            ...
            if clitel.codcont = 0  
            then next.
            find tt-func where tt-func.etbcod = clitel.etbcobra and
                               tt-func.codigo = clitel.funcod no-error.
            find func where func.funcod = clitel.funcod and
                            func.etbcod = clitel.etbcobra no-lock.
            if not avail tt-func
            then create tt-func.
            assign tt-func.codigo  = clitel.funcod
                   tt-func.etbcod  = clitel.etbcobra
                   tt-func.apelido = func.funape.
            find tt-sinte where tt-sinte.funcod  = func.funcod and
                                tt-sinte.etbcod  = func.etbcod and
    ...
    */
    ...
    for each clitel where             
        ...
        find first clien where 
                  clien.clicod = clitel.clicod no-lock.
        vdivida = 0.
...
/*        vnumdias = today - clitel.dtven.*/
        create tt-liga.
        assign
            tt-liga.telhor = string(clitel.telhor,"HH:MM:SS")
            tt-liga.clfcod = clien.clicod
            tt-liga.clfnom = clien.clinom
/*            tt-liga.matras = clien.dtven*/
...
vtitulo = "SINTETICO DE SITUACAO DE COBRANCA - PERIODO DE " +
    ...
    if opsys = "UNIX"
    then varquivo = "../relat/funtel" + string(time) .
    else varquivo = "l:~\relat~\funtel" + string(time).
...
form
    ...
    with frame f-func
        centered
        down
        title " Cobranças - Pesquisa por Funcionario " .
{setbrw.i}.
do :         
        assign
            i-seeid = -1
        ...
        {abrowse.i
            &File = tt-func
            &CField = tt-func.apelido
            &Ofield = " tt-func.etbcod
                        tt-func.codigo tt-func.apelido " 
            &Where = "true" 
            &AftSelect1 = "vfuncod = tt-func.codigo.
                           vetbcod = tt-func.etbcod. 
                   leave keys-loop. "
            &locktype = "no-lock" 
            &Form = " frame f-func" 
        } .
