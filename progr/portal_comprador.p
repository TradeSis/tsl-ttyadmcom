...
define temp-table tt-valores no-undo serialize-name 'comprador'
...
for each compr no-lock:
    create tt-valores.
    assign tt-valores.codigo    = compr.comcod
           tt-valores.descricao = fnremoveacento(compr.comnom)
...
RUN PostRequest (INPUT 'produtos/integracaoComprador', 
                 INPUT lo-arquivo).
