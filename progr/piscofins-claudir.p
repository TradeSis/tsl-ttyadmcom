...
then do.
    ...
    disp tt-movim.movpc tt-movim.movipi / tt-movim.movqtm
        tt-movim.movpc + (tt-movim.movipi / tt-movim.movqtm).
    ...
    then do.
        if tipmov.movtnota
        then vclfcod = tt-plani.desti. /* Emite  */
        else vclfcod = tt-plani.emite. /* Digita */
    ...
    disp vbasepiscof / tt-movim.movqtm valiqpis valiqcofins .
    pause.
    assign
        vpis    = round(vbasepiscof * valiqpis    / 100, 2)
        vcofins = round(vbasepiscof * valiqcofins / 100, 2).
    disp (vpis / tt-movim.movqtm) (vcofins / tt-movim.movqtm) .
    pause.    
end.
else do.
    assign
...
procedure cal-mva-entrada:
    ...
    assign 
        ...
        disp vmovcusto clafis.mva_oestado1. pause.
