...
def var vlistavendas as log format "Sim/Nao".
...
repeat with 1 down side-label width 80 row 3:
    ...
    vlistavendas = no.
    if vetbcod > 0
    then update vlistavendas colon 25 label "Listar vendas".
                        ...
                        find first titulo
                             where titulo.empcod = 19
                               ...
                               and titulo.titpar   = 0
                                        no-lock no-error.
                        if avail titulo
                        then vvndentrada = titulo.titvlcob.
                    end.
                end.                            
  ...
  then do:
    ...
    for each wfresumo where wfresumo.tipocxa = vtipocxa    
                        and wfresumo.avista = yes
                        and wfresumo.modal  = "VEN" .
        disp
            wfresumo.catcod
            wfresumo.vista 
            wfresumo.vndprodu column-label "VENPRD!Produtos!"
            wfresumo.recarga  column-label "VENCC!Recarga!+"
            wfresumo.garantia column-label "VENGR!Garantia/RFQ!+"
            wfresumo.vndCP    column-label "CECP!Cred Pes!"
            wfresumo.vndPEDO  column-label "VENPN!PED!"
            wfresumo.vndencar column-label "Encargos"
            wfresumo.vndserv  column-label "Abate!-"
            with frame f-resumo-99 width 136 down no-box.
    end.    
    ...
    then
        do vdt = vdtini to vdtfin:
            for each plani where 
                ...
                disp
                    plani.cxacod
                    plani.serie  format "x(8)"
                    plani.numero
                    plani.crecod
                    plani.pladat
                    plani.protot (total by plani.crecod)
                    plani.platot (total by plani.crecod)
                    vcontnum     format ">>>>>>>>9"
                    vvltotal     (total by plani.crecod)
                    vvndgarantia (total by plani.crecod)
                            column-label "Garantia"
                    vvndentrada  (total by plani.crecod) column-label "Entrada"
                    plani.vlserv (total by plani.crecod) column-label "Bonus"
                    with frame f-plani width 136 down no-box.
            end.
        end.
        ...
        each titulo where titulo.etbcobra = estab.etbcod and
            ...
            */
            ...
            vljuro = 0.
                ...
                else if titulo.modcod begins "CP" and
                     titulo.cxacod < 30
                    then val-pago = titulo.titvlcob.
            ...
            find first wfresumo where 
                ...
                then do:
                    ...
                    wfresumo.vlsemnovcob  = wfresumo.vlsemnovcob + 
                                            titulo.titvlcob.  
                    wfresumo.vlsemnovpag  = wfresumo.vlsemnovpag + 
                                                titulo.titvlpag.
                    wfresumo.vlsemnovjur = wfresumo.vlsemnovjur + 
                    if titulo.modcod begins "CP" and
                                         titulo.cxacod < 30
                       then 0
                       else if titulo.titvlpag > titulo.titvlcob
                            then (titulo.titvlpag - titulo.titvlcob)
                            else 0.
                end.
