...
repeat:
   ...
   if opsys = "UNIX"
   then do:
        varquivo = "/admcom/relat-auto/convloj." + string(time) + ".txt".
        /*
    ...
    */
        ...
        if frame-index = 2 
        then do:
            run convgen1.p (input vfabcod).
        end.
        else do:
            run convgen0.p (input vfabcod).
        end.        
        */
  ...
  do d = vdti to vdtf:
    for each estab where estab.etbcod = (if vetbcod <> 0 
        ...
        then do:
            ...
            find first ttloja where ttloja.etbcod = movim.etbcod no-error.
            if not avail ttloja
            then do:
                create ttloja.
                assign ttloja.etbcod = movim.etbcod
                       ttloja.etbnom = estab.etbnom
                       ttloja.metlj  = v-total.
            end.
        ...
        then do:
            ...
            then next.
                    ...
                    find first ttloja where 
                               ttloja.etbcod = movim.etbcod no-error.
                    if not avail ttloja
                    then do:
                        create ttloja.
                        assign ttloja.etbcod = movim.etbcod
                    ...
                    val_dev =  ((movim.movpc * movim.movqtm) / plani.platot) * 
                                plani.vlserv.
                    if val_dev = ? then val_dev = 0.
                    if (plani.platot - plani.vlserv - plani.descprod) < plani.b~iss
                    then
                    val_fin =  ((((movim.movpc * movim.movqtm) - val_dev - val_~des) /
                                (plani.platot - plani.vlserv - plani.descprod))
                                * plani.biss) - ((movim.movpc * movim.movqtm) - 
                                val_dev - val_des).
                    if val_fin = ? then val_fin = 0.
