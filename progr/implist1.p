...
assign tt-remetente.data-emissao           = today                 .
    /* tt-remetente.pagina                 =  */
       ...
       tt-remetente.cod-administrativo     = 9033734                .
       tt-remetente.contrato               = "9912231982"          .
       tt-remetente.numerolista            = 0                    .
...
for each wbsdocbvol where wbsdocbvol.dtfec = vdata no-lock.
    ...
    then find first pedecom where pedecom.pednum = pedid.pednum       
                              ...
                              and pedecom.pedtdc = pedid.pedtdc
                                  no-lock no-error.
    if not avail pedecom or (pedecom.tipo <> "SEDEX" and pedecom.tipo <> "PAC")
    then next.
    ...
    assign tt-destinatario.destinatario        = clien.clinom                .
           ...
           tt-destinatario.declara-valor       = "NAO"                       .
           tt-destinatario.val-declarado       = 0                        .
           tt-destinatario.valor-cobrado-desti = 0                        .
           tt-destinatario.inf-compl           = ""                     .
           ...
           tt-destinatario.volume              = string(wbsdocbvol.dcbseq) +
                                           "/" + string(vqtdvol).            .
           tt-destinatario.servico             = "40436 SEDEX CONTRATO"   .
           tt-destinatario.peso-tarifado       = ""                    .
           tt-destinatario.servico-adic        = "25"                    .
           tt-destinatario.valor-pagar         = ""                     .
...
assign vcha-caminho-arq = "/admcom/web/correios/listagem/list-"
...
put stream str-html unformatted
    ...
    "<tr>" skip
    "    <td width='17%' nowrap><img src='logo_correios.jpg'/></td>" skip
    "    <td width='59%' align='Left' nowrap><font size='3'><b>Empresa Brasileira de Correios e Telégrafos</b></font></td>" skip
...
for each tt-destinatario no-lock:
    ...
    put stream str-html unformatted
        ...
        "            <tr height='20' valign='top'>" skip
        "                <td class='destinatario' width='1%'></td>" skip
        "                <td class='destinatario'>" tt-destinatario.declara-valor  "</td>" skip
        "                <td class='destinatario'>" tt-destinatario.val-declarado "</td>" skip            
        "                <td class='destinatario'>" tt-destinatario.valor-cobrado-desti "</td>" skip
        "            </tr>" skip
        "            <tr>" skip
        "                <td class='destinatario' width='1%'></td>" skip
