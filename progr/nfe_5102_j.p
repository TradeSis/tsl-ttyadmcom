...
message "Esta NF terá produtos usados?" update vprod-usado.

for each tt-movim:
    
    find produ where produ.procod = tt-movim.procod no-lock.
    if produ.proipiper = 99 and not vprod-usado
    then tt-movim.movcsticms = "060".
    else do.
        if tt-movim.movcsticms = ""
        then tt-movim.movcsticms = "000".
        ...
        for each tt-movim where tt-movim.etbcod = tt-plani.etbcod and
            ...
            then do:
                    ...
                    assign
                        ...
                        int(substr(tt-movim.movcsticms,1,1))
                        N01_icms.modbc = 3
                        N01_icms.cst   = 
                        int(substr(tt-movim.movcsticms,2,2)).
