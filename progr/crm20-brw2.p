{admcom_funcoes.i}                 
def var vlim-total as dec.
def var vlim-dispo  as dec.
...
def var v-tipo-cartao as char format "x(1)".
def var v-indi-paberta as log format "sim/nao" init no.
...
def var esc-fil as char format "x(25)" extent 3
        ...
        then do:
            ...
            then do:
                ...
                then do with frame f-filtro on error undo.
                    ...
                    disp  skip(1)
                          esc-fil[1] format "x(30)" skip 
                          esc-fil[2] format "x(30)" skip
                          esc-fil[3] format "x(30)" skip
                          with frame f-esc-fil title " Tipo de Filtro " 
                                row 7 width 32
                             centered color with/black no-label overlay. 
...
procedure p-total.
    ...
    for each brfvtot where 
        ...
        vlim-total = vlim-total + brfvtot.lim-credito.
        vlim-dispo = vlim-dispo + brfvtot.lim-disponivel. 
    end.
    ...
    disp skip(3)
         ...
         space(4)   vtotqtd no-label skip(2)
         
         /*
         "LIMITE DISPONIVEL" SKIP
         SPACE(4)   VLIM-DISPO format ">>>,>>>,>>9.99" no-label skip
         */
...
procedure p-rfv-ini:                
    ...
    def var vlimite-credito as dec init 0.
    def var vsaldo-aberto as dec init 0.
    vrec = 0. 
    vfre = 0. 
    vval = 0.

    for each estab where (if vetbcod > 0
                           then estab.etbcod = vetbcod else true) and
                           estab.etbnom begins "DREBES-FIL"
                           NO-LOCK:
    disp "Processando RFV ... Aguarde! "
            "Filial " estab.etbcod  
            with frame f-disprfv 1 down centered row 10 no-box no-label.
        pause 0.

    for each crm use-index ch02crm where
                crm.etbcod = estab.etbcod:
        ...
        find first rfvtot where rfvtot.rfv = crm.rfv and
                   rfvtot.etbcod = vetbcod no-error.
        if not avail rfvtot
        then do:
            create rfvtot.
            assign rfvtot.rfv            = crm.rfv
                   rfvtot.flag           = no
                   rfvtot.etbcod         = vetbcod.
            ...
            do icont = 1 to length(string(crm.rfv)):
               ...
               then do:
                   if icont = 1
                   then assign rfvtot.recencia      = rfv-tipo.descricao.
                   if icont = 2
        ...
        assign rfvtot.qtd-ori    = rfvtot.qtd-ori + 1
               rfvtot.per-tot    = (rfvtot.qtd-ori * 100) / i
               vqtd = vqtd + 1.

        if vqtd mod 1000 = 0 
        then do:
            disp vqtd no-label
                 with frame f-disprfv .
            pause 0.     
        end.
        
        /**
        find clien where clien.clicod = crm.clicod no-lock.
        vlimite-credito = limite-cred-scor(recid(clien)).
        vsaldo-aberto   = saldo-aberto-cliente(recid(clien)).
    ...
    for each crm where 
        ...
        /**/
            ...
            then do: /*Filtra*/
                ...
                if crm.mostra = no then next.  /* Antonio - Sol 26885*/
...
procedure p-acao:
    ...
    then do:
        ...
        */ 
        ...
        for each rfvtot use-index iflagetb where 
             rfvtot.flag = yes and
             rfvtot.etbcod = if vetbcod = 0
                              then rfvtot.etbcod
                              else vetbcod no-lock:
            
            for each crm use-index ch04crm where 
                crm.rfv    = rfvtot.rfv and
                      crm.etbcod = if vetbcod = 0
                                   then crm.etbcod
                                   else vetbcod no-lock:

                 if rfvtot.qtd-ori = rfvtot.qtd-sel or
                    crm.mostra = yes
                 then.
                 else next.
                ...
                if vconfirma = yes
                then do:
                    for each bacao where
                         bacao.dtini   >= vdata1           and
                         bacao.dtini   <= vdata2           or
                         bacao.dtfin   >= vdata1           and
                         bacao.dtfin   <= vdata2           no-lock:

                        find first bacao-cli where
                                   bacao-cli.acaocod = bacao.acaocod and
                                   bacao-cli.clicod  = crm.clicod
                                   no-lock no-error.
                        if avail bacao-cli
                        then do:
                            vok = yes.
