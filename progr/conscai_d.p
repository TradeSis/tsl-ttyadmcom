...
def shared var vEstab     as log  format "Sim/Nao"  init yes.
...
def shared var vdtini      like d.titulo.titdtemi.
def shared var vdtfin      like d.titulo.titdtemi.
...
/*       if vetbcod <> 0 and fin.caixa.cxacod = 20 then next.*/
       ...
       for each chq where chq.datemi = vdata no-lock:
          ...
          then do:
             run Pi-Cria-Anali(input "cheque", input 15,
                               input "", input chq.valor, 1).
             if v-gravou-ana = yes 
             then assign vpredre = vpredre + chq.valor
    ...
    for each d.titulo  
            ...
            */
            ...
            if d.titulo.moecod <> "CAR" then next.

            qtd-cart = qtd-cart + 1.
            vl-cart = vl-cart + d.titulo.titvlpag.
            tt-dados.qtd-cart = tt-dados.qtd-cart + 1.
            tt-dados.vl-cart = tt-dados.vl-cart + d.titulo.titvlpag.
    ...
    for each fin.caixa where fin.caixa.etbcod = estab.etbcod no-lock:
        ...
        for each d.titulo where d.titulo.empcod = 19
            ...
            tt-dados.total-cheque-devolvido = total-cheque-devolvido
                                            + d.titulo.titvlpag
            tt-dados.conta-cheque-devolvido = tt-dados.conta-cheque-devolvido
                                            + 1.
             end.
        end.
    ...
    for each d.titulo where d.titulo.etbcobra = estab.etbcod and
            ...
            then  for each titpag where
                    ...
                    then do:
                        ...
                        then assign vlpagcartao = vlpagcartao + titpag.titvlpag
                           tt-dados.vlpagcartao = tt-dados.vlpagcartao 
                                                + titpag.titvlpag.
                        else do:
                     ...
                     tt-dados.vljurpre = tt-dados.vljurpre + d.titulo.titjuro.
            ...
            then do:
                ...
                then do:
                   ...
                   run Pi-Cria-Anali(input "titulo", input 4, 
                    ...
                    tt-dados.vlpres = tt-dados.vlpres + d.titulo.titvlcob.
                end.    
                ...
                run Pi-Cria-Anali(input "titulo", input 4, 
                           ...
                           input 1).
                if v-gravou-ana = yes then
                assign vlpres = vlpres + d.titulo.titvlcob
                tt-dados.vlpres = tt-dados.vlpres + d.titulo.titvlcob.
            end.    
            ...
            then do:
               ...
               then assign vlnov  = vlnov + d.titulo.titvlcob
                           ct-nov = ct-nov + 1
                           tt-dados.vlnov = tt-dados.vlnov + d.titulo.titvlcob
        ...
        for each d.titulo where d.titulo.datexp = vdata no-lock.
            ...
            then do:
                ...
                then   assign ct-entr = ct-entr + 1
                       ...
                       tt-dados.vlentr = tt-dados.vlentr + d.titulo.titvlcob.
            end.
        end.
 end.
 ...
 for each fin.caixa where fin.caixa.etbcod = estab.etbcod no-lock:
         ...
         for each d.titulo where d.titulo.datexp = vdata 
            ...
            then do:
                ...
                then do:
                        ...
                        assign vldesc = vldesc + d.titulo.titdesc
                          tt-dados.vldesc = tt-dados.vldesc + d.titulo.titdesc.
                end.
            end.
            ...
            then do:
                if bmoeda.moetit = yes
                then assign tt-dados.ct-pagcartao = tt-dados.ct-pagcartao 
                                           + (if d.titulo.titvlcob > 0
                                              then 1 else 0)
                     ct-pagcartao = ct-pagcartao + (if d.titulo.titvlcob > 0
                                                    then 1 else 0).
                else assign ct-pres = ct-pres + (if d.titulo.titvlcob > 0
                                                then 1 else 0)
            ...
            tt-dados.ct-juro = tt-dados.ct-juro
                             + if d.titulo.titjuro > 0
                               then 1 else 0. 
            ct-desc = ct-desc + if d.titulo.titdesc > 0
                                then 1
                                else 0.
            tt-dados.ct-desc + if d.titulo.titdesc > 0
                               then 1 else 0.     
         end.
 end.
 ...
 for each fin.caixa where fin.caixa.etbcod = estab.etbcod no-lock:
        ...
        for each d.titulo where d.titulo.datexp = vdata
                ...
                then do:
                 ...
                 tt-dados.vdevolucao = tt-dados.vdevolucao + d.titulo.titvlpag
                 tt-dados.ct-devolucao = tt-dados.ct-devolucao + 1.
                end.
        ...
        for each plani use-index pladat where plani.movtdc = 5 and
            ...
            then do:
                ...
                then do:
                    ...
                    then do:
                        for each tt-cartpre.
                            ...
                            tt-dados.vvalor-cartpre = tt-dados.vvalor-cartpre
                                       + tt-cartpre.valor.
                        end.
                    end.
                ...
                then do:
                        ...
                        if v-gravou-ana = yes 
                        then assign vlpraz = vlpraz - plani.vlserv
