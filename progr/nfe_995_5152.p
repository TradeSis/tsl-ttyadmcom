...
do:
    DO:
        ...
        /* em 16/06 coloquei exclusive  */
        find first A01_infnfe where   A01_infnfe.emite = tt-plani.emite and
...
find first tt-pedid where tt-pedid.etbcod > 0 no-error.
if avail tt-pedid
then do:
    find pedid where pedid.etbcod = tt-pedid.etbcod and
                     pedid.pednum = tt-pedid.pednum and
                     pedid.pedtdc = tt-pedid.pedtdc
                     no-error.
    if avail pedid 
    then do:
        buffer-copy tt-pedid to pedid.
        for each tt-liped where 
                 tt-liped.etbcod = pedid.etbcod and
                 tt-liped.pednum = pedid.pednum and
