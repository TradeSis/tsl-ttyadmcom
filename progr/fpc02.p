...
repeat:
    ...
    update vcla-cod label "Clase" colon 16 with frame f-dep.
    /*find bclase where bclase.clacod = vcla-cod no-lock no-error.*/
...
/******/    
    ...
    update vetbi no-label format ">>9"
           ...
           vetbf no-label format ">>9" 
                    with frame f-etb centered color blue/cyan row 16
                                    title " Filial ".
    for each tt-curva:
        delete tt-curva.
    end.
    ...
    for each tt-forfilho,
        each tt-cla /*clase where clase.clasup = vcla-cod*/ no-lock,
        each produ where (produ.catcod = vcatcod or
                          produ.catcod = vcatcod2) and
                          produ.fabcod = tt-forfilho.fabcod and
                          /* fabri.fabcod and    antonio antes */
                          produ.clacod = tt-cla.clacod
                           no-lock:
    ...
    then do:
        ...
        then assign fila = string(impress.dfimp)
                    varquivo = "/admcom/relat/fpc"
                                        + string(time)
                                        + STRING(day(today)) +
                                        string(xclase.clacod,"9999999999").
    ...
    for each tt-curva by tt-curva.pos:
        ...
        disp tt-curva.pos format "9999" column-label "Pos."
             ...
             tt-curva.giro when tt-curva.giro > 0
                     ...
                     with frame f-imp width 180 down.
    end.
    output close.
