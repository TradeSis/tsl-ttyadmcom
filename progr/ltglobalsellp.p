...
vselecao = string(par-dtini) + " a " + string(par-dtfin) 
    + " valor acima de " + string(par-valor).



find lotcretp where lotcretp.ltcretcod = "GLOBAL" no-lock.
for each d.titulo where 
         d.titulo.empcod = 19
         and d.titulo.titnat = no
         and d.titulo.modcod = "CRE"
         and d.titulo.titdtven >= par-dtini
         and d.titulo.titdtven <= par-dtfin
         and d.titulo.titvlcob >= par-valor 
         and d.titulo.etbcod = par-estab 
         and d.titulo.titsit = "LIB"
         and d.titulo.titvlpag = 0
         and d.titulo.cobcod = 2
         no-lock.


         find first d.contnf where contnf.etbcod = d.titulo.etbcod and        
         d.contnf.contnum = int(d.titulo.titnum) no-lock no-error.        
            
            /* mandar as novacoes com parcela acima de 31 */
            if not avail d.contnf and
                d.titulo.titpar < 31
            then next.                                                 
            ...
            do on error undo.
                find lotcreag where lotcreag.ltcrecod = lotcre.ltcrecod
                    and lotcreag.clfcod   = d.titulo.clifor
                  no-error.
