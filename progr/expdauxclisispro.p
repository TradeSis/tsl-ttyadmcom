...
vnomarq = "SP_INT_CTB_LCT_EX".
        
varqexp = "/admcom/sispro/" + vnomarq + "_" + 
...
varqres = "/admcom/sispro/" + vnomarq + "_RESUMO_" + 
...
procedure put-emissao.
    create tt-arquivo.
    assign
        tt-arquivo.CD_ESTAB = string(tabdac.etblan,"999")
        tt-arquivo.DT_INI_PERIODO = int(string(vdti,"99999999"))   
        tt-arquivo.AN_LIVRO = "DIARIO"
        tt-arquivo.DT_LCTO  = int(string(tabdac.datlan,"99999999"))
        tt-arquivo.AN_NUM_LCTO = ""
        tt-arquivo.AN_LANCTO_LOTE = "9999"         
        tt-arquivo.NR_BOLETIM  = 99999          
        tt-arquivo.NR_LANCTO   = vnum-lanca       
        tt-arquivo.DT_FIM_PERIODO = int(string(vdtf,"99999999"))       
        tt-arquivo.CD_COD_CTA = "1.1.02.01.001"
        tt-arquivo.CD_COD_CCUS  = ""
        tt-arquivo.VL_DC        = tabdac.vallan
        tt-arquivo.IN_IND_DC    = "D"
        tt-arquivo.IN_IND_LCTO  = "N"
        tt-arquivo.AN_NUM_ARQ   = ""
        tt-arquivo.AN_COD_HIST_PAD = "0"
        tt-arquivo.AN_HIST = "*LIVRE@" + tabdac.hislan
        tt-arquivo.AN_COD_PART  = ""
        tt-arquivo.CD_DOCUMENTO = ""
        tt-arquivo.IN_ROTINA    = ""
        tt-arquivo.CD_USUARIO   = ""
        tt-arquivo.DT_ALTERACAO = 99999999       
        tt-arquivo.HR_ALTERACAO = ""
        tt-arquivo.CD_STID      = ""
        tt-arquivo.AN_COD_PART_AUX = string(tabdac.clicod)
        tt-arquivo.DT_OPER         = int(string(tabdac.datlan,"99999999"))
        tt-arquivo.AN_TP_OPER      = tabdac.tiplan
        tt-arquivo.AN_TP_DOC       = "TIT"         
        tt-arquivo.AN_DOC          = tabdac.numlan
        .  
    
    if avail clien
    then tt-arquivo.AN_NM_PART_AUX  = clien.clinom.
    ...
    put unformat
     ...
     year(tabdac.datemi) format "9999"          /* 44-51 */
     ...
     year(tabdac.datemi) format "9999"  /* 96-103 */
     month(tabdac.datemi) format "99"  
     day(tabdac.datemi) format "99"
...
procedure put-arquivo:
    find first tt-arquivo no-error.
    if avail tt-arquivo
    then
    put unformatted
        "01" + tt-arquivo.CD_ESTAB        format "x(16)"      
        tt-arquivo.DT_INI_PERIODO  format "99999999"   
        "CLIENTE" /*tt-arquivo.AN_LIVRO*/        format "x(10)" 
        tt-arquivo.DT_LCTO         format "99999999"
        "" /*tt-arquivo.AN_NUM_LCTO*/     format "x(20)"          
        tt-arquivo.AN_LANCTO_LOTE  format "x(10)"          
        "" /*tt-arquivo.NR_BOLETIM*/      format "x(5)"          
        int(tt-arquivo.NR_LANCTO)     format ">>>>>>>9"       
        tt-arquivo.DT_FIM_PERIODO  format "99999999"       
        tt-arquivo.CD_COD_CTA      format "x(52)"          
        "" /*tt-arquivo.CD_COD_CCUS*/     format "x(30)"          
        /*tt-arquivo.VL_DC           format "99999999999999.99" 
        */
        replace(string(tt-arquivo.VL_DC,"999999999999999.99"),".","")
            format "x(17)"
        tt-arquivo.IN_IND_DC       format "x"              
        tt-arquivo.IN_IND_LCTO     format "x"              
        tt-arquivo.AN_NUM_ARQ      format "x(50)"          
        tt-arquivo.AN_COD_HIST_PAD format "x(5)"           
        tt-arquivo.AN_HIST         format "x(254)"         
        tt-arquivo.AN_COD_PART     format "x(22)"          
        tt-arquivo.CD_DOCUMENTO    format "x(10)"          
        tt-arquivo.IN_ROTINA       format "x(30)"          
        tt-arquivo.CD_USUARIO      format "x(16)"          
        "" /*tt-arquivo.DT_ALTERACAO*/    format "x(8)"       
        tt-arquivo.HR_ALTERACAO    format "x(8)"           
        tt-arquivo.CD_STID         format "x(30)"          
        "" format "x(62)"
        tt-arquivo.AN_COD_PART_AUX format "x(22)"          
        tt-arquivo.AN_NM_PART_AUX  format "x(100)"          
        "" format "x(8)" /*tt-arquivo.DT_OPER         format "99999999" */      
        tt-arquivo.AN_TP_OPER      format "x(20)"          
        tt-arquivo.AN_TP_DOC       format "x(10)"          
        tt-arquivo.AN_DOC          format "x(20)"          
        skip
    .
...
procedure resumo:
    ...
    find first tt-resumo where
               tt-resumo.data = p-data no-error.
