/*  resutel1.p  */
def input  parameter p-clfcod like clien.clicod.
def output parameter p-fone   as   char.

run zresutel.p (input p-clfcod, output p-fone).     
...
/*def var vclfcod like clifor.clfcod init 7570.*/
...
find clifor where clifor.clfcod = p-clfcod no-lock no-error.
find cpfis of clifor no-lock no-error.
/*find clicpl where clicpl.clfcod = clifor.clfcod no-lock no-error.*/

v-titulo = "Resumo de Telefones - "
         + string(clifor.clfcod)
         + " "
         + clifor.clfnom.
...
CREATE tt-fone.
ASSIGN tt-fone.fone    =  clifor.fone /* (IF AVAIL cpfis
                          THEN cpfis.fonepes
                          ELSE "") */
...
CREATE tt-fone.
ASSIGN tt-fone.fone    = clifor.celular /*(IF AVAIL cpfis
                          THEN cpfis.fone 
                          ELSE "") */
...
CREATE tt-fone.
ASSIGN tt-fone.fone    = (IF AVAIL cpfis
                          THEN cpfis.fonepes
                          ELSE "") 
...
ASSIGN tt-fone.fone    = (IF AVAIL cpfis 
                          THEN cpfis.conjfone 
                          ELSE "") 
       tt-fone.tipo    = "CONJUGE"
       tt-fone.relacao = cpfis.conjuge
       tt-fone.ordem   = 40.
...
CREATE tt-fone. 
ASSIGN tt-fone.fone    = cpfis.rpesfone[1] 
       tt-fone.relacao = cpfis.rpesnome[1]
       tt-fone.tipo    = cpfis.rpestiprel[1]
       tt-fone.ordem   = 50.

CREATE tt-fone. 
ASSIGN tt-fone.fone    = cpfis.rpesfone[2] 
       tt-fone.relacao = cpfis.rpesnome[2]
       tt-fone.tipo    = cpfis.rpestiprel[2]
       tt-fone.ordem   = 60.
...
/*create tt-fone.
assign tt-fone.fone = "C O N T R A T O S"
       tt-fone.tipo = ""
       tt-fone.relacao = ""
       tt-fone.ordem = 62.*/
...
assign tt-fone.fone    = "F O N E"
       ...
       tt-fone.relacao = "D A T A    N O M E"
       tt-fone.ordem   = 62.
...
procedure frame-a.
    ...
    display tt-fone.fone     format "x(22)" column-label "Fone"
            ...
            tt-fone.relacao  format "x(30)" column-label "Relacionamento"
...
procedure mostra-contratos:
    ...
    for each titulo where titulo.titnat = no
                      and titulo.clfcod = clifor.clfcod no-lock
        ...
        then do:
            ...
            find bclifor where bclifor.clfcod = titulo.titdep no-lock no-error.
            ...
            then do:
                if tt-fone.relacao <> ""
                then 
                    tt-fone.relacao = tt-fone.relacao + "   " + bclifor.clfnom.
                else
...
/*            disp string(plani.hiccod) format "(999)999-9999-9999"
                 ...
                 bclifor.clfnom       column-label "Nome" format "x(21)"
                                      when avail bclifor
