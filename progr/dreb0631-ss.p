...
def temp-table wfcomis field etbcod       like estab.etbcod
                       ...
                       field acre         as dec
                       field remarca as dec
                       .  
def var val-remarca as dec.
...
repeat:
    ...
    tipo = 0.
    update vtprcod  colon 20 with title " Relatorio Vendas sem o seguro ".
    tipo = vtprcod.
    ...
    */
    ...
    do vdt = vdtini to vdtfin:
        ...
        for each estab where if vetbcod = 0
            ...
            for each movim where movim.etbcod = plani.etbcod and
                ...
                then assign
                    ...
                    find first hispre where
                       hispre.procod = movim.procod and
                       hispre.dtalt  <= movim.movdat and
                       hispre.funcod = 234
                       no-lock no-error.
                    if avail hispre
                    then val-remarca = val-remarca +
                            ( movim.movpc * movim.movqtm).
                            
                    /*
                    find first aux_estoq where 
                           aux_estoq.etbcod      = movim.etbcod and
                           aux_estoq.procod      = movim.procod and
                           aux_estoq.Nome_Campo  = "PRICE_TYPE" and
                           aux_estoq.valor_campo = "C"
                           no-lock no-error.
                    if avail aux_estoq and
                         /*aux_estoq.datexp >= vdtini and
                         aux_estoq.datexp <= vdtfin and*/
                         aux_estoq.datexp <= movim.movdat
                    then val-remarca = val-remarca + 
                                    ( movim.movpc * movim.movqtm).
                    */
                end.
            ...
            then des-recarga = 0.
            if val-remarca < 0 or val-remarca = ?
            then val-remarca = 0.
            ...
            valtotal = valtotal + 
                    ((plani.platot - val-recarga - plani.seguro) -
                  val_des + val_acr) .
            /*
            message plani.seguro plani.platot val-recarga val_des val_acr
            valtotal. pause.
            */
            ...
            then do:
                ...
                assign wfcomis.etbcod = wf-com.etbcod
                       ...
                       wfcomis.remarca = wfcomis.remarca +
                            ((wf-com.compra - valor-bonus) * 
                                         (val-remarca / plani.platot))
                                        .
    ...
    then do:
        ...
        for each wfcomis break by wfcomis.etbcod 
            ...
            find first piso where piso.pismes = month(vdtfin) and
            ...
            find first func where func.etbcod = wfcomis.etbcod and
            ...
            display if wfcomis.modcod = "VVI"
                ...
                wfcomis.remarca(total by wfcomis.vencod)
                            column-label "Remarca" when tipo = 1
                            with frame 
...
procedure devolucao-venda:
    ...
    for each estab where if vetbcod = 0
        ...
        for each ctdevven where
            ...
            find first tippro where tippro.tprcod = tipo no-lock no-error.
            disp cplani.etbcod 
                 cplani.pladat 
            ...
            val_acr = 0.
            val_des = 0.
            if cplani.biss > (cplani.platot - cplani.vlserv - cplani.seguro)
            then val_acr = cplani.biss -
                        (cplani.platot - cplani.vlserv - cplani.seguro).
            ...
            then val_acr = 0.
            valtotal = valtotal + 
                            ((cplani.platot - val-recarga - cplani.seguro) -
                  val_des + val_acr) .
