...
def var vnomearq as char format "x(15)".

def input parameter p-catcod    like categoria.catcod.
def input parameter p-sresp     as   log.
def input parameter p-dti       as   date format "99/99/9999".
def input parameter p-dtf       as   date format "99/99/9999".
def input parameter p-analitico as   log format "Analitico/Sintetico".
def input parameter p-clacod    like clase.clacod.
def input parameter p-etbcodi   like estab.etbcod.
def input parameter p-etbcodf   like estab.etbcod.
    ...
    */
    ...
    /*repeat:
        ...
        do on error undo:
            ...
            update vetbcodi label "Filial Inicial"
                   vetbcodf label "Filial Final"
                   with frame f-etb centered side-label color blue/cyan row 15.
            
        end.*/
        
        vetbcodi = p-etbcodi. vetbcodf = p-etbcodf.

        for each estab where estab.etbcod >= vetbcodi 
                         and estab.etbcod <= vetbcodf no-lock:
    ...
    then do:
        for each tt-estab:
            vnom = vnom + string(tt-estab.etbcod,">99") + " ".
        end.
    end.
...
/*        disp fabri.fabnom no-label with frame f-depf.*/
    ...
    then do:
        ...
        assign vtime    = 0
               vtime    = time
               varquivo = "/admcom/relat/resu" + string(vtime)
               vnomearq = ""
               vnomearq = "resu" + string(vtime).
        
        output to /admcom/relat/imp-resu.bat append.
            put "type " vnomearq " > prn " skip.
        output close.
    ...
    {mdad.i
        ...
        &Tit-Rel   = """FILIAL "" + string(vetbcodi,"">99"")
                        + "" ATE "" + string(vetbcodf,"">99"")
                        + "" ""
                        + vfabnom + "" ""
                        + string(vfabcod,"">>>>>>>9"") + 
                        "" PERIODO DE "" +
    ...
    for each tt-cla no-lock:
        ...
        for each tt-catcod,         
            ...
            output stream stela to terminal.
                disp stream stela produ.procod format ">>>>>>>>>>>>>"
                            produ.clacod
