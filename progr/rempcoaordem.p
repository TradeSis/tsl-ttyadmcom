...
form
    ...
    vobs[3] /*opcom.opcnom*/  no-label format "x(30)"
    vpladat       colon 17
...
repeat:
   ...
   do on error undo, retry:
    ...
    find btipmov where btipmov.movtdc = 75 no-lock.
    if btipmov.movtlin
    then run tt-outsai.p(input forne.forcod,
                         input vetbcod).       
...
procedure emissao-NFe:
    ...
    do on error undo:
        ...
        for each w-movim where w-movim.marca = "*":
            vmovseq = vmovseq + 1.
            find produ where recid(produ) = w-movim.wrec no-lock no-error.                  if not avail produ 
