...
def var vcombo as dec.
def var vtotal-mov as dec.
...
def temp-table wplani
    ...
    field   wcbo-nf like plani.platot.
...
def temp-table wmovim
    ...
    field icms     as dec
    field combo    as dec
    index iq etbcod placod movtdc movdat procod.
    ...
    for each estab where estab.etbcod >= vetbi and
        ...
        do dt = vdti to vdtf:
            ...
            for each plani where plani.movtdc = 5             and
                ...
                create tt-plani.
                buffer-copy aux-plani to tt-plani.
                ...
                for each aux-movim where aux-movim.etbcod = aux-plani.etbcod and
                       ...
                       vtotal-mov = vtotal-mov + (aux-movim.movpc * aux-movim.movqtm).
                 ...
                 if vtotal-mov <> aux-plani.platot
                 then next.
                 
                 for each aux-movim where 
                                     aux-movim.etbcod = aux-plani.etbcod and
                                     aux-movim.placod = aux-plani.placod and
                                     aux-movim.movtdc = aux-plani.movtdc and
                                     aux-movim.movdat = aux-plani.pladat
                                     no-lock:
                    ...
                    then vcat = 31.
                        ...
                        if (vcat = 31 or
                            ...
                            vcat = 50)
                        then do:
                            cus-m = cus-m + (aux-movim.movqtm * vcustom).
                            if substr(aux-plani.notped,1,1) = "C" and
                               aux-plani.ufemi <> ""
                            then cus-mm = cus-mm + (aux-movim.movqtm * vcustom).
                            else cus-ms = cus-ms + (aux-movim.movqtm * vcustom).
                        end.            
                        else do:
                            cus-c = cus-c + (aux-movim.movqtm * vcustom).
                            if substr(aux-plani.notped,1,1) = "C" and
                               aux-plani.ufemi <> ""
                            then do:
                                if produ.fabcod = 5027
                                then
                                cus-cm = cus-cm +  /*(aux-movim.movqtm * vcustom).
                                */    ((aux-movim.movqtm * vcustom) - 
                                    (((aux-movim.movqtm * vcustom) /* / 2*/) * 
                                    (14.05 / 100))). /**/
                                else
                                cus-cm = cus-cm +  (aux-movim.movqtm * vcustom).
                            end.
                            else cus-cs = cus-cs +  (aux-movim.movqtm * vcustom).
                        end.
                        
                        find first wmovim where 
                               wmovim.etbcod = aux-movim.etbcod and
                               wmovim.placod = aux-movim.placod and
                               wmovim.movtdc = aux-movim.movtdc and
                               wmovim.movdat = aux-movim.movdat and 
                               wmovim.procod = aux-movim.procod 
                               no-error.
                        if not avail wmovim
                        then do:
                            create wmovim.
                            assign
                                wmovim.etbcod = aux-movim.etbcod
                                wmovim.placod = aux-movim.placod
                                wmovim.movtdc = aux-movim.movtdc
                                wmovim.numero = aux-plani.numero
                                wmovim.movdat = aux-movim.movdat  
                                wmovim.procod = aux-movim.procod 
                                wmovim.movqtm = aux-movim.movqtm 
                                wmovim.movpc  = aux-movim.movpc  
                                wmovim.movcto = vcustom
                                wmovim.custo  = estoq.estcusto
                                wmovim.alpis  = aux-movim.movalpis
                                wmovim.pis    = aux-movim.movpis  / aux-movim.movqtm
                                wmovim.alcofins = aux-movim.movalcofins
                                wmovim.cofins = aux-movim.movcofins / aux-movim.movqtm
                                wmovim.alicms = aux-movim.movalicms
                                wmovim.icms   = ((aux-movim.movpc * aux-movim.movqtm) *
                                               (aux-movim.movalicms / 100)).
                        ...
                        vcombo = 0.
                        if acha("COMBO-" + string(wmovim.procod),
                                aux-plani.notobs[1]) <> ?
                        then do:
                            find first movim where
                                       movim.etbcod = plani.etbcod and
                                       movim.placod = plani.placod and
                                       movim.procod = wmovim.procod 
                                       no-lock no-error.
                            vcombo = (dec(acha("COMBO-" + string(movim.procod),
                                      plani.notobs[1])) - movim.movpc)
                                      * movim.movqtm
                                     .
                            if vcombo <> ? and
                               vcombo > 0
                            then assign
                                    wmovim.combo = wmovim.combo + vcombo
                                    cbo-nf = cbo-nf + vcombo
                                    .
                        end.        
                        create tt-movim.
                        buffer-copy aux-movim to tt-movim.
                    end. /* movim */
                ...
                then do:
                    ...
                    assign vvldesc  = vvldesc  + aux-plani.descprod
                           vvlacre  = vvlacre  + wacr.
                end.
                /*
                else assign vvldesc  = vvldesc  + aux-plani.descprod
                            vvlacre  = vvlacre  + aux-plani.acfprod.
                */
                ...
                if (vcat = 31 or
                    ...
                    vcat = 50)
                then do:
                    assign acum-m = acum-m + (aux-plani.platot - aux-plani.vlserv -
                                               vvldesc + vvlacre)
    ...
    for each estab where estab.etbcod >= vetbi and
        ...
        do dt = vdti to vdtf:
            ...
            for each plani where plani.movtdc = 5
                ...
                for each aux-movim where aux-movim.etbcod = plani.etbcod and
                                     aux-movim.placod = plani.placod and
                                     aux-movim.movtdc = plani.movtdc and
                                     aux-movim.movdat = plani.pladat
                                      no-lock:
                ...
                for each aux-movim where aux-movim.etbcod = aux-plani.etbcod and
                    ...
                    then do:
                        ...
                        then do:
                            ...
                            if substr(aux-plani.notped,1,1) = "C" and
                               aux-plani.ufemi <> ""
                            then cus-cm = cus-cm +  (aux-movim.movqtm * vcustom).
                            else cus-cs = cus-cs +  (aux-movim.movqtm * vcustom).
                        end.
                        /*
                        find first wmovim where 
                               wmovim.placod = aux-plani.placod and
                               wmovim.numero = aux-plani.numero and
                               wmovim.movdat = aux-movim.movdat and 
                               wmovim.procod = aux-movim.procod and
                               wmovim.movqtm = aux-movim.movqtm and
                               wmovim.movpc  = aux-movim.movpc  and
                               wmovim.movcto = vcustom
                               no-error.
                        */
                        find first wmovim where 
                    ...
                    then do:
                        if aux-plani.biss > (aux-plani.platot - aux-plani.vlserv)
                        then wacr = ((aux-movim.movpc * aux-movim.movqtm) /
                                      aux-plani.biss) * 
                                   (aux-plani.biss - (aux-plani.platot - aux-plani.vlserv))
                                    .
                        else wacr = ((aux-movim.movpc * aux-movim.movqtm) /
                                      (aux-plani.platot - aux-plani.vlserv))
                                        * aux-plani.acfprod.
                    ...
                    then do:
                        assign acum-m = acum-m + 
                                ((aux-movim.movpc * aux-movim.movqtm) 
                                    - aux-plani.vlserv - vvldesc  + vvlacre)
                                ven-m  = ven-m + (aux-movim.movpc * aux-movim.movqtm)
                                des-m  = des-m + vvldesc
                                acr-m  = acr-m + vvlacre.
                        if substr(aux-plani.notped,1,1) = "C" and
                              aux-plani.ufemi <> ""
                        then do:
                            assign
                                acr-mm  = acr-mm + vvlacre.
                            for each tt-movim 
                                where tt-movim.procod = aux-movim.procod:
                                find produ where produ.procod = tt-movim.procod
                                    no-lock no-error.
                                if not avail produ then next.
                                assign
                    ...
                    then do:
                        assign
                            acum-c = acum-c + 
                                    ((aux-movim.movpc * aux-movim.movqtm) 
                                        - aux-plani.vlserv - vvldesc + vvlacre)
                            ven-c  = ven-c + (aux-movim.movpc * aux-movim.movqtm)
                            des-c  = des-c + vvldesc
                            acr-c  = acr-c + vvlacre. 
                        if substr(aux-plani.notped,1,1) = "C" and
                            aux-plani.ufemi <> ""
    ...
    then do:
        for each wplani by wplani.wetbcod:
            ...
            disp estab.etbcod column-label "Fl"
                 ...
                 ****/   
                    with frame f-imp width 360 down.
            ...
            disp estab.etbcod column-label "Fl"
                 ...
                 */
                 ...
                 (wplani.wven-m - wplani.wdes-m + wplani.wacr-m
                  ...
                  column-label "Venda(3)" format "->>>>>,>>9.99"
                 */  
                 wplani.wcbo-nf(total) column-label "Combo"
                    format ">>>>>,>>9.99"
                     with frame f-imp1 width 360 down.
        ...
        for each wmovim break by margem-v:
            ...
            if (categoria.catcod <> 41 and produ.catcod = 41) or
               (categoria.catcod = 41 and produ.catcod <> 41)
            then.
            else do:
                tot-medio = tot-medio + wmovim.custo.
                tot-custo = tot-custo + wmovim.movcto.
            end.         
        end.
...
procedure  analitico:
    ...
    def var t-custom as dec.
    def var t-combo as dec.
    ...
    for each wplani:
            ...
            if wplani.wven-c = 0 and wplani.wven-m = 0
            then next.

            t-movpc = t-movpc + wplani.wven-c + wplani.wven-m.
    ...
    */        
    ...
    for each wmovim break by margem-v:
        ...
        disp wmovim.procod 
             ...
             wmovim.custo(total)   format "->>>>>>>9.99"
             wmovim.subst(total)   format "->>>>>>>9.99"
             wmovim.credito(total) format "->>>>>>>9.99"
