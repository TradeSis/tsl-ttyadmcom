...
def buffer ntitulo for fin.titulo.
...
def var vcobra as int.

for each estab where estab.etbcod > vetblan no-lock.
...
for each tt-estab  no-lock:       
    ...
    do vdata1 = vdti to vdtf:
        ...
        for each plani where plani.etbcod  = tt-estab.etbcod and
            ...
            then do:
                ...
                tt-venda.venda_cartaopresente[2] =
                     tt-venda.venda_cartaopresente[2] + vendap-cartaopresente.
                     .
            end.
            ...
            then do:
                ...
                then do:
                    ...
                    find first titulo where
                              ...
                              titulo.titpar = 1
                              no-lock no-error.
                    if avail envfinan or
                        (avail titulo and titulo.cobcod = 10)
                    then vcobra = 10.
                    else vcobra = 2.
        ...
        def var f-origem as dec.
        def var af-origem as dec.
        def var l-origem as dec.
        def var al-origem as dec.
        def var v-juro as dec.
        for each tt-contnum: 
            find contrato where contrato.contnum = tt-contnum.contnum no-lock.
    
            find first ntitulo where ntitulo.empcod = 19 and
                          ntitulo.titnat = no and
                          ntitulo.modcod = contrato.modcod and
                          ntitulo.etbcod = contrato.etbcod and
                          ntitulo.clifor = contrato.clicod and
                          ntitulo.titnum = string(contrato.contnum)  and
                          ntitulo.titpar > 0
                          no-lock no-error.
                          
            if not avail ntitulo then next.
            
            if ntitulo.tpcontrato <> "N" and
               ntitulo.tpcontrato <> "L" and  
               ntitulo.titpar <> 31      and
               ntitulo.titpar <> 51             
            then next.
            ...
            for each tit_novacao where
                ...
                if titulo.titpar > 30
                then v-origem = tit_novacao.ori_titvlcob -                                     (tit_novacao.ori_titvlcob * .20). 
                else v-origem = tit_novacao.ori_titvlcob /*+
                                    (tit_novacao.ori_titvlcob * .30)*/.
                val-origem = val-origem + v-origem. 
                if titulo.cobcod = 10
                then f-origem = f-origem + v-origem. 
                else l-origem = l-origem + v-origem.
                ...
                then do:
                    if titulo.cobcod = 10
                    then do:    
                        vnov-finan = vnov-finan + titulo.titvlcob. 
                        p-tiplan = "RECNOVACAO".
                        p-sitlan = "FINANCEIRA".
                        run put-recebimento-novacao.
                    end.
                    else do:
                        vnov-lebes = vnov-lebes + titulo.titvlcob.
                        p-tiplan = "RECNOVACAO".
                        p-sitlan = "LEBES".
                        run put-recebimento-novacao.
                    end.
            ...
            then for each titulo where
                          ...
                          titulo.etbcobra = contrato.etbcod and
                          titulo.clifor = contrato.clicod and
                          titulo.moecod = "NOV"
                          no-lock:
                if titulo.titpar > 30
                then v-origem = titulo.titvlcob - (titulo.titvlcob * .20). 
                else v-origem = titulo.titvlcob.
                val-origem = val-origem + v-origem. 
                if titulo.cobcod = 10
                then f-origem = f-origem + v-origem. 
                else l-origem = l-origem + v-origem.
            ...
            for each tit_novacao where
                ...
                find first titulo where
                           ...
                           no-lock no-error.
                if not avail titulo then next. 
                val-origem = val-origem + tit_novacao.ori_titvlcob.
                if titulo.cobcod = 10
                then f-origem = f-origem + tit_novacao.ori_titvlcob.
                else l-origem = l-origem + tit_novacao.ori_titvlcob.
            ...
            ***********************/
            ...
            for each btitulo where btitulo.empcod = 19 and
                          ...
                          btitulo.titnum = string(contrato.contnum)  
                ...
                then v-financiado = v-financiado + btitulo.titvlcob.
                else v-clientes = v-clientes + btitulo.titvlcob. 
                if acha("FEIRAO-NOME-LIMPO",btitulo.titobs[1]) <> ?
                then.
                else if v-juro = 0
                then v-juro = dec(acha("JURO-ATU",btitulo.titobs[1])).
            end.
            if v-financiado + v-clientes < 3
            then next.
            if v-juro = ? then  v-juro = 0.
            if v-juro < contrato.vltotal - v-principal
            then v-principal = val-origem + v-juro.
            ...
            v-acrescimo = v-acrescimo +
                    ((contrato.vltotal - contrato.vlentra) -
                     (v-principal + v-acrescimo)).

            assign
                /*l-origem = vtitvlcob *
                           (vnov-lebes / (vnov-finan + vnov-lebes))*/
                f-origem = v-principal *
                           (vnov-finan / (vnov-finan + vnov-lebes)).
                l-origem = v-principal - f-origem.       
            ...
            then do:
                if f-origem > 0
                then f-origem = f-origem +
                            (v-juro * (f-origem / val-origem)).
                if l-origem > 0
                then l-origem = l-origem + 
                            (v-juro * (l-origem / val-origem))
                            .
            end.*/ 
            if v-acrescimo > 0
            then do:
                /*
                val-origem = f-origem + l-origem.
                if f-origem > 0
                then do:
                    if f-origem > val-origem
                    then f-origem = val-origem.
                    af-origem = af-origem +
                                (v-acrescimo * (f-origem / val-origem))
                                .
                end.
                if l-origem > 0
                then do:
                    if l-origem > val-origem
                    then l-origem = val-origem.
                    al-origem = al-origem +
                                (v-acrescimo * (l-origem / val-origem))
                                .
                end.
                */

                af-origem = af-origem +
                   (v-acrescimo * (vnov-finan / (vnov-finan + vnov-lebes))).
                al-origem = al-origem +
                   (v-acrescimo * (vnov-lebes / (vnov-finan + vnov-lebes))).
            ...
            then do:
                if f-origem > 0
                then af-origem = af-origem +
                                (v-acrescimo * (f-origem / val-origem))
                                .
                if l-origem > 0
                then al-origem = al-origem +
                                (v-acrescimo * (l-origem / val-origem))
                                .
            end.         
        ...
        *******/        
        ...
        run pdv-moeda(input tt-estab.etbcod, input vdata1).
    ...
    run grava-registros.
    /*
    output to value("/admcom/custom/Claudir/tt-plani-outras"
                    + string(tt-estab.etbcod,"999")).
        ...
        then do:
            ...
            for each   pag-titmoe where 
                ...
                find first tt-recmoeda where
                       ...
                       tt-recmoeda.moecod = pag-titmoe.moecod
                       no-error.
                ...
                tt-recmoeda.valor = tt-recmoeda.valor + pag-titmoe.titvlpag.
                *****/
                 ...
                 do vi = 1 to 15:
                    ...
                    then do:
                        ...
                        assign
                            ...
                            tt-receb.receb_moeda_principal_lebes[vi] +
                         (p-principal * (pag-titmoe.titvlpag / titulo.titvlpag))
                            tt-receb.receb_moeda_acrescimo_lebes[vi] = 
                            tt-receb.receb_moeda_acrescimo_lebes[vi] +
                         (p-acrescimo * (pag-titmoe.titvlpag / titulo.titvlpag))
                            tt-receb.receb_moeda_juros_lebes[vi] = 
                            tt-receb.receb_moeda_juros_lebes[vi] +
                      (titulo.titjuro * (pag-titmoe.titvlpag / titulo.titvlpag))
                            .
                        else if p-sitlan = "FINANCEIRA"
                            then 
                            assign
                         ...
                         (p-acrescimo * (pag-titmoe.titvlpag / titulo.titvlpag))
                            ...
                            tt-receb.receb_moeda_juros_fdrebes[vi] +
                       (titulo.titjur * (pag-titmoe.titvlpag / titulo.titvlpag))
                            .
                        leave.
                    end.    
                 end.     
...
procedure put-contratonov:
    ...
    then do on error undo: 
     ...
     if p-sitlan = "LEBES"
     then assign
            tt-venda.novacao_lebes = tt-venda.novacao_lebes + vtitvlcob
            tt-venda.novacao_lebes_ol = tt-venda.novacao_lebes_ol + l-origem
            tt-venda.novacao_lebes_of = tt-venda.novacao_lebes_of + f-origem
            .

     else  tt-venda.novacao_fdrebes = tt-venda.novacao_fdrebes + vtitvlcob.
     ...
     then do:           
        ...
        then assign
             ...
             tt-venda.acrescimo_novacao_lebes_ol =
                    tt-venda.acrescimo_novacao_lebes_ol + al-origem 
                    /*(vacrescimo * (l-origem / vtitvlcob))*/
             tt-venda.acrescimo_novacao_lebes_of =
                    tt-venda.acrescimo_novacao_lebes_of + af-origem
                    /*(vacrescimo * (f-origem / vtitvlcob))*/
             .
            ...
            then do:
                ...
                then do:
                    ...
                    for each titpag where
                          ...
                          titpag.titpar = titulo.titpar
                          no-lock:
                        vtotal = vtotal + titpag.titvlpag.
                    end.
                    if venda-fiscal < vtotal
                    then do:
                    ...
                    for each titpag where
                        ...
                        val-pago = titpag.titvlpag -  
                                ((vtotal - venda-fiscal) * 
                                    (titpag.titvlpag / vtotal)).
                        ...
                        do vi = 1 to 15:
                            ...
                            then do:
                                ...
                                tt-venda.venda_moeda[vi] + val-pago.
                                leave.
                            end.    
                        end.         
                        tt-venda.venda_vista =
                               tt-venda.venda_vista + val-pago.
