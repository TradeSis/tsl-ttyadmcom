...
define temp-table tt-valores no-undo serialize-name 'subCaracteristica'
    field codigo    as integer   serialize-name 'codigo'
    field descricao as character serialize-name 'descricao'
    field cod-car   as integer   serialize-name 'codigoCaracteristica'
...
for each subcaract no-lock:
    create tt-valores.
    assign tt-valores.codigo    = subcaract.subcod
           tt-valores.descricao = fnremoveacento(subcaract.subdes)
           tt-valores.cod-car   = subcaract.carcod
...
RUN PostRequest (INPUT 'produtos/integracaoSubCaracteristica', 
                                 INPUT lo-arquivo).
