...
repeat:
    ...
    update vclacod label "Clase" colon 16 with frame f-dep.

    /*find bclase where bclase.clacod = vclasup no-lock no-error.
...
/******/    
    ...
    for each tt-cla /*clase where clase.clasup = vclacod*/ no-lock,
        each produ where (produ.catcod = vcatcod or
                         produ.catcod = vcatcod2) and
                         produ.clacod = tt-cla.clacod /*clase.clacod*/
                         no-lock:
    ...
    for each tt-curpro by tt-curpro.pos:
        ...
        disp produ.procod column-label "Codigo"
             produ.pronom when avail produ format "x(28)" column-label "Nome"
             tt-curpro.qtdven(total) format ">>>>>9" column-label "Qtd!Ven"
             tt-curpro.valcus(total) format "->>>,>>9" column-label "Val.Cus"
             tt-curpro.valven(total) format ">>>,>>9" column-label "Val.Ven"
