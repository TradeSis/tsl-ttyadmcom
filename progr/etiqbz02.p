...
if opsys = "UNIX"
then do:
    output to /admcom/zebra/sp/eti-sp.bat append.
    ...
    output to l:~\zebra~\sp~\eti-sp.bat append.
    ...
    */
    ...
    then do:
      for each wfetq break by linha:
        ...
        if opsys = "UNIX"
        then varquivo = "/admcom/zebra/sp/c" + string(produ.procod)
                               + "." + trim(string(par-taman,"x(3)")).
        else varquivo = "l:~\zebra~\sp~\c" + string(produ.procod) 
                       + "." + trim(string(par-taman,"x(3)")).
        output to value(varquivo). 
        
        /*
        output to value("l:\zebra\sp\c" + string(produ.procod) 
                       + "." + string(par-taman,"x(3)")).
        */
            for each wfetq break by linha.
    ...
    then do:
      for each wfetq break by linha:
        ...
        if opsys = "UNIX"
        then varquivo = "/admcom/zebra/sp/cr" + string(produ.procod) + 
                            "." + trim(string(par-taman,"x(3)")).
        else varquivo = "l:~\zebra~\sp~\cr" + string(produ.procod) +
                            "." + trim(string(par-taman,"x(3)")).
        output to value(varquivo).         
        /*
        output to value("l:~\zebra~\sp~\cr" 
                       + string(produ.procod) 
                       + "." + string(par-taman,"x(3)")).
        */
        for each wfetq break by linha.
