...
def var vsetor like teste.clilig.setor.
def var vsettipo like teste.clilig.settipo.
...
def var vdatopc7    as date label "A partir de".
...
find first teste.clilig where teste.clilig.DtProxLig >= vtoday - 1 no-lock no-error.
if not avail teste.clilig
...
/*
find first teste.clilig where (if vsetor <> "" then teste.clilig.setor = vsetor else true)
        no-lock no-error.
if not avail teste.clilig
then do on error undo.
    message "Processo para selecionar as Ligacoes deve ser processado."
...
def buffer bclilig       for teste.clilig.
def buffer celclilig     for teste.clilig.
def var vclilig         like teste.clilig.clicod.
...
form
    ...
    teste.clilig.valdivtot           label "Divida"
...
then  do.
    if vdatopc8ini <> ?
    then
        for each teste.clilig where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and
                              teste.clilig.dtven >= vdatopc8ini and
                              teste.clilig.dtven <= vdatopc8fin and
                              teste.clilig.dtacor    = ?      and                   
                              teste.clilig.DtUltLig  < vtoday  and                  
                              teste.clilig.titvlcob >= vvalini     and              
                        (if vetbcod = 0                                 
                               then true                                      
                               else if can-find(first cliletb where
                                     cliletb.clicod = teste.clilig.clicod and
                                     cliletb.etbcod = vetbcod)
                                    then true
                                    else false)
                                    
                                      and                                                                         (if vrgccod = 0                                                                     then true                                                                 else teste.clilig.rgccod = vrgccod) /*and              
                               (if vmodcod = "GERAL" or vnovacao = yes                                       then true                                      
                                 else teste.clilig.modcod = vmodcod)*/ and
                       (if ((vnovacao = yes and teste.clilig.modcod = "NOV") or
                           (vcrediario = yes and teste.clilig.modcod <> "NOV"))
                                then true
                                else false)
                     and (if vnovos = yes then teste.clilig.tipo = "N" else true)
                                use-index clilig2
                                                     no-lock.                        
            vqtd = vqtd + 1.                                                        end.    
    else 
    for each teste.clilig where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and teste.clilig.dtven    = vdtini  and
                                teste.clilig.dtacor    = ?      and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
                                (if vetbcod = 0                                 
                               then true                                      
                                ...
                                (if vrgccod = 0
                                 then true
                                 else teste.clilig.rgccod = vrgccod) /*and
                                (if vmodcod = "GERAL"
                                 then true
                                 else teste.clilig.modcod = vmodcod)*/ and
                       (if (
                            (vnovacao = yes and teste.clilig.modcod = "NOV") or
                            (vcrediario = yes and teste.clilig.modcod <> "NOV")
                            )
                        then true
                        else false)
    ...
    if vdatopc7 <> ?
    then 
        for each teste.clilig  where  /*teste.clilig.dtacor    >= vdatopc7*/
                                 teste.clilig.dtacor <> ? and
                                (if teste.clilig.modcod = "CRE"
                                 then teste.clilig.dtacor = vdatopc7
                                 else teste.clilig.dtacor >= vdatopc7) /*Lu and 
                                teste.clilig.dtacor    < vtoday Lu*/      and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
...
and (if vnovos = yes then teste.clilig.tipo = "N" else true)                                
    ...
    for each teste.clilig  where 
                                teste.clilig.dtacor    < vtoday      and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
                                (if vetbcod = 0                                 
                               then true                                      
...
repeat:
    ...
    then
        ...
        find teste.clilig where recid(teste.clilig) = recatu1 no-lock.
    if not available teste.clilig
    then esqvazio = yes.
    else esqvazio = no.
    ...
    repeat with frame frame-a:
        ...
        then do:
            find teste.clilig where recid(teste.clilig) = recatu1 no-lock.
            find clien where
                    clien.clicod = teste.clilig.clicod no-lock.
...
find clien where clien.clicod = teste.clilig.clicod no-lock.
        display clien.clicod 
            ...
            choose field teste.clilig.clicod help ""
        ...
        then do.
            ...
            then do:
                ...
                if not avail teste.clilig
                then next.
                color display normal teste.clilig.clicod with frame frame-a.
                if frame-line(frame-a) = frame-down(frame-a)
            ...
            then do:
                ...
                then do on error undo:
                    ...
                    then do:
                        ...
                        */
                        find teste.clilig where teste.clilig.clicod = vclicod no-lock
                                                    no-error.
                        if not avail teste.clilig
                        then do.
                            create teste.clilig.
                            teste.clilig.clicod = vclicod.
                            if vsetor <> "" and vsetor <> teste.clilig.setor
                            then do:
                                message "Cliente em " teste.clilig.setor view-as alert-box.
                            end.
                        end.
                        recatu1 = recid(clilig).
                        leave.
...
/*                             cpfis.estado-fone label "Tipo"*/
                        ...
                        find last clitel where 
                                clitel.clicod = teste.clilig.clicod 
                                    no-lock no-error.
                        if avail clitel and
                           cliTEL.teldat = vtoday
                        then do:
                            find teste.clilig where recid(teste.clilig) = recatu1.
                            recatu1 = ?.
                            teste.clilig.dtultlig = today.
                            /*
...
procedure frame-a.
    ...
    for each titulo where titulo.titnat = no and
                          ...
                          titulo.clifor = teste.clilig.clicod and
    ...
    */
    ...
    vnumdias = vtoday - teste.clilig.dtven.
    if vnumdias < 0
    then vnumdias = 0.

    find last clitel where clitel.clicod = teste.clilig.clicod /*and
               clitel.teldat > teste.clilig.dtven and
    ...
    for each bclitel of teste.clilig where bclitel.dtpagcont  <> ? no-lock.
         
         vdataprom = bclitel.dtpagcont.  
    end.
    
    vdataprom = teste.clilig.dtacor.
                
    find func of clitel no-lock no-error.
    find clien of teste.clilig no-lock no-error.
    vnumdias = vtoday - teste.clilig.dtven.
    if vnumdias < 0
    then vnumdias = 0.
    display
       teste.clilig.clicod column-label "Conta" format ">>>>>>>>>9"
       teste.clilig.dtven format "99/99/99"   column-label "Maior!Atraso"
       vnumdias
       teste.clilig.valdivtot column-label "Sld Aberto" format "->>>,>>9.99"
...
then  
    ...
    then  
        if vdatopc8ini <> ?
        then
        find first teste.clilig where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and teste.clilig.dtven    >= vdatopc8ini and
                                teste.clilig.dtven    <= vdatopc8fin and
                                teste.clilig.dtacor    = ?      and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
...
and (if vnovos = yes then teste.clilig.tipo = "N" else true)                                
      ...
      if vdatopc7 <> ?
      then
            find last teste.clilig  where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and /*teste.clilig.dtacor    >= vdatopc7*/
                                 teste.clilig.dtacor <> ? and
                                (if teste.clilig.modcod = "CRE"
                                 then teste.clilig.dtacor = vdatopc7
                                 else teste.clilig.dtacor >= vdatopc7) /*Luand
                                teste.clilig.dtacor    <= vtoday - 1Lu*/     and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
...
and (if vnovos = yes then teste.clilig.tipo = "N" else true)                                
        ...
        find last teste.clilig  where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                                ...
                                teste.clilig.dtacor    <= vtoday - 1     and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
...
then  
    ...
    then  
        if vdatopc8ini <> ?
        then 
        find next teste.clilig  where  (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and teste.clilig.dtven    >= vdatopc8ini and
                                 teste.clilig.dtven <= vdatopc8fin and
                                teste.clilig.dtacor    = ?      and
...
and (if vnovos = yes then teste.clilig.tipo = "N" else true)                                
      ...
      if vdatopc7 <> ?
      then 
        find prev teste.clilig  where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and /*teste.clilig.dtacor    >= vdatopc7*/
                                 teste.clilig.dtacor <> ? and
                                (if teste.clilig.modcod = "CRE"
...
and (if vnovos = yes then teste.clilig.tipo = "N" else true)                                
        ...
        find prev teste.clilig    where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                               else true) and teste.clilig.dtacor    <= vtoday - 1     and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
(if vetbcod = 0                                 
                               then true                                      
    ...
    then 
                   find prev teste.clilig where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                                 ...
                                 teste.clilig.dtven  >= vdatopc8ini
                                      and teste.clilig.dtven <= vdatopc8fin and
                                teste.clilig.dtacor    = ?      and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
(if vetbcod = 0                                 
      ...
      then do:
        for each clilig  where (if vsetor <> ""
                               then teste.clilig.setor = vsetor
                                ...
                                (if teste.clilig.modcod = "CRE"
                                 then teste.clilig.dtacor = vdatopc7
                                 else teste.clilig.dtacor >= vdatopc7) /*and
                                teste.clilig.dtacor    <= vtoday - 1*/     and
                                teste.clilig.DtUltLig  < vtoday  and
                                teste.clilig.titvlcob >= vvalini     and
(if vetbcod = 0                                 
...
and (if vnovos = yes then teste.clilig.tipo = "N" else true)                                                            no-lock.
            run p-imprime.
        end.
     end.                                         
      else do:
