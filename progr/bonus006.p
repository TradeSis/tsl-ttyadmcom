...
find first btt-cli no-lock no-error.
if not avail btt-cli
then do:
    message "Nenhum participante encontrado.". pause 2 no-message.
    leave.
end.
...
procedure frame-a.
    display tt-cli.clicod    column-label "Codigo"
            tt-cli.clinom    column-label "Cliente" format "x(30)"
            tt-cli.conjuge   column-label "Conjuge" format "x(14)"
            tt-cli.dtaniconj column-label "Ani.Conj"
...
then  
    if esqascend  
    then  
        find first tt-cli where true no-lock no-error.
    else  
        find last tt-cli  where true no-lock no-error.
                                             
if par-tipo = "seg" or par-tipo = "down" 
then  
    if esqascend  
    then  
        find next tt-cli  where true no-lock no-error.
    else  
        find prev tt-cli  where true no-lock no-error.
