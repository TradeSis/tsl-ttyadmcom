...
def workfile wfsaldo
    ...
    field venda as int  column-label "" format ">>>>9"          
            extent 2
    field venda-val as dec column-label "" format ">>,>>9.99" extent 2
...
/**/
def var vttvenda-val    like wfsaldo.venda-val[1].
/**/
...
def input parameter par-dti as date format "99/99/9999".
def input parameter par-dtf as date format "99/99/9999".

/***
def var par-procod as char init "10218".
def var par-dti    as date format "99/99/9999" init today.
def var par-dtf    as date format "99/99/9999" init today.
***/
    ...
    then do:
        ...
        for each movim where
            ...
            wfsaldo.venda[vconta] = wfsaldo.venda[vconta]
                            + movim.movqtm.
            wfsaldo.venda-val[vconta] = wfsaldo.venda-val[vconta]
                            + venda-liquida-item(recid(movim)).
                            /*
...
for each wfsaldo.
    assign
        ...
        vttvenda-val = vttvenda-val + wfsaldo.venda-val[1] +
                                      wfsaldo.venda-val[2]
        vttreserva = vttreserva + wfsaldo.reserva[1] + wfsaldo.reserva[2]
        /*vttCobertura  = vttCobertura  + wfsaldo.Cobertura[1]  + wfsaldo.Cobertu~ ra[2]*/
...
def buffer bwfsaldo for wfsaldo.

for each bwfsaldo where bwfsaldo.filial[1] <> 0:

    find first wfsaldo where wfsaldo.linha     = 0
                         and wfsaldo.filial[1] = 0 no-error.
    if avail wfsaldo
    then assign wfsaldo.venda[1]     =
                    wfsaldo.venda[1]     + (bwfsaldo.venda[1] +
                                            bwfsaldo.venda[2])
                wfsaldo.venda-val[1] = 
                    wfsaldo.venda-val[1] + (bwfsaldo.venda-val[1] +
                                            bwfsaldo.venda-val[2])
                wfsaldo.saldo[1]     = 
                    wfsaldo.saldo[1]     + (bwfsaldo.saldo[1] +
                                            bwfsaldo.saldo[2])
                wfsaldo.reserva[1]   = 
                    wfsaldo.reserva[1]   + (bwfsaldo.reserva[1] +
                                            bwfsaldo.reserva[2]).
end.
    ...
    */
    ...
    then do with frame frame-a:
        ...
        vmostra1 = wfsaldo.saldo[1] <> 0 or
                   wfsaldo.venda[1] <> 0 or
                   wfsaldo.venda-val[1] <> 0.
                   
        vmostra2 = wfsaldo.saldo[2] <> 0 or
                   wfsaldo.venda[2] <> 0 or
                   wfsaldo.venda-val[2] <> 0.
                   
        disp
            wfsaldo.filial[1] format ">>9"  column-label "Fil" when vmostra1
            wfsaldo.venda[1]  column-label "Qtd.Ven" when vmostra1
            wfsaldo.venda-val[1] column-label "R$ Ven" when vmostra1
            wfsaldo.saldo[1]  column-label "Estoque" when vmostra1
            wfsaldo.reserva[1] column-label "Reser" when vmostra1
/*            wfsaldo.Cobertura[1] column-label "Cobert" when vmostra1*/
            ...
            wfsaldo.venda[2]   column-label "Qtd.Ven" when vmostra2
            wfsaldo.venda-val[2] column-label "R$ Ven" when vmostra2
            wfsaldo.saldo[2]   column-label "Estoque"  when vmostra2
            wfsaldo.reserva[2] column-label "Reser" when vmostra2
    ...
    */
    ...
    then repeat:
        ...
        disp
            ...
            wfsaldo.venda-val[1] when vmostra1
...
/*            wfsaldo.Cobertura[1] when vmostra1*/
            ...
            wfsaldo.venda-val[2] when vmostra2
