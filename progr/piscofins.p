...
do on error undo.
    ...
    if tipmov.movtdc = 5 or tipmov.movtdc = 81
    then vopccod = 5102.
    ...
    then do.
        ...
        then
            assign
                ...
                valiqcofins = 4.6.
        if clafis.log1
        then assign valiqpis = 0 valiqcofins = 0.
        run cal-mva-entrada.
     end.
...
procedure cal-mva-entrada:
    ...
    assign 
            ...
            vmovfre = if tt-plani.frete > 0 and
                         tt-movim.movdev > 0
                      then tt-movim.movdev
                      else if tt-movim.movdev > 0   
                           then (tt-movim.movdev / tt-movim.movqtm)
                           else if tt-plani.frete > 0
                               then (tt-plani.frete / tt-plani.protot) 
                                    * tt-movim.movpc
                               else 0 
            ...
            vmovsubst = if tt-movim.movbsubst > 0 and
                           tt-movim.movsubst  > 0
                        then tt-movim.movsubst / tt-movim.movqtm
                        else 0
        ...
        if vmovbsubst = 0 and tt-plani.bsubst > 0
        then vmovbsubst = ((tt-plani.bsubst / tt-plani.protot) * vmovtot)
                            / tt-movim.movqtm.
 
        if vmovbsubst > 0 /*and vmovsubst = 0*/
        then vmovsubst = vmovbsubst * .18.
