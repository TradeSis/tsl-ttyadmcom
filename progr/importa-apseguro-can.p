...
for each tt-cap:
    ...
    then do:
        ...
        find first apseguro where apseguro.apolice = apolice_seguro
                     no-error.
        ...
        then do:
            ...
            find first vndseguro where
                       ...
                       vndseguro.certifi = substr(apolice_seguro,15,11)
                       no-lock no-error.
            if avail vndseguro
            then disp vndseguro.
            */
            ...
            assign
                ...
                apseguro.premio_total = ((- 1) * dec(tt-cap.valor[15]))
                apseguro.premio_liquido = ((- 1) * dec(tt-cap.valor[16]))
                apseguro.iof = dec(tt-cap.valor[17])
                apseguro.prolabore = ((- 1) * dec(tt-cap.valor[18]))
                apseguro.comissao  = dec(tt-cap.valor[19])
                apseguro.lote = tt-cap.valor[20]
                apseguro.campo_char1 = tt-cap.dados
                apseguro.campo_log1 = no
                apseguro.campo_log2 = yes
                .
            ...
            find first vndseguro where
                       vndseguro.tpseguro = 1 and
                       vndseguro.certifi = substr(apolice,15,11)
                       no-lock no-error.
            if avail vndseguro
            then disp vndseguro.
            ***/
